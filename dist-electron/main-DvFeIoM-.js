...
import { ipcMain as PR, app as CE, Notification as Ne, shell as Pt, BrowserWindow as bR } from "electron";
...
import Vt from "net";
import mR from "tls";
import Ee from "timers";
import YR from "stream";
import WR from "buffer";
import mt from "string_decoder";
import Yt from "zlib";
import HR from "util";
import Wt from "url";
...
var re;
function Xt() {
  if (re) return RE.exports;
  re = 1;
  const E = Gt, n = Ft, e = Qt, _ = GE, N = yt.version, T = /(?:^|^)\s*(?:export\s+)?([\w.-]+)(?:\s*=\s*?|:\s+?)(\s*'(?:\\'|[^'])*'|\s*"(?:\\"|[^"])*"|\s*`(?:\\`|[^`])*`|[^#\r\n]+)?\s*(?:#.*)?(?:$|$)/mg;
  function A(M) {
    const h = {};
    let d = M.toString();
    d = d.replace(/\r\n?/mg, `
...
`), P = P.replace(/\\r/g, "\r")), h[b] = P;
  ...
  function I(M) {
    ...
    const U = c(M).split(","), b = U.length;
    let P;
    for (let w = 0; w < b; w++)
      try {
        const L = U[w].trim(), f = O(d, L);
  ...
  function t(M) {
    console.log(`[dotenv@${N}][WARN] ${M}`);
  }
  function a(M) {
    console.log(`[dotenv@${N}][DEBUG] ${M}`);
  }
  function i(M) {
    console.log(`[dotenv@${N}] ${M}`);
  }
  function c(M) {
  ...
  function O(M, h) {
    ...
    const P = `DOTENV_VAULT_${b.toUpperCase()}`, w = M.parsed[P];
    if (!w) {
  ...
  function S(M) {
    ...
    if (M && M.path && M.path.length > 0)
      ...
      h = n.resolve(process.cwd(), ".env.vault");
    return E.existsSync(h) ? h : null;
  }
  function u(M) {
    return M[0] === "~" ? n.join(e.homedir(), M.slice(1)) : M;
  }
  function s(M) {
    ...
    let b = process.env;
    return M && M.processEnv != null && (b = M.processEnv), B.populate(b, U, M), { parsed: U };
  }
  function r(M) {
    const h = n.resolve(process.cwd(), ".env");
    let d = "utf8";
    const U = !!(M && M.debug), b = M && "quiet" in M ? M.quiet : !0;
    ...
    let f = process.env;
    if (M && M.processEnv != null && (f = M.processEnv), B.populate(f, L, M), U || !b) {
      const l = Object.keys(L).length, g = [];
      for (const V of P)
        try {
          const W = n.relative(process.cwd(), V);
          g.push(W);
        } catch (W) {
          U && a(`Failed to load ${V} ${W.message}`), w = W;
        }
  ...
  function D(M, h) {
    ...
    const b = U.subarray(0, 12), P = U.subarray(-16);
    U = U.subarray(12, -16);
    try {
      const w = _.createDecipheriv("aes-256-gcm", d, b);
  ...
  function o(M, h, d = {}) {
    const U = !!(d && d.debug), b = !!(d && d.override);
    if (typeof h != "object") {
    ...
    for (const P of Object.keys(h))
      Object.prototype.hasOwnProperty.call(M, P) ? (b === !0 && (M[P] = h[P]), U && a(b === !0 ? `"${P}" is already defined and WAS overwritten` : `"${P}" is already defined and was NOT overwritten`)) : M[P] = h[P];
  }
  const B = {
    configDotenv: r,
    _configVault: s,
...
async function qt(E, n) {
  ...
  const A = new AbortController(), I = setTimeout(() => A.abort(), 2e4);
  try {
    const t = await T(_, {
      method: "POST",
      headers: N,
      body: JSON.stringify({ channel: E, args: n }),
      signal: A.signal
    }), a = await t.text();
    let i = null;
    try {
...
function F(E, n) {
  PR.handle(E, async (e, ..._) => {
    try {
      return Zt(E) ? await qt(E, _) : await n(e, ..._);
    } catch (R) {
...
const XR = VR(import.meta.url), pt = x.dirname(XR);
typeof globalThis < "u" && (globalThis.__filename = XR, globalThis.__dirname = pt);
let m = null, t_ = !1;
function G() {
  if (m)
    return console.log(" [DB] Reutilizando conexión existente"), m;
  if (t_ && (console.log("⏳ [DB] Conexión en progreso, esperando..."), m))
    return m;
  t_ = !0;
  try {
    if (!CE.isReady())
      throw new Error("Electron app not ready");
    const E = CE.getPath("userData"), n = x.join(E, "reservas.db");
    if (z.existsSync(E) || z.mkdirSync(E, { recursive: !0 }), console.log(" [DB] Creando nueva conexión a:", n), m = new bt(n, {
      readonly: !1,
      fileMustExist: !1,
      timeout: 3e4
    }), console.log(" [DB] Configurando pragmas..."), m.pragma("query_only = FALSE"), m.pragma("journal_mode = OFF"), m.pragma("synchronous = OFF"), m.pragma("cache_size = -64000"), m.pragma("temp_store = MEMORY"), m.pragma("foreign_keys = ON"), m.pragma("busy_timeout = 100000"), console.log(" [DB] Pragmas configurados correctamente"), m.exec(`
  ...
  `), m.exec("CREATE INDEX IF NOT EXISTS idx_aprontes_fecha_hora ON aprontes (fecha, hora)"), m.exec(`
  ...
  `).get().total === 0) {
    ...
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN notas TEXT"), console.log(' [DB] Columna "notas" agregada a reservas');
    } catch (_) {
    ...
    console.log(" DB inicializada en:", n);
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN particular_tipo TEXT"), console.log('âœ… [DB] Columna "particular_tipo" agregada a reservas');
    } catch (_) {
    ...
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN garantia_tipo TEXT"), console.log('âœ… [DB] Columna "garantia_tipo" agregada a reservas');
    } catch (_) {
    ...
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN garantia_fecha_compra TEXT"), console.log('âœ… [DB] Columna "garantia_fecha_compra" agregada a reservas');
    } catch (_) {
    ...
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN garantia_numero_service TEXT"), console.log('âœ… [DB] Columna "garantia_numero_service" agregada a reservas');
    } catch (_) {
    ...
    try {
      m.exec("ALTER TABLE reservas ADD COLUMN garantia_problema TEXT"), console.log('âœ… [DB] Columna "garantia_problema" agregada a reservas');
    } catch (_) {
    ...
    try {
      m.exec("ALTER TABLE aprontes ADD COLUMN observaciones TEXT"), console.log(' [DB] Columna "observaciones" agregada a aprontes');
    } catch (_) {
...
function xt() {
  return se || (se = 1, (function(E) {
    ...
    n.escapeId = function(a, i) {
      if (Array.isArray(a)) {
        for (var c = "", O = 0; O < a.length; O++)
          c += (O === 0 ? "" : ", ") + n.escapeId(a[O], i);
        return c;
      } else return i ? "`" + String(a).replace(e, "``") + "`" : "`" + String(a).replace(e, "``").replace(_, "`.`") + "`";
    }, n.escape = function(a, i, c) {
      if (a == null)
        return "NULL";
      switch (typeof a) {
        case "boolean":
        ...
        case "number":
          return a + "";
        case "object":
          return Object.prototype.toString.call(a) === "[object Date]" ? n.dateToString(a, c || "local") : Array.isArray(a) ? n.arrayToList(a, c) : Buffer.isBuffer(a) ? n.bufferToString(a) : typeof a.toSqlString == "function" ? String(a.toSqlString()) : i ? T(a.toString()) : n.objectToValues(a, c);
        default:
          return T(a);
      }
    }, n.arrayToList = function(a, i) {
      for (var c = "", O = 0; O < a.length; O++) {
        var S = a[O];
        Array.isArray(S) ? c += (O === 0 ? "" : ", ") + "(" + n.arrayToList(S, i) + ")" : c += (O === 0 ? "" : ", ") + n.escape(S, !0, i);
      }
      return c;
    }, n.format = function(a, i, c, O) {
      if (i == null)
        return a;
      Array.isArray(i) || (i = [i]);
      for (var S = 0, u = /\?+/g, s = "", r = 0, C; r < i.length && (C = u.exec(a)); ) {
        var D = C[0].length;
        if (!(D > 2)) {
          var o = D === 2 ? n.escapeId(i[r]) : n.escape(i[r], c, O);
          s += a.slice(S, C.index) + o, S = u.lastIndex, r++;
        }
      }
      return S === 0 ? a : S < a.length ? s + a.slice(S) : s;
    }, n.dateToString = function(a, i) {
      var c = new Date(a);
      if (isNaN(c.getTime()))
        return "NULL";
      var O, S, u, s, r, C, D;
      if (i === "local")
        O = c.getFullYear(), S = c.getMonth() + 1, u = c.getDate(), s = c.getHours(), r = c.getMinutes(), C = c.getSeconds(), D = c.getMilliseconds();
      else {
        var o = I(i);
        o !== !1 && o !== 0 && c.setTime(c.getTime() + o * 6e4), O = c.getUTCFullYear(), S = c.getUTCMonth() + 1, u = c.getUTCDate(), s = c.getUTCHours(), r = c.getUTCMinutes(), C = c.getUTCSeconds(), D = c.getUTCMilliseconds();
      }
      var B = A(O, 4) + "-" + A(S, 2) + "-" + A(u, 2) + " " + A(s, 2) + ":" + A(r, 2) + ":" + A(C, 2) + "." + A(D, 3);
      return T(B);
    }, n.bufferToString = function(a) {
      return "X" + T(a.toString("hex"));
    }, n.objectToValues = function(a, i) {
      var c = "";
      for (var O in a) {
        var S = a[O];
        typeof S != "function" && (c += (c.length === 0 ? "" : ", ") + n.escapeId(O) + " = " + n.escape(S, !0, i));
      }
      return c;
    }, n.raw = function(a) {
      if (typeof a != "string")
    ...
    function T(t) {
      for (var a = R.lastIndex = 0, i = "", c; c = R.exec(t); )
        i += t.slice(a, c.index) + N[c[0]], a = R.lastIndex;
...
var I_, Ce;
function _e() {
  return Ce || (Ce = 1, I_ = xt()), I_;
}
var LE = {}, De;
function $E() {
  if (De) return LE;
  De = 1, Object.defineProperty(LE, "__esModule", { value: !0 }), LE.createLRU = void 0;
  const E = (n) => {
    let { max: e } = n;
    if (!(Number.isInteger(e) && e > 0))
      throw new TypeError("`max` must be a positive integer");
    let _ = 0, R = 0, N = 0, T = [];
    const { onEviction: A } = n, I = /* @__PURE__ */ new Map(), t = new Array(e).fill(void 0), a = new Array(e).fill(void 0), i = new Array(e).fill(0), c = new Array(e).fill(0), O = (r) => {
      i[N] = r, c[r] = N, i[r] = 0, N = r;
    }, S = (r) => {
      if (r === N)
        return;
      const C = i[r], D = c[r];
      r === R ? R = C : i[D] = C, c[C] = D, O(r);
    }, u = (r) => {
      let C = N;
      const D = Math.min(_, r), o = _ - D, B = new Array(D), M = new Array(D);
      for (let h = 0; h < o; h++) {
        const d = t[R];
        A == null || A(d, a[R]), I.delete(d), R = i[R];
      }
      for (let h = D - 1; h >= 0; h--)
        B[h] = t[C], M[h] = a[C], I.set(t[C], h), C = c[C];
      R = 0, N = D - 1, _ = D, t.length = r, a.length = r, i.length = r, c.length = r;
      for (let h = 0; h < D; h++)
      ...
      t.length = r, a.length = r, i.length = r, c.length = r, t.fill(void 0, e), a.fill(void 0, e), i.fill(0, e), c.fill(0, e);
    };
    return {
      ...
      set(r, C) {
        ...
        if (D === void 0) {
          if (_ === e) {
            D = R;
            const o = t[D];
            A == null || A(o, a[D]), I.delete(o), R = i[D], c[R] = 0;
          } else
            D = T.length > 0 ? T.pop() : _, _++;
          I.set(r, D), t[D] = r, a[D] = C, _ === 1 ? R = N = D : O(D);
        } else
          A == null || A(r, a[D]), a[D] = C, S(D);
      },
      ...
      *entries() {
      ...
      delete(r) {
        ...
        if (C === void 0)
          return !1;
        A == null || A(r, a[C]), I.delete(r), T.push(C), t[C] = void 0, a[C] = void 0;
        const D = c[C], o = i[C];
        return C === R ? R = o : i[D] = o, C === N ? N = D : c[o] = D, _--, !0;
      },
      ...
      evict: (r) => {
        ...
        for (; C > 0; ) {
          const D = R, o = t[D];
          A == null || A(o, a[D]), I.delete(o), t[D] = void 0, a[D] = void 0, R = i[D], c[R] = 0, _--, T.push(D), C--;
        }
        _ === 0 && (R = N = 0);
      },
      ...
      clear() {
        ...
        I.clear(), t.fill(void 0), a.fill(void 0), T = [], _ = 0, R = N = 0;
      },
...
function ne() {
  ...
  function _(T, A, I, t, a) {
    const i = e(T, A, I, t);
    let c = n.get(i);
    return c || (c = a(A, I, t), n.set(i, c), c);
  }
  function R(T) {
    n.resize(T);
  }
...
function KR() {
  ...
  function E(n, _) {
    var _ = _ || {};
    this._capacity = _.capacity, this._head = 0, this._tail = 0, Array.isArray(n) ? this._fromArray(n) : (this._capacityMask = 3, this._list = new Array(4));
  }
  return E.prototype.peekAt = function(e) {
    ...
    if (_ === (_ | 0) && this._head !== this._tail) {
      var R = this.size(), N = this._list.length;
      if (!(_ >= R || _ < -R)) {
        _ < 0 && (_ += R), _ = this._head + _ & this._capacityMask;
        var T = this._list[_], A;
        if (e < R / 2) {
          for (A = e; A > 0; A--)
            this._list[_] = this._list[_ = _ - 1 + N & this._capacityMask];
          this._list[_] = void 0, this._head = this._head + 1 + N & this._capacityMask;
        } else {
          for (A = R - 1 - e; A > 0; A--)
            this._list[_] = this._list[_ = _ + 1 + N & this._capacityMask];
          this._list[_] = void 0, this._tail = this._tail - 1 + N & this._capacityMask;
        }
        return T;
      }
    }
    ...
    if (R === (R | 0) && this._head !== this._tail) {
      var A = this.size(), I = this._list.length;
      if (!(R >= A || R < -A || _ < 1)) {
        if (R < 0 && (R += A), _ === 1 || !_)
          return N = new Array(1), N[0] = this.removeOne(R), N;
        if (R === 0 && R + _ >= A)
          return N = this.toArray(), this.clear(), N;
        R + _ > A && (_ = A - R);
        var t;
        for (N = new Array(_), t = 0; t < _; t++)
          N[t] = this._list[this._head + R + t & this._capacityMask];
        if (R = this._head + R & this._capacityMask, e + _ === A) {
        ...
        if (R < A / 2) {
          for (this._head = this._head + e + _ + I & this._capacityMask, t = e; t > 0; t--)
            this.unshift(this._list[R = R - 1 + I & this._capacityMask]);
          for (R = this._head - 1 + I & this._capacityMask; T > 0; )
            this._list[R = R - 1 + I & this._capacityMask] = void 0, T--;
          e < 0 && (this._tail = R);
        } else {
          for (this._tail = R, R = R + _ + I & this._capacityMask, t = A - (_ + e); t > 0; t--)
            this.push(this._list[R++]);
          for (R = this._tail; T > 0; )
            this._list[R = R + 1 + I & this._capacityMask] = void 0, T--;
        }
        return this._head < 2 && this._tail > 1e4 && this._tail <= I >>> 2 && this._shrinkArray(), N;
      }
    }
    ...
    if (R === (R | 0)) {
      ...
      if (R < 0 && (R += N), !(R > N))
        if (arguments.length > 2) {
          var T, A, I, t = arguments.length, a = this._list.length, i = 2;
          if (!N || R < N / 2) {
            for (A = new Array(R), T = 0; T < R; T++)
              A[T] = this._list[this._head + T & this._capacityMask];
            for (_ === 0 ? (I = [], R > 0 && (this._head = this._head + R + a & this._capacityMask)) : (I = this.remove(R, _), this._head = this._head + R + a & this._capacityMask); t > i; )
            ...
            var c = A.length;
            for (T = 0; T < c; T++)
              A[T] = this._list[this._head + R + _ + T & this._capacityMask];
            for (_ === 0 ? (I = [], R != N && (this._tail = this._head + R + a & this._capacityMask)) : (I = this.remove(R, _), this._tail = this._tail - c + a & this._capacityMask); i < t; )
    ...
    for (var N = 0; N < _; N++) this._list[N] = e[N];
  }, E.prototype._copyArray = function(e, _) {
    var R = this._list, N = R.length, T = this.length;
    if (_ = _ | T, _ == T && this._head < this._tail)
      return this._list.slice(this._head, this._tail);
    var A = new Array(_), I = 0, t;
    if (e || this._head > this._tail) {
      for (t = this._head; t < N; t++) A[I++] = R[t];
      for (t = 0; t < this._tail; t++) A[I++] = R[t];
    } else
      for (t = this._head; t < this._tail; t++) A[I++] = R[t];
    return A;
...
var UE = { exports: {} }, $t = UE.exports, Be;
function Ei() {
  return Be || (Be = 1, (function(E, n) {
    (function(e, _) {
      function R(N) {
        return N.default || N;
      }
      ...
      typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : $t,
      function(e) {
        ...
        function t(f, l) {
          var g, V, W;
          return l ? (f >>>= 0, (W = 0 <= f && f < 256) && (V = I[f], V) ? V : (g = i(f, 0, !0), W && (I[f] = g), g)) : (f |= 0, (W = -128 <= f && f < 128) && (V = A[f], V) ? V : (g = i(f, f < 0 ? -1 : 0, !1), W && (A[f] = g), g));
        }
        R.fromInt = t;
        function a(f, l) {
        ...
        R.fromString = O;
        function S(f, l) {
          return typeof f == "number" ? a(f, l) : typeof f == "string" ? O(f, l) : i(
            f.low,
            f.high,
        ...
        R.fromValue = S;
        var u = 65536, s = 1 << 24, r = u * u, C = r * r, D = C / 2, o = t(s), B = t(0);
        R.ZERO = B;
        var M = t(0, !0);
        R.UZERO = M;
        var h = t(1);
        ...
        L.toInt = function() {
          ...
          if (this.isNegative())
            if (this.eq(w)) {
              var g = a(l), V = this.div(g), W = V.mul(g).sub(this);
              return V.toString(l) + W.toInt().toString(l);
          ...
          return N(l) || (l = S(l)), this.unsigned !== l.unsigned && this.high >>> 31 === 1 && l.high >>> 31 === 1 ? !1 : this.high === l.high && this.low === l.low;
          ...
          if (N(l) || (l = S(l)), this.eq(l)) return 0;
          ...
          N(l) || (l = S(l));
          var g = this.high >>> 16, V = this.high & 65535, W = this.low >>> 16, H = this.low & 65535, k = l.high >>> 16, X = l.high & 65535, EE = l.low >>> 16, q = l.low & 65535, j = 0, _E = 0, J = 0, $ = 0;
          return $ += H + q, J += $ >>> 16, $ &= 65535, J += W + EE, _E += J >>> 16, J &= 65535, _E += V + X, j += _E >>> 16, _E &= 65535, j += g + k, j &= 65535, i(J << 16 | $, j << 16 | _E, this.unsigned);
        }, L.subtract = function(l) {
          return N(l) || (l = S(l)), this.add(l.neg());
          ...
          var V = this.high >>> 16, W = this.high & 65535, H = this.low >>> 16, k = this.low & 65535, X = l.high >>> 16, EE = l.high & 65535, q = l.low >>> 16, j = l.low & 65535, _E = 0, J = 0, $ = 0, VE = 0;
          return VE += k * j, $ += VE >>> 16, VE &= 65535, $ += H * j, J += $ >>> 16, $ &= 65535, $ += k * q, J += $ >>> 16, $ &= 65535, J += W * j, _E += J >>> 16, J &= 65535, J += H * q, _E += J >>> 16, J &= 65535, J += k * EE, _E += J >>> 16, J &= 65535, _E += V * j + W * q + H * EE + k * X, _E &= 65535, i($ << 16 | VE, _E << 16 | J, this.unsigned);
        }, L.mul = L.multiply, L.divide = function(l) {
          if (N(l) || (l = S(l)), l.isZero()) throw Error("division by zero");
          if (_) {
          ...
          if (this.unsigned) {
            ...
            if (this.eq(w)) {
              ...
              if (l.eq(w)) return h;
              var k = this.shr(1);
              return V = k.div(l).shl(1), V.eq(B) ? l.isNegative() ? h : U : (W = this.sub(l.mul(V)), H = V.add(W.div(l)), H);
          ...
          for (W = this; W.gte(l); ) {
            V = Math.max(1, Math.floor(W.toNumber() / l.toNumber()));
            for (var X = Math.ceil(Math.log(V) / Math.LN2), EE = X <= 48 ? 1 : c(2, X - 48), q = a(V), j = q.mul(l); j.isNegative() || j.gt(W); )
              V -= EE, q = a(V, this.unsigned), j = q.mul(l);
          ...
          return N(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l < 32 ? i(
            this.low << l,
          ...
          var g;
          return N(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l === 32 ? i(this.high, this.low, this.unsigned) : l < 32 ? (g = 32 - l, i(
          ...
          var V = Number(BigInt.asIntN(32, l)), W = Number(BigInt.asIntN(32, l >> BigInt(32)));
          return i(V, W, g);
...
var mE = {}, XE = { exports: {} }, c_, Me;
function TE() {
  if (Me) return c_;
  Me = 1;
  var E = WR, n = E.Buffer, e = {}, _;
  for (_ in E)
    E.hasOwnProperty(_) && (_ === "SlowBuffer" || _ === "Buffer" || (e[_] = E[_]));
  var R = e.Buffer = {};
  for (_ in n)
    n.hasOwnProperty(_) && (_ === "allocUnsafe" || _ === "allocUnsafeSlow" || (R[_] = n[_]));
  if (e.Buffer.prototype = n.prototype, (!R.from || R.from === Uint8Array.from) && (R.from = function(N, T, A) {
    if (typeof N == "number")
    ...
    return n(N, T, A);
  }), R.alloc || (R.alloc = function(N, T, A) {
    if (typeof N != "number")
    ...
    var I = n(N);
    return !T || T.length === 0 ? I.fill(0) : typeof A == "string" ? I.fill(T, A) : I.fill(T), I;
  }), !e.kStringMaxLength)
    try {
  ...
  }, e.kStringMaxLength && (e.constants.MAX_STRING_LENGTH = e.kStringMaxLength)), c_ = e, c_;
}
var YE = {}, he;
function _i() {
  if (he) return YE;
...
function ni() {
  ...
  function n(t, a) {
    this.enc = t.encodingName, this.bomAware = t.bomAware, this.enc === "base64" ? this.encoder = N : this.enc === "utf8" ? this.encoder = I : this.enc === "cesu8" && (this.enc = "utf8", this.encoder = T, E.from("eda0bdedb2a9", "hex").toString() !== "💩" && (this.decoder = A, this.defaultCharUnicode = a.defaultCharUnicode));
  }
  n.prototype.encoder = R, n.prototype.decoder = _;
  var e = mt.StringDecoder;
  function _(t, a) {
    this.decoder = new e(a.enc);
  }
  ...
  T.prototype.write = function(t) {
    for (var a = E.alloc(t.length * 3), i = 0, c = 0; c < t.length; c++) {
      ...
      O < 128 ? a[i++] = O : O < 2048 ? (a[i++] = 192 + (O >>> 6), a[i++] = 128 + (O & 63)) : (a[i++] = 224 + (O >>> 12), a[i++] = 128 + (O >>> 6 & 63), a[i++] = 128 + (O & 63));
    }
    return a.slice(0, i);
  ...
  A.prototype.write = function(t) {
    for (var a = this.acc, i = this.contBytes, c = this.accBytes, O = "", S = 0; S < t.length; S++) {
      var u = t[S];
      (u & 192) !== 128 ? (i > 0 && (O += this.defaultCharUnicode, i = 0), u < 128 ? O += String.fromCharCode(u) : u < 224 ? (a = u & 31, i = 1, c = 1) : u < 240 ? (a = u & 15, i = 2, c = 1) : O += this.defaultCharUnicode) : i > 0 ? (a = a << 6 | u & 63, i--, c++, i === 0 && (c === 2 && a < 128 && a > 0 ? O += this.defaultCharUnicode : c === 3 && a < 2048 ? O += this.defaultCharUnicode : O += String.fromCharCode(a))) : O += this.defaultCharUnicode;
    }
    return this.acc = a, this.contBytes = i, this.accBytes = c, O;
...
function ei() {
  ...
  aE.utf32le = { type: "_utf32", isLE: !0 }, aE.utf32be = { type: "_utf32", isLE: !1 }, aE.ucs4le = "utf32le", aE.ucs4be = "utf32be", n.prototype.encoder = e, n.prototype.decoder = _;
  function e(t, a) {
    this.isLE = a.isLE, this.highSurrogate = 0;
  }
  e.prototype.write = function(t) {
    for (var a = E.from(t, "ucs2"), i = E.alloc(a.length * 2), c = this.isLE ? i.writeUInt32LE : i.writeUInt32BE, O = 0, S = 0; S < a.length; S += 2) {
      var u = a.readUInt16LE(S), s = u >= 55296 && u < 56320, r = u >= 56320 && u < 57344;
      if (this.highSurrogate)
        if (s || !r)
          c.call(i, this.highSurrogate, O), O += 4;
        else {
          var C = (this.highSurrogate - 55296 << 10 | u - 56320) + 65536;
          c.call(i, C, O), O += 4, this.highSurrogate = 0;
          continue;
        }
      s ? this.highSurrogate = u : (c.call(i, u, O), O += 4, this.highSurrogate = 0);
    }
  ...
  _.prototype.write = function(t) {
    if (t.length === 0)
      return "";
    var a = 0, i = 0, c = E.alloc(t.length + 4), O = 0, S = this.isLE, u = this.overflow, s = this.badChar;
    if (u.length > 0) {
      for (; a < t.length && u.length < 4; a++)
        u.push(t[a]);
      u.length === 4 && (S ? i = u[a] | u[a + 1] << 8 | u[a + 2] << 16 | u[a + 3] << 24 : i = u[a + 3] | u[a + 2] << 8 | u[a + 1] << 16 | u[a] << 24, u.length = 0, O = R(c, O, i, s));
    }
    for (; a < t.length - 3; a += 4)
      S ? i = t[a] | t[a + 1] << 8 | t[a + 2] << 16 | t[a + 3] << 24 : i = t[a + 3] | t[a + 2] << 8 | t[a + 1] << 16 | t[a] << 24, O = R(c, O, i, s);
    for (; a < t.length; a++)
      u.push(t[a]);
    return c.slice(0, O).toString("ucs2");
  };
  function R(t, a, i, c) {
  ...
  }, aE.utf32 = N, aE.ucs4 = "utf32";
  function N(t, a) {
    this.iconv = a;
  }
  N.prototype.encoder = T, N.prototype.decoder = A;
  function T(t, a) {
  ...
  function I(t, a) {
    var i = [], c = 0, O = 0, S = 0, u = 0, s = 0;
    E:
      for (var r = 0; r < t.length; r++)
        for (var C = t[r], D = 0; D < C.length; D++)
          if (i.push(C[D]), i.length === 4) {
            ...
            if ((i[0] !== 0 || i[1] > 16) && S++, (i[3] !== 0 || i[2] > 16) && O++, i[0] === 0 && i[1] === 0 && (i[2] !== 0 || i[3] !== 0) && s++, (i[0] !== 0 || i[1] !== 0) && i[2] === 0 && i[3] === 0 && u++, i.length = 0, c++, c >= 100)
              break E;
          }
    return s - S > u - O ? "utf-32be" : s - S < u - O ? "utf-32le" : a || "utf-32le";
  }
  return aE;
}
var WE = {}, de;
function Ai() {
  ...
  n.prototype.encoder = e, n.prototype.decoder = _, n.prototype.bomAware = !0;
  function e() {
  }
  ...
  R.prototype.encoder = N, R.prototype.decoder = T;
  function N(I, t) {
  ...
  T.prototype.write = function(I) {
    ...
    if (!this.decoder) {
      var I = A(this.initialBufs, this.options.defaultEncoding);
  ...
  function A(I, t) {
    ...
    E:
      for (var S = 0; S < I.length; S++)
        for (var u = I[S], s = 0; s < u.length; s++)
          if (a.push(u[s]), a.length === 2) {
            if (i === 0) {
              if (a[0] === 255 && a[1] === 254) return "utf-16le";
              if (a[0] === 254 && a[1] === 255) return "utf-16be";
            }
            if (a[0] === 0 && a[1] !== 0 && O++, a[0] !== 0 && a[1] === 0 && c++, a.length = 0, i++, i >= 100)
              break E;
          }
    return O > c ? "utf-16be" : O < c ? "utf-16le" : t || "utf-16le";
  }
  return WE;
}
var fE = {}, Ue;
function Ri() {
  ...
  Ue = 1;
  var E = TE().Buffer;
  fE.utf7 = n, fE.unicode11utf7 = "utf7";
  function n(u, s) {
    this.iconv = s;
  }
  n.prototype.encoder = _, n.prototype.decoder = R, n.prototype.bomAware = !0;
  var e = /[^A-Za-z0-9'\(\),-\.\/:\? \n\r\t]+/g;
  function _(u, s) {
    this.iconv = s.iconv;
  }
  _.prototype.write = function(u) {
    return E.from(u.replace(e, (function(s) {
  ...
  for (var N = /[A-Za-z0-9\/+]/, T = [], A = 0; A < 256; A++)
    T[A] = N.test(String.fromCharCode(A));
  var I = 43, t = 45, a = 38;
  R.prototype.write = function(u) {
    for (var s = "", r = 0, C = this.inBase64, D = this.base64Accum, o = 0; o < u.length; o++)
      if (!C)
        u[o] == I && (s += this.iconv.decode(u.slice(r, o), "ascii"), r = o + 1, C = !0);
      else if (!T[u[o]]) {
        if (o == r && u[o] == t)
          s += "+";
        else {
          var B = D + this.iconv.decode(u.slice(r, o), "ascii");
          s += this.iconv.decode(E.from(B, "base64"), "utf16-be");
        }
        u[o] != t && o--, r = o + 1, C = !1, D = "";
      }
    if (!C)
      s += this.iconv.decode(u.slice(r), "ascii");
    else {
      var B = D + this.iconv.decode(u.slice(r), "ascii"), M = B.length - B.length % 8;
      D = B.slice(M), B = B.slice(0, M), s += this.iconv.decode(E.from(B, "base64"), "utf16-be");
    }
  ...
  i.prototype.encoder = c, i.prototype.decoder = O, i.prototype.bomAware = !0;
  function c(u, s) {
    this.iconv = s.iconv, this.inBase64 = !1, this.base64Accum = E.alloc(6), this.base64AccumIdx = 0;
  }
  c.prototype.write = function(u) {
    for (var s = this.inBase64, r = this.base64Accum, C = this.base64AccumIdx, D = E.alloc(u.length * 5 + 10), o = 0, B = 0; B < u.length; B++) {
      var M = u.charCodeAt(B);
      M >= 32 && M <= 126 ? (s && (C > 0 && (o += D.write(r.slice(0, C).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), o), C = 0), D[o++] = t, s = !1), s || (D[o++] = M, M === a && (D[o++] = t))) : (s || (D[o++] = a, s = !0), s && (r[C++] = M >> 8, r[C++] = M & 255, C == r.length && (o += D.write(r.toString("base64").replace(/\//g, ","), o), C = 0)));
    }
    return this.inBase64 = s, this.base64AccumIdx = C, D.slice(0, o);
  }, c.prototype.end = function() {
    var u = E.alloc(10), s = 0;
    return this.inBase64 && (this.base64AccumIdx > 0 && (s += u.write(this.base64Accum.slice(0, this.base64AccumIdx).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), s), this.base64AccumIdx = 0), u[s++] = t, this.inBase64 = !1), u.slice(0, s);
  };
  function O(u, s) {
    this.iconv = s.iconv, this.inBase64 = !1, this.base64Accum = "";
  }
  var S = T.slice();
  return S[44] = !0, O.prototype.write = function(u) {
    for (var s = "", r = 0, C = this.inBase64, D = this.base64Accum, o = 0; o < u.length; o++)
      if (!C)
        u[o] == a && (s += this.iconv.decode(u.slice(r, o), "ascii"), r = o + 1, C = !0);
      else if (!S[u[o]]) {
        if (o == r && u[o] == t)
          s += "&";
        else {
          var B = D + this.iconv.decode(u.slice(r, o), "ascii").replace(/,/g, "/");
          s += this.iconv.decode(E.from(B, "base64"), "utf16-be");
        }
        u[o] != t && o--, r = o + 1, C = !1, D = "";
      }
    if (!C)
      ...
      var B = D + this.iconv.decode(u.slice(r), "ascii").replace(/,/g, "/"), M = B.length - B.length % 8;
      D = B.slice(M), B = B.slice(0, M), s += this.iconv.decode(E.from(B, "base64"), "utf16-be");
    }
    return this.inBase64 = C, this.base64Accum = D, s;
...
function ti() {
  ...
  function n(R, N) {
    ...
    for (var I = E.alloc(65536, N.defaultCharSingleByte.charCodeAt(0)), A = 0; A < R.chars.length; A++)
      I[R.chars.charCodeAt(A)] = A;
    this.encodeBuf = I;
  }
  n.prototype.encoder = e, n.prototype.decoder = _;
  function e(R, N) {
    this.encodeBuf = N.encodeBuf;
  }
  e.prototype.write = function(R) {
    for (var N = E.alloc(R.length), T = 0; T < R.length; T++)
      N[T] = this.encodeBuf[R.charCodeAt(T)];
    return N;
  }, e.prototype.end = function() {
  };
  function _(R, N) {
  ...
  return _.prototype.write = function(R) {
    for (var N = this.decodeBuf, T = E.alloc(R.length * 2), A = 0, I = 0, t = 0; t < R.length; t++)
      A = R[t] * 2, I = t * 2, T[I] = N[A], T[I + 1] = N[A + 1];
...
function Ii() {
  ...
  var E = TE().Buffer;
  o_._dbcs = I;
  for (var n = -1, e = -2, _ = -10, R = -1e3, N = new Array(256), T = -1, A = 0; A < 256; A++)
    N[A] = n;
  function I(c, O) {
    ...
    if (typeof c.gb18030 == "function") {
      ...
      for (var C = this.decodeTables[0], u = 129; u <= 254; u++)
        for (var D = this.decodeTables[R - C[u]], o = 48; o <= 57; o++) {
          if (D[o] === n)
            D[o] = R - s;
          else if (D[o] > R)
            throw new Error("gb18030 decode tables conflict at byte 2");
          for (var B = this.decodeTables[R - D[o]], M = 129; M <= 254; M++) {
            if (B[M] === n)
              B[M] = R - r;
            else {
              if (B[M] === R - r)
            ...
            for (var h = this.decodeTables[R - B[M]], d = 48; d <= 57; d++)
              h[d] === n && (h[d] = e);
          }
        }
    }
    ...
    if (c.encodeSkipVals)
      for (var u = 0; u < c.encodeSkipVals.length; u++) {
        var b = c.encodeSkipVals[u];
        if (typeof b == "number")
          U[b] = !0;
        else
          for (var o = b.from; o <= b.to; o++)
            U[o] = !0;
      }
    ...
    this.defCharSB = this.encodeTable[0][O.defaultCharSingleByte.charCodeAt(0)], this.defCharSB === n && (this.defCharSB = this.encodeTable[0]["?"]), this.defCharSB === n && (this.defCharSB = 63);
  }
  I.prototype.encoder = t, I.prototype.decoder = a, I.prototype._getDecodeTrieNode = function(c) {
    for (var O = []; c > 0; c >>>= 8)
      O.push(c & 255);
    O.length == 0 && O.push(0);
    for (var S = this.decodeTables[0], u = O.length - 1; u > 0; u--) {
      var s = S[O[u]];
      if (s == n)
        S[O[u]] = R - this.decodeTables.length, this.decodeTables.push(S = N.slice(0));
      else if (s <= R)
    ...
    var O = parseInt(c[0], 16), S = this._getDecodeTrieNode(O);
    O = O & 255;
      ...
      if (typeof s == "string")
        for (var r = 0; r < s.length; ) {
          ...
          if (C >= 55296 && C < 56320) {
            ...
            for (var o = 4095 - C + 2, B = [], M = 0; M < o; M++)
              B.push(s.charCodeAt(r++));
            S[O++] = _ - this.decodeTableSeq.length, this.decodeTableSeq.push(B);
          } else
            S[O++] = C;
        }
      else if (typeof s == "number")
        for (var h = S[O - 1] + 1, r = 0; r < s; r++)
          S[O++] = h++;
      else
        throw new Error("Incorrect type '" + typeof s + "' given in " + this.encodingName + " at chunk " + c[0]);
    }
    if (O > 255)
      throw new Error("Incorrect chunk in " + this.encodingName + " at addr " + c[0] + ": too long" + O);
  }, I.prototype._getEncodeBucket = function(c) {
    var O = c >> 8;
    return this.encodeTable[O] === void 0 && (this.encodeTable[O] = N.slice(0)), this.encodeTable[O];
  }, I.prototype._setEncodeChar = function(c, O) {
    var S = this._getEncodeBucket(c), u = c & 255;
    S[u] <= _ ? this.encodeTableSeq[_ - S[u]][T] = O : S[u] == n && (S[u] = O);
  }, I.prototype._setEncodeSequence = function(c, O) {
    var S = c[0], u = this._getEncodeBucket(S), s = S & 255, r;
    u[s] <= _ ? r = this.encodeTableSeq[_ - u[s]] : (r = {}, u[s] !== n && (r[T] = u[s]), u[s] = _ - this.encodeTableSeq.length, this.encodeTableSeq.push(r));
    for (var C = 1; C < c.length - 1; C++) {
      var D = r[S];
      typeof D == "object" ? r = D : (r = r[S] = {}, D !== void 0 && (r[T] = D));
    }
    S = c[c.length - 1], r[S] = O;
  }, I.prototype._fillEncodeTable = function(c, O, S) {
    for (var u = this.decodeTables[c], s = !1, r = {}, C = 0; C < 256; C++) {
      var D = u[C], o = O + C;
      if (!S[o])
        if (D >= 0)
          this._setEncodeChar(D, o), s = !0;
        else if (D <= R) {
          var B = R - D;
          if (!r[B]) {
            var M = o << 8 >>> 0;
            this._fillEncodeTable(B, M, S) ? s = !0 : r[B] = !0;
          }
        } else D <= _ && (this._setEncodeSequence(this.decodeTableSeq[_ - D], o), s = !0);
    }
    return s;
  };
  function t(c, O) {
  ...
  t.prototype.write = function(c) {
    for (var O = E.alloc(c.length * (this.gb18030 ? 4 : 3)), S = this.leadSurrogate, u = this.seqObj, s = -1, r = 0, C = 0; ; ) {
      if (s === -1) {
        if (r == c.length) break;
      ...
      if (D >= 55296 && D < 57344)
        if (D < 56320)
          ...
          S !== -1 ? (D = 65536 + (S - 55296) * 1024 + (D - 56320), S = -1) : D = n;
      else S !== -1 && (s = D, D = n, S = -1);
      var o = n;
      if (u !== void 0 && D != n) {
        var B = u[D];
        if (typeof B == "object") {
          u = B;
          continue;
        } else typeof B == "number" ? o = B : B == null && (B = u[T], B !== void 0 && (o = B, s = D));
        u = void 0;
      } else if (D >= 0) {
        var M = this.encodeTable[D >> 8];
        if (M !== void 0 && (o = M[D & 255]), o <= _) {
          u = this.encodeTableSeq[_ - o];
          continue;
        }
        if (o == n && this.gb18030) {
          var h = i(this.gb18030.uChars, D);
          if (h != -1) {
            var o = this.gb18030.gbChars[h] + (D - this.gb18030.uChars[h]);
            O[C++] = 129 + Math.floor(o / 12600), o = o % 12600, O[C++] = 48 + Math.floor(o / 1260), o = o % 1260, O[C++] = 129 + Math.floor(o / 10), o = o % 10, O[C++] = 48 + o;
            continue;
          }
        }
      }
      o === n && (o = this.defaultCharSingleByte), o < 256 ? O[C++] = o : o < 65536 ? (O[C++] = o >> 8, O[C++] = o & 255) : o < 16777216 ? (O[C++] = o >> 16, O[C++] = o >> 8 & 255, O[C++] = o & 255) : (O[C++] = o >>> 24, O[C++] = o >>> 16 & 255, O[C++] = o >>> 8 & 255, O[C++] = o & 255);
    }
    return this.seqObj = u, this.leadSurrogate = S, O.slice(0, C);
  }, t.prototype.end = function() {
    if (!(this.leadSurrogate === -1 && this.seqObj === void 0)) {
      var c = E.alloc(10), O = 0;
      if (this.seqObj) {
        var S = this.seqObj[T];
        S !== void 0 && (S < 256 ? c[O++] = S : (c[O++] = S >> 8, c[O++] = S & 255)), this.seqObj = void 0;
      }
      return this.leadSurrogate !== -1 && (c[O++] = this.defaultCharSingleByte, this.leadSurrogate = -1), c.slice(0, O);
    }
  ...
  a.prototype.write = function(c) {
    for (var O = E.alloc(c.length * 2), S = this.nodeIdx, u = this.prevBytes, s = this.prevBytes.length, r = -this.prevBytes.length, C, D = 0, o = 0; D < c.length; D++) {
      var B = D >= 0 ? c[D] : u[D + s], C = this.decodeTables[S][B];
      if (!(C >= 0)) if (C === n)
        C = this.defaultCharUnicode.charCodeAt(0), D = r;
      else if (C === e) {
        if (D >= 3)
          var M = (c[D - 3] - 129) * 12600 + (c[D - 2] - 48) * 1260 + (c[D - 1] - 129) * 10 + (B - 48);
        else
          var M = (u[D - 3 + s] - 129) * 12600 + ((D - 2 >= 0 ? c[D - 2] : u[D - 2 + s]) - 48) * 1260 + ((D - 1 >= 0 ? c[D - 1] : u[D - 1 + s]) - 129) * 10 + (B - 48);
        ...
        for (var d = this.decodeTableSeq[_ - C], U = 0; U < d.length - 1; U++)
          C = d[U], O[o++] = C & 255, O[o++] = C >> 8;
        C = d[d.length - 1];
      } else
        throw new Error("iconv-lite internal error: invalid decoding table value " + C + " at " + S + "/" + B);
      if (C >= 65536) {
        C -= 65536;
        var b = 55296 | C >> 10;
        O[o++] = b & 255, O[o++] = b >> 8, C = 56320 | C & 1023;
      }
      O[o++] = C & 255, O[o++] = C >> 8, S = 0, r = D + 1;
    }
    return this.nodeIdx = S, this.prevBytes = r >= 0 ? Array.prototype.slice.call(c, r) : u.slice(r + s).concat(Array.prototype.slice.call(c)), O.slice(0, o).toString("ucs2");
  }, a.prototype.end = function() {
    for (var c = ""; this.prevBytes.length > 0; ) {
      c += this.defaultCharUnicode;
      var O = this.prevBytes.slice(1);
      this.prevBytes = [], this.nodeIdx = 0, O.length > 0 && (c += this.write(O));
    }
...
function Si() {
  ...
  Ye = 1;
  var E = TE().Buffer;
  return M_ = function(n) {
    var e = n.Transform;
    function _(N, T) {
      this.conv = N, T = T || {}, T.decodeStrings = !1, e.call(this, T);
    }
    _.prototype = Object.create(e.prototype, {
      constructor: { value: _ }
    }), _.prototype._transform = function(N, T, A) {
      if (typeof N != "string")
        return A(new Error("Iconv encoding stream needs strings as its input."));
      try {
        var I = this.conv.write(N);
        I && I.length && this.push(I), A();
      } catch (t) {
        A(t);
      }
    ...
    }, _.prototype.collect = function(N) {
      var T = [];
    ...
    function R(N, T) {
      this.conv = N, T = T || {}, T.encoding = this.encoding = "utf8", e.call(this, T);
    }
    return R.prototype = Object.create(e.prototype, {
      constructor: { value: R }
    }), R.prototype._transform = function(N, T, A) {
      if (!E.isBuffer(N) && !(N instanceof Uint8Array))
        return A(new Error("Iconv decoding stream needs buffers as its input."));
      try {
        var I = this.conv.write(N);
        I && I.length && this.push(I, this.encoding), A();
      } catch (t) {
        A(t);
      }
      ...
      try {
        var T = this.conv.end();
        T && T.length && this.push(T, this.encoding), N();
      } catch (A) {
        N(A);
      }
...
function oi() {
  return We || (We = 1, (function(E) {
    var n = TE().Buffer, e = _i(), _ = jR();
    E.exports.encodings = null, E.exports.defaultCharUnicode = "�", E.exports.defaultCharSingleByte = "?", E.exports.encode = function(T, A, I) {
      T = "" + (T || "");
      var t = E.exports.getEncoder(A, I), a = t.write(T), i = t.end();
      return i && i.length > 0 ? n.concat([a, i]) : a;
    }, E.exports.decode = function(T, A, I) {
      typeof T == "string" && (E.exports.skipDecodeWarning || (console.error("Iconv-lite warning: decode()-ing strings is deprecated. Refer to https://github.com/ashtuchkin/iconv-lite/wiki/Use-Buffers-when-decoding"), E.exports.skipDecodeWarning = !0), T = n.from("" + (T || ""), "binary"));
      var t = E.exports.getDecoder(A, I), a = t.write(T), i = t.end();
      return i ? a + i : a;
      ...
      if (!E.exports.encodings) {
        var A = Di();
        E.exports.encodings = { __proto__: null }, _(E.exports.encodings, A);
      }
      for (var I = E.exports._canonicalizeEncoding(T), t = {}; ; ) {
        ...
        switch (typeof i) {
          ...
          default:
            throw new Error("Encoding not recognized: '" + T + "' (searched as: '" + I + "')");
        }
      }
    }, E.exports._canonicalizeEncoding = function(N) {
      return ("" + N).toLowerCase().replace(/:\d{4}$|[^0-9a-z]/g, "");
    }, E.exports.getEncoder = function(T, A) {
      var I = E.exports.getCodec(T), t = new I.encoder(A, I);
      return I.bomAware && A && A.addBOM && (t = new e.PrependBOM(t, A)), t;
    }, E.exports.getDecoder = function(T, A) {
      var I = E.exports.getCodec(T), t = new I.decoder(A, I);
      return I.bomAware && !(A && A.stripBOM === !1) && (t = new e.StripBOM(t, A)), t;
    }, E.exports.enableStreamingAPI = function(T) {
      if (!E.exports.supportsStreams) {
        var A = Si()(T);
        E.exports.IconvLiteEncoderStream = A.IconvLiteEncoderStream, E.exports.IconvLiteDecoderStream = A.IconvLiteDecoderStream, E.exports.encodeStream = function(t, a) {
...
function E_() {
  if (He) return mE;
  He = 1;
  const E = oi(), { createLRU: n } = $E(), e = n({
    max: 500
  });
  return mE.decode = function(_, R, N, T, A) {
    if (Buffer.isEncoding(R))
      return _.toString(R, N, T);
    let I;
    if (!A)
      I = e.get(R), I || (I = E.getDecoder(R), e.set(R, I));
    else {
      const i = { encoding: R, options: A }, c = JSON.stringify(i);
      I = e.get(c), I || (I = E.getDecoder(i.encoding, i.options), e.set(c, I));
    }
    const t = I.write(_.slice(N, T)), a = I.end();
    return a ? t + a : t;
  }, mE.encode = function(_, R, N) {
    if (Buffer.isEncoding(R))
      return Buffer.from(_, R);
    const T = E.getEncoder(R, N || {}), A = T.write(_), I = T.end();
    return I && I.length > 0 ? Buffer.concat([A, I]) : A;
  }, mE;
}
var Y = { exports: {} }, ve;
function IE() {
  return ve || (ve = 1, Y.exports = {
    0: "DECIMAL",
    // aka DECIMAL
    1: "TINY",
    // aka TINYINT, 1 byte
...
function K() {
  if (ye) return h_;
  ye = 1;
  const E = kR(), n = WR.Buffer, e = Ei(), _ = E_(), R = IE(), N = /* @__PURE__ */ new Date(NaN), T = "000000000000";
  function A(S, u) {
    const s = u.toString();
    return s.length >= S ? s : (T + s).slice(-S);
  }
  const I = 45, t = 43, a = 46, i = 101, c = 69;
  class O {
    constructor(u, s, r, C) {
      this.sequenceId = u, this.numPackets = 1, this.buffer = s, this.start = r, this.offset = r + 4, this.end = C;
    }
    // ==============================
    // readers
    // ==============================
    ...
    readInt64JSNumber() {
      const u = this.readInt32(), s = this.readInt32();
      return new e(u, s, !0).toNumber();
    }
    readSInt64JSNumber() {
      const u = this.readInt32(), s = this.readInt32();
      return s & 2147483648 ? new e(u, s, !1).toNumber() : u + 4294967296 * s;
    }
    readInt64String() {
      const u = this.readInt32(), s = this.readInt32();
      return new e(u, s, !0).toString();
    }
    readSInt64String() {
      const u = this.readInt32(), s = this.readInt32();
      return new e(u, s, !1).toString();
    }
    readInt64() {
      const u = this.readInt32(), s = this.readInt32();
      let r = new e(u, s, !0);
    ...
    readLengthCodedNumber(u, s) {
      const r = this.buffer[this.offset++];
      return r < 251 ? r : this.readLengthCodedNumberExt(r, u, s);
    }
    readLengthCodedNumberSigned(u) {
      return this.readLengthCodedNumber(u, !0);
    }
    readLengthCodedNumberExt(u, s, r) {
      let C, D, o;
      if (u === 251)
        return null;
      if (u === 252)
      ...
      if (u === 254) {
        ...
        o = new e(C, D, !r);
        const B = o.toNumber(), M = o.toString();
        return o = B.toString() === M ? B : M, s ? M : o;
      }
    ...
    readDateTime(u) {
      if (!u || u === "Z" || u === "local") {
        ...
        if (r === 251)
          return null;
        let C = 0, D = 0, o = 0, B = 0, M = 0, h = 0, d = 0;
        return r > 3 && (C = this.readInt16(), D = this.readInt8(), o = this.readInt8()), r > 6 && (B = this.readInt8(), M = this.readInt8(), h = this.readInt8()), r > 10 && (d = this.readInt32() / 1e3), C + D + o + B + M + h + d === 0 ? N : u === "Z" ? new Date(Date.UTC(C, D - 1, o, B, M, h, d)) : new Date(C, D - 1, o, B, M, h, d);
      }
      let s = this.readDateTimeString(6, "T", null);
      return s.length === 10 && (s += "T00:00:00"), new Date(s + u);
    }
    readDateTimeString(u, s, r) {
      const C = this.readInt8();
      let D = 0, o = 0, B = 0, M = 0, h = 0, d = 0, U = 0, b;
      return C > 3 && (D = this.readInt16(), o = this.readInt8(), B = this.readInt8(), b = [A(4, D), A(2, o), A(2, B)].join("-")), C > 6 ? (M = this.readInt8(), h = this.readInt8(), d = this.readInt8(), b += `${s || " "}${[
        A(2, M),
        A(2, h),
        A(2, d)
      ].join(":")}`) : r === R.DATETIME && (b += " 00:00:00"), C > 10 && (U = this.readInt32(), b += ".", u && (U = A(6, U), U.length > u && (U = U.substring(0, u))), b += U), b;
    }
    ...
    readTimeString(u) {
      ...
      let C = 0, D = 0, o = 0, B = 0, M = 0;
      return s > 6 && (C = this.readInt32(), D = this.readInt8(), o = this.readInt8(), B = this.readInt8()), s > 10 && (M = this.readInt32()), u ? (D += C * 24, o += D * 60, B += o * 60, M += B * 1e3, M *= r, M) : (r === -1 ? "-" : "") + [A(2, C * 24 + D), A(2, o), A(2, B)].join(":") + (M ? `.${M}`.replace(/0+$/, "") : "");
    }
    ...
    readNullTerminatedString(u) {
      ...
      for (; this.buffer[r]; )
        r = r + 1;
      return this.offset = r + 1, _.decode(this.buffer, u, s, r);
    }
    // TODO reuse?
    readString(u, s) {
      return typeof u == "string" && typeof s > "u" && (s = u, u = void 0), typeof u > "u" && (u = this.end - this.offset), this.offset += u, _.decode(
        this.buffer,
        s,
        this.offset - u,
        this.offset
    ...
    parseInt(u, s) {
      ...
      let r = 0;
      const C = this.offset, D = this.offset + u;
      let o = 1;
      if (u === 0)
        return 0;
      this.buffer[this.offset] === I && (this.offset++, o = -1);
      let B;
      const M = D - this.offset;
      if (s) {
        if (M >= 15)
          return B = this.readString(D - this.offset, "binary"), r = parseInt(B, 10), r.toString() === B ? o * r : o === -1 ? `-${B}` : B;
        if (M > 16)
          return B = this.readString(D - this.offset), o === -1 ? `-${B}` : B;
      }
      for (this.buffer[this.offset] === t && this.offset++; this.offset < D; )
        r *= 10, r += this.buffer[this.offset] - 48, this.offset++;
      const h = r * o;
      return !s || (B = this.buffer.toString("ascii", C, D), h.toString() === B) ? h : B;
    }
    ...
    parseGeometryValue() {
      ...
      if (u === null || !u.length)
        return null;
      function r() {
        let C, D, o, B, M, h, d = null;
        const U = u.readUInt8(s);
        s += 1;
        const b = U ? u.readUInt32LE(s) : u.readUInt32BE(s);
        switch (s += 4, b) {
          case 1:
            C = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, D = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, d = { x: C, y: D };
            break;
          case 2:
            for (M = U ? u.readUInt32LE(s) : u.readUInt32BE(s), s += 4, d = [], o = M; o > 0; o--)
              C = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, D = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, d.push({ x: C, y: D });
            break;
          case 3:
            const P = U ? u.readUInt32LE(s) : u.readUInt32BE(s);
            for (s += 4, d = [], o = P; o > 0; o--) {
              for (M = U ? u.readUInt32LE(s) : u.readUInt32BE(s), s += 4, h = [], B = M; B > 0; B--)
                C = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, D = U ? u.readDoubleLE(s) : u.readDoubleBE(s), s += 8, h.push({ x: C, y: D });
              d.push(h);
            }
            break;
          case 4:
          ...
          // WKBMultiPolygon
          case 7:
            const w = U ? u.readUInt32LE(s) : u.readUInt32BE(s);
            for (s += 4, d = [], o = w; o > 0; o--)
              d.push(r());
            break;
        }
    ...
    parseVector() {
      const u = this.readLengthCodedNumber(), s = this.offset + u, r = [];
    ...
    parseDate(u) {
      ...
      return !u || u === "local" ? new Date(r, C - 1, D) : u === "Z" ? new Date(Date.UTC(r, C - 1, D)) : /* @__PURE__ */ new Date(
        `${A(4, r)}-${A(2, C)}-${A(2, D)}T00:00:00${u}`
      );
    }
    parseDateTime(u) {
      const s = this.readLengthCodedString("binary");
      return s === null ? null : !u || u === "local" ? new Date(s) : /* @__PURE__ */ new Date(`${s}${u}`);
    }
    parseFloat(u) {
      if (u === null)
        return null;
      ...
      if (u === 0)
        return 0;
      for (this.buffer[this.offset] === I && (this.offset++, C = -1), this.buffer[this.offset] === t && this.offset++; this.offset < r; )
        if (o = this.buffer[this.offset], o === a)
          D = !0, this.offset++;
        else if (o === i || o === c) {
          this.offset++;
          const B = this.parseInt(r - this.offset);
          return s / C * Math.pow(10, B);
        } else
    ...
    asError(u) {
      ...
      const C = this.readString(void 0, u), D = new Error(C);
    ...
    writeNullTerminatedString(u, s) {
      const r = _.encode(u, s);
    ...
    writeHeader(u) {
      const s = this.offset;
      this.offset = 0, this.writeInt24(this.buffer.length - 4), this.writeInt8(u), this.offset = s;
    }
    clone() {
    ...
    static lengthCodedStringLength(u, s) {
      const C = _.encode(u, s).length;
      return O.lengthCodedNumberLength(C) + C;
    }
    static MockBuffer() {
      const u = function() {
      }, s = Buffer.alloc(0);
      for (const r in n.prototype)
        typeof s[r] == "function" && (s[r] = u);
      return s;
    }
  }
  return h_ = O, h_;
...
function JR() {
  if (Xe) return L_;
  Xe = 1;
  const E = K(), n = 16777215;
  function e(R, N) {
    const T = R[N], A = R[N + 1], I = R[N + 2];
    return A + I === 0 ? T : T + (A << 8) + (I << 16);
  }
  class _ {
    constructor(N, T) {
      typeof T > "u" && (T = 4), this.buffer = [], this.bufferLength = 0, this.packetHeaderLength = T, this.headerLen = 0, this.length = 0, this.largePacketParts = [], this.firstPacketSequenceId = 0, this.onPacket = N, this.execute = _.prototype.executeStart, this._flushLargePacket = T === 7 ? this._flushLargePacket7 : this._flushLargePacket4;
    }
    _flushLargePacket4() {
      const N = this.largePacketParts.length;
      this.largePacketParts.unshift(Buffer.from([0, 0, 0, 0]));
      const T = Buffer.concat(this.largePacketParts), A = new E(this.firstPacketSequenceId, T, 0, T.length);
      this.largePacketParts.length = 0, A.numPackets = N, this.onPacket(A);
    }
    _flushLargePacket7() {
      const N = this.largePacketParts.length;
      ...
      const A = new E(this.firstPacketSequenceId, T, 0, T.length);
      A.numPackets = N, this.onPacket(A);
    }
    executeStart(N) {
      let T = 0;
      const A = N.length;
      for (; A - T >= 3; )
        if (this.length = e(N, T), A - T >= this.length + this.packetHeaderLength) {
          const I = N[T + 3];
          this.length < n && this.largePacketParts.length === 0 ? this.onPacket(
            new E(
              I,
              N,
              T,
          ...
          ), this.length < n && this._flushLargePacket()), T += this.packetHeaderLength + this.length;
        } else {
          this.buffer = [N.slice(T + 3, A)], this.bufferLength = A - T - 3, this.execute = _.prototype.executePayload;
          return;
        }
      A - T > 0 && (this.headerLen = A - T, this.length = N[T], this.headerLen === 2 ? (this.length = N[T] + (N[T + 1] << 8), this.execute = _.prototype.executeHeader3) : this.execute = _.prototype.executeHeader2);
    }
    executePayload(N) {
      let T = 0;
      const A = N.length, I = this.length - this.bufferLength + this.packetHeaderLength - 3;
      if (A - T >= I) {
        ...
        N.copy(t, a, T, T + I);
        const i = t[3];
        if (this.length < n && this.largePacketParts.length === 0 ? this.onPacket(
          new E(
            i,
            t,
            0,
        ...
        ), this.length < n && this._flushLargePacket()), this.buffer = [], this.bufferLength = 0, this.execute = _.prototype.executeStart, T += I, A - T > 0)
          return this.execute(N.slice(T, A));
      } else
        this.buffer.push(N), this.bufferLength += N.length;
      return null;
    }
    executeHeader2(N) {
      return this.length += N[0] << 8, N.length > 1 ? (this.length += N[1] << 16, this.execute = _.prototype.executePayload, this.executePayload(N.slice(2))) : (this.execute = _.prototype.executeHeader3, null);
    }
    executeHeader3(N) {
      return this.length += N[0] << 16, this.execute = _.prototype.executePayload, this.executePayload(N.slice(1));
    }
  }
  return L_ = _, L_;
}
var HE = { exports: {} }, f_, Ke;
function li() {
  if (Ke) return f_;
  Ke = 1;
  const E = K();
  class n {
    ...
    toPacket(_) {
      const R = 6 + this.pluginName.length + this.pluginData.length, N = Buffer.allocUnsafe(R), T = new E(0, N, 0, R);
    ...
    static fromPacket(_, R) {
      _.readInt8();
      const N = _.readNullTerminatedString(R), T = _.readBuffer();
      return new n({
        pluginName: N,
        pluginData: T
      });
...
function Bi() {
  ...
  class n {
    ...
    toPacket() {
      const _ = 6 + this.pluginName.length + this.pluginData.length, R = Buffer.allocUnsafe(_), N = new E(0, R, 0, _);
      return N.offset = 4, N.writeInt8(254), N.writeNullTerminatedString(this.pluginName, "cesu8"), N.writeBuffer(this.pluginData), N;
    }
    static fromPacket(_) {
      _.readInt8();
      const R = _.readNullTerminatedString("cesu8"), N = _.readBuffer();
      return new n({
        pluginName: R,
        pluginData: N
      });
...
function Mi() {
  ...
  class n {
    ...
    toPacket() {
      const _ = 5 + this.data.length, R = Buffer.allocUnsafe(_), N = new E(0, R, 0, _);
      return N.offset = 4, N.writeInt8(1), N.writeBuffer(this.data), N;
    }
    static fromPacket(_) {
      _.readInt8();
...
function hi() {
  ...
  class n {
    ...
    toPacket() {
      const _ = 4 + this.data.length, R = Buffer.allocUnsafe(_), N = new E(0, R, 0, _);
      return N.offset = 4, N.writeBuffer(this.data), N;
    }
    static fromPacket(_) {
      const R = _.readBuffer();
      return new n(R);
...
function Li() {
  if (ze) return w_;
  ze = 1;
  const E = IE(), n = K(), e = new Array(256);
  class _ {
    constructor(N) {
      this.columns = N || [];
    }
    static toPacket(N, T) {
      ...
      N.forEach((O) => {
        ...
        I += n.lengthCodedStringLength(O.toString(10), T);
      }), I = I + 2;
      const t = Buffer.allocUnsafe(I + 4), a = new n(0, t, 0, I + 4);
      a.offset = 4, a.writeInt8(0);
      let i = 0, c = 1;
      return N.forEach((O) => {
        O.type === E.NULL && (i += c), c *= 2, c === 256 && (a.writeInt8(i), i = 0, c = 1);
      }), c !== 1 && a.writeInt8(i), N.forEach((O) => {
        if (O === null) {
          a.writeNull();
          return;
        }
        ...
        a.writeLengthCodedString(O.toString(10), T);
      }), a;
    }
    // TODO: complete list of types...
    static fromPacket(N, T) {
      const A = new Array(N.length);
      T.readInt8();
      const I = Math.floor((N.length + 7 + 2) / 8);
      T.skip(I);
      for (let t = 0; t < A.length; ++t)
        A[t] = e[N[t].columnType].apply(T);
      return new _(A);
    }
  }
  return e[E.DECIMAL] = n.prototype.readLengthCodedString, e[1] = n.prototype.readInt8, e[2] = n.prototype.readInt16, e[3] = n.prototype.readInt32, e[4] = n.prototype.readFloat, e[5] = n.prototype.readDouble, e[6] = n.prototype.assertInvalid, e[7] = n.prototype.readTimestamp, e[8] = n.prototype.readInt64, e[9] = n.prototype.readInt32, e[10] = n.prototype.readTimestamp, e[11] = n.prototype.readTime, e[12] = n.prototype.readDateTime, e[13] = n.prototype.readInt16, e[E.VAR_STRING] = n.prototype.readLengthCodedString, w_ = _, w_;
}
var G_, Ze;
function tE() {
  return Ze || (Ze = 1, G_ = {
...
function fi() {
  ...
  class e {
    ...
    toPacket() {
      const R = 15 + Buffer.byteLength(this.filename, "utf8"), N = Buffer.allocUnsafe(R), T = new E(0, N, 0, R);
      return T.offset = 4, T.writeInt8(n.BINLOG_DUMP), T.writeInt32(this.binlogPos), T.writeInt16(this.flags), T.writeInt32(this.serverId), T.writeString(this.filename), T;
    }
  }
  return F_ = e, F_;
}
...
function BE() {
  return xe || (xe = 1, (function(E) {
    const n = GE;
    function e(T, A, I) {
      const t = n.createHash("sha1");
      return t.update(T), A && t.update(A), I && t.update(I), t.digest();
    }
    function _(T, A) {
      const I = Buffer.allocUnsafe(T.length);
    ...
    function R(T, A, I) {
      ...
      const t = e(T);
      return E.calculateTokenFromPasswordSha(t, A, I);
    }
    E.calculateTokenFromPasswordSha = function(T, A, I) {
      const t = A.slice(0, 8), a = I.slice(0, 12), i = e(T), c = e(t, a, i);
      return _(c, T);
    }, E.calculateToken = R, E.verifyToken = function(T, A, I, t) {
      const a = _(I, e(T, A, t));
      return e(a).compare(t) === 0;
    }, E.doubleSha1 = function(T) {
      return e(e(T));
    };
    function N(T, A) {
      const I = Buffer.allocUnsafe(T.length), t = A.length;
...
function gi() {
  if (EA) return b_;
  EA = 1;
  const E = tE(), n = rE(), e = K(), _ = BE(), R = iE();
  class N {
    constructor(A) {
      this.flags = A.flags, this.user = A.user || "", this.database = A.database || "", this.password = A.password || "", this.passwordSha1 = A.passwordSha1, this.authPluginData1 = A.authPluginData1, this.authPluginData2 = A.authPluginData2, this.connectAttributes = A.connectAttrinutes || {};
      let I;
      this.passwordSha1 ? I = _.calculateTokenFromPasswordSha(
        ...
        this.authPluginData1,
        this.authPluginData2
      ), this.authToken = I, this.charsetNumber = A.charsetNumber;
    }
    // TODO
    // ChangeUser.fromPacket = function(packet)
    // };
    serializeToBuffer(A) {
      const I = (i) => this.flags & n[i], t = new e(0, A, 0, A.length);
      t.offset = 4;
      const a = R[this.charsetNumber];
      if (t.writeInt8(E.CHANGE_USER), t.writeNullTerminatedString(this.user, a), I("SECURE_CONNECTION") ? (t.writeInt8(this.authToken.length), t.writeBuffer(this.authToken)) : (t.writeBuffer(this.authToken), t.writeInt8(0)), t.writeNullTerminatedString(this.database, a), t.writeInt16(this.charsetNumber), I("PLUGIN_AUTH") && t.writeNullTerminatedString("mysql_native_password", "latin1"), I("CONNECT_ATTRS")) {
        ...
        for (let S = 0; S < c.length; ++S)
          O += e.lengthCodedStringLength(c[S], a), O += e.lengthCodedStringLength(
            i[c[S]],
            a
          );
        t.writeLengthCodedNumber(O);
        for (let S = 0; S < c.length; ++S)
          t.writeLengthCodedString(c[S], a), t.writeLengthCodedString(
            i[c[S]],
            a
          );
      }
    ...
    toPacket() {
      ...
      const A = this.serializeToBuffer(e.MockBuffer());
      return this.serializeToBuffer(Buffer.allocUnsafe(A.offset));
    }
  }
  return b_ = N, b_;
}
...
function di() {
  ...
  class e {
    ...
    toPacket() {
      ...
      return R.offset = 4, R.writeInt8(n.STMT_CLOSE), R.writeInt32(this.id), R;
    }
  }
  return V_ = e, V_;
}
var Z = {}, nA;
function ee() {
  return nA || (nA = 1, Z.NOT_NULL = 1, Z.PRI_KEY = 2, Z.UNIQUE_KEY = 4, Z.MULTIPLE_KEY = 8, Z.BLOB = 16, Z.UNSIGNED = 32, Z.ZEROFILL = 64, Z.BINARY = 128, Z.ENUM = 256, Z.AUTO_INCREMENT = 512, Z.TIMESTAMP = 1024, Z.SET = 2048, Z.NO_DEFAULT_VALUE = 4096, Z.ON_UPDATE_NOW = 8192, Z.NUM = 32768), Z;
}
var m_, eA;
function Ui() {
  if (eA) return m_;
  eA = 1;
  const E = K(), n = E_(), e = iE(), _ = ["catalog", "schema", "table", "orgTable", "name", "orgName"];
  class R {
    constructor(A, I) {
      this._buf = A.buffer, this._clientEncoding = I, this._catalogLength = A.readLengthCodedNumber(), this._catalogStart = A.offset, A.offset += this._catalogLength, this._schemaLength = A.readLengthCodedNumber(), this._schemaStart = A.offset, A.offset += this._schemaLength, this._tableLength = A.readLengthCodedNumber(), this._tableStart = A.offset, A.offset += this._tableLength, this._orgTableLength = A.readLengthCodedNumber(), this._orgTableStart = A.offset, A.offset += this._orgTableLength;
      const t = A.readLengthCodedNumber(), a = A.offset;
      A.offset += t, this._orgNameLength = A.readLengthCodedNumber(), this._orgNameStart = A.offset, A.offset += this._orgNameLength, A.skip(1), this.characterSet = A.readInt16(), this.encoding = e[this.characterSet], this.name = n.decode(
        this._buf,
        this.encoding === "binary" ? this._clientEncoding : this.encoding,
        a,
        a + t
      ), this.columnLength = A.readInt32(), this.columnType = A.readInt8(), this.type = this.columnType, this.flags = A.readInt16(), this.decimals = A.readInt8();
    }
    inspect() {
      return {
    ...
    [Symbol.for("nodejs.util.inspect.custom")](A, I, t) {
      const a = IE(), i = [];
      for (const r in a)
        i[a[r]] = r;
      const c = ee(), O = [], S = this.flags;
      for (const r in c)
        S & c[r] && (r === "PRI_KEY" ? O.push("PRIMARY KEY") : r === "NOT_NULL" ? O.push("NOT NULL") : r === "BINARY" || r === "MULTIPLE_KEY" || r === "NO_DEFAULT_VALUE" || r === "BLOB" || r === "UNSIGNED" || r === "TIMESTAMP" || (r === "ON_UPDATE_NOW" ? O.push("ON UPDATE CURRENT_TIMESTAMP") : O.push(r)));
      if (A > 1)
        return t({
          ...this.inspect(),
      ...
      return s === "BLOB" ? this.columnLength === 4294967295 ? s = "LONGTEXT" : this.columnLength === 67108860 ? s = "MEDIUMTEXT" : this.columnLength === 262140 ? s = "TEXT" : this.columnLength === 1020 ? s = "TINYTEXT" : s = `BLOB(${this.columnLength})` : s === "VAR_STRING" ? s = `VARCHAR(${Math.ceil(this.columnLength / 4)})` : s === "TINY" ? this.columnLength === 3 && u || this.columnLength === 4 && !u ? s = "TINYINT" : s = `TINYINT(${this.columnLength})` : s === "LONGLONG" ? this.columnLength === 20 ? s = "BIGINT" : s = `BIGINT(${this.columnLength})` : s === "SHORT" ? u && this.columnLength === 5 || !u && this.columnLength === 6 ? s = "SMALLINT" : s = `SMALLINT(${this.columnLength})` : s === "LONG" ? u && this.columnLength === 10 || !u && this.columnLength === 11 ? s = "INT" : s = `INT(${this.columnLength})` : s === "INT24" ? u && this.columnLength === 8 || !u && this.columnLength === 9 ? s = "MEDIUMINT" : s = `MEDIUMINT(${this.columnLength})` : s === "DOUBLE" ? this.columnLength === 22 && this.decimals === 31 ? s = "DOUBLE" : s = `DOUBLE(${this.columnLength},${this.decimals})` : s === "FLOAT" ? this.columnLength === 12 && this.decimals === 31 ? s = "FLOAT" : s = `FLOAT(${this.columnLength},${this.decimals})` : s === "NEWDECIMAL" ? this.columnLength === 11 && this.decimals === 0 ? s = "DECIMAL" : this.decimals === 0 ? u ? s = `DECIMAL(${this.columnLength})` : s = `DECIMAL(${this.columnLength - 1})` : s = `DECIMAL(${this.columnLength - 2},${this.decimals})` : s = `${i[this.columnType]}(${this.columnLength})`, u && (s += " UNSIGNED"), `\`${this.name}\` ${[s, ...O].join(" ")}`;
    }
    static toPacket(A, I) {
      let t = 17;
      _.forEach((O) => {
      ...
      return i.offset = 4, _.forEach(c), i.writeInt8(12), i.writeInt16(A.characterSet), i.writeInt32(A.columnLength), i.writeInt8(A.columnType), i.writeInt16(A.flags), i.writeInt8(A.decimals), i.writeInt16(0), i;
    }
  ...
  const N = function(T) {
    Object.defineProperty(R.prototype, T, {
      get: function() {
        const A = this[`_${T}Start`], I = A + this[`_${T}Length`], t = n.decode(
          this._buf,
          this.encoding === "binary" ? this._clientEncoding : this.encoding,
          A,
          I
  ...
  return N("catalog"), N("schema"), N("table"), N("orgTable"), N("orgName"), m_ = R, m_;
}
var Y_, AA;
function wi() {
  return AA || (AA = 1, Y_ = {
...
function Gi() {
  if (RA) return W_;
  RA = 1;
  const E = wi(), n = tE(), e = IE(), _ = K(), R = iE();
  function N(I) {
    return Array.isArray(I) || I.constructor === Object || typeof I.toJSON == "function" && !Buffer.isBuffer(I);
  }
  function T(I, t, a) {
    let i = e.VAR_STRING, c, O = function(S) {
      return _.prototype.writeLengthCodedString.call(this, S, t);
    };
    if (I !== null)
      switch (typeof I) {
        case "undefined":
        ...
        case "number":
          i = e.DOUBLE, c = 8, O = _.prototype.writeDouble;
          break;
        case "boolean":
          I = I | 0, i = e.TINY, c = 1, O = _.prototype.writeInt8;
          break;
        case "object":
          Object.prototype.toString.call(I) === "[object Date]" ? (i = e.DATETIME, c = 12, O = function(S) {
            return _.prototype.writeDate.call(this, S, a);
          }) : N(I) ? (I = JSON.stringify(I), i = e.JSON) : Buffer.isBuffer(I) && (c = _.lengthCodedNumberLength(I.length) + I.length, O = _.prototype.writeLengthCodedBuffer);
          break;
        default:
          I = I.toString();
      }
      ...
      I = "", i = e.NULL;
    return c || (c = _.lengthCodedStringLength(I, t)), { value: I, type: i, length: c, writer: O };
  }
  class A {
    constructor(t, a, i, c) {
      this.id = t, this.parameters = a, this.encoding = R[i], this.timezone = c;
    }
    static fromPacket(t, a) {
      const i = t.readInt32(), c = t.readInt8(), O = t.readInt32();
      let S = t.offset;
      for (; S < t.end - 1 && !((t.buffer[S + 1] === e.VAR_STRING || t.buffer[S + 1] === e.NULL || t.buffer[S + 1] === e.DOUBLE || t.buffer[S + 1] === e.TINY || t.buffer[S + 1] === e.DATETIME || t.buffer[S + 1] === e.JSON) && t.buffer[S] === 1 && t.buffer[S + 2] === 0); )
        t.readInt8(), S++;
      const u = [];
      for (let r = t.offset + 1; r < t.end - 1; r++)
        (t.buffer[r] === e.VAR_STRING || t.buffer[r] === e.NULL || t.buffer[r] === e.DOUBLE || t.buffer[r] === e.TINY || t.buffer[r] === e.DATETIME || t.buffer[r] === e.JSON) && t.buffer[r + 1] === 0 && (u.push(t.buffer[r]), t.skip(2));
      t.skip(1);
      const s = [];
      for (let r = 0; r < u.length; r++)
        u[r] === e.VAR_STRING ? s.push(t.readLengthCodedString(a)) : u[r] === e.DOUBLE ? s.push(t.readDouble()) : u[r] === e.TINY ? s.push(t.readInt8()) : u[r] === e.DATETIME ? s.push(t.readDateTime()) : u[r] === e.JSON && s.push(JSON.parse(t.readLengthCodedString(a))), u[r] === e.NULL && s.push(null);
      return { stmtId: i, flags: c, iterationCount: O, values: s };
    }
    toPacket() {
      let t = 14, a;
      this.parameters && this.parameters.length > 0 && (t += Math.floor((this.parameters.length + 7) / 8), t += 1, t += 2 * this.parameters.length, a = this.parameters.map(
        (O) => T(O, this.encoding, this.timezone)
      ), t += a.reduce(
        (O, S) => O + S.length,
        0
      ));
      const i = Buffer.allocUnsafe(t), c = new _(0, i, 0, t);
      if (c.offset = 4, c.writeInt8(n.STMT_EXECUTE), c.writeInt32(this.id), c.writeInt8(E.NO_CURSOR), c.writeInt32(1), a) {
        let O = 0, S = 1;
        a.forEach((u) => {
          u.type === e.NULL && (O += S), S *= 2, S === 256 && (c.writeInt8(O), O = 0, S = 1);
        }), S !== 1 && c.writeInt8(O), c.writeInt8(1), a.forEach((u) => {
          c.writeInt8(u.type), c.writeInt8(0);
        }), a.forEach((u) => {
          u.type !== e.NULL && u.writer.call(c, u.value);
        });
      }
      return c;
    }
...
function Fi() {
  ...
  class e {
    ...
    toPacket(R) {
      const N = 68 + Buffer.byteLength(this.serverVersion, "utf8"), T = Buffer.alloc(N + 4, 0), A = new E(R, T, 0, N + 4);
      ...
      return I.writeUInt32LE(this.capabilityFlags, 0), A.writeBuffer(I.slice(0, 2)), A.writeInt8(this.characterSet), A.writeInt16(this.statusFlags), A.writeBuffer(I.slice(2, 4)), A.writeInt8(21), A.skip(10), A.writeBuffer(this.authPluginData2), A.writeInt8(0), A.writeString("mysql_native_password", "latin1"), A.writeInt8(0), A;
    }
    static fromPacket(R) {
      const N = {};
      N.protocolVersion = R.readInt8(), N.serverVersion = R.readNullTerminatedString("cesu8"), N.connectionId = R.readInt32(), N.authPluginData1 = R.readBuffer(8), R.skip(1);
      const T = Buffer.allocUnsafe(4);
      if (T[0] = R.readInt8(), T[1] = R.readInt8(), R.haveMoreData() ? (N.characterSet = R.readInt8(), N.statusFlags = R.readInt16(), T[2] = R.readInt8(), T[3] = R.readInt8(), N.capabilityFlags = T.readUInt32LE(0), N.capabilityFlags & n.PLUGIN_AUTH ? N.authPluginDataLength = R.readInt8() : (N.authPluginDataLength = 0, R.skip(1)), R.skip(10)) : N.capabilityFlags = T.readUInt16LE(0), N.capabilityFlags & n.SECURE_CONNECTION) {
        const I = N.authPluginDataLength;
        if (I === 0)
          N.authPluginDataLength = 20, N.authPluginData2 = R.readBuffer(12), R.skip(1);
        else {
          const t = Math.max(13, I - 8);
          N.authPluginData2 = R.readBuffer(t);
        }
      }
      return N.capabilityFlags & n.PLUGIN_AUTH && (N.authPluginName = R.readNullTerminatedString("ascii")), new e(N);
    }
  }
  return H_ = e, H_;
}
...
function Qi() {
  ...
  class R {
    constructor(T) {
      ...
      let A;
      this.passwordSha1 ? A = _.calculateTokenFromPasswordSha(
        this.passwordSha1,
        this.authPluginData1,
        this.authPluginData2
      ) : A = _.calculateToken(
        ...
        this.authPluginData1,
        this.authPluginData2
      ), this.authToken = A, this.charsetNumber = T.charsetNumber, this.encoding = n[T.charsetNumber], this.connectAttributes = T.connectAttributes;
    }
    serializeResponse(T) {
      const A = (i) => this.clientFlags & E[i], I = new e(0, T, 0, T.length);
      ...
      let a;
      if (A("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (I.writeLengthCodedNumber(this.authToken.length), I.writeBuffer(this.authToken)) : A("SECURE_CONNECTION") ? (I.writeInt8(this.authToken.length), I.writeBuffer(this.authToken)) : (I.writeBuffer(this.authToken), I.writeInt8(0)), A("CONNECT_WITH_DB") && I.writeNullTerminatedString(this.database, t), A("PLUGIN_AUTH") && I.writeNullTerminatedString("mysql_native_password", "latin1"), A("CONNECT_ATTRS")) {
        const i = this.connectAttributes || {}, c = Object.keys(i);
        let O = 0;
        for (a = 0; a < c.length; ++a)
          O += e.lengthCodedStringLength(c[a], t), O += e.lengthCodedStringLength(
            i[c[a]],
            t
          );
        for (I.writeLengthCodedNumber(O), a = 0; a < c.length; ++a)
    ...
    static fromPacket(T) {
      const A = {};
      A.clientFlags = T.readInt32();
      function I(i) {
        return A.clientFlags & E[i];
      }
      A.maxPacketSize = T.readInt32(), A.charsetNumber = T.readInt8();
      const t = n[A.charsetNumber];
      A.encoding = t, T.skip(23), A.user = T.readNullTerminatedString(t);
      let a;
      if (I("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (a = T.readLengthCodedNumber(t), A.authToken = T.readBuffer(a)) : I("SECURE_CONNECTION") ? (a = T.readInt8(), A.authToken = T.readBuffer(a)) : A.authToken = T.readNullTerminatedString(t), I("CONNECT_WITH_DB") && (A.database = T.readNullTerminatedString(t)), I("PLUGIN_AUTH") && (A.authPluginName = T.readNullTerminatedString(t)), I("CONNECT_ATTRS")) {
        const i = T.readLengthCodedNumber(t), c = T.offset + i, O = {};
        for (; T.offset < c; )
          O[T.readLengthCodedString(t)] = T.readLengthCodedString(t);
        A.connectAttributes = O;
      }
      return A;
    }
  }
...
function Pi() {
  if (aA) return y_;
  aA = 1;
  const E = K(), n = tE(), e = E_(), _ = iE();
  class R {
    constructor(T, A) {
      this.query = T, this.charsetNumber = A, this.encoding = _[A];
    }
    toPacket() {
      const T = e.encode(this.query, this.encoding), A = 5 + T.length, I = Buffer.allocUnsafe(A), t = new E(0, I, 0, A);
      return t.offset = 4, t.writeInt8(n.STMT_PREPARE), t.writeBuffer(T), t;
    }
  }
  return y_ = R, y_;
}
...
function Vi() {
  ...
  class R {
    ...
    toPacket() {
      const T = e.encode(this.query, this.encoding), A = 5 + T.length, I = Buffer.allocUnsafe(A), t = new E(0, I, 0, A);
      return t.offset = 4, t.writeInt8(n.QUERY), t.writeBuffer(T), t;
    }
  }
  return K_ = R, K_;
}
...
function Hi() {
  if (OA) return z_;
  OA = 1;
  const E = K(), n = rE(), e = zR(), _ = Yi(), R = Wi();
  class N {
    constructor(A, I) {
      const t = I.config.bigNumberStrings, a = I.serverEncoding, i = I._handshakePacket.capabilityFlags, c = function(u) {
        return i & n[u];
      };
      if (A.buffer[A.offset] !== 0) {
        this.fieldCount = A.readLengthCodedNumber(), this.fieldCount === null && (this.infileName = A.readString(void 0, a));
        return;
      }
      this.fieldCount = A.readInt8(), this.affectedRows = A.readLengthCodedNumber(t), this.insertId = A.readLengthCodedNumberSigned(t), this.info = "", c("PROTOCOL_41") ? (this.serverStatus = A.readInt16(), this.warningStatus = A.readInt16()) : c("TRANSACTIONS") && (this.serverStatus = A.readInt16());
      let O = null;
      if (c("SESSION_TRACK") && A.offset < A.end) {
        if (this.info = A.readLengthCodedString(a), this.serverStatus && e.SERVER_SESSION_STATE_CHANGED) {
          let u = A.offset < A.end ? A.readLengthCodedNumber() : 0;
          const s = A.offset + u;
          let r, C, D;
          for (u > 0 && (O = {
            systemVariables: {},
            ...
            if (r = A.readInt8(), u = A.readLengthCodedNumber(), D = A.offset + u, r === R.SYSTEM_VARIABLES) {
              C = A.readLengthCodedString(a);
              const o = A.readLengthCodedString(a);
              if (O.systemVariables[C] = o, C === "character_set_client") {
                const B = _[o];
                typeof B < "u" && (I.config.charsetNumber = B);
              }
            } else if (r === R.SCHEMA)
              C = A.readLengthCodedString(a), O.schema = C;
            else if (r === R.STATE_CHANGE)
              O.trackStateChange = A.readLengthCodedString(a);
            else if (r === R.STATE_GTIDS) {
              A.readLengthCodedString(a);
              const o = A.readLengthCodedString(a);
              O.gtids = o.split(",");
            }
            A.offset = D;
        ...
        this.info = A.readString(void 0, a);
    ...
    static toPacket(A, I) {
      let t = 4 + E.lengthCodedNumberLength(A);
      typeof I < "u" && (t += E.lengthCodedNumberLength(I));
      const a = Buffer.allocUnsafe(t), i = new E(0, a, 0, t);
      return i.offset = 4, i.writeLengthCodedNumber(A), typeof I < "u" && i.writeLengthCodedNumber(I), i;
    }
  }
  return z_ = N, z_;
}
...
function vi() {
  ...
  class e {
    constructor(R, N) {
      this.clientFlags = R | E.SSL, this.charset = N;
    }
    toPacket() {
      const N = Buffer.allocUnsafe(36), T = new n(0, N, 0, 36);
      return N.fill(0), T.offset = 4, T.writeInt32(this.clientFlags), T.writeInt32(0), T.writeInt8(this.charset), T;
    }
  }
  return Z_ = e, Z_;
}
  ...
  class n {
    ...
    static toPacket(_, R) {
      ...
      _.forEach((t) => {
        ...
        T += E.lengthCodedStringLength(t.toString(10), R);
      });
      const A = Buffer.allocUnsafe(T + 4), I = new E(0, A, 0, T + 4);
      return I.offset = 4, _.forEach((t) => {
        ...
        I.writeLengthCodedString(t.toString(10), R);
      }), I;
    }
  }
  return q_ = n, q_;
...
function eE() {
  return DA || (DA = 1, (function(E, n) {
    const e = xE, _ = li(), R = Bi(), N = Mi(), T = hi(), A = Li(), I = fi(), t = gi(), a = di(), i = Ui(), c = Gi(), O = Fi(), S = Qi(), u = Pi(), s = bi(), r = Vi(), C = mi(), D = Hi(), o = vi(), B = yi();
    Object.entries({
      AuthNextFactor: _,
      AuthSwitchRequest: R,
      ...
      if (E.exports[P] = w, e.env.NODE_DEBUG && w.prototype.toPacket) {
    ...
    class d {
      static toPacket(w, L) {
        w = w || {};
        const f = w.affectedRows || 0, l = w.insertId || 0, g = w.serverStatus || 0, V = w.warningCount || 0, W = w.message || "";
        let H = 9 + h.lengthCodedNumberLength(f);
        H += h.lengthCodedNumberLength(l);
        const k = Buffer.allocUnsafe(H), X = new h(0, k, 0, H);
        return X.offset = 4, X.writeInt8(0), X.writeLengthCodedNumber(f), X.writeLengthCodedNumber(l), X.writeInt16(g), X.writeInt16(V), X.writeString(W, L), X._name = "OK", X;
      }
    }
    n.OK = d;
    class U {
    ...
    class b {
      ...
      static fromPacket(w) {
        ...
        const f = w.readNullTerminatedString("utf8"), l = new b();
...
function AE() {
  ...
  class e extends E {
    ...
    execute(R, N) {
      if (this.next || (this.next = this.start, N._resetSequenceId()), R && R.isError()) {
        const T = R.asError(N.clientEncoding);
        return T.sql = this.sql || this.query, this.queryTimeout && (n.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult ? (this.onResult(T), this.emit("end")) : (this.emit("error", T), this.emit("end")), !0;
      }
      return this.next = this.next(R, N), this.next ? !1 : (this.emit("end"), !0);
    }
  }
  return p_ = e, p_;
}
...
function Xi() {
  if (oA) return x_;
  oA = 1;
  const E = "sha256_password", n = GE, { xorRotating: e } = BE(), _ = mR, R = Buffer.from([1]), N = 0, T = 1, A = -1;
  function I(t, a, i) {
    const c = e(Buffer.from(`${t}\0`, "utf8"), a);
    return n.publicEncrypt(i, c);
  }
  return x_ = (t = {}) => ({ connection: a }) => {
    let i = 0, c = null;
    const O = a.config.password, S = (u) => {
      const s = I(O, c, u);
      return i = A, s;
    };
    return (u) => {
      switch (i) {
        case N:
          return a.stream instanceof _.TLSSocket && a.stream.encrypted === !0 ? Buffer.from(`${O}\0`, "utf8") : (c = u.slice(0, 20), t.serverPublicKey ? S(t.serverPublicKey) : (i = T, R));
        case T:
          return t.onServerPublicKey && t.onServerPublicKey(u), S(u);
        case A:
          throw new Error(
...
function Ki() {
  if (lA) return $_;
  lA = 1;
  const E = "caching_sha2_password", n = GE, { xor: e, xorRotating: _ } = BE(), R = Buffer.from([2]), N = Buffer.from([3]), T = Buffer.from([4]), A = 0, I = 1, t = 2, a = -1;
  function i(S) {
    const u = n.createHash("sha256");
    return u.update(S), u.digest();
  }
  function c(S, u) {
    if (!S)
      return Buffer.alloc(0);
    const s = i(Buffer.from(S)), r = i(s), C = i(Buffer.concat([r, u]));
    return e(s, C);
  }
  function O(S, u, s) {
    const r = _(Buffer.from(`${S}\0`, "utf8"), u);
    return n.publicEncrypt(
      {
        key: s,
        padding: n.constants.RSA_PKCS1_OAEP_PADDING
      },
      r
    );
  }
  return $_ = (S = {}) => ({ connection: u }) => {
    let s = 0, r = null;
    const C = u.config.password, D = (o) => {
      const B = O(C, r, o);
      return s = a, B;
    };
    return (o) => {
      switch (s) {
        case A:
          return r = o.slice(0, 20), s = I, c(C, r);
        case I:
          if (N.equals(o))
            return s = a, null;
          if (T.equals(o))
            return (typeof S.overrideIsSecure > "u" ? u.config.ssl || u.config.socketPath : S.overrideIsSecure) ? (s = a, Buffer.from(`${C}\0`, "utf8")) : S.serverPublicKey ? D(S.serverPublicKey) : (s = t, R);
          throw new Error(
            `Invalid AuthMoreData packet received by ${E} plugin in STATE_TOKEN_SENT state.`
          );
        case t:
          return S.onServerPublicKey && S.onServerPublicKey(o), D(o);
        case a:
          throw new Error(
            `Unexpected data in AuthMoreData packet received by ${E} plugin in STATE_FINAL state.`
          );
...
function ki() {
  ...
  BA = 1;
  const E = BE();
  return En = (n) => ({ connection: e, command: _ }) => {
    const R = _.password || n.password || e.config.password, N = _.passwordSha1 || n.passwordSha1 || e.config.passwordSha1;
    return (T) => {
      const A = T.slice(0, 8), I = T.slice(8, 20);
      let t;
      return N ? t = E.calculateTokenFromPasswordSha(
        N,
        A,
        I
      ) : t = E.calculateToken(
...
function ji() {
  ...
  return _n = (e) => function({ connection: R, command: N }) {
    const T = N.password || e.password || R.config.password;
    return function() {
      return E(T);
    };
  }, _n;
...
function Ji() {
  if (hA) return nn;
  hA = 1;
  const E = eE(), n = Xi(), e = Ki(), _ = ki(), R = ji(), N = {
    sha256_password: n({}),
  ...
  function I(a, i, c) {
    const { pluginName: O, pluginData: S } = E.AuthSwitchRequest.fromPacket(a);
    let u = i.config.authPlugins && i.config.authPlugins[O];
    if (i.config.authSwitchHandler && O !== "mysql_native_password") {
      const s = i.config.authSwitchHandler;
      T(), s({ pluginName: O, pluginData: S }, (r, C) => {
        if (r)
          return A(r, c);
    ...
    i._authPlugin = u({ connection: i, command: c }), Promise.resolve(i._authPlugin(S)).then((s) => {
  ...
  function t(a, i, c) {
    const { data: O } = E.AuthSwitchRequestMoreData.fromPacket(a);
    if (i.config.authSwitchHandler) {
      const S = i.config.authSwitchHandler;
      T(), S({ pluginData: O }, (u, s) => {
        if (u)
          return A(u, c);
...
function zi() {
  return LA || (LA = 1, (function(E) {
    var n = NE.EventEmitter, e = HR, _ = 3e3, R = 0, N = "closed", T = "drained", A = function(t) {
      n.call(this), t && t > 0 ? this.timeout = t : this.timeout = _, this.status = I.STATUS_IDLE, this.curId = R, this.queue = [];
    };
    e.inherits(A, n), A.prototype.push = function(t, a, i) {
      ...
      this.status !== I.STATUS_IDLE && this.status !== I.STATUS_BUSY || (t ? (this.status = I.STATUS_DRAINED, this.timerId && (clearTimeout(this.timerId), this.timerId = void 0), this.emit(T)) : (this.status = I.STATUS_CLOSED, this.emit(N)));
      ...
      if (!(t !== this.curId || this.status !== I.STATUS_BUSY && this.status !== I.STATUS_CLOSED)) {
        ...
        if (!a) {
          this.status === I.STATUS_BUSY ? (this.status = I.STATUS_IDLE, this.curId++) : (this.status = I.STATUS_DRAINED, this.emit(T));
          return;
        }
        var i = this;
        a.id = ++this.curId;
        ...
        try {
          ...
          i.emit("error", O, a), process.nextTick(function() {
            i._next(a.id);
          });
        }
      }
...
function qi() {
  ...
  const E = Yt, n = JR();
  function e(N) {
    const T = this, A = N.readInt24(), I = N.readBuffer();
    A !== 0 ? T.inflateQueue.push((t) => {
      E.inflate(I, (a, i) => {
        if (a) {
          T._handleNetworkError(a);
          return;
        }
        T._bumpCompressedSequenceId(N.numPackets), T._inflatedPacketsParser.execute(i), t.done();
      });
    }) : T.inflateQueue.push((t) => {
      T._bumpCompressedSequenceId(N.numPackets), T._inflatedPacketsParser.execute(I), t.done();
    });
  }
  function _(N) {
    let A;
    if (N.length > 16777210) {
      for (A = 0; A < N.length; A += 16777210)
        _.call(
          // eslint-disable-next-line no-invalid-this
          this,
          N.slice(A, A + 16777210)
        );
      return;
    }
    const I = this;
    ...
    (function(i) {
      I.deflateQueue.push((c) => {
        E.deflate(N, (O, S) => {
          ...
          let u = S.length;
          u < t ? (a.writeUInt8(u & 255, 0), a.writeUInt16LE(u >> 8, 1), a.writeUInt8(i, 3), a.writeUInt8(t & 255, 4), a.writeUInt16LE(t >> 8, 5), I.writeUncompressed(a), I.writeUncompressed(S)) : (u = t, t = 0, a.writeUInt8(u & 255, 0), a.writeUInt16LE(u >> 8, 1), a.writeUInt8(i, 3), a.writeUInt8(t & 255, 4), a.writeUInt16LE(t >> 8, 5), I.writeUncompressed(a), I.writeUncompressed(N)), c.done();
        });
      });
    })(I.compressedSequenceId), I._bumpCompressedSequenceId(1);
  }
  function R(N) {
    N._lastWrittenPacketId = 0, N._lastReceivedPacketId = 0, N._handleCompressedPacket = e, N._inflatedPacketsParser = new n((A) => {
      N.handlePacket(A);
    }, 4), N._inflatedPacketsParser._lastPacket = 0, N.packetParser = new n((A) => {
      N._handleCompressedPacket(A);
    }, 7), N.writeUncompressed = N.write, N.write = _;
    const T = Zi();
    N.inflateQueue = T.createQueue(), N.deflateQueue = T.createQueue();
  }
  return Rn = {
    enableCompression: R
  }, Rn;
...
function ZR() {
  if (dA) return tn;
  dA = 1;
  const E = AE(), n = eE(), e = rE(), _ = iE(), R = BE();
  function N(A) {
    const I = [];
    for (const t in e)
      A & e[t] && I.push(t.replace(/_/g, " ").toLowerCase());
    return I;
  }
  class T extends E {
    constructor(I) {
    ...
    sendCredentials(I) {
      ...
      const t = new n.HandshakeResponse({
    ...
    handshakeInit(I, t) {
      ...
      }), this.handshake = n.Handshake.fromPacket(I), t.config.debug && console.log(
        "Server hello packet: capability flags:%d=(%s)",
        this.handshake.capabilityFlags,
        N(this.handshake.capabilityFlags).join(", ")
      ...
      const a = this.handshake.capabilityFlags & e.SSL, i = this.handshake.capabilityFlags & e.MULTI_FACTOR_AUTHENTICATION;
      if (this.clientFlags = this.clientFlags | i, t.config.compress = t.config.compress && this.handshake.capabilityFlags & e.COMPRESS, this.clientFlags = this.clientFlags | t.config.compress, t.config.ssl) {
        if (!a) {
        ...
        this.clientFlags |= e.SSL, this.sendSSLRequest(t), t.startTLS((c) => {
          if (c) {
        ...
        this.sendCredentials(t);
      return i && (this.authenticationFactor = 1), T.prototype.handshakeResult;
    }
    handshakeResult(I, t) {
      const a = I.peekByte();
      if (a === 254 || a === 1 || a === 2) {
        const i = Ji();
        try {
          return a === 1 ? i.authSwitchRequestMoreData(I, t, this) : (this.authenticationFactor !== 0 && (t.config.password = this[`password${this.authenticationFactor}`], this.authenticationFactor += 1), i.authSwitchRequest(I, t, this)), T.prototype.handshakeResult;
        } catch (c) {
  ...
  class R extends e {
    ...
    start(T, A) {
      ...
      return this.serverHello = I, I.setScrambleData((t) => {
        if (t) {
          A.emit("error", new Error("Error generating random bytes"));
          return;
        }
        A.writePacket(I.toPacket(0));
      }), R.prototype.readClientReply;
    }
    readClientReply(T, A) {
      const I = _.HandshakeResponse.fromPacket(T);
      return A.clientHelloReply = I, this.args.authCallback ? this.args.authCallback(
        {
          user: I.user,
          database: I.database,
          address: A.stream.remoteAddress,
      ...
      ) : A.writeOk(), R.prototype.dispatchCommands;
    }
    _isStatement(T, A) {
      return T.split(" ")[0].toUpperCase() === A;
    }
    dispatchCommands(T, A) {
      let I = !0;
      const t = A.clientHelloReply.encoding, a = T.readInt8();
      switch (a) {
        case E.STMT_PREPARE:
          if (A.listeners("stmt_prepare").length) {
            const i = T.readString(void 0, t);
            A.emit("stmt_prepare", i);
          } else
            A.writeError({
              code: n.HA_ERR_INTERNAL_ERROR,
              message: "No query handler for prepared statements."
            });
          break;
        case E.STMT_EXECUTE:
          if (A.listeners("stmt_execute").length) {
            const { stmtId: i, flags: c, iterationCount: O, values: S } = _.Execute.fromPacket(T, t);
            A.emit(
              "stmt_execute",
              i,
              c,
          ...
          break;
        case E.QUIT:
          A.listeners("quit").length ? A.emit("quit") : A.stream.end();
          break;
        case E.INIT_DB:
          if (A.listeners("init_db").length) {
            const i = T.readString(void 0, t);
            A.emit("init_db", i);
          } else
            A.writeOk();
          break;
        case E.QUERY:
          if (A.listeners("query").length) {
            const i = T.readString(void 0, t);
            this._isStatement(i, "PREPARE") || this._isStatement(i, "SET") ? A.emit("stmt_prepare", i) : this._isStatement(i, "EXECUTE") ? A.emit("stmt_execute", null, null, null, null, i) : A.emit("query", i);
          } else
            A.writeError({
              code: n.HA_ERR_INTERNAL_ERROR,
          ...
          break;
        case E.FIELD_LIST:
          if (A.listeners("field_list").length) {
            const i = T.readNullTerminatedString(t), c = T.readString(void 0, t);
            A.emit("field_list", i, c);
          } else
            A.writeError({
              code: n.ER_WARN_DEPRECATED_SYNTAX,
              message: "As of MySQL 5.7.11, COM_FIELD_LIST is deprecated and will be removed in a future version of MySQL."
            });
          break;
        case E.PING:
          A.listeners("ping").length ? A.emit("ping") : A.writeOk();
          break;
        default:
          I = !1;
      }
      return A.listeners("packet").length ? A.emit("packet", T.clone(), I, a) : I || console.log("Unknown command:", a), R.prototype.dispatchCommands;
    }
  }
  return an = R, an;
}
...
function __() {
  ...
  let n, e = !1;
  try {
    n = xi("cardinal").highlight;
  } catch {
    n = (A) => (e || (console.log("For nicer debug output consider install cardinal@^2.0.0"), e = !0), A);
  }
  function _(A, I) {
    console.log(`
...
`);
  ...
  function R(A, I, t) {
    return Array.isArray(I) ? I.some((a) => A === t[a]) : !!I;
  }
  uE.typeMatch = R;
  ...
  uE.privateObjectProps = N;
  const T = (A, I = !0) => {
    if (N.has(A))
      throw new Error(
        `The field name (${A}) can't be the same as an object's private property.`
      );
    return I ? E(A) : A;
  };
...
`), C = 0; C < r.length; C++)
    ...
    return u.scope = {}, u.formats = I, u.sym = function(s) {
      return (!s || !A(s)) && (s = "tmp"), c[s] || (c[s] = 0), s + (c[s]++ || "");
    }, u.property = function(s, r) {
      return arguments.length === 1 && (r = s, s = ""), r = r + "", n(r) ? s ? s + "." + r : r : s ? s + "[" + JSON.stringify(r) + "]" : JSON.stringify(r);
    }, u.toString = function() {
      return a.join(`
`);
    }, u.toFunction = function(s) {
      s || (s = {});
      var r = "return (" + u.toString() + ")";
      Object.keys(u.scope).forEach(function(o) {
        s[o] || (s[o] = u.scope[o]);
      });
      var C = Object.keys(s).map(function(o) {
        return o;
      }), D = C.map(function(o) {
        return s[o];
      });
  ...
  return t.formats = I, Tn = t, Tn;
}
var rn, PA;
function Ea() {
  if (PA) return rn;
  PA = 1;
  const E = IE(), n = ME(), e = __(), _ = qR(), R = ne(), N = [];
  for (const t in E)
    N[E[t]] = t;
  function T(t, a, i, c, O) {
    const S = !!(O.supportBigNumbers || c.supportBigNumbers), u = !!(O.bigNumberStrings || c.bigNumberStrings), s = O.timezone || c.timezone, r = O.dateStrings || c.dateStrings;
    switch (t) {
      case E.TINY:
      case E.SHORT:
      case E.LONG:
      ...
      case E.DATE:
        return e.typeMatch(t, r, E) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDate(${e.srcEscape(s)})`;
      case E.DATETIME:
      case E.TIMESTAMP:
        return e.typeMatch(t, r, E) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDateTime(${e.srcEscape(s)})`;
      case E.TIME:
      ...
      default:
        return a === n.BINARY ? "packet.readLengthCodedBuffer()" : `packet.readLengthCodedString(${i})`;
    }
  }
  function A(t, a, i) {
    ...
    function c(D, o) {
      return {
        ...
        table: D.table,
        name: D.name,
        string: function(B = D.encoding) {
          return D.columnType === E.JSON && B === D.encoding && console.warn(
          ...
          ), o.packet.readLengthCodedString(B);
        },
        buffer: function() {
          return o.packet.readLengthCodedBuffer();
        },
        geometry: function() {
          return o.packet.parseGeometryValue();
        }
      };
    }
    const O = _();
    O("(function () {")("return class TextRow {"), O("constructor(fields) {"), typeof a.typeCast == "function" && (O("const _this = this;"), O("for(let i=0; i<fields.length; ++i) {"), O("this[`wrap${i}`] = wrap(fields[i], _this);"), O("}")), O("}"), O("next(packet, fields, options) {"), O("this.packet = packet;"), a.rowsAsArray ? O(`const result = new Array(${t.length});`) : O("const result = {};");
    const S = {};
    let u = [];
    if (a.nestTables === !0) {
      ...
      for (let D = 0; D < u.length; D++)
        O(`result[${e.fieldEscape(u[D])}] = {};`);
    }
    let s = "", r = "", C = "";
    for (let D = 0; D < t.length; D++)
      if (r = e.fieldEscape(t[D].name), typeof a.nestTables == "string" ? s = `result[${e.fieldEscape(t[D].table + a.nestTables + t[D].name)}]` : a.nestTables === !0 ? (C = e.fieldEscape(t[D].table), O(`if (!result[${C}]) result[${C}] = {};`), s = `result[${C}][${r}]`) : a.rowsAsArray ? s = `result[${D.toString(10)}]` : s = `result[${r}]`, a.typeCast === !1)
        O(`${s} = packet.readLengthCodedBuffer();`);
      else {
        const o = `fields[${D}].encoding`, B = T(
          t[D].columnType,
          t[D].characterSet,
          o,
          i,
        ...
        typeof a.typeCast == "function" ? O(
          `${s} = options.typeCast(this.wrap${D}, function() { return ${B} });`
        ) : O(`${s} = ${B};`);
      }
    return O("return result;"), O("}"), O("};")("})()"), i.debug && e.printDebugWithCode(
      "Compiled text protocol row parser",
      O.toString()
    ), typeof a.typeCast == "function" ? O.toFunction({ wrap: c }) : O.toFunction();
  }
  function I(t, a, i) {
    return R.getParser("text", t, a, i, A);
  }
  return rn = I, rn;
}
var cn, bA;
function _a() {
  if (bA) return cn;
  bA = 1;
  const E = IE(), n = ME(), e = __(), _ = [];
  for (const A in E)
    _[E[A]] = A;
  function R({ packet: A, type: I, charset: t, encoding: a, config: i, options: c }) {
    const O = !!(c.supportBigNumbers || i.supportBigNumbers), S = !!(c.bigNumberStrings || i.bigNumberStrings), u = c.timezone || i.timezone, s = c.dateStrings || i.dateStrings;
    switch (I) {
      case E.TINY:
      case E.SHORT:
      case E.LONG:
      case E.INT24:
      case E.YEAR:
        return A.parseLengthCodedIntNoBigCheck();
      case E.LONGLONG:
        return O && S ? A.parseLengthCodedIntString() : A.parseLengthCodedInt(O);
      case E.FLOAT:
      case E.DOUBLE:
        return A.parseLengthCodedFloat();
      case E.NULL:
      case E.DECIMAL:
      case E.NEWDECIMAL:
        return i.decimalNumbers ? A.parseLengthCodedFloat() : A.readLengthCodedString("ascii");
      case E.DATE:
        return e.typeMatch(I, s, E) ? A.readLengthCodedString("ascii") : A.parseDate(u);
      case E.DATETIME:
      case E.TIMESTAMP:
        return e.typeMatch(I, s, E) ? A.readLengthCodedString("ascii") : A.parseDateTime(u);
      case E.TIME:
        return A.readLengthCodedString("ascii");
      case E.GEOMETRY:
        return A.parseGeometryValue();
      case E.VECTOR:
        return A.parseVector();
      case E.JSON:
        return i.jsonStrings ? A.readLengthCodedString("utf8") : JSON.parse(A.readLengthCodedString("utf8"));
      default:
        return t === n.BINARY ? A.readLengthCodedBuffer() : A.readLengthCodedString(a);
    }
  }
  function N(A, I) {
    return {
      ...
      table: A.table,
      name: A.name,
      string: function(t = A.encoding) {
        return A.columnType === E.JSON && t === A.encoding && console.warn(
  ...
  function T(A, I, t) {
    return {
      next(a, i, c) {
        const O = c.rowsAsArray ? [] : {};
        for (let S = 0; S < i.length; S++) {
          const u = i[S], s = c.typeCast ? c.typeCast : t.typeCast, r = () => R({
            packet: a,
            type: u.columnType,
            encoding: u.encoding,
          ...
          let C;
          if (c.typeCast === !1 ? C = a.readLengthCodedBuffer() : typeof s == "function" ? C = s(N(u, a), r) : C = r(), c.rowsAsArray)
            O.push(C);
          else if (typeof c.nestTables == "string")
            O[`${e.fieldEscape(u.table, !1)}${c.nestTables}${e.fieldEscape(u.name, !1)}`] = C;
          else if (c.nestTables) {
            const D = e.fieldEscape(u.table, !1);
            O[D] || (O[D] = {}), O[D][e.fieldEscape(u.name, !1)] = C;
          } else
            O[e.fieldEscape(u.name, !1)] = C;
        }
        return O;
      }
    };
...
function pR() {
  if (VA) return un;
  VA = 1;
  const E = xE, n = Ee, e = YR.Readable, _ = AE(), R = eE(), N = Ea(), T = _a(), A = zR(), I = new R.Packet(0, Buffer.allocUnsafe(4), 0, 4);
  class t extends _ {
    constructor(i, c) {
    ...
    start(i, c) {
      ...
      return c.writePacket(O.toPacket(1)), t.prototype.resultsetHeader;
    }
    done() {
      if (this._unpipeStream(), this.timeout && !this.queryTimeout)
        return null;
      if (this.queryTimeout && (n.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult) {
        let i, c;
    ...
    doneInsert(i) {
      return this._localStreamError ? (this.onResult ? this.onResult(this._localStreamError, i) : this.emit("error", this._localStreamError), null) : (this._rows.push(i), this._fields.push(void 0), this.emit("fields", void 0), this.emit("result", i), i.serverStatus & A.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, this.resultsetHeader) : this.done());
    }
    resultsetHeader(i, c) {
      const O = new R.ResultSetHeader(i, c);
      return this._fieldCount = O.fieldCount, c.config.debug && console.log(
        `        Resultset header received, expecting ${O.fieldCount} column definition packets`
      ), this._fieldCount === 0 ? this.doneInsert(O) : this._fieldCount === null ? this._streamLocalInfile(c, O.infileName) : (this._receivedFieldsCount = 0, this._rows.push([]), this._fields.push([]), this.readField);
    }
    _streamLocalInfile(i, c) {
      ...
      const O = () => {
        ...
        const o = Buffer.allocUnsafe(D.length + 4);
        D.copy(o, 4), i.writePacket(
          new R.Packet(0, o, 0, o.length)
        );
      }, r = () => {
        i.removeListener("error", O), i.writePacket(I);
      }, C = (D) => {
        this._localStreamError = D, i.removeListener("error", O), i.writePacket(I);
      };
      return this._unpipeStream = () => {
        i.stream.removeListener("pause", u), i.stream.removeListener("drain", S), this._localStream.removeListener("data", s), this._localStream.removeListener("end", r), this._localStream.removeListener("error", C);
      }, i.stream.on("pause", u), i.stream.on("drain", S), this._localStream.on("data", s), this._localStream.on("end", r), this._localStream.on("error", C), i.once("error", O), this.infileOk;
    }
    readField(i, c) {
      if (this._receivedFieldsCount++, this._fields[this._resultIndex].length !== this._fieldCount) {
        ...
        this._fields[this._resultIndex].push(O), c.config.debug && (console.log("        Column definition:"), console.log(`          name: ${O.name}`), console.log(`          type: ${O.columnType}`), console.log(`         flags: ${O.flags}`));
      }
      if (this._receivedFieldsCount === this._fieldCount) {
        const O = this._fields[this._resultIndex];
        return this.emit("fields", O), this.options.disableEval ? this._rowParser = T(O, this.options, c.config) : this._rowParser = new (N(
          O,
          this.options,
          c.config
        ))(O), t.prototype.fieldsEOF;
      }
    ...
    row(i, c) {
      if (i.isEOF())
        return i.eofStatusFlags() & A.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, t.prototype.resultsetHeader) : this.done();
      let O;
      try {
      ...
      return this.onResult ? this._rows[this._resultIndex].push(O) : this.emit("result", O, this._resultIndex), t.prototype.row;
    }
    infileOk(i, c) {
      const O = new R.ResultSetHeader(i, c);
    ...
    stream(i) {
      ...
      return c._destroy = (r, C) => {
        this._connection && this._connection.resume(), this.removeListener("result", O), this.removeListener("fields", S), this.removeListener("end", u), this.removeListener("error", s), C(r);
      }, this.on("result", O), this.on("fields", S), this.on("end", u), this.on("error", s), c;
    }
    _setTimeout() {
      if (this.timeout) {
        const i = this._handleTimeoutError.bind(this);
        this.queryTimeout = n.setTimeout(i, this.timeout);
      }
    }
    _handleTimeoutError() {
      this.queryTimeout && (n.clearTimeout(this.queryTimeout), this.queryTimeout = null);
  ...
  return t.prototype.catch = t.prototype.then, un = t, un;
}
var On, mA;
function xR() {
  if (mA) return On;
  ...
  class e extends E {
    ...
    start(R, N) {
      return N.writePacket(new n.CloseStatement(this.id).toPacket(1)), null;
    }
  }
  return On = e, On;
}
...
function na() {
  if (YA) return sn;
  YA = 1;
  const E = ee(), n = ME(), e = IE(), _ = __(), R = qR(), N = ne(), T = [];
  for (const a in e)
    T[e[a]] = a;
  function A(a, i, c, O) {
    const S = !!(c.supportBigNumbers || i.supportBigNumbers), u = !!(c.bigNumberStrings || i.bigNumberStrings), s = c.timezone || i.timezone, r = c.dateStrings || i.dateStrings, C = a.flags & E.UNSIGNED;
    switch (a.columnType) {
      case e.TINY:
      ...
      case e.TIMESTAMP:
      case e.NEWDATE:
        return _.typeMatch(a.columnType, r, e) ? `packet.readDateTimeString(${parseInt(a.decimals, 10)}, null, ${a.columnType});` : `packet.readDateTime(${_.srcEscape(s)});`;
      case e.TIME:
      ...
      case e.LONGLONG:
        return S ? u ? C ? "packet.readInt64String();" : "packet.readSInt64String();" : C ? "packet.readInt64();" : "packet.readSInt64();" : C ? "packet.readInt64JSNumber();" : "packet.readSInt64JSNumber();";
      default:
        return a.characterSet === n.BINARY ? "packet.readLengthCodedBuffer();" : `packet.readLengthCodedString(fields[${O}].encoding)`;
    }
  }
  function I(a, i, c) {
    const O = R(), S = Math.floor((a.length + 7 + 2) / 8);
    function u(B, M) {
      return {
        type: T[B.columnType],
        ...
        string: function(h = B.encoding) {
          if (B.columnType === e.JSON && h === B.encoding && console.warn(
          ...
          return B.columnType === e.TIME ? M.readTimeString() : M.readLengthCodedString(h);
        },
        buffer: function() {
    ...
    O("(function(){"), O("return class BinaryRow {"), O("constructor() {"), O("}"), O("next(packet, fields, options) {"), i.rowsAsArray ? O(`const result = new Array(${a.length});`) : O("const result = {};"), typeof c.typeCast == "function" && typeof i.typeCast != "function" && (i.typeCast = c.typeCast), O("packet.readInt8();");
    for (let B = 0; B < S; ++B)
      O(`const nullBitmaskByte${B} = packet.readInt8();`);
    let s = "", r = 4, C = 0, D = "", o = "";
    for (let B = 0; B < a.length; B++) {
      if (D = _.fieldEscape(a[B].name), typeof i.nestTables == "string" ? s = `result[${_.fieldEscape(a[B].table + i.nestTables + a[B].name)}]` : i.nestTables === !0 ? (o = _.fieldEscape(a[B].table), O(`if (!result[${o}]) result[${o}] = {};`), s = `result[${o}][${D}]`) : i.rowsAsArray ? s = `result[${B.toString(10)}]` : s = `result[${D}]`, O(`if (nullBitmaskByte${C} & ${r}) `), O(`${s} = null;`), O("else {"), i.typeCast === !1)
        O(`${s} = packet.readLengthCodedBuffer();`);
      else {
        const M = `fieldWrapper${B}`;
        O(`const ${M} = wrap(fields[${B}], packet);`);
        const h = A(a[B], c, i, B);
        typeof i.typeCast == "function" ? O(
          `${s} = options.typeCast(${M}, function() { return ${h} });`
        ) : O(`${s} = ${h};`);
      }
      O("}"), r *= 2, r === 256 && (r = 1, C++);
    }
    return O("return result;"), O("}"), O("};")("})()"), c.debug && _.printDebugWithCode(
      "Compiled binary protocol row parser",
      O.toString()
    ), O.toFunction({ wrap: u });
  }
  function t(a, i, c) {
    return N.getParser("binary", a, i, c, I);
  }
  return sn = t, sn;
}
var Cn, WA;
function ea() {
  if (WA) return Cn;
  WA = 1;
  const E = ee(), n = ME(), e = IE(), _ = __(), R = [];
  for (const T in e)
    R[e[T]] = T;
  function N(T, A, I) {
    function t(a, i, c, O, S) {
      const u = !!(c.supportBigNumbers || i.supportBigNumbers), s = !!(c.bigNumberStrings || i.bigNumberStrings), r = c.timezone || i.timezone, C = c.dateStrings || i.dateStrings, D = a.flags & E.UNSIGNED;
      switch (a.columnType) {
        case e.TINY:
        ...
        case e.TIMESTAMP:
        case e.NEWDATE:
          return _.typeMatch(a.columnType, C, e) ? S.readDateTimeString(
        ...
        case e.LONGLONG:
          return u ? s ? D ? S.readInt64String() : S.readSInt64String() : D ? S.readInt64() : S.readSInt64() : D ? S.readInt64JSNumber() : S.readSInt64JSNumber();
        default:
          return a.characterSet === n.BINARY ? S.readLengthCodedBuffer() : S.readLengthCodedString(T[O].encoding);
      }
    }
    return class {
      constructor() {
      ...
      next(i, c, O) {
        i.readInt8();
        const S = Math.floor((c.length + 7 + 2) / 8), u = new Array(S);
        for (let D = 0; D < S; D++)
          u[D] = i.readInt8();
        const s = O.rowsAsArray ? new Array(c.length) : {};
        let r = 4, C = 0;
        for (let D = 0; D < c.length; D++) {
          const o = c[D], B = O.typeCast !== void 0 ? O.typeCast : I.typeCast;
          let M;
          if (u[C] & r)
            M = null;
            ...
            M = typeof B == "function" ? B(
              {
                ...
                table: o.table,
                name: o.name,
                string: function(d = o.encoding) {
                  if (o.columnType === e.JSON && d === o.encoding && console.warn(
                    ...
                    e.TIMESTAMP,
                    e.DATE
                  ].includes(o.columnType))
                    return i.readDateTimeString(
                      parseInt(o.decimals, 10),
                      " ",
                      o.columnType
                    );
                  if (o.columnType === e.TINY) {
                    const U = o.flags & E.UNSIGNED;
                    return String(
                      U ? i.readInt8() : i.readSInt8()
                    );
                  }
                  return o.columnType === e.TIME ? i.readTimeString() : i.readLengthCodedString(d);
                },
                buffer: function() {
                  return i.readLengthCodedBuffer();
                },
          ...
          if (O.rowsAsArray)
            ...
            const h = _.fieldEscape(
              o.table + O.nestTables + o.name,
              !1
            );
            s[h] = M;
          } else if (O.nestTables === !0) {
            const h = _.fieldEscape(o.table, !1);
            s[h] || (s[h] = {});
            const d = _.fieldEscape(o.name, !1);
            s[h][d] = M;
          } else {
            const h = _.fieldEscape(o.name, !1);
            s[h] = M;
          }
          r *= 2, r === 256 && (r = 1, C++);
        }
...
function $R() {
  if (HA) return Dn;
  HA = 1;
  const E = AE(), n = pR(), e = eE(), _ = na(), R = ea();
  class N extends E {
    constructor(A, I) {
      super(), this.statement = A.statement, this.sql = A.sql, this.values = A.values, this.onResult = I, this.parameters = A.values, this.insertId = 0, this.timeout = A.timeout, this.queryTimeout = null, this._rows = [], this._fields = [], this._result = [], this._fieldCount = 0, this._rowParser = null, this._executeOptions = A, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
      }, this._streamFactory = A.infileStreamFactory, this._connection = null;
    }
    buildParserFromFields(A, I) {
      return this.options.disableEval ? R(A, this.options, I.config) : _(A, this.options, I.config);
    }
    start(A, I) {
      this._connection = I, this.options = Object.assign({}, I.config, this._executeOptions), this._setTimeout();
      const t = new e.Execute(
        this.statement.id,
        this.parameters,
        I.config.charsetNumber,
        I.config.timezone
      ...
      return N.prototype.resultsetHeader;
    }
    readField(A, I) {
      let t;
      const a = new e.ColumnDefinition(
        A,
        I.clientEncoding
      );
      return this._receivedFieldsCount++, this._fields[this._resultIndex].push(a), this._receivedFieldsCount === this._fieldCount ? (t = this._fields[this._resultIndex], this.emit("fields", t, this._resultIndex), N.prototype.fieldsEOF) : N.prototype.readField;
    }
    fieldsEOF(A, I) {
      return A.isEOF() ? (this._rowParser = new (this.buildParserFromFields(
        this._fields[this._resultIndex],
        I
      ))(), N.prototype.row) : I.protocolError("Expected EOF packet");
    }
  }
  return N.prototype.done = n.prototype.done, N.prototype.doneInsert = n.prototype.doneInsert, N.prototype.resultsetHeader = n.prototype.resultsetHeader, N.prototype._findOrCreateReadStream = n.prototype._findOrCreateReadStream, N.prototype._streamLocalInfile = n.prototype._streamLocalInfile, N.prototype._setTimeout = n.prototype._setTimeout, N.prototype._handleTimeoutError = n.prototype._handleTimeoutError, N.prototype.row = n.prototype.row, N.prototype.stream = n.prototype.stream, Dn = N, Dn;
}
var Sn, vA;
function Aa() {
  if (vA) return Sn;
  vA = 1;
  const E = eE(), n = AE(), e = xR(), _ = $R();
  class R {
    constructor(A, I, t, a, i) {
      this.query = A, this.id = I, this.columns = t, this.parameters = a, this.rowParser = null, this._connection = i;
    }
    close() {
      return this._connection.addCommand(new e(this.id));
    }
    execute(A, I) {
      return typeof A == "function" && (I = A, A = []), this._connection.addCommand(
        new _({ statement: this, values: A }, I)
      );
    }
  }
  class N extends n {
    constructor(A, I) {
      super(), this.query = A.sql, this.onResult = I, this.id = 0, this.fieldCount = 0, this.parameterCount = 0, this.fields = [], this.parameterDefinitions = [], this.options = A;
    }
    start(A, I) {
      ...
      return I.writePacket(i.toPacket(1)), N.prototype.prepareHeader;
    }
    prepareHeader(A, I) {
      const t = new E.PreparedStatementHeader(A);
      return this.id = t.id, this.fieldCount = t.fieldCount, this.parameterCount = t.parameterCount, this.parameterCount > 0 ? N.prototype.readParameter : this.fieldCount > 0 ? N.prototype.readField : this.prepareDone(I);
    }
    readParameter(A, I) {
      if (A.isEOF())
        return this.fieldCount > 0 ? N.prototype.readField : this.prepareDone(I);
      const t = new E.ColumnDefinition(A, I.clientEncoding);
      return this.parameterDefinitions.push(t), this.parameterDefinitions.length === this.parameterCount ? N.prototype.parametersEOF : this.readParameter;
    }
    readField(A, I) {
      if (A.isEOF())
        return this.prepareDone(I);
      const t = new E.ColumnDefinition(A, I.clientEncoding);
      return this.fields.push(t), this.fields.length === this.fieldCount ? N.prototype.fieldsEOF : N.prototype.readField;
    }
    parametersEOF(A, I) {
      return A.isEOF() ? this.fieldCount > 0 ? N.prototype.readField : this.prepareDone(I) : I.protocolError("Expected EOF packet after parameters");
    }
    fieldsEOF(A, I) {
      return A.isEOF() ? this.prepareDone(I) : I.protocolError("Expected EOF packet after fields");
    }
    prepareDone(A) {
      const I = new R(
        this.query,
      ...
      return A._statements.set(this.key, I), this.onResult && this.onResult(null, I), null;
    }
  }
  return Sn = N, Sn;
}
...
function Ra() {
  ...
  class _ extends E {
    constructor(N) {
      super(), this.onResult = N;
    }
    start(N, T) {
      const A = new e(
        0,
        Buffer.from([1, 0, 0, 0, n.PING]),
        0,
        5
      );
      return T.writePacket(A), _.prototype.pingResponse;
    }
    pingResponse() {
...
function ta() {
  ...
  class e extends E {
    constructor(R, N) {
      super(), this.onResult = N, this.opts = R;
    }
    start(R, N) {
      const T = new n.RegisterSlave(this.opts);
      return N.writePacket(T.toPacket(1)), e.prototype.registerResponse;
    }
    registerResponse() {
      return this.onResult && process.nextTick(this.onResult.bind(this)), null;
    }
      ...
      switch (N = e[R++], N) {
        ...
        case E.UPDATED_DB_NAMES:
          for (T = e[R++], _.updatedDBs = []; T; --T) {
            for (A = R; e[R++] && R < e.length; )
              ;
            _.updatedDBs.push(
              e.toString("utf8", A, R - 1)
            );
          }
          break;
        case E.MICROSECONDS:
...
function aa() {
  ...
  class R extends E {
    ...
    start(a, i) {
      const c = new n.BinlogDump(this.opts);
      return i.writePacket(c.toPacket(1)), R.prototype.binlogData;
    }
    binlogData(a) {
      if (a.isEOF())
        return this.emit("eof"), null;
      a.readInt8();
      const i = new _(a), c = e[i.eventType];
      let O;
      return c ? O = new c(a) : O = {
        name: "UNKNOWN"
  ...
  class A {
    constructor(a) {
      ...
      const O = a.readInt16(), S = a.readBuffer(O);
      this.schema = a.readString(c), a.readInt8(), this.statusVars = i(S), this.query = a.readString(), this.name = "QueryEvent";
    }
  }
  class I {
    constructor(a) {
  ...
  return e[2] = A, e[4] = N, e[15] = T, e[16] = I, Mn = R, Mn;
}
var hn, jA;
function Ia() {
  if (jA) return hn;
  jA = 1;
  const E = AE(), n = eE(), e = rE(), _ = ZR(), R = iE();
  class N extends E {
    constructor(A, I) {
      super(), this.onResult = I, this.user = A.user, this.password = A.password, this.password1 = A.password, this.password2 = A.password2, this.password3 = A.password3, this.database = A.database, this.passwordSha1 = A.passwordSha1, this.charsetNumber = A.charsetNumber, this.currentConfig = A.currentConfig, this.authenticationFactor = 0;
    }
    start(A, I) {
      const t = new n.ChangeUser({
      ...
      return this.currentConfig.user = this.user, this.currentConfig.password = this.password, this.currentConfig.database = this.database, this.currentConfig.charsetNumber = this.charsetNumber, I.clientEncoding = R[this.charsetNumber], I._statements.clear(), I.writePacket(t.toPacket()), I.serverCapabilityFlags & e.MULTI_FACTOR_AUTHENTICATION && (this.authenticationFactor = 1), N.prototype.handshakeResult;
    }
  }
  return N.prototype.handshakeResult = _.prototype.handshakeResult, N.prototype.calculateNativePasswordAuthToken = _.prototype.calculateNativePasswordAuthToken, hn = N, hn;
}
var Ln, JA;
function Na() {
  if (JA) return Ln;
  ...
  class _ extends E {
    ...
    start(N, T) {
      ...
      const A = new e(
        0,
        Buffer.from([1, 0, 0, 0, n.QUIT]),
        0,
        5
      );
      return this.onResult && this.onResult(), T.writePacket(A), null;
    }
  }
  return Ln = _, Ln;
}
...
function Ta() {
  if (zA) return fn;
  zA = 1;
  const E = ZR(), n = pi(), e = pR(), _ = Aa(), R = xR(), N = $R(), T = Ra(), A = ta(), I = aa(), t = Ia(), a = Na();
  return fn = {
    ClientHandshake: E,
    ServerHandshake: n,
    Query: e,
...
var gn = {}, SE = { exports: {} }, gE = {}, ZA;
function ua() {
  return ZA || (ZA = 1, Object.defineProperty(gE, "__esModule", { value: !0 }), gE.defaults = void 0, gE.defaults = [
    `-----BEGIN CERTIFICATE-----
...
var dE = {}, qA;
function Oa() {
  return qA || (qA = 1, Object.defineProperty(dE, "__esModule", { value: !0 }), dE.proxies = void 0, dE.proxies = [
    `-----BEGIN CERTIFICATE-----
...
var sa = SE.exports, pA;
function Ca() {
  if (pA) return SE.exports;
  pA = 1, Object.defineProperty(sa, "__esModule", { value: !0 });
  const E = ua(), n = Oa(), e = {
    ca: n.proxies
  }, _ = {
    ca: [...E.defaults, ...n.proxies]
  };
...
function Ae() {
  if ($A) return dn;
  $A = 1;
  const { URL: E } = Wt, n = rE(), e = ME(), { version: _ } = ca;
  let R = null;
  const N = {
    authPlugins: 1,
    authSwitchHandler: 1,
  ...
  class T {
    constructor(I) {
      if (typeof I == "string")
        ...
        const a = T.parseUrl(I.uri);
        for (const i in a)
          Object.prototype.hasOwnProperty.call(a, i) && (I[i] || (I[i] = a[i]));
      }
      for (const a in I)
        Object.prototype.hasOwnProperty.call(I, a) && N[a] !== 1 && console.error(
      ...
      ), this.timezone = "Z") : this.timezone = I.timezone || "local", this.queryFormat = I.queryFormat, this.pool = I.pool || void 0, this.ssl = typeof I.ssl == "string" ? T.getSSLProfile(I.ssl) : I.ssl || !1, this.multipleStatements = I.multipleStatements || !1, this.rowsAsArray = I.rowsAsArray || !1, this.namedPlaceholders = I.namedPlaceholders || !1, this.nestTables = I.nestTables === void 0 ? void 0 : I.nestTables, this.typeCast = I.typeCast === void 0 ? !0 : I.typeCast, this.disableEval = !!I.disableEval, this.timezone[0] === " " && (this.timezone = `+${this.timezone.slice(1)}`), this.ssl) {
      ...
      this.maxPacketSize = 0, this.charsetNumber = I.charset ? T.getCharsetNumber(I.charset) : I.charsetNumber || e.UTF8MB4_UNICODE_CI, this.compress = I.compress || !1, this.authPlugins = I.authPlugins, this.authSwitchHandler = I.authSwitchHandler, this.clientFlags = T.mergeFlags(
        T.getDefaultFlags(I),
        I.flags || ""
      );
      const t = {
    ...
    static mergeFlags(I, t) {
      ...
      for (i in I)
        t.indexOf(`-${I[i]}`) >= 0 || (a |= n[I[i]] || 0);
      for (i in t)
        t[i][0] !== "-" && (I.indexOf(t[i]) >= 0 || (a |= n[t[i]] || 0));
      return a;
    }
...
var vE = { exports: {} }, ER;
function Sa() {
  if (ER) return vE.exports;
  ER = 1;
  const E = /(?:\?)|(?::(\d+|(?:[a-zA-Z][a-zA-Z0-9_]*)))/g, n = 34, e = 39, _ = 92;
  function R(A) {
    let I = E.exec(A), t = 0, a = 0, i;
    const c = [];
    let O = !1, S = !1, u;
    const s = [];
    let r = 0, C = 0, D;
      ...
      do {
        for (D = t, i = I.index; D < i; ++D) {
          ...
          if (o === _) S = !S;
            ...
            if (O && o === u) {
              if (A.charCodeAt(D + 1) === u) {
                ++D;
                continue;
              }
              O = !1;
            } else !O && (o === n || o === e) && (O = !0, u = o);
          }
        }
        O || (c.push(A.substring(a, i)), s.push(I[0].length === 1 ? r++ : I[1]), a = i + I[0].length, C = a), t = i + I[0].length;
      } while (I = E.exec(A));
      if (s.length)
        return t < A.length && c.push(A.substring(C)), [c, s];
    }
    return [A];
  }
  function N(A) {
    A || (A = {}), A.placeholder || (A.placeholder = "?");
    let I = 100, t;
    typeof A.cache == "number" && (I = A.cache), typeof A.cache == "object" && (t = A.cache), A.cache !== !1 && !t && (t = $E().createLRU({ max: I }));
    function a(S, u) {
      const s = [];
      if (S.length === 1)
        return [S[0], []];
    ...
    function c(S) {
      ...
      for (let r = 1; r < S[0].length; ++r)
        S[0][r - 1].slice(-1) === ":" && (u += A.placeholder), u += A.placeholder, u += i(S[0][r]);
      const s = S[0][S[0].length - 1];
      return S[0].length === S[1].length && (s.slice(-1) === ":" && (u += A.placeholder), u += A.placeholder), [u, S[1]];
    }
    function O(S, u) {
      let s;
      return t && (s = t.get(S)) || (s = c(R(S)), t && t.set(S, s)), a(s, u);
    }
    return O.parse = R, O;
  }
  function T(A, I) {
    ...
    for (let s = 0; s < t[0].length; ++s)
      S = i[t[1][s]], S || (S = ++c, i[t[1][s]] = S), t[1][s] ? (u[S - 1] = t[1][s], O += `${t[0][s]}$${S}`) : O += t[0][s];
    return [O, u.map((s) => I[s])];
  }
  return vE.exports = N, vE.exports.toNumbered = T, vE.exports;
}
var Un, _R;
function n_() {
  if (_R) return Un;
  _R = 1;
  const E = Vt, n = mR, e = Ee, _ = NE.EventEmitter, R = YR.Readable, N = KR(), T = _e(), { createLRU: A } = $E(), I = JR(), t = eE(), a = Ta(), i = Ae(), c = iE();
  let O = 0, S = null;
  class u extends _ {
    constructor(r) {
      ...
      }), this.stream.setNoDelay(!0)), this._internalId = O++, this._commands = new N(), this._command = null, this._paused = !1, this._paused_packets = new N(), this._statements = A({
        max: this.config.maxPreparedStatements,
        onEviction: function(D, o) {
          o.close();
        }
      }), this.serverCapabilityFlags = 0, this.authorized = !1, this.sequenceId = 0, this.compressedSequenceId = 0, this.threadId = null, this._handshakePacket = null, this._fatalError = null, this._protocolError = null, this._outOfOrderPackets = [], this.clientEncoding = c[this.config.charsetNumber], this.stream.on("error", this._handleNetworkError.bind(this)), this.packetParser = new I((D) => {
        this.handlePacket(D);
    ...
    _notifyError(r) {
      ...
      for (this._command && this._command.onResult ? (this._command.onResult(r), this._command = null) : this._command && this._command.constructor === a.ClientHandshake && this._commands.length > 0 || (D = !0); C = this._commands.shift(); )
    ...
    writePacket(r) {
      ...
      if (D < 16777215)
        ...
        for (this.config.debug && (console.log(
          ...
          `${this._internalId} ${this.connectionId} <== ${this._command._commandName}#${this._command.stateName()}(${[this.sequenceId, r._name, r.length()].join(",")})`
        )), B = 4; B < 4 + D; B += 16777215)
          o = r.buffer.slice(B, B + 16777215), o.length === 16777215 ? M = Buffer.from([255, 255, 255, this.sequenceId]) : M = Buffer.from([
            o.length & 255,
            o.length >> 8 & 255,
            o.length >> 16 & 255,
            this.sequenceId
          ]), this._bumpSequenceId(1), this.write(M), this.write(o);
    }
    // 0.11+ environment
    startTLS(r) {
      this.config.debug && console.log("Upgrading connection to TLS");
      const C = n.createSecureContext({
      ...
      }), D = this.config.ssl.rejectUnauthorized, o = this.config.ssl.verifyIdentity, B = E.isIP(this.config.host) ? void 0 : this.config.host;
      let M = !1;
      ...
      const h = n.connect(
        {
          ...
          requestCert: D,
          checkServerIdentity: o ? n.checkServerIdentity : function() {
          },
          secureContext: C,
          isServer: !1,
          socket: this.stream,
        ...
        () => {
          if (M = !0, D && typeof B == "string" && o) {
            const d = h.getPeerCertificate(!0), U = n.checkServerIdentity(
              B,
              d
            );
            if (U) {
    ...
    format(r, C) {
      ...
      return this._resolveNamedPlaceholders(D), T.format(
        D.sql,
        D.values,
    ...
    _resolveNamedPlaceholders(r) {
      ...
      if (this.config.namedPlaceholders || r.namedPlaceholders) {
        if (Array.isArray(r.values))
          return;
        S === null && (S = Sa()()), C = S(r.sql, r.values), r.sql = C[0], r.values = C[1];
      }
    }
    query(r, C, D) {
      let o;
      r.constructor === a.Query ? o = r : o = u.createQuery(r, C, D, this.config), this._resolveNamedPlaceholders(o);
      const B = this.format(
        o.sql,
        o.values !== void 0 ? o.values : []
      );
      return o.sql = B, this.addCommand(o);
    }
    pause() {
    ...
    unprepare(r) {
      ...
      const D = u.statementKey(C), o = this._statements.get(D);
      return o && (this._statements.delete(D), o.close()), o;
    }
    execute(r, C, D) {
      let o = {
        infileStreamFactory: this.config.infileStreamFactory
      };
      if (typeof r == "object" ? (o = {
        ...o,
        ...r,
        sql: r.sql,
        values: r.values
      }, typeof C == "function" ? D = C : o.values = o.values || C) : typeof C == "function" ? (D = C, o.sql = r, o.values = void 0) : (o.sql = r, o.values = C), this._resolveNamedPlaceholders(o), o.values) {
      ...
      const B = new a.Execute(o, D), M = new a.Prepare(o, (h, d) => {
        if (h) {
          B.start = function() {
            return null;
          }, D ? D(h) : B.emit("error", h), B.emit("end");
          return;
        }
        B.statement = d;
      });
    ...
    writeTextResult(r, C, D = !1) {
      this.writeColumns(C), r.forEach((o) => {
        const B = new Array(C.length);
        C.forEach((M) => {
          B.push(o[M.name]);
        }), D ? this.writeBinaryRow(B) : this.writeTextRow(B);
      }), this.writeEof();
    }
    writeEof(r, C) {
    ...
    static createQuery(r, C, D, o) {
      let B = {
        rowsAsArray: o.rowsAsArray,
        infileStreamFactory: o.infileStreamFactory
      };
      return typeof r == "object" ? (B = {
        ...
        sql: r.sql,
        values: r.values
      }, typeof C == "function" ? D = C : C !== void 0 && (B.values = C)) : typeof C == "function" ? (D = C, B.sql = r, B.values = void 0) : (B.sql = r, B.values = C), new a.Query(B, D);
    }
...
function Et() {
  ...
  return n.statementKey = E.statementKey, wn = n, n.prototype._realEnd = E.prototype.end, wn;
}
var Gn, eR;
function FE() {
  if (eR) return Gn;
  ...
  function E(n, e, _) {
    return function(R, N, T) {
      R ? (_.message = R.message, _.code = R.code, _.errno = R.errno, _.sql = R.sql, _.sqlState = R.sqlState, _.sqlMessage = R.sqlMessage, e(_)) : n([N, T]);
    };
  }
  return Gn = E, Gn;
}
...
function oa() {
  ...
  class n {
    ...
    execute(_) {
      const R = this.statement, N = new Error();
...
function Re() {
  ...
  function E(n, e, _) {
    const R = {};
    e.on("newListener", (N) => {
      _.indexOf(N) >= 0 && !e.listenerCount(N) && n.on(
        N,
        R[N] = function() {
          const T = [].slice.call(arguments);
          T.unshift(N), e.emit.apply(e, T);
        }
      );
    }).on("removeListener", (N) => {
      _.indexOf(N) >= 0 && !e.listenerCount(N) && (n.removeListener(N, R[N]), delete R[N]);
    });
  }
  return Qn = E, Qn;
}
...
function te() {
  if (tR) return Pn;
  tR = 1;
  const E = NE.EventEmitter, n = oa(), e = FE(), _ = Re(), R = n_();
  class N extends E {
    constructor(A, I) {
      super(), this.connection = A, this.Promise = I || Promise, _(A, this, [
        "error",
        "drain",
        "connect",
        "end",
    ...
    query(A, I) {
      ...
      return new this.Promise((i, c) => {
        const O = e(i, c, a);
        I !== void 0 ? t.query(A, I, O) : t.query(A, O);
      });
    }
    execute(A, I) {
      const t = this.connection, a = new Error();
      ...
      return new this.Promise((i, c) => {
        const O = e(i, c, a);
        I !== void 0 ? t.execute(A, I, O) : t.execute(A, O);
      });
    }
    end() {
      return new this.Promise((A) => {
        this.connection.end(A);
      });
    }
    beginTransaction() {
    ...
    prepare(A) {
      ...
      return new this.Promise((i, c) => {
        I.prepare(A, (O, S) => {
          if (O)
            a.message = O.message, a.code = O.code, a.errno = O.errno, a.sqlState = O.sqlState, a.sqlMessage = O.sqlMessage, c(a);
          else {
            const u = new n(
              S,
              t
  ...
  return (function(T) {
    for (let A = 0; T && A < T.length; A++) {
      const I = T[A];
      typeof R.prototype[I] == "function" && N.prototype[I] === void 0 && (N.prototype[I] = /* @__PURE__ */ (function(a) {
        return function() {
          return R.prototype[a].apply(
            this.connection,
            arguments
          );
        };
...
function e_() {
  ...
  class e extends E {
    ...
    destroy() {
      return n.prototype.destroy.apply(
        this.connection,
        arguments
      );
    }
...
function _t() {
  if (IR) return mn;
  IR = 1;
  const E = xE, n = _e(), e = NE.EventEmitter, _ = la(), R = KR(), N = n_();
  function T(I, t) {
    const a = I.length;
    for (let i = 0; i < a; i++)
      if (I.get(i) === t) {
  ...
  class A extends e {
    constructor(t) {
      super(), this.config = t.config, this.config.connectionConfig.pool = this, this._allConnections = new R(), this._freeConnections = new R(), this._connectionQueue = new R(), this._closed = !1, this.config.maxIdle < this.config.connectionLimit && this._removeIdleTimeoutConnections();
    }
    getConnection(t) {
      if (this._closed)
    ...
    releaseConnection(t) {
      ...
      this._closed = !0, clearTimeout(this._removeIdleTimeoutConnectionsTimer), typeof t != "function" && (t = function(S) {
        if (S)
          throw S;
      });
      let a = !1, i = 0, c;
      const O = (function(S) {
        if (!a && (S || ++i >= this._allConnections.length)) {
          a = !0, t(S);
          return;
        }
      }).bind(this);
      ...
      for (let S = 0; S < this._allConnections.length; S++)
        c = this._allConnections.get(S), c._realEnd(O);
    }
    query(t, a, i) {
      const c = N.createQuery(
        t,
        a,
        i,
        this.config.connectionConfig
      );
      return typeof c.namedPlaceholders > "u" && (c.namedPlaceholders = this.config.connectionConfig.namedPlaceholders), this.getConnection((O, S) => {
        if (O) {
          typeof c.onResult == "function" ? c.onResult(O) : c.emit("error", O);
          return;
        }
        try {
          S.query(c).once("end", () => {
    ...
    execute(t, a, i) {
      typeof a == "function" && (i = a, a = []), this.getConnection((c, O) => {
        if (c)
          return i(c);
        try {
          O.execute(t, a, i).once("end", () => {
            O.release();
          });
        } catch (S) {
    ...
    _removeConnection(t) {
      T(this._allConnections, t), T(this._freeConnections, t), this.releaseConnection(t);
    }
...
function nt() {
  if (NR) return Yn;
  NR = 1;
  const E = NE.EventEmitter, n = FE(), e = e_(), _ = Re(), R = _t();
  class N extends E {
    constructor(A, I) {
      super(), this.pool = A, this.Promise = I || Promise, _(A, this, ["acquire", "connection", "enqueue", "release"]);
    }
    getConnection() {
      const A = this.pool;
      return new this.Promise((I, t) => {
        A.getConnection((a, i) => {
          a ? t(a) : I(new e(i, this.Promise));
        });
      });
    }
    releaseConnection(A) {
      A instanceof e && A.release();
    }
    query(A, I) {
    ...
    execute(A, I) {
      ...
      return new this.Promise((i, c) => {
        const O = n(i, c, a);
        I ? t.execute(A, I, O) : t.execute(A, O);
      });
    }
    end() {
  ...
  return (function(T) {
    for (let A = 0; T && A < T.length; A++) {
      ...
      typeof R.prototype[I] == "function" && N.prototype[I] === void 0 && (N.prototype[I] = /* @__PURE__ */ (function(a) {
        return function() {
          return R.prototype[a].apply(this.pool, arguments);
        };
      })(I));
    }
  })([
...
function tt() {
  if (uR) return yn;
  uR = 1;
  const E = xE, n = et(), e = At(), _ = Rt(), R = NE.EventEmitter, N = {
    RR() {
      let a = 0;
  ...
  class I {
    constructor(i, c, O) {
      this._cluster = i, this._pattern = c, this._selector = N[O]();
    }
    getConnection(i) {
      ...
      return this._cluster._getConnection(c, (O, S) => O ? this._cluster._canRetry && this._cluster._findNodeIds(this._pattern).length !== 0 ? (this._cluster.emit("warn", O), this.getConnection(i)) : i(O) : i(null, S));
    }
    /**
     * pool cluster query
     * @param {*} sql
     ...
     */
    query(i, c, O) {
      const S = _.createQuery(i, c, O, {});
      return this.getConnection((u, s) => {
        if (u) {
     ...
     */
    execute(i, c, O) {
      typeof c == "function" && (O = c, c = []), this.getConnection((S, u) => {
        if (S)
          return O(S);
        try {
          u.execute(i, c, O).once("end", () => {
            u.release();
          });
        } catch (s) {
          throw u.release(), s;
  ...
  class t extends R {
    ...
    of(i, c) {
      i = i || "*", c = c || this._defaultSelector, c = c.toUpperCase(), !N[c] === "undefined" && (c = this._defaultSelector);
      const O = i + c;
      return typeof this._namespaces[O] > "u" && (this._namespaces[O] = new I(this, i, c)), this._namespaces[O];
    }
    add(i, c) {
      typeof i == "object" && (c = i, i = `CLUSTER::${++this._lastId}`), typeof this._nodes[i] > "u" && (this._nodes[i] = {
        id: i,
        errorCount: 0,
        pool: new n({ config: new e(c) }),
        _offlineUntil: 0
    ...
    remove(i) {
      ...
      for (let O = 0; O < c.length; O++) {
        const S = this._getNode(c[O]);
        S && this._removeNode(S);
      }
    }
    getConnection(i, c, O) {
      let S;
      typeof i == "function" ? (O = i, S = this.of()) : (typeof c == "function" && (O = c, c = this._defaultSelector), S = this.of(i, c)), S.getConnection(O);
    }
    end(i) {
      const c = i !== void 0 ? i : (s) => {
        if (s)
          throw s;
      };
      if (this._closed) {
    ...
    _findNodeIds(i, c) {
      ...
      return this._findCaches[i] = S, c ? S : S.filter((u) => {
        const s = this._getNode(u);
        return s._offlineUntil ? (O || (O = T()), s._offlineUntil <= O) : !0;
      });
    }
    _getNode(i) {
    ...
    _increaseErrorCount(i) {
      ...
      if (!(this._removeNodeErrorCount > c)) {
        if (this._restoreNodeTimeout > 0) {
          i._offlineUntil = T() + this._restoreNodeTimeout, this.emit("offline", i.id);
          return;
        }
    ...
    _getConnection(i, c) {
      i.pool.getConnection((O, S) => O ? (this._increaseErrorCount(i), c(O)) : (this._decreaseErrorCount(i), S._clusterId = i.id, c(null, S)));
    }
    _removeNode(i) {
...
function La() {
  ...
  class e {
    constructor(R, N) {
      this.poolNamespace = R, this.Promise = N || Promise;
    }
    getConnection() {
      const R = this.poolNamespace;
      return new this.Promise((N, T) => {
        R.getConnection((A, I) => {
          A ? T(A) : N(new E(I, this.Promise));
        });
      });
    }
    query(R, N) {
      const T = this.poolNamespace, A = new Error();
...
function fa() {
  return SR || (SR = 1, (function(E) {
    const n = _e(), e = NE.EventEmitter, _ = ne(), R = tt(), N = Ba(), T = Ma(), A = ha(), I = te(), t = nt(), a = FE(), i = e_(), c = Re(), O = La();
    function S(C) {
      const D = N(C), o = new Error(), B = C.Promise || Promise;
      if (!B)
        throw new Error(
      ...
      return new B((M, h) => {
        D.once("connect", () => {
          ...
          o.message = d.message, o.code = d.code, o.errno = d.errno, o.sqlState = d.sqlState, h(o);
        });
      });
    }
    function u(C) {
      const D = T(C), o = C.Promise || Promise;
      if (!o)
        throw new Error(
          "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
        );
    ...
    class s extends e {
      constructor(D, o) {
        super(), this.poolCluster = D, this.Promise = o || Promise, c(D, this, ["warn", "remove", "online", "offline"]);
      }
      getConnection(D, o) {
      ...
      of(D, o) {
        return new O(
          this.poolCluster.of(D, o),
          this.Promise
        );
      }
      end() {
        const D = this.poolCluster, o = new Error();
        return new this.Promise((B, M) => {
          D.end((h) => {
            h ? (o.message = h.message, o.code = h.code, o.errno = h.errno, o.sqlState = h.sqlState, o.sqlMessage = h.sqlMessage, M(o)) : B();
          });
        });
      }
    }
    (function(C) {
      for (let D = 0; C && D < C.length; D++) {
        const o = C[D];
        typeof R.prototype[o] == "function" && s.prototype[o] === void 0 && (s.prototype[o] = /* @__PURE__ */ (function(M) {
          return function() {
            return R.prototype[M].apply(
              this.poolCluster,
              arguments
            );
          };
    ...
    function r(C) {
      const D = A(C), o = C && C.Promise || Promise;
      if (!o)
        throw new Error(
          "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
        );
    ...
    E.createConnection = S, E.createPool = u, E.createPoolCluster = r, E.escape = n.escape, E.escapeId = n.escapeId, E.format = n.format, E.raw = n.raw, E.PromisePool = t, E.PromiseConnection = I, E.PromisePoolConnection = i, E.__defineGetter__("Types", () => IE()), E.__defineGetter__(
      "Charsets",
      () => ME()
    ), E.__defineGetter__(
      "CharsetToEncoding",
...
function wa() {
  if (!KE) {
    const E = process.env.MYSQL_PORT ? Number(process.env.MYSQL_PORT) : 3306, n = (process.env.MYSQL_SSL || "").toLowerCase(), e = n === "1" || n === "true" || n === "yes", _ = (process.env.MYSQL_SSL_REJECT_UNAUTHORIZED || "").toLowerCase(), R = !(_ === "0" || _ === "false" || _ === "no");
    KE = da.createPool({
...
async function Q(E) {
  ...
  try {
    return { ok: !0, value: await E(wa()) };
  } catch (n) {
    return console.warn("[MySQL] Error:", n), { ok: !1, error: n };
  }
}
function hE(E) {
  const [n, e] = E.split(":").map(Number);
  if (isNaN(n) || isNaN(e))
...
function Qa(E) {
  let e = G().prepare(`
    ...
    ORDER BY h.hora
  `).all(E, E);
  return it(E) && (e = e.filter((_) => _.hora < "12:00")), e;
}
async function Pa(E) {
  const n = new Date(E).toISOString().split("T")[0], e = await Q(async (_) => {
    const [R] = await _.execute(
      `
        SELECT h.hora
        FROM horarios_base h
  ...
  if (e.ok) {
    let _ = e.value;
    return it(E) && (_ = _.filter((R) => R.hora < "12:00")), _;
  }
  return Qa(n);
}
function oR(E) {
...
async function Va(E) {
  if (console.log("[Service] Desactivando horario:", E), (await Q(async (e) => {
...
function ma() {
  ...
  `).all();
  return console.log("[Service] Horarios inactivos encontrados:", n.length), n;
}
async function Ya() {
  console.log("[Service] Obteniendo horarios inactivos");
  const E = await Q(async (n) => {
    const [e] = await n.execute(
      "SELECT id, hora FROM horarios_base WHERE activo = 0 ORDER BY hora"
    );
    return e;
  });
  return E.ok ? E.value : ma();
}
function BR(E) {
  const n = G();
  n.transaction(() => {
...
function MR(E, n, e) {
  console.log("[Service] Bloqueando horario:", { fecha: E, hora: n, motivo: e });
  const _ = G(), R = new Date(E).toISOString().split("T")[0], N = hE(n);
  console.log("[Service] Fecha normalizada:", E, "->", R), console.log("[Service] Hora normalizada:", n, "->", N);
  try {
    _.transaction(() => {
      if (_.prepare(`
      ...
      _.prepare(`
        ...
        VALUES ( ?, ?, ?)
      `).run(R, N, e ?? ""), console.log("[Service] Horario bloqueado");
    })();
  } catch (T) {
    throw console.error("[Service] Error en bloquearHorario:", T), T;
  }
}
async function Ha(E, n, e) {
  console.log("[Service] Bloqueando horario:", { fecha: E, hora: n, motivo: e });
  const _ = new Date(E).toISOString().split("T")[0], R = hE(n);
  if ((await Q(async (T) => {
    const [A] = await T.execute(
      "SELECT id FROM bloqueos_horarios WHERE fecha = ? AND hora = ?",
      [_, R]
    );
    A.length || await T.execute(
...
function hR(E, n) {
  console.log("[Service] Desbloqueando horario:", { fecha: E, hora: n });
  const e = G(), _ = new Date(E).toISOString().split("T")[0], R = hE(n);
  console.log("[Service] Fecha normalizada:", E, "->", _), console.log("[Service] Hora normalizada:", n, "->", R);
  try {
    e.transaction(() => {
      e.prepare(`
...
async function va(E, n) {
  console.log("[Service] Desbloqueando horario:", { fecha: E, hora: n });
  const e = new Date(E).toISOString().split("T")[0], _ = hE(n);
  if ((await Q(async (N) => {
    await N.execute(
    ...
    try {
      ...
      console.warn("[Service] Backup SQLite fallo en desbloquearHorario:", N);
    }
    return;
  }
  return hR(e, _);
...
function ya(E) {
  ...
  const n = G(), e = new Date(E).toISOString().split("T")[0];
  ...
  const R = n.prepare("SELECT * FROM bloqueos_horarios ORDER BY fecha, hora").all();
...
async function Ka(E) {
  if (console.log("[Service] Borrando horario permanentemente:", E), (await Q(async (e) => {
...
async function y(E) {
  return new Promise((n, e) => {
    const _ = `op_${++ka}`, R = Date.now();
    sE.push({ id: _, fn: E, resolve: n, reject: e, createdAt: R }), console.log(`[Lock] ${_} encolada. Cola: ${sE.length} operaciones. Locked: ${kE}`), at();
  });
}
async function at() {
  if (kE) {
  ...
  try {
    ...
    kE = !1, console.log(`[Lock] ${E.id} liberada. Quedan: ${sE.length}`), sE.length > 0 && (console.log("[Lock] Procesando siguiente..."), setImmediate(() => at()));
  }
}
function ja() {
  F("horarios:base", async () => {
    ...
    "horarios:crear",
    async (E, n) => await y(() => ba(n))
  ), F("horarios:desactivar", async (E, n) => {
    console.log("[IPC] Desactivando horario:", n);
    const e = await y(() => Va(n));
    return console.log("[IPC] Horario desactivado exitosamente"), e;
  }), F("horarios:activar", async (E, n) => {
    console.log("[IPC] Activando horario:", n);
    const e = await y(() => Wa(n));
    ...
    "horarios:bloquear",
    async (E, n) => await y(
      () => Ha(n.fecha, n.hora, n.motivo)
    )
  ), F("horarios:desbloquear", async (E, n) => {
    console.log("[IPC] Desbloqueando horario:", n);
    const e = await y(
      () => va(n.fecha, n.hora)
    );
    return console.log("[IPC] Horario desbloqueado exitosamente"), e;
  }), F("horarios:bloqueados", async (E, n) => {
    console.log("[IPC] Obteniendo horarios bloqueados para:", n);
    const e = await y(() => Xa(n));
    return console.log("[IPC] Horarios bloqueados obtenidos:", e), e;
  }), F("horarios:borrar", async (E, n) => {
    console.log("[IPC] Borrando horario permanentemente:", n);
    const e = await y(() => Ka(n));
...
function It(E) {
  const n = E.tipo_turno;
  return n !== "Garantía" && (E.garantia_tipo = null, E.garantia_fecha_compra = null, E.garantia_numero_service = null, E.garantia_problema = null), n !== "Particular" && (E.particular_tipo = null), E;
}
function qa(E) {
  return String(E || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
}
function pa(E) {
  const n = E.tipo_turno, e = /^\d+$/.test(String(E.km || "").trim());
  if (n === "Particular") {
...
async function Nt(E, n = 0) {
  try {
    return console.log(`[Service] Intento ${n + 1}/${fR}`), E();
  } catch (e) {
    if (e.code === "SQLITE_BUSY" && n < fR - 1)
      return console.warn(`[Service] SQLITE_BUSY, reintentando en ${gR}ms...`), await new Promise((_) => setTimeout(_, gR)), Nt(E, n + 1);
    throw e;
  }
}
async function dR(E, n) {
  return Nt(() => {
    const e = G(), _ = e.transaction(() => {
      ...
      console.log("[Service] Reserva insertada con ID:", N.lastInsertRowid);
      const T = e.prepare(`
...
async function $a(E) {
  console.log("[Service] Iniciando crearReserva..."), Za(E), pa(E);
  const n = It({ ...E }), e = new Date(n.fecha).toISOString().split("T")[0];
  console.log("[Service] Fecha normalizada:", n.fecha, "->", e);
  try {
    const _ = await xa(n, e);
    try {
      await dR(n, e);
    } catch (R) {
    ...
    return console.warn("[Service] MySQL no disponible, usando SQLite local"), await dR(n, e);
  }
}
async function Jn(E) {
  ...
  return n.ok ? n.value : G().prepare("SELECT * FROM reservas WHERE id = ?").get(E);
}
async function EI(E) {
  if (console.log("[Service] Borrando reserva:", E), (await Q(async (_) => {
    const [R] = await _.execute("SELECT * FROM reservas WHERE id = ?", [E]), N = R[0];
    if (!N) {
    ...
    try {
      const _ = G();
      _.transaction(() => {
        const N = _.prepare("SELECT * FROM reservas WHERE id = ?").get(E);
        N && (_.prepare("DELETE FROM reservas WHERE id = ?").run(E), _.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
...
async function _I(E, n, e) {
  if (console.log("[Service] Moviendo reserva:", { id: E, nuevaFecha: n, nuevaHora: e }), (await Q(async (N) => {
    const [T] = await N.execute(
    ...
    try {
      const N = G();
      N.transaction(() => {
        const A = N.prepare("SELECT fecha, hora FROM reservas WHERE id = ?").get(E);
        A && (N.prepare("UPDATE reservas SET fecha = ?, hora = COALESCE( ?, hora) WHERE id = ?").run(n, e ?? null, E), n !== A.fecha && N.prepare(`
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
            VALUES ( ?, 'fecha', ?, ?, datetime('now'))
          `).run(E, A.fecha, n), e && e !== A.hora && N.prepare(`
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
      ...
      console.warn("[Service] Backup SQLite fallo en moverReserva:", N);
    }
    return;
  }
  const R = G();
  try {
    R.transaction(() => {
      ...
      R.prepare("UPDATE reservas SET fecha = ?, hora = COALESCE( ?, hora) WHERE id = ?").run(n, e ?? null, E), n !== T.fecha && R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
          VALUES ( ?, 'fecha', ?, ?, datetime('now'))
        `).run(E, T.fecha, n), e && e !== T.hora && R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
          VALUES ( ?, 'hora', ?, ?, datetime('now'))
        `).run(E, T.hora, e);
    ...
    throw console.error("[Service] Error en moverReserva:", N), N;
  }
}
async function nI(E, n) {
  console.log("[Service] Actualizando reserva:", E, n);
  const e = Number(E || (n == null ? void 0 : n.id) || 0);
  if (!e)
    throw new Error("ID de reserva invalido.");
  const _ = qa((n == null ? void 0 : n.matricula) || "").slice(0, 10);
  if (_ && !/^[A-Z0-9]{3,10}$/.test(_))
    throw new Error("Matricula invalida. Usa solo letras y numeros.");
  const R = It({
    nombre: String((n == null ? void 0 : n.nombre) ?? ""),
    cedula: String((n == null ? void 0 : n.cedula) ?? ""),
    telefono: String((n == null ? void 0 : n.telefono) ?? ""),
    marca: String((n == null ? void 0 : n.marca) ?? ""),
    modelo: String((n == null ? void 0 : n.modelo) ?? ""),
    km: String((n == null ? void 0 : n.km) ?? ""),
    matricula: _,
    tipo_turno: String((n == null ? void 0 : n.tipo_turno) ?? ""),
    particular_tipo: (n == null ? void 0 : n.particular_tipo) ?? null,
    garantia_tipo: (n == null ? void 0 : n.garantia_tipo) ?? null,
    garantia_fecha_compra: (n == null ? void 0 : n.garantia_fecha_compra) ?? null,
    garantia_numero_service: (n == null ? void 0 : n.garantia_numero_service) ?? null,
    garantia_problema: (n == null ? void 0 : n.garantia_problema) ?? null,
    fecha: String((n == null ? void 0 : n.fecha) ?? ""),
    hora: String((n == null ? void 0 : n.hora) ?? ""),
    detalles: String((n == null ? void 0 : n.detalles) ?? "")
  }), N = String((n == null ? void 0 : n.estado) ?? "pendiente"), T = { ...R, estado: N }, A = Object.keys(T);
  if ((await Q(async (a) => {
    ...
    await a.execute(
      ...
      [
        ...
        T.matricula,
        T.tipo_turno,
        T.particular_tipo ?? null,
        T.garantia_tipo ?? null,
        T.garantia_fecha_compra ?? null,
        T.garantia_numero_service ?? null,
        T.garantia_problema ?? null,
        T.fecha,
        T.hora,
        T.estado,
        T.detalles,
...
function ie(E) {
  if (!(!Array.isArray(E) || E.length === 0))
    try {
      const n = G(), e = n.prepare("SELECT id FROM reservas WHERE id = ?"), _ = n.prepare(`
    ...
    `);
      n.transaction(() => {
        for (const A of E) {
          const I = A != null && A.id ? Number(A.id) : null;
          let t = null;
          if (I) {
            const i = e.get(I);
            i != null && i.id && (t = i.id);
          ...
          if (!t) {
            const i = _.get(
              (A == null ? void 0 : A.fecha) ?? "",
              (A == null ? void 0 : A.hora) ?? "",
              (A == null ? void 0 : A.cedula) ?? "",
              (A == null ? void 0 : A.matricula) ?? ""
            );
            i != null && i.id && (t = i.id);
          }
          const a = [
            ...
            (A == null ? void 0 : A.cedula) ?? "",
            (A == null ? void 0 : A.telefono) ?? "",
            (A == null ? void 0 : A.marca) ?? "",
            (A == null ? void 0 : A.modelo) ?? "",
            (A == null ? void 0 : A.km) ?? "",
            (A == null ? void 0 : A.matricula) ?? "",
            (A == null ? void 0 : A.tipo_turno) ?? "",
            (A == null ? void 0 : A.particular_tipo) ?? null,
            (A == null ? void 0 : A.garantia_tipo) ?? null,
            (A == null ? void 0 : A.garantia_fecha_compra) ?? null,
            (A == null ? void 0 : A.garantia_numero_service) ?? null,
            (A == null ? void 0 : A.garantia_problema) ?? null,
            (A == null ? void 0 : A.fecha) ?? "",
            (A == null ? void 0 : A.hora) ?? "",
            (A == null ? void 0 : A.detalles) ?? null,
            (A == null ? void 0 : A.estado) ?? "pendiente",
            (A == null ? void 0 : A.notas) ?? null
          ];
          t ? N.run(
            ...a,
            t
      ...
      console.warn("[Service] Error sincronizando reservas MySQL -> SQLite:", n);
    }
}
async function eI(E, n) {
  console.log("[Service] Obteniendo reservas entre:", E, "y", n);
  const e = new Date(E).toISOString().split("T")[0], _ = new Date(n).toISOString().split("T")[0];
  console.log("[Service] Fechas normalizadas:", e, "a", _);
  const R = await Q(async (A) => {
    const [I] = await A.execute(
      `
  ...
  return R.ok ? (ie(R.value), R.value) : G().prepare(`
    SELECT * FROM reservas
    WHERE fecha >= ? AND fecha <= ?
    ORDER BY fecha, hora
  `).all(e, _);
...
async function Tt(E) {
  const n = new Date(E).toISOString().split("T")[0], e = await Q(async (R) => {
  ...
  return e.ok ? (ie(e.value), e.value) : G().prepare(`
    SELECT * FROM reservas
    WHERE fecha = ?
    ORDER BY hora
  `).all(n);
...
async function RI(E, n) {
  if (console.log("[Service] Actualizando notas para reserva:", E), (await Q(async (R) => {
    const [N] = await R.execute(
      "SELECT notas FROM reservas WHERE id = ?",
      [E]
    ), T = N[0];
    T && (await R.execute("UPDATE reservas SET notas = ? WHERE id = ?", [n, E]), await R.execute(
      `
        INSERT INTO historial_reservas
        (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
        VALUES ( ?, 'notas', ?, ?, NOW())
    ...
    try {
      const R = G(), N = R.prepare("SELECT notas FROM reservas WHERE id = ?").get(E);
      if (!N) return;
      R.transaction(() => {
        R.prepare("UPDATE reservas SET notas = ? WHERE id = ?").run(n, E), R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
  ...
  try {
    ...
    _.transaction(() => {
      _.prepare("UPDATE reservas SET notas = ? WHERE id = ?").run(n, E), _.prepare(`
        INSERT INTO historial_reservas
        (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
        VALUES ( ?, 'notas', ?, ?, datetime('now'))
      `).run(E, R.notas || "", n);
...
async function tI(E, n = 0, e = 200) {
  console.log("[Service] Buscando cambios de reservas desde:", E, "id>", n);
  const _ = await Q(async (T) => {
    const [A] = await T.execute(
      `
...
const iI = "daily-summary.json", oE = {
  enabled: !1,
  sendTime: "07:30",
  recipients: [],
  lastSentDate: ""
...
function ct(E) {
  ...
  if (!/^\d{2}:\d{2}$/.test(n)) return oE.sendTime;
  const [e, _] = n.split(":").map(Number);
  return !Number.isFinite(e) || !Number.isFinite(_) || e < 0 || e > 23 || _ < 0 || _ > 59 ? oE.sendTime : `${String(e).padStart(2, "0")}:${String(_).padStart(2, "0")}`;
}
function ut(E) {
  const n = /* @__PURE__ */ new Set();
  for (const e of E || []) {
    const _ = String(e || "").trim().toLowerCase();
    _ && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(_) && n.add(_);
  }
  return Array.from(n);
}
function Zn() {
...
function QE() {
  try {
    ...
    if (!z.existsSync(E))
      return { ...oE };
    const n = z.readFileSync(E, "utf-8"), e = JSON.parse(n);
    return !e || typeof e != "object" ? { ...oE } : {
      enabled: !!e.enabled,
      sendTime: ct(String(e.sendTime || oE.sendTime)),
      recipients: ut(Array.isArray(e.recipients) ? e.recipients : []),
      lastSentDate: String(e.lastSentDate || "")
    };
  } catch (E) {
    return console.warn("[ResumenDiario] No se pudo leer config:", E), { ...oE };
  }
}
function aI(E) {
  const n = rt(), e = x.dirname(n);
  z.existsSync(e) || z.mkdirSync(e, { recursive: !0 }), z.writeFileSync(n, JSON.stringify(E, null, 2), "utf-8");
}
function pE(E) {
  const n = QE(), e = {
    enabled: typeof E.enabled == "boolean" ? E.enabled : n.enabled,
    sendTime: E.sendTime ? ct(E.sendTime) : n.sendTime,
    recipients: Array.isArray(E.recipients) ? ut(E.recipients) : n.recipients,
    lastSentDate: typeof E.lastSentDate == "string" ? E.lastSentDate : n.lastSentDate
  };
  return aI(e), e;
}
function II() {
...
function NI(E) {
  const n = (process.env.SMTP_SECURE || "").toLowerCase();
  return n === "1" || n === "true" || n === "yes" ? !0 : n === "0" || n === "false" || n === "no" ? !1 : E === 465;
}
function TI() {
...
function rI() {
  const E = String(process.env.SMTP_HOST || "").trim(), n = String(process.env.SMTP_USER || "").trim(), e = String(process.env.SMTP_PASS || "").trim(), _ = String(process.env.SMTP_FROM || n).trim(), R = II(), N = NI(R), T = TI();
  return { host: E, port: R, secure: N, user: n, pass: e, from: _, rejectUnauthorized: T };
}
function cI(E, n) {
  const e = [
    ...
    const N = R != null && R.hora ? String(R.hora) : "--:--", T = R != null && R.nombre ? String(R.nombre) : "Sin nombre", A = R != null && R.telefono ? String(R.telefono) : "-", I = R != null && R.matricula ? String(R.matricula) : "-", t = R != null && R.estado ? String(R.estado) : "Pendiente";
    return `${N} | ${T} | ${A} | ${I} | ${t}`;
  });
...
async function Ot(E) {
  const n = QE();
  if (!n.enabled) return { ok: !1, reason: "disabled" };
  if (!n.recipients.length) return { ok: !1, reason: "no_recipients" };
  const e = rI();
  if (!e.host || !e.user || !e.pass || !e.from)
    return { ok: !1, reason: "smtp_missing" };
  const _ = await Tt(E), R = (Array.isArray(_) ? _ : []).filter((I) => String((I == null ? void 0 : I.estado) || "").trim().toLowerCase() !== "cancelada"), N = cI(E, R);
  return await (await import("./nodemailer-DmKQdW4w.js").then((I) => I.n)).createTransport({
    host: e.host,
    port: e.port,
    secure: e.secure,
    tls: {
    ...
    text: N
  }), pE({ lastSentDate: E }), { ok: !0, count: R.length };
}
function uI(E) {
  const n = /* @__PURE__ */ new Date(), e = String(n.getHours()).padStart(2, "0"), _ = String(n.getMinutes()).padStart(2, "0");
  return `${e}:${_}` === E.sendTime;
}
async function wR() {
  const E = QE();
  if (!E.enabled || !uI(E)) return;
  const n = Zn();
  if (E.lastSentDate !== n)
    try {
      const e = await Ot(n);
...
function OI() {
  UR || (UR = !0, wR(), setInterval(wR, 60 * 1e3));
}
async function sI(E) {
  const n = String(E || Zn()).trim(), e = /^\d{4}-\d{2}-\d{2}$/.test(n) ? n : Zn(), _ = QE();
  _.enabled || pE({ enabled: !0 });
  try {
    return await Ot(e);
  } finally {
    _.enabled || pE({ enabled: !1 });
  }
}
function CI() {
  const E = (e, _) => {
    for (const R of bR.getAllWindows())
    ...
    const T = N ? {
      ...
      fecha: N.fecha,
      hora: N.hora,
      tipo_turno: N.tipo_turno
    } : { id: _, ...R || {} }, A = e === "creada" ? "Nueva reserva" : e === "eliminada" ? "Reserva eliminada" : "Reserva modificada", t = [
...
` + "=".repeat(50)), console.log("[IPC] Recibiendo solicitud de reserva:"), console.log(_), console.log("=".repeat(50)), console.log("[IPC] Esperando lock...");
    const N = await y(async () => (console.log("[IPC] Lock adquirido, ejecutando crearReserva"), await $a(_))), T = Date.now() - R;
    return console.log(`[IPC] Reserva creada exitosamente en ${T}ms, retornando ID:`, N), console.log("=".repeat(50) + `
`), typeof N == "number" && await n("creada", N, {
      nombre: _ == null ? void 0 : _.nombre,
      fecha: _ == null ? void 0 : _.fecha,
      hora: _ == null ? void 0 : _.hora,
      tipo_turno: _ == null ? void 0 : _.tipo_turno
    }), N;
  }), F("reservas:obtener", (e, _) => (console.log("[IPC] Obteniendo reserva:", _), Jn(_))), F("reservas:borrar", async (e, _) => {
    ...
    const N = await y(() => EI(_));
    return console.log("[IPC] Reserva borrada exitosamente"), await n("eliminada", _, {
      nombre: R == null ? void 0 : R.nombre,
      fecha: R == null ? void 0 : R.fecha,
      hora: R == null ? void 0 : R.hora,
      tipo_turno: R == null ? void 0 : R.tipo_turno
    }), N;
  }), F("reservas:mover", async (e, _) => {
    console.log("[IPC] Moviendo reserva:", _);
    const R = await y(
      () => _I(_.id, _.nuevaFecha, _.nuevaHora)
    );
    return console.log("[IPC] Reserva movida exitosamente"), _ != null && _.id && await n("modificada", _.id, {
      fecha: _.nuevaFecha,
      hora: _.nuevaHora
    }), R;
  }), F("reservas:actualizar", async (e, _) => {
    console.log("[IPC] Actualizando reserva:", _);
    const R = await y(
      () => nI(_.id, _)
    );
    return console.log("[IPC] Reserva actualizada exitosamente"), _ != null && _.id && await n("modificada", _.id, {
      nombre: _ == null ? void 0 : _.nombre,
      fecha: _ == null ? void 0 : _.fecha,
      hora: _ == null ? void 0 : _.hora,
      tipo_turno: _ == null ? void 0 : _.tipo_turno
    }), R;
  }), F("reservas:semana", async (e, _) => {
    console.log("[IPC] Obteniendo reservas de semana:", _);
    const R = await eI(_.desde, _.hasta);
  ...
  }), F("reservas:actualizar-notas", async (e, _, R) => {
    console.log("[IPC] Actualizando notas para reserva:", _);
    const N = await y(() => RI(_, R));
    return console.log("[IPC] Notas actualizadas exitosamente"), _ && await n("modificada", _), N;
  }), F("reservas:cambios", async (e, _) => {
    const R = (_ == null ? void 0 : _.since) || (/* @__PURE__ */ new Date(0)).toISOString(), N = Number((_ == null ? void 0 : _.lastId) || 0), T = Number((_ == null ? void 0 : _.limit) || 200);
    return tI(R, N, T);
  }), F("reservas:dia", async (e, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return R ? Tt(R) : [];
  }), F("resumen-diario:config:get", async () => QE()), F("resumen-diario:config:set", async (e, _) => pE({
  ...
  })), F("resumen-diario:enviar", async (e, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return sI(R);
  });
}
...
function st(E, n, e) {
  return E === "creación" ? "Reserva creada" : E === "eliminación" ? "Reserva eliminada" : n === null && e !== null ? `Se estableció ${yE(E)}: ${e}` : n !== null && e === null ? `Se eliminó ${yE(E)}` : n !== e ? `Cambió ${yE(E)} de "${n}" a "${e}"` : `Actualización de ${yE(E)}`;
}
function DI(E) {
...
function fI() {
  F("vehiculos:todos", async () => await y(() => MI())), F("vehiculos:historial", async (E, n) => await y(() => LI(n))), F("vehiculos:mysql-by-matricula", async (E, n) => await y(() => hI(n)));
}
const gI = "mysql.env", dI = [
  "MYSQL_HOST",
  "MYSQL_PORT",
  "MYSQL_USER",
  "MYSQL_PASSWORD",
...
function UI(E) {
  ...
  for (const _ of e) {
    ...
    const N = R.indexOf("=");
    if (N === -1) continue;
    const T = R.slice(0, N).trim();
    let A = R.slice(N + 1).trim();
    (A.startsWith('"') && A.endsWith('"') || A.startsWith("'") && A.endsWith("'")) && (A = A.slice(1, -1)), T && (n[T] = A);
  }
  return n;
}
function wI() {
...
function Ct() {
  ...
  const n = z.readFileSync(E, "utf-8"), e = UI(n);
...
function Dt(E) {
  const n = ae(), e = x.dirname(n);
  z.existsSync(e) || z.mkdirSync(e, { recursive: !0 }), z.writeFileSync(n, E, "utf-8");
}
function FI() {
  F("config:env:get", async () => GI()), F("config:env:set", async (E, n) => (Dt(n || ""), Ct(), Ga(), { ok: !0 })), F("config:db:test", async () => {
    const E = await Q(async (n) => {
      const [e] = await n.query("SELECT 1 as ok");
      return e;
    });
...
async function PE(E) {
  ...
  await Q(async (_) => (await _.query(
    `INSERT INTO auditoria_usuarios ( actor_username, actor_role, accion, target_username, detalle)
       VALUES ( ?, ?, ?, ?, ?)`,
    [n.actor_username, n.actor_role, n.accion, n.target_username, n.detalle]
  ), !0)), G().prepare(
    `INSERT INTO auditoria_usuarios ( actor_username, actor_role, accion, target_username, detalle)
     VALUES ( ?, ?, ?, ?, ?)`
  ).run(n.actor_username, n.actor_role, n.accion, n.target_username, n.detalle);
}
async function QI() {
  await St();
  const E = await Q(async (e) => {
...
function lE(E) {
  return E === "superadmin" ? [...qn] : E === "super" ? [...qn] : E === "admin" ? ["agenda", "reservas", "aprontes", "historial", "ajustes", "vehiculos"] : ["reservas", "historial"];
}
function nE(E) {
  return E === "superadmin" || E === "super" || E === "admin" || E === "user" ? E : "user";
}
function Ie(E) {
  const n = qE.randomBytes(16), e = qE.scryptSync(E, n, 32);
  return `scrypt$${n.toString("hex")}$${e.toString("hex")}`;
}
function PI(E, n) {
  const e = n.split("$");
  if (e.length !== 3 || e[0] !== "scrypt") return !1;
  const _ = Buffer.from(e[1], "hex"), R = Buffer.from(e[2], "hex"), N = qE.scryptSync(E, _, 32);
  return qE.timingSafeEqual(R, N);
}
function bE(E, n) {
  const e = nE(E), _ = new Set(qn);
  if (!n || n.length === 0)
    return lE(e);
...
function pn(E, n) {
  ...
  try {
    ...
    return Array.isArray(_) ? bE(e, _) : lE(e);
  } catch {
    return lE(e);
  }
}
function FR(E) {
  ...
  return (E == null ? void 0 : E.code) === "SQLITE_CONSTRAINT_UNIQUE" || (E == null ? void 0 : E.code) === "ER_DUP_ENTRY" || n.includes("UNIQUE constraint failed: usuarios.username") || n.toLowerCase().includes("duplicate entry");
}
async function ot(E) {
  await cE();
  const n = JSON.stringify(bE(E.role, E.permissions)), e = E.activo ?? 1;
  return Q(async (_) => (await _.query(
    ...
    [E.nombre, E.username, E.passwordHash, E.role, n, e]
  ), !0));
}
function xn(E) {
  const n = G(), e = JSON.stringify(bE(E.role, E.permissions)), _ = E.activo ?? 1;
  n.prepare(
    `INSERT INTO usuarios (nombre, username, password_hash, role, permissions_json, activo)
     VALUES ( ?, ?, ?, ?, ?, ?)
  ...
  n.transaction((R) => {
    for (const N of R)
      e.run(
        ...
        N.password_hash,
        nE(N.role),
        N.permissions_json ?? JSON.stringify(lE(nE(N.role))),
        Number(N.activo) || 0
      );
  })(E);
}
async function lt() {
  const E = process.env.SUPERADMIN_USER || "superadmin", n = process.env.SUPERADMIN_PASS || "rodrigoSayasDev724", e = process.env.SUPERADMIN_NAME || "", _ = "superadmin", R = lE(_), N = Ie(n);
  await cE();
  const T = await Q(async (a) => {
  ...
  let A = !1;
  try {
    A = !!G().prepare("SELECT id FROM usuarios WHERE username = ? LIMIT 1").get(E);
  } catch {
    A = !1;
  }
  const I = T.ok, t = T.ok && T.value;
  if (!(!I && A) && (t || await ot({
    nombre: e,
    username: E,
    passwordHash: N,
...
async function Bt() {
  ...
  if (E.ok) {
    ...
    return E.value.map((_) => ({
      ...
      username: _.username,
      role: nE(_.role),
      permissions: pn(_.permissions_json, nE(_.role)),
...
async function VI() {
  return (await Bt()).filter((n) => n.activo).map((n) => ({
    id: n.id,
    nombre: n.nombre,
    username: n.username,
    role: n.role,
...
async function mI(E, n) {
  ...
  let _ = null;
  return e.ok ? _ = e.value : _ = G().prepare(
    "SELECT id, nombre, username, password_hash, role, permissions_json, activo FROM usuarios WHERE username = ? LIMIT 1"
  ).get(E), !_ || !_.password_hash ? { ok: !1, error: "Usuario o contraseña inválida" } : _.activo ? PI(n, _.password_hash) ? (await PE({
...
async function YI(E) {
  await cE();
  const n = nE(E.role), e = bE(n, E.permissions), _ = Ie(E.password);
  if (!(await ot({
    nombre: E.nombre,
    username: E.username,
    passwordHash: _,
    ...
    try {
      ...
      console.warn("[Usuarios] Error sincronizando ? sqlite:", N);
    }
  await PE({
...
async function WI(E) {
  await cE();
  const n = nE(E.role), e = bE(n, E.permissions), _ = await Q(async (N) => (await N.query(
    `UPDATE usuarios SET nombre = ?, username = ?, role = ?, permissions_json = ?, activo = ?
       WHERE id = ?`,
    [E.nombre, E.username, n, JSON.stringify(e), E.activo ?? 1, E.id]
  ), !0));
  if (!_.ok && FR(_.error))
  ...
  try {
    R.prepare(
      `UPDATE usuarios SET nombre = ?, username = ?, role = ?, permissions_json = ?, activo = ?
       WHERE id = ?`
    ).run(E.nombre, E.username, n, JSON.stringify(e), E.activo ?? 1, E.id);
  } catch (N) {
    if (!_.ok)
      throw FR(N) ? new Error("El usuario ya existe") : N;
    console.warn("[Usuarios] Error sincronizando SQLite al actualizar usuario:", N);
  }
  _.ok || console.warn("[Usuarios] MySQL no disponible, actualizado solo en SQLite"), await PE({
    ...
    target_username: E.username,
    detalle: `Rol: ${n} | activo: ${E.activo ?? 1}`
  });
}
async function HI(E, n) {
  const e = await Mt(E);
  await cE(), await Q(async (R) => (await R.query("DELETE FROM usuarios WHERE id = ?", [E]), !0)), G().prepare("DELETE FROM usuarios WHERE id = ?").run(E), await PE({
...
async function vI(E, n, e) {
  const _ = await Mt(E);
  await cE();
  const R = Ie(n);
  await Q(async (T) => (await T.query("UPDATE usuarios SET password_hash = ? WHERE id = ?", [R, E]), !0)), G().prepare("UPDATE usuarios SET password_hash = ? WHERE id = ?").run(R, E), await PE({
...
async function Mt(E) {
  const n = await Q(async (R) => {
    var T;
    const [N] = await R.query(
      "SELECT username FROM usuarios WHERE id = ? LIMIT 1",
      [E]
    );
    return ((T = N[0]) == null ? void 0 : T.username) || null;
  });
  return n.ok ? n.value || null : G().prepare("SELECT username FROM usuarios WHERE id = ? LIMIT 1").get(E).username || null;
}
function yI() {
  F("usuarios:bootstrap", async () => (await lt(), { ok: !0 })), F("usuarios:login-list", async () => VI()), F("auth:login", async (E, n, e) => mI(n, e)), F("usuarios:list", async () => Bt()), F("usuarios:create", async (E, n) => {
    try {
      return await YI(n), { ok: !0 };
    } catch (e) {
      return { ok: !1, error: e.message || "Error al crear usuario" };
    }
  }), F("usuarios:update", async (E, n) => {
    try {
      return await WI(n), { ok: !0 };
    } catch (e) {
      return { ok: !1, error: e.message || "Error al actualizar usuario" };
    }
  }), F("usuarios:delete", async (E, n) => {
    try {
      return await HI(n.id, n.actor), { ok: !0 };
    } catch (e) {
      return { ok: !1, error: e.message || "Error al eliminar usuario" };
    }
  }), F("usuarios:password", async (E, n) => {
    try {
      return await vI(n.id, n.password, n.actor), { ok: !0 };
    } catch (e) {
...
function jE(E) {
  ...
  const e = Number(n[0]), _ = Number(n[1]);
  if (!Number.isFinite(e) || !Number.isFinite(_))
    throw new Error("Formato de hora invalido");
  if (e < 0 || e > 23 || _ < 0 || _ > 59)
    throw new Error("Formato de hora invalido");
  return `${String(e).padStart(2, "0")}:${String(_).padStart(2, "0")}`;
}
function JE(E) {
  const n = [
    "nombre",
  ...
  for (const e of n)
    if (!String(E[e] || "").trim())
      throw new Error("Campo requerido: " + e);
}
function zE(E) {
  return {
    nombre: OE(E.nombre, 255),
    fecha: E.fecha,
    hora: E.hora,
    telefono: OE(E.telefono, 30),
    localidad: OE(E.localidad, 100),
    observaciones: OE(E.observaciones, 500),
    marca: OE(E.marca, 100),
    modelo: OE(E.modelo, 100),
    factura: OE(E.factura, 100)
  };
}
async function ht(E, n, e, _) {
  var a, i;
  ...
  const N = Number(((a = R[0]) == null ? void 0 : a.cupo) || 0);
  if (N < 1)
    throw new Error("Cupo invalido para el horario");
  let T = "SELECT COUNT(*) AS total FROM aprontes WHERE fecha = ? AND hora = ?";
  const A = [n, e];
  _ && (T += " AND id <> ?", A.push(_));
  const [I] = await E.execute(T, A);
  if (Number(((i = I[0]) == null ? void 0 : i.total) || 0) >= N)
...
function $n(E, n, e, _) {
  ...
  if (!R)
    throw new Error("Horario de apronte no disponible");
  const N = Number(R.cupo || 0);
  if (N < 1)
    throw new Error("Cupo invalido para el horario");
  let T = "SELECT COUNT(*) AS total FROM aprontes WHERE fecha = ? AND hora = ?";
  const A = [n, e];
  _ && (T += " AND id <> ?", A.push(_));
  const I = E.prepare(T).get(...A);
  if (Number((I == null ? void 0 : I.total) || 0) >= N)
    throw new Error("No hay cupos disponibles para ese horario");
}
function Lt(E) {
  if (!(!Array.isArray(E) || E.length === 0))
    try {
      ...
      n.transaction((R) => {
        for (const N of R) {
          const T = N != null && N.id ? Number(N.id) : null;
          T && e.run(
            T,
            ...
            (N == null ? void 0 : N.localidad) ?? "",
            (N == null ? void 0 : N.observaciones) ?? "",
            (N == null ? void 0 : N.marca) ?? "",
            (N == null ? void 0 : N.modelo) ?? "",
            (N == null ? void 0 : N.factura) ?? "",
            (N == null ? void 0 : N.created_at) ?? null
          );
        }
      })(E);
    } catch (n) {
      console.warn("[Aprontes] Error sync sqlite:", n);
    }
}
async function KI(E, n, e) {
  const _ = G();
...
async function jI(E) {
  JE(E);
  const n = zE({ ...E }), e = wE(n.fecha), _ = jE(n.hora);
  try {
    const R = await kI(n, e, _);
    try {
      G().prepare(
        `INSERT INTO aprontes (
      ...
      console.warn("[Aprontes] Backup SQLite fallo en crear:", N);
    }
    return R;
  } catch {
    return console.warn("[Aprontes] MySQL no disponible, usando SQLite local"), KI(n, e, _);
  }
}
async function JI(E) {
  const n = Number(E);
  if (!n) return null;
  const e = await Q(async (R) => {
    const [N] = await R.execute("SELECT * FROM aprontes WHERE id = ?", [n]);
    return N[0] ?? null;
  });
  return e.ok ? e.value : G().prepare("SELECT * FROM aprontes WHERE id = ?").get(n);
}
async function zI(E) {
  const n = wE(E), e = await Q(async (R) => {
    const [N] = await R.execute(
  ...
  return e.ok ? (Lt(e.value), e.value) : G().prepare(
    "SELECT * FROM aprontes WHERE fecha = ? ORDER BY hora"
  ).all(n);
}
async function ZI() {
...
async function qI(E, n) {
  ...
  if ((await Q(async (i) => {
    const [c] = await i.execute("SELECT * FROM aprontes WHERE id = ?", [e]), O = c[0];
    if (!O) return;
    const S = { ...O, ...n };
    JE(S);
    const u = zE(S), s = wE(u.fecha), r = jE(u.hora);
    s === O.fecha && r === O.hora || await ht(i, s, r, e), await i.execute(
      `UPDATE aprontes
    ...
    try {
      const i = G(), c = i.prepare("SELECT * FROM aprontes WHERE id = ?").get(e);
      if (!c) return;
      const O = { ...c, ...n };
      JE(O);
      const S = zE(O), u = wE(S.fecha), s = jE(S.hora);
      u === c.fecha && s === c.hora || $n(i, u, s, e), i.prepare(
        `UPDATE aprontes
         SET nombre = ?, fecha = ?, hora = ?,
             telefono = ?, localidad = ?, observaciones = ?,
             marca = ?, modelo = ?, factura = ?
  ...
  const R = G(), N = R.prepare("SELECT * FROM aprontes WHERE id = ?").get(e);
  if (!N) return;
  const T = { ...N, ...n };
  JE(T);
  const A = zE(T), I = wE(A.fecha), t = jE(A.hora);
  I === N.fecha && t === N.hora || $n(R, I, t, e), R.prepare(
    `UPDATE aprontes
     SET nombre = ?, fecha = ?, hora = ?,
         telefono = ?, localidad = ?, observaciones = ?,
         marca = ?, modelo = ?, factura = ?
...
async function pI(E) {
  ...
  if (!n) return;
  if ((await Q(async (R) => {
    await R.execute("DELETE FROM aprontes WHERE id = ?", [n]);
  })).ok) {
    try {
      G().prepare("DELETE FROM aprontes WHERE id = ?").run(n);
    } catch (R) {
      console.warn("[Aprontes] Backup SQLite fallo en borrar:", R);
    }
    return;
  }
  G().prepare("DELETE FROM aprontes WHERE id = ?").run(n);
...
function gt(E) {
  if (!(!Array.isArray(E) || E.length === 0))
    try {
      ...
      n.transaction((N) => {
        for (const T of N) {
          const A = T != null && T.id ? Number(T.id) : null, I = String((T == null ? void 0 : T.hora) || ""), t = Number((T == null ? void 0 : T.cupo) || 1), a = typeof (T == null ? void 0 : T.activo) == "number" ? Number(T.activo) : 1;
          A ? e.run(A, I, t, a) : I && _.run(I, t, a);
        }
      })(E);
    } catch (n) {
      console.warn("[HorariosAprontes] Error sync sqlite:", n);
    }
}
async function EN() {
  const E = await Q(async (e) => {
  ...
  return E.ok ? (gt(E.value), E.value) : G().prepare(
    `SELECT id, hora, cupo, activo
     FROM horarios_aprontes
     WHERE activo = 1
     ORDER BY hora`
...
async function _N() {
  ...
  return E.ok ? (gt(E.value.map((e) => ({ ...e, activo: 0 }))), E.value) : G().prepare(
    `SELECT id, hora, cupo
     FROM horarios_aprontes
     WHERE activo = 0
     ORDER BY hora`
...
async function eN(E, n = 1) {
  const e = xI(E), _ = ft(n), R = await Q(async (I) => {
...
async function AN(E, n) {
  ...
  if ((await Q(async (A) => {
    ...
    try {
      ...
      console.warn("[HorariosAprontes] Backup SQLite fallo en cupo:", A);
    }
    return;
  }
  const N = G();
  if (!N.prepare(
    ...
    throw new Error("Horario no encontrado");
  N.prepare("UPDATE horarios_aprontes SET cupo = ? WHERE id = ?").run(_, e);
}
async function RN(E) {
  const n = Number(E);
  if (!n)
    throw new Error("ID de horario invalido");
  if ((await Q(async (R) => {
    await R.execute("UPDATE horarios_aprontes SET activo = 0 WHERE id = ?", [n]);
  })).ok) {
    try {
      G().prepare("UPDATE horarios_aprontes SET activo = 0 WHERE id = ?").run(n);
    } catch (R) {
      console.warn("[HorariosAprontes] Backup SQLite fallo en desactivar:", R);
    }
    return;
  }
  G().prepare("UPDATE horarios_aprontes SET activo = 0 WHERE id = ?").run(n);
...
async function tN(E) {
  ...
  if (!n)
    throw new Error("ID de horario invalido");
  if ((await Q(async (R) => {
    await R.execute("UPDATE horarios_aprontes SET activo = 1 WHERE id = ?", [n]);
  })).ok) {
    try {
      G().prepare("UPDATE horarios_aprontes SET activo = 1 WHERE id = ?").run(n);
    } catch (R) {
      console.warn("[HorariosAprontes] Backup SQLite fallo en activar:", R);
    }
    return;
  }
  G().prepare("UPDATE horarios_aprontes SET activo = 1 WHERE id = ?").run(n);
...
async function iN(E) {
  ...
  if (!n)
    throw new Error("ID de horario invalido");
  if ((await Q(async (N) => {
    const [T] = await N.execute("SELECT id FROM horarios_aprontes WHERE id = ?", [n]);
    if (!T.length)
      throw new Error("Horario no encontrado");
    await N.execute("DELETE FROM horarios_aprontes WHERE id = ?", [n]);
  })).ok) {
    try {
      G().prepare("DELETE FROM horarios_aprontes WHERE id = ?").run(n);
    } catch (N) {
      console.warn("[HorariosAprontes] Backup SQLite fallo en borrar:", N);
    }
    return;
  }
  const _ = G();
  if (!_.prepare("SELECT id FROM horarios_aprontes WHERE id = ?").get(n))
    throw new Error("Horario no encontrado");
  _.prepare("DELETE FROM horarios_aprontes WHERE id = ?").run(n);
}
function aN() {
  F(
    "aprontes:crear",
    async (E, n) => await y(() => jI(n))
  ), F(
    "aprontes:obtener",
    (E, n) => JI(n)
  ), F(
    "aprontes:borrar",
    async (E, n) => await y(() => pI(n))
  ), F(
    "aprontes:actualizar",
    async (E, n) => await y(() => qI(n == null ? void 0 : n.id, n))
  ), F(
    "aprontes:fecha",
    (E, n) => zI(n)
  ), F(
    ...
    async (E, n) => await y(() => eN(n == null ? void 0 : n.hora, n == null ? void 0 : n.cupo))
  ), F(
    "horarios-aprontes:actualizar-cupo",
    async (E, n) => await y(() => AN(n == null ? void 0 : n.id, n == null ? void 0 : n.cupo))
  ), F(
    "horarios-aprontes:desactivar",
    async (E, n) => await y(() => RN(n))
  ), F(
    "horarios-aprontes:activar",
    async (E, n) => await y(() => tN(n))
  ), F(
    "horarios-aprontes:borrar",
    async (E, n) => await y(() => iN(n))
  );
}
function IN() {
  console.log(` 
 Cargando IPC handlers  
`), ja(), CI(), lI(), fI(), FI(), yI(), XI(), aN(), console.log(` 
...
function rN() {
  const E = /* @__PURE__ */ new Date(), n = (e) => String(e).padStart(2, "0");
  return `${E.getFullYear()}-${n(E.getMonth() + 1)}-${n(E.getDate())}_${n(E.getHours())}-${n(E.getMinutes())}-${n(E.getSeconds())}`;
}
function QR() {
  try {
    const E = NN();
    ...
    const e = x.join(n, `reservas_${rN()}.db`);
...
function cN() {
  QR(), setInterval(QR, 3600 * 1e3);
}
const dt = VR(import.meta.url), Ut = x.dirname(dt);
globalThis.__filename = dt;
globalThis.__dirname = Ut;
process.env.APP_ROOT = x.join(Ut, "..");
const ZE = process.env.VITE_DEV_SERVER_URL, uN = x.join(process.env.APP_ROOT, "dist-electron"), wt = x.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = ZE ? x.join(process.env.APP_ROOT, "public") : wt;
let DE = null;
function ON() {
  DE = new bR({
    width: 1366,
    ...
    webPreferences: {
      preload: x.join(uN, "preload.mjs"),
      nodeIntegration: !1,
      contextIsolation: !0
    }
  }), ZE && DE.webContents.openDevTools({ mode: "detach" }), DE.maximize(), DE.on("page-title-updated", (E) => E.preventDefault()), ZE ? DE.loadURL(ZE) : DE.loadFile(x.join(wt, "index.html"));
}
CE.whenReady().then(async () => {
  Ct();
  const E = yR();
  E ? console.log("[Main] Modo API remota activo. Se omite DB local y schedulers locales.") : (G(), await lt()), IN(), E || (cN(), OI()), ON(), PR.on("settings:update", (n, e) => {
    if (!e || typeof e != "object") return;
    const _ = e.soundEnabled, R = e.theme;
    Ja({
