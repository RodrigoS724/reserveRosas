...
import { ipcMain as gR, app as sE, Notification as Ae, shell as Mt, BrowserWindow as UR } from "electron";
import { fileURLToPath as dR } from "node:url";
import x from "node:path";
import ht from "better-sqlite3";
import z from "node:fs";
import TE from "events";
import JE from "process";
import Lt from "net";
import wR from "tls";
import zn from "timers";
import GR from "stream";
import FR from "buffer";
import ot from "string_decoder";
import ft from "zlib";
import QR from "util";
import gt from "url";
...
function Gt() {
  ...
  te = 1;
  const E = St, t = Bt, n = lt, _ = dE, T = wt.version, a = /(?:^|^)\s*(?:export\s+)?([\w.-]+)(?:\s*=\s*?|:\s+?)(\s*'(?:\\'|[^'])*'|\s*"(?:\\"|[^"])*"|\s*`(?:\\`|[^`])*`|[^#\r\n]+)?\s*(?:#.*)?(?:$|$)/mg;
  function e(h) {
    const L = {};
    let U = h.toString();
    U = U.replace(/\r\n?/mg, `
...
`), G = G.replace(/\\r/g, "\r")), L[F] = G;
  ...
  function s(h) {
    ...
    (L || !U) && i("Loading env from encrypted .env.vault");
  ...
  function r(h) {
    ...
    if (h && h.processEnv != null && (f = h.processEnv), M.populate(f, o, h), d || !F) {
      ...
      for (const V of G)
        try {
          const Y = t.relative(process.cwd(), V);
          g.push(Y);
        } catch (Y) {
          d && I(`Failed to load ${V} ${Y.message}`), w = Y;
        }
...
async function Yt(E, t) {
  ...
  const e = new AbortController(), N = setTimeout(() => e.abort(), 2e4);
  try {
    const A = await a(_, {
      method: "POST",
      headers: T,
      body: JSON.stringify({ channel: E, args: t }),
      signal: e.signal
    }), I = await A.text();
    let i = null;
    try {
    ...
    if (!A.ok) {
      const u = (i == null ? void 0 : i.error) || (i == null ? void 0 : i.message) || I || `HTTP ${A.status}`;
      throw new Error(`API remota error (${A.status}): ${u}`);
    }
    if (i && typeof i == "object") {
      if (i.__ipc_error) {
        const u = new Error(i.message || "Error remoto");
        throw u.stack = i.stack || u.stack, u;
      }
      if (i.ok === !1)
    ...
    throw (A == null ? void 0 : A.name) === "AbortError" ? new Error("Timeout llamando API remota (20s)") : A;
  } finally {
    clearTimeout(N);
  }
}
function P(E, t) {
  gR.handle(E, async (n, ..._) => {
    try {
      return mt(E) ? await Yt(E, _) : await t(n, ..._);
    } catch (R) {
...
const bR = dR(import.meta.url), Wt = x.dirname(bR);
typeof globalThis < "u" && (globalThis.__filename = bR, globalThis.__dirname = Wt);
let W = null, __ = !1;
function Q() {
  if (W)
    return console.log(" [DB] Reutilizando conexión existente"), W;
  if (__ && (console.log("⏳ [DB] Conexión en progreso, esperando..."), W))
    return W;
  __ = !0;
  try {
    if (!sE.isReady())
      throw new Error("Electron app not ready");
    const E = sE.getPath("userData"), t = x.join(E, "reservas.db");
    if (z.existsSync(E) || z.mkdirSync(E, { recursive: !0 }), console.log(" [DB] Creando nueva conexión a:", t), W = new ht(t, {
      readonly: !1,
      fileMustExist: !1,
      timeout: 3e4
...
function vt() {
  return Te || (Te = 1, (function(E) {
    ...
    t.escapeId = function(I, i) {
      if (Array.isArray(I)) {
        for (var u = "", O = 0; O < I.length; O++)
          u += (O === 0 ? "" : ", ") + t.escapeId(I[O], i);
        return u;
      } else return i ? "`" + String(I).replace(n, "``") + "`" : "`" + String(I).replace(n, "``").replace(_, "`.`") + "`";
    }, t.escape = function(I, i, u) {
      if (I == null)
        return "NULL";
      switch (typeof I) {
        case "boolean":
        ...
        case "number":
          return I + "";
        case "object":
          return Object.prototype.toString.call(I) === "[object Date]" ? t.dateToString(I, u || "local") : Array.isArray(I) ? t.arrayToList(I, u) : Buffer.isBuffer(I) ? t.bufferToString(I) : typeof I.toSqlString == "function" ? String(I.toSqlString()) : i ? a(I.toString()) : t.objectToValues(I, u);
        default:
          return a(I);
      }
    }, t.arrayToList = function(I, i) {
      for (var u = "", O = 0; O < I.length; O++) {
        var S = I[O];
        Array.isArray(S) ? u += (O === 0 ? "" : ", ") + "(" + t.arrayToList(S, i) + ")" : u += (O === 0 ? "" : ", ") + t.escape(S, !0, i);
      }
      return u;
    }, t.format = function(I, i, u, O) {
      if (i == null)
        return I;
      Array.isArray(i) || (i = [i]);
      for (var S = 0, c = /\?+/g, s = "", r = 0, C; r < i.length && (C = c.exec(I)); ) {
        var D = C[0].length;
        if (!(D > 2)) {
          var B = D === 2 ? t.escapeId(i[r]) : t.escape(i[r], u, O);
          s += I.slice(S, C.index) + B, S = c.lastIndex, r++;
        }
      }
      return S === 0 ? I : S < I.length ? s + I.slice(S) : s;
    }, t.dateToString = function(I, i) {
      var u = new Date(I);
      if (isNaN(u.getTime()))
        return "NULL";
      var O, S, c, s, r, C, D;
    ...
    }, t.objectToValues = function(I, i) {
      var u = "";
      for (var O in I) {
        var S = I[O];
        typeof S != "function" && (u += (u.length === 0 ? "" : ", ") + t.escapeId(O) + " = " + t.escape(S, !0, i));
      }
      return u;
    }, t.raw = function(I) {
      if (typeof I != "string")
    ...
    function a(A) {
      for (var I = R.lastIndex = 0, i = "", u; u = R.exec(A); )
        i += A.slice(I, u.index) + T[u[0]], I = R.lastIndex;
      return I === 0 ? "'" + A + "'" : I < A.length ? "'" + i + A.slice(I) + "'" : "'" + i + "'";
    }
    function e(A, I) {
...
var A_, ae;
function Zn() {
  return ae || (ae = 1, A_ = vt()), A_;
}
var LE = {}, re;
function zE() {
  if (re) return LE;
  re = 1, Object.defineProperty(LE, "__esModule", { value: !0 }), LE.createLRU = void 0;
  const E = (t) => {
    let { max: n } = t;
    if (!(Number.isInteger(n) && n > 0))
      throw new TypeError("`max` must be a positive integer");
    let _ = 0, R = 0, T = 0, a = [];
    const { onEviction: e } = t, N = /* @__PURE__ */ new Map(), A = new Array(n).fill(void 0), I = new Array(n).fill(void 0), i = new Array(n).fill(0), u = new Array(n).fill(0), O = (r) => {
      i[T] = r, u[r] = T, i[r] = 0, T = r;
    }, S = (r) => {
      if (r === T)
        return;
      const C = i[r], D = u[r];
      r === R ? R = C : i[D] = C, u[C] = D, O(r);
    }, c = (r) => {
      let C = T;
      const D = Math.min(_, r), B = _ - D, M = new Array(D), h = new Array(D);
      for (let L = 0; L < B; L++) {
        const U = A[R];
        e == null || e(U, I[R]), N.delete(U), R = i[R];
      }
      for (let L = D - 1; L >= 0; L--)
        M[L] = A[C], h[L] = I[C], N.set(A[C], L), C = u[C];
      R = 0, T = D - 1, _ = D, A.length = r, I.length = r, i.length = r, u.length = r;
      for (let L = 0; L < D; L++)
        A[L] = M[L], I[L] = h[L], i[L] = L + 1, u[L] = L - 1;
      a = [];
      for (let L = D; L < r; L++)
        a.push(L);
    }, s = (r) => {
      A.length = r, I.length = r, i.length = r, u.length = r, A.fill(void 0, n), I.fill(void 0, n), i.fill(0, n), u.fill(0, n);
    };
    return {
      ...
      set(r, C) {
        ...
        if (D === void 0) {
          if (_ === n) {
            D = R;
            const B = A[D];
            e == null || e(B, I[D]), N.delete(B), R = i[D], u[R] = 0;
          } else
            D = a.length > 0 ? a.pop() : _, _++;
          N.set(r, D), A[D] = r, I[D] = C, _ === 1 ? R = T = D : O(D);
        } else
      ...
      *entries() {
      ...
      delete(r) {
        ...
        if (C === void 0)
          return !1;
        e == null || e(r, I[C]), N.delete(r), a.push(C), A[C] = void 0, I[C] = void 0;
        const D = u[C], B = i[C];
        return C === R ? R = B : i[D] = B, C === T ? T = D : u[B] = D, _--, !0;
      },
      ...
      evict: (r) => {
        ...
        for (; C > 0; ) {
          const D = R, B = A[D];
          e == null || e(B, I[D]), N.delete(B), A[D] = void 0, I[D] = void 0, R = i[D], u[R] = 0, _--, a.push(D), C--;
        }
        _ === 0 && (R = T = 0);
      },
      ...
      clear() {
        ...
        N.clear(), A.fill(void 0), I.fill(void 0), a = [], _ = 0, R = T = 0;
      },
...
function qn() {
  ...
  function _(a, e, N, A, I) {
    const i = n(a, e, N, A);
    let u = t.get(i);
    return u || (u = I(e, N, A), t.set(i, u), u);
  }
  function R(a) {
    t.resize(a);
  }
...
function mR() {
  ...
  return E.prototype.peekAt = function(n) {
    ...
    if (R === (R | 0) && this._head !== this._tail) {
      var e = this.size(), N = this._list.length;
      if (!(R >= e || R < -e || _ < 1)) {
        if (R < 0 && (R += e), _ === 1 || !_)
        ...
        if (R < e / 2) {
          for (this._head = this._head + n + _ + N & this._capacityMask, A = n; A > 0; A--)
          ...
          for (this._tail = R, R = R + _ + N & this._capacityMask, A = e - (_ + n); A > 0; A--)
            this.push(this._list[R++]);
          for (R = this._tail; a > 0; )
            this._list[R = R + 1 + N & this._capacityMask] = void 0, a--;
        }
        return this._head < 2 && this._tail > 1e4 && this._tail <= N >>> 2 && this._shrinkArray(), T;
      }
    }
    ...
    if (R === (R | 0)) {
      ...
      if (R < 0 && (R += T), !(R > T))
        if (arguments.length > 2) {
          var a, e, N, A = arguments.length, I = this._list.length, i = 2;
          if (!T || R < T / 2) {
            for (e = new Array(R), a = 0; a < R; a++)
              e[a] = this._list[this._head + a & this._capacityMask];
            for (_ === 0 ? (N = [], R > 0 && (this._head = this._head + R + I & this._capacityMask)) : (N = this.remove(R, _), this._head = this._head + R + I & this._capacityMask); A > i; )
            ...
            for (_ === 0 ? (N = [], R != T && (this._tail = this._head + R + I & this._capacityMask)) : (N = this.remove(R, _), this._tail = this._tail - u + I & this._capacityMask); i < A; )
    ...
    var e = new Array(_), N = 0, A;
    if (n || this._head > this._tail) {
      for (A = this._head; A < T; A++) e[N++] = R[A];
      for (A = 0; A < this._tail; A++) e[N++] = R[A];
    } else
      for (A = this._head; A < this._tail; A++) e[N++] = R[A];
    return e;
...
var UE = { exports: {} }, Ht = UE.exports, se;
function yt() {
  return se || (se = 1, (function(E, t) {
    (function(n, _) {
      ...
      typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : Ht,
      function(n) {
        ...
        function A(f, l) {
          var g, V, Y;
          return l ? (f >>>= 0, (Y = 0 <= f && f < 256) && (V = N[f], V) ? V : (g = i(f, 0, !0), Y && (N[f] = g), g)) : (f |= 0, (Y = -128 <= f && f < 128) && (V = e[f], V) ? V : (g = i(f, f < 0 ? -1 : 0, !1), Y && (e[f] = g), g));
        }
        R.fromInt = A;
        function I(f, l) {
        ...
        function O(f, l, g) {
          ...
          var V;
          if ((V = f.indexOf("-")) > 0) throw Error("interior hyphen");
          if (V === 0)
            return O(f.substring(1), l, g).neg();
          for (var Y = I(u(g, 8)), v = M, K = 0; K < f.length; K += 8) {
            var y = Math.min(8, f.length - K), EE = parseInt(f.substring(K, K + y), g);
            if (y < 8) {
              var q = I(u(g, y));
              v = v.mul(q).add(I(EE));
            } else
              v = v.mul(Y), v = v.add(I(EE));
          }
          return v.unsigned = l, v;
        }
        R.fromString = O;
        function S(f, l) {
          return typeof f == "number" ? I(f, l) : typeof f == "string" ? O(f, l) : i(
            f.low,
            f.high,
        ...
        o.toInt = function() {
          ...
          if (this.isNegative())
            if (this.eq(w)) {
              var g = I(l), V = this.div(g), Y = V.mul(g).sub(this);
              return V.toString(l) + Y.toInt().toString(l);
            } else return "-" + this.neg().toString(l);
          for (var v = I(u(l, 6), this.unsigned), K = this, y = ""; ; ) {
            var EE = K.div(v), q = K.sub(EE.mul(v)).toInt() >>> 0, j = q.toString(l);
          ...
          var g = this.isNegative(), V = l.isNegative();
          return g && !V ? -1 : !g && V ? 1 : this.unsigned ? l.high >>> 0 > this.high >>> 0 || l.high === this.high && l.low >>> 0 > this.low >>> 0 ? -1 : 1 : this.sub(l).isNegative() ? -1 : 1;
        }, o.comp = o.compare, o.negate = function() {
          return !this.unsigned && this.eq(w) ? w : this.not().add(L);
        }, o.neg = o.negate, o.add = function(l) {
          T(l) || (l = S(l));
          var g = this.high >>> 16, V = this.high & 65535, Y = this.low >>> 16, v = this.low & 65535, K = l.high >>> 16, y = l.high & 65535, EE = l.low >>> 16, q = l.low & 65535, j = 0, _E = 0, k = 0, $ = 0;
          return $ += v + q, k += $ >>> 16, $ &= 65535, k += Y + EE, _E += k >>> 16, k &= 65535, _E += V + y, j += _E >>> 16, _E &= 65535, j += g + K, j &= 65535, i(k << 16 | $, j << 16 | _E, this.unsigned);
          ...
          var V = this.high >>> 16, Y = this.high & 65535, v = this.low >>> 16, K = this.low & 65535, y = l.high >>> 16, EE = l.high & 65535, q = l.low >>> 16, j = l.low & 65535, _E = 0, k = 0, $ = 0, PE = 0;
          return PE += K * j, $ += PE >>> 16, PE &= 65535, $ += v * j, k += $ >>> 16, $ &= 65535, $ += K * q, k += $ >>> 16, $ &= 65535, k += Y * j, _E += k >>> 16, k &= 65535, k += v * q, _E += k >>> 16, k &= 65535, k += K * EE, _E += k >>> 16, k &= 65535, _E += V * j + Y * q + v * EE + K * y, _E &= 65535, i($ << 16 | PE, _E << 16 | k, this.unsigned);
          ...
          if (this.unsigned) {
            ...
            if (this.eq(w)) {
              ...
              if (l.eq(w)) return L;
              var K = this.shr(1);
              return V = K.div(l).shl(1), V.eq(M) ? l.isNegative() ? L : d : (Y = this.sub(l.mul(V)), v = V.add(Y.div(l)), v);
          ...
          for (Y = this; Y.gte(l); ) {
            V = Math.max(1, Math.floor(Y.toNumber() / l.toNumber()));
            for (var y = Math.ceil(Math.log(V) / Math.LN2), EE = y <= 48 ? 1 : u(2, y - 48), q = I(V), j = q.mul(l); j.isNegative() || j.gt(Y); )
              V -= EE, q = I(V, this.unsigned), j = q.mul(l);
            q.isZero() && (q = L), v = v.add(q), Y = Y.sub(j);
          }
          return v;
        ...
        }, R.fromBytes = function(l, g, V) {
          return V ? R.fromBytesLE(l, g) : R.fromBytesBE(l, g);
          ...
          var V = Number(BigInt.asIntN(32, l)), Y = Number(BigInt.asIntN(32, l >> BigInt(32)));
          return i(V, Y, g);
...
function aE() {
  ...
  if (n.Buffer.prototype = t.prototype, (!R.from || R.from === Uint8Array.from) && (R.from = function(T, a, e) {
    ...
    var N = t(T);
    return !a || a.length === 0 ? N.fill(0) : typeof e == "string" ? N.fill(a, e) : N.fill(a), N;
  }), !n.kStringMaxLength)
    try {
  ...
  }, n.kStringMaxLength && (n.constants.MAX_STRING_LENGTH = n.kStringMaxLength)), I_ = n, I_;
}
var bE = {}, De;
function Xt() {
  if (De) return bE;
...
function Kt() {
  ...
  function t(A, I) {
    this.enc = A.encodingName, this.bomAware = A.bomAware, this.enc === "base64" ? this.encoder = T : this.enc === "utf8" ? this.encoder = N : this.enc === "cesu8" && (this.enc = "utf8", this.encoder = a, E.from("eda0bdedb2a9", "hex").toString() !== "💩" && (this.decoder = e, this.defaultCharUnicode = I.defaultCharUnicode));
  }
  t.prototype.encoder = R, t.prototype.decoder = _;
  var n = ot.StringDecoder;
  function _(A, I) {
  ...
  a.prototype.write = function(A) {
    for (var I = E.alloc(A.length * 3), i = 0, u = 0; u < A.length; u++) {
      var O = A.charCodeAt(u);
      O < 128 ? I[i++] = O : O < 2048 ? (I[i++] = 192 + (O >>> 6), I[i++] = 128 + (O & 63)) : (I[i++] = 224 + (O >>> 12), I[i++] = 128 + (O >>> 6 & 63), I[i++] = 128 + (O & 63));
    }
    return I.slice(0, i);
  ...
  e.prototype.write = function(A) {
    for (var I = this.acc, i = this.contBytes, u = this.accBytes, O = "", S = 0; S < A.length; S++) {
      var c = A[S];
      (c & 192) !== 128 ? (i > 0 && (O += this.defaultCharUnicode, i = 0), c < 128 ? O += String.fromCharCode(c) : c < 224 ? (I = c & 31, i = 1, u = 1) : c < 240 ? (I = c & 15, i = 2, u = 1) : O += this.defaultCharUnicode) : i > 0 ? (I = I << 6 | c & 63, i--, u++, i === 0 && (u === 2 && I < 128 && I > 0 ? O += this.defaultCharUnicode : u === 3 && I < 2048 ? O += this.defaultCharUnicode : O += String.fromCharCode(I))) : O += this.defaultCharUnicode;
    }
    return this.acc = I, this.contBytes = i, this.accBytes = u, O;
...
function jt() {
  ...
  IE.utf32le = { type: "_utf32", isLE: !0 }, IE.utf32be = { type: "_utf32", isLE: !1 }, IE.ucs4le = "utf32le", IE.ucs4be = "utf32be", t.prototype.encoder = n, t.prototype.decoder = _;
  function n(A, I) {
    this.isLE = I.isLE, this.highSurrogate = 0;
  }
  n.prototype.write = function(A) {
    for (var I = E.from(A, "ucs2"), i = E.alloc(I.length * 2), u = this.isLE ? i.writeUInt32LE : i.writeUInt32BE, O = 0, S = 0; S < I.length; S += 2) {
      var c = I.readUInt16LE(S), s = c >= 55296 && c < 56320, r = c >= 56320 && c < 57344;
      if (this.highSurrogate)
        if (s || !r)
          u.call(i, this.highSurrogate, O), O += 4;
        else {
          var C = (this.highSurrogate - 55296 << 10 | c - 56320) + 65536;
          u.call(i, C, O), O += 4, this.highSurrogate = 0;
          continue;
        }
      s ? this.highSurrogate = c : (u.call(i, c, O), O += 4, this.highSurrogate = 0);
    }
    return O < i.length && (i = i.slice(0, O)), i;
  }, n.prototype.end = function() {
    if (this.highSurrogate) {
      var A = E.alloc(4);
  ...
  _.prototype.write = function(A) {
    if (A.length === 0)
      return "";
    var I = 0, i = 0, u = E.alloc(A.length + 4), O = 0, S = this.isLE, c = this.overflow, s = this.badChar;
    if (c.length > 0) {
      for (; I < A.length && c.length < 4; I++)
        c.push(A[I]);
      c.length === 4 && (S ? i = c[I] | c[I + 1] << 8 | c[I + 2] << 16 | c[I + 3] << 24 : i = c[I + 3] | c[I + 2] << 8 | c[I + 1] << 16 | c[I] << 24, c.length = 0, O = R(u, O, i, s));
    }
    for (; I < A.length - 3; I += 4)
      S ? i = A[I] | A[I + 1] << 8 | A[I + 2] << 16 | A[I + 3] << 24 : i = A[I + 3] | A[I + 2] << 8 | A[I + 1] << 16 | A[I] << 24, O = R(u, O, i, s);
    for (; I < A.length; I++)
      c.push(A[I]);
    return u.slice(0, O).toString("ucs2");
  };
  function R(A, I, i, u) {
    if ((i < 0 || i > 1114111) && (i = u), i >= 65536) {
      i -= 65536;
      var O = 55296 | i >> 10;
      A[I++] = O & 255, A[I++] = O >> 8;
      var i = 56320 | i & 1023;
    }
    return A[I++] = i & 255, A[I++] = i >> 8, I;
  }
  _.prototype.end = function() {
    this.overflow.length = 0;
  }, IE.utf32 = T, IE.ucs4 = "utf32";
  function T(A, I) {
    this.iconv = I;
  }
  ...
  e.prototype.write = function(A) {
    if (!this.decoder) {
      ...
      for (var i = "", u = 0; u < this.initialBufs.length; u++)
        i += this.decoder.write(this.initialBufs[u]);
    ...
    if (!this.decoder) {
      ...
      for (var I = "", i = 0; i < this.initialBufs.length; i++)
        I += this.decoder.write(this.initialBufs[i]);
  ...
  function N(A, I) {
    var i = [], u = 0, O = 0, S = 0, c = 0, s = 0;
    E:
      for (var r = 0; r < A.length; r++)
        for (var C = A[r], D = 0; D < C.length; D++)
          if (i.push(C[D]), i.length === 4) {
            if (u === 0) {
            ...
            if ((i[0] !== 0 || i[1] > 16) && S++, (i[3] !== 0 || i[2] > 16) && O++, i[0] === 0 && i[1] === 0 && (i[2] !== 0 || i[3] !== 0) && s++, (i[0] !== 0 || i[1] !== 0) && i[2] === 0 && i[3] === 0 && c++, i.length = 0, u++, u >= 100)
              break E;
          }
...
function kt() {
  ...
  _.prototype.write = function(N) {
    if (N.length == 0)
      return "";
    var A = E.alloc(N.length + 1), I = 0, i = 0;
    for (this.overflowByte !== -1 && (A[0] = N[0], A[1] = this.overflowByte, I = 1, i = 2); I < N.length - 1; I += 2, i += 2)
      A[i] = N[I + 1], A[i + 1] = N[I];
    return this.overflowByte = I == N.length - 1 ? N[N.length - 1] : -1, A.slice(0, i).toString("ucs2");
  }, _.prototype.end = function() {
    this.overflowByte = -1;
  }, mE.utf16 = R;
  function R(N, A) {
  ...
  a.prototype.write = function(N) {
    ...
    if (!this.decoder) {
      var N = e(this.initialBufs, this.options.defaultEncoding);
      ...
      return i && (A += i), this.initialBufs.length = this.initialBufsLen = 0, A;
    }
    return this.decoder.end();
  };
  function e(N, A) {
...
function Jt() {
  ...
  he = 1;
  var E = aE().Buffer;
  oE.utf7 = t, oE.unicode11utf7 = "utf7";
  function t(c, s) {
    this.iconv = s;
  }
  ...
  R.prototype.write = function(c) {
    for (var s = "", r = 0, C = this.inBase64, D = this.base64Accum, B = 0; B < c.length; B++)
      if (!C)
        c[B] == N && (s += this.iconv.decode(c.slice(r, B), "ascii"), r = B + 1, C = !0);
      else if (!a[c[B]]) {
        if (B == r && c[B] == A)
          s += "+";
        else {
  ...
  i.prototype.encoder = u, i.prototype.decoder = O, i.prototype.bomAware = !0;
  function u(c, s) {
...
function zt() {
  ...
  function t(R, T) {
    ...
    for (var N = E.alloc(65536, T.defaultCharSingleByte.charCodeAt(0)), e = 0; e < R.chars.length; e++)
      N[R.chars.charCodeAt(e)] = e;
    this.encodeBuf = N;
  }
  ...
  return _.prototype.write = function(R) {
    for (var T = this.decodeBuf, a = E.alloc(R.length * 2), e = 0, N = 0, A = 0; A < R.length; A++)
      e = R[A] * 2, N = A * 2, a[N] = T[e], a[N + 1] = T[e + 1];
...
function pt() {
  ...
  N.prototype.encoder = A, N.prototype.decoder = I, N.prototype._getDecodeTrieNode = function(u) {
    for (var O = []; u > 0; u >>>= 8)
      O.push(u & 255);
    O.length == 0 && O.push(0);
    ...
    return S;
  }, N.prototype._addDecodeChunk = function(u) {
  ...
  }, N.prototype._fillEncodeTable = function(u, O, S) {
  ...
  I.prototype.write = function(u) {
    for (var O = E.alloc(u.length * 2), S = this.nodeIdx, c = this.prevBytes, s = this.prevBytes.length, r = -this.prevBytes.length, C, D = 0, B = 0; D < u.length; D++) {
      ...
      if (!(C >= 0)) if (C === t)
        ...
        var L = i(this.gb18030.gbChars, h);
...
function ii() {
  ...
  return D_ = function(t) {
    ...
    return R.prototype = Object.create(n.prototype, {
      ...
      try {
        var N = this.conv.write(T);
        N && N.length && this.push(N, this.encoding), e();
      } catch (A) {
        e(A);
      }
...
function Ii() {
  return Qe || (Qe = 1, (function(E) {
    var t = aE().Buffer, n = Xt(), _ = WR();
    E.exports.encodings = null, E.exports.defaultCharUnicode = "�", E.exports.defaultCharSingleByte = "?", E.exports.encode = function(a, e, N) {
      a = "" + (a || "");
      var A = E.exports.getEncoder(e, N), I = A.write(a), i = A.end();
      return i && i.length > 0 ? t.concat([I, i]) : I;
    }, E.exports.decode = function(a, e, N) {
      ...
      var A = E.exports.getDecoder(e, N), I = A.write(a), i = A.end();
      return i ? I + i : I;
      ...
      for (var N = E.exports._canonicalizeEncoding(a), A = {}; ; ) {
        var I = E.exports._codecDataCache[N];
        if (I)
          return I;
        var i = E.exports.encodings[N];
        switch (typeof i) {
          case "string":
            N = i;
            break;
          case "object":
            for (var u in i)
              A[u] = i[u];
            A.encodingName || (A.encodingName = N), N = i.type;
            break;
          case "function":
            return A.encodingName || (A.encodingName = N), I = new i(A, E.exports), E.exports._codecDataCache[A.encodingName] = I, I;
          default:
            throw new Error("Encoding not recognized: '" + a + "' (searched as: '" + N + "')");
        }
      }
    }, E.exports._canonicalizeEncoding = function(T) {
      return ("" + T).toLowerCase().replace(/:\d{4}$|[^0-9a-z]/g, "");
    }, E.exports.getEncoder = function(a, e) {
      var N = E.exports.getCodec(a), A = new N.encoder(e, N);
      return N.bomAware && e && e.addBOM && (A = new n.PrependBOM(A, e)), A;
    }, E.exports.getDecoder = function(a, e) {
      var N = E.exports.getCodec(a), A = new N.decoder(e, N);
      return N.bomAware && !(e && e.stripBOM === !1) && (A = new n.StripBOM(A, e)), A;
...
function ZE() {
  if (Pe) return VE;
  Pe = 1;
  const E = Ii(), { createLRU: t } = zE(), n = t({
    max: 500
  });
  return VE.decode = function(_, R, T, a, e) {
    if (Buffer.isEncoding(R))
      return _.toString(R, T, a);
    let N;
    if (!e)
      N = n.get(R), N || (N = E.getDecoder(R), n.set(R, N));
    else {
      const i = { encoding: R, options: e }, u = JSON.stringify(i);
      N = n.get(u), N || (N = E.getDecoder(i.encoding, i.options), n.set(u, N));
    }
    const A = N.write(_.slice(T, a)), I = N.end();
    return I ? A + I : A;
  }, VE.encode = function(_, R, T) {
    if (Buffer.isEncoding(R))
      return Buffer.from(_, R);
    const a = E.getEncoder(R, T || {}), e = a.write(_), N = a.end();
    return N && N.length > 0 ? Buffer.concat([e, N]) : e;
  }, VE;
}
var m = { exports: {} }, Ve;
function NE() {
  return Ve || (Ve = 1, m.exports = {
    0: "DECIMAL",
    // aka DECIMAL
    1: "TINY",
    // aka TINYINT, 1 byte
...
function X() {
  ...
  class O {
    ...
    parseIntNoBigCheck(c) {
      ...
      if (c === 0)
        return 0;
      for (this.buffer[this.offset] === N && (this.offset++, C = -1), this.buffer[this.offset] === A && this.offset++; this.offset < r; )
...
function vR() {
  ...
  function n(R, T) {
    const a = R[T], e = R[T + 1], N = R[T + 2];
    return e + N === 0 ? a : a + (e << 8) + (N << 16);
  }
  class _ {
    constructor(T, a) {
    ...
    executePayload(T) {
      let a = 0;
      const e = T.length, N = this.length - this.bufferLength + this.packetHeaderLength - 3;
      if (e - a >= N) {
        ...
        ), this.length < t && this._flushLargePacket()), this.buffer = [], this.bufferLength = 0, this.execute = _.prototype.executeStart, a += N, e - a > 0)
...
function ui() {
  if (ye) return o_;
  ye = 1;
  const E = NE(), t = X(), n = new Array(256);
  class _ {
    constructor(T) {
      this.columns = T || [];
    }
    static toPacket(T, a) {
      ...
      T.forEach((O) => {
        ...
        N += t.lengthCodedStringLength(O.toString(10), a);
      }), N = N + 2;
      const A = Buffer.allocUnsafe(N + 4), I = new t(0, A, 0, N + 4);
      I.offset = 4, I.writeInt8(0);
      let i = 0, u = 1;
      return T.forEach((O) => {
        O.type === E.NULL && (i += u), u *= 2, u === 256 && (I.writeInt8(i), i = 0, u = 1);
      }), u !== 1 && I.writeInt8(i), T.forEach((O) => {
        if (O === null) {
          I.writeNull();
          return;
        }
    ...
    static fromPacket(T, a) {
      ...
      a.readInt8();
      const N = Math.floor((T.length + 7 + 2) / 8);
      a.skip(N);
  ...
  return n[E.DECIMAL] = t.prototype.readLengthCodedString, n[1] = t.prototype.readInt8, n[2] = t.prototype.readInt16, n[3] = t.prototype.readInt32, n[4] = t.prototype.readFloat, n[5] = t.prototype.readDouble, n[6] = t.prototype.assertInvalid, n[7] = t.prototype.readTimestamp, n[8] = t.prototype.readInt64, n[9] = t.prototype.readInt32, n[10] = t.prototype.readTimestamp, n[11] = t.prototype.readTime, n[12] = t.prototype.readDateTime, n[13] = t.prototype.readInt16, n[E.VAR_STRING] = t.prototype.readLengthCodedString, o_ = _, o_;
}
var f_, Xe;
function tE() {
  return Xe || (Xe = 1, f_ = {
...
var H = {}, je;
function rE() {
  return je || (je = 1, H.LONG_PASSWORD = 1, H.FOUND_ROWS = 2, H.LONG_FLAG = 4, H.CONNECT_WITH_DB = 8, H.NO_SCHEMA = 16, H.COMPRESS = 32, H.ODBC = 64, H.LOCAL_FILES = 128, H.IGNORE_SPACE = 256, H.PROTOCOL_41 = 512, H.INTERACTIVE = 1024, H.SSL = 2048, H.IGNORE_SIGPIPE = 4096, H.TRANSACTIONS = 8192, H.RESERVED = 16384, H.SECURE_CONNECTION = 32768, H.MULTI_STATEMENTS = 65536, H.MULTI_RESULTS = 131072, H.PS_MULTI_RESULTS = 262144, H.PLUGIN_AUTH = 524288, H.CONNECT_ATTRS = 1048576, H.PLUGIN_AUTH_LENENC_CLIENT_DATA = 2097152, H.CAN_HANDLE_EXPIRED_PASSWORDS = 4194304, H.SESSION_TRACK = 8388608, H.SSL_VERIFY_SERVER_CERT = 1073741824, H.REMEMBER_OPTIONS = 2147483648, H.MULTI_FACTOR_AUTHENTICATION = 268435456), H;
}
var U_ = {}, ke;
function lE() {
  return ke || (ke = 1, (function(E) {
    ...
    function n(a, e, N) {
      ...
      return A.update(a), e && A.update(e), N && A.update(N), A.digest();
    }
    function _(a, e) {
      const N = Buffer.allocUnsafe(a.length);
    ...
    function R(a, e, N) {
      ...
      const A = n(a);
      return E.calculateTokenFromPasswordSha(A, e, N);
    }
    E.calculateTokenFromPasswordSha = function(a, e, N) {
      const A = e.slice(0, 8), I = N.slice(0, 12), i = n(a), u = n(A, I, i);
      return _(u, a);
    }, E.calculateToken = R, E.verifyToken = function(a, e, N, A) {
      const I = _(N, n(a, e, A));
    ...
    function T(a, e) {
      const N = Buffer.allocUnsafe(a.length), A = e.length;
  ...
  class T {
    constructor(e) {
      ...
      this.passwordSha1 ? N = _.calculateTokenFromPasswordSha(
        ...
        this.authPluginData1,
        this.authPluginData2
      ), this.authToken = N, this.charsetNumber = e.charsetNumber;
    }
    // TODO
    // ChangeUser.fromPacket = function(packet)
    // };
    serializeToBuffer(e) {
      const N = (i) => this.flags & t[i], A = new n(0, e, 0, e.length);
      A.offset = 4;
      ...
      if (A.writeInt8(E.CHANGE_USER), A.writeNullTerminatedString(this.user, I), N("SECURE_CONNECTION") ? (A.writeInt8(this.authToken.length), A.writeBuffer(this.authToken)) : (A.writeBuffer(this.authToken), A.writeInt8(0)), A.writeNullTerminatedString(this.database, I), A.writeInt16(this.charsetNumber), N("PLUGIN_AUTH") && A.writeNullTerminatedString("mysql_native_password", "latin1"), N("CONNECT_ATTRS")) {
        const i = this.connectAttributes, u = Object.keys(i);
        let O = 0;
...
var Z = {}, qe;
function pn() {
  return qe || (qe = 1, Z.NOT_NULL = 1, Z.PRI_KEY = 2, Z.UNIQUE_KEY = 4, Z.MULTIPLE_KEY = 8, Z.BLOB = 16, Z.UNSIGNED = 32, Z.ZEROFILL = 64, Z.BINARY = 128, Z.ENUM = 256, Z.AUTO_INCREMENT = 512, Z.TIMESTAMP = 1024, Z.SET = 2048, Z.NO_DEFAULT_VALUE = 4096, Z.ON_UPDATE_NOW = 8192, Z.NUM = 32768), Z;
}
var F_, pe;
function Ci() {
  if (pe) return F_;
  pe = 1;
  const E = X(), t = ZE(), n = iE(), _ = ["catalog", "schema", "table", "orgTable", "name", "orgName"];
  class R {
    constructor(e, N) {
      this._buf = e.buffer, this._clientEncoding = N, this._catalogLength = e.readLengthCodedNumber(), this._catalogStart = e.offset, e.offset += this._catalogLength, this._schemaLength = e.readLengthCodedNumber(), this._schemaStart = e.offset, e.offset += this._schemaLength, this._tableLength = e.readLengthCodedNumber(), this._tableStart = e.offset, e.offset += this._tableLength, this._orgTableLength = e.readLengthCodedNumber(), this._orgTableStart = e.offset, e.offset += this._orgTableLength;
    ...
    [Symbol.for("nodejs.util.inspect.custom")](e, N, A) {
      const I = NE(), i = [];
      for (const r in I)
        i[I[r]] = r;
      const u = pn(), O = [], S = this.flags;
      for (const r in u)
      ...
      let s = i[this.columnType];
      return s === "BLOB" ? this.columnLength === 4294967295 ? s = "LONGTEXT" : this.columnLength === 67108860 ? s = "MEDIUMTEXT" : this.columnLength === 262140 ? s = "TEXT" : this.columnLength === 1020 ? s = "TINYTEXT" : s = `BLOB(${this.columnLength})` : s === "VAR_STRING" ? s = `VARCHAR(${Math.ceil(this.columnLength / 4)})` : s === "TINY" ? this.columnLength === 3 && c || this.columnLength === 4 && !c ? s = "TINYINT" : s = `TINYINT(${this.columnLength})` : s === "LONGLONG" ? this.columnLength === 20 ? s = "BIGINT" : s = `BIGINT(${this.columnLength})` : s === "SHORT" ? c && this.columnLength === 5 || !c && this.columnLength === 6 ? s = "SMALLINT" : s = `SMALLINT(${this.columnLength})` : s === "LONG" ? c && this.columnLength === 10 || !c && this.columnLength === 11 ? s = "INT" : s = `INT(${this.columnLength})` : s === "INT24" ? c && this.columnLength === 8 || !c && this.columnLength === 9 ? s = "MEDIUMINT" : s = `MEDIUMINT(${this.columnLength})` : s === "DOUBLE" ? this.columnLength === 22 && this.decimals === 31 ? s = "DOUBLE" : s = `DOUBLE(${this.columnLength},${this.decimals})` : s === "FLOAT" ? this.columnLength === 12 && this.decimals === 31 ? s = "FLOAT" : s = `FLOAT(${this.columnLength},${this.decimals})` : s === "NEWDECIMAL" ? this.columnLength === 11 && this.decimals === 0 ? s = "DECIMAL" : this.decimals === 0 ? c ? s = `DECIMAL(${this.columnLength})` : s = `DECIMAL(${this.columnLength - 1})` : s = `DECIMAL(${this.columnLength - 2},${this.decimals})` : s = `${i[this.columnType]}(${this.columnLength})`, c && (s += " UNSIGNED"), `\`${this.name}\` ${[s, ...O].join(" ")}`;
    }
    static toPacket(e, N) {
      let A = 17;
      _.forEach((O) => {
      ...
      const I = Buffer.allocUnsafe(A), i = new E(N, I, 0, A);
      function u(O) {
      ...
      return i.offset = 4, _.forEach(u), i.writeInt8(12), i.writeInt16(e.characterSet), i.writeInt32(e.columnLength), i.writeInt8(e.columnType), i.writeInt16(e.flags), i.writeInt8(e.decimals), i.writeInt16(0), i;
    }
  ...
  const T = function(a) {
    Object.defineProperty(R.prototype, a, {
      get: function() {
        const e = this[`_${a}Start`], N = e + this[`_${a}Length`], A = t.decode(
          this._buf,
          this.encoding === "binary" ? this._clientEncoding : this.encoding,
          e,
          N
  ...
  return T("catalog"), T("schema"), T("table"), T("orgTable"), T("orgName"), F_ = R, F_;
}
var Q_, xe;
function Di() {
  return xe || (xe = 1, Q_ = {
  ...
  function a(N, A, I) {
    ...
    if (N !== null)
      switch (typeof N) {
        ...
        case "number":
          i = n.DOUBLE, u = 8, O = _.prototype.writeDouble;
          break;
        case "boolean":
          N = N | 0, i = n.TINY, u = 1, O = _.prototype.writeInt8;
          break;
        case "object":
          Object.prototype.toString.call(N) === "[object Date]" ? (i = n.DATETIME, u = 12, O = function(S) {
            return _.prototype.writeDate.call(this, S, I);
          }) : T(N) ? (N = JSON.stringify(N), i = n.JSON) : Buffer.isBuffer(N) && (u = _.lengthCodedNumberLength(N.length) + N.length, O = _.prototype.writeLengthCodedBuffer);
          break;
        default:
          N = N.toString();
      }
      ...
      N = "", i = n.NULL;
    return u || (u = _.lengthCodedStringLength(N, A)), { value: N, type: i, length: u, writer: O };
  }
  class e {
    constructor(A, I, i, u) {
      this.id = A, this.parameters = I, this.encoding = R[i], this.timezone = u;
    }
    static fromPacket(A, I) {
      const i = A.readInt32(), u = A.readInt8(), O = A.readInt32();
      let S = A.offset;
      ...
      return { stmtId: i, flags: u, iterationCount: O, values: s };
    }
    toPacket() {
      let A = 14, I;
      ...
      const i = Buffer.allocUnsafe(A), u = new _(0, i, 0, A);
...
function Bi() {
  ...
  class n {
    ...
    toPacket(R) {
      ...
      return N.writeUInt32LE(this.capabilityFlags, 0), e.writeBuffer(N.slice(0, 2)), e.writeInt8(this.characterSet), e.writeInt16(this.statusFlags), e.writeBuffer(N.slice(2, 4)), e.writeInt8(21), e.skip(10), e.writeBuffer(this.authPluginData2), e.writeInt8(0), e.writeString("mysql_native_password", "latin1"), e.writeInt8(0), e;
    }
    static fromPacket(R) {
      const T = {};
      ...
      if (a[0] = R.readInt8(), a[1] = R.readInt8(), R.haveMoreData() ? (T.characterSet = R.readInt8(), T.statusFlags = R.readInt16(), a[2] = R.readInt8(), a[3] = R.readInt8(), T.capabilityFlags = a.readUInt32LE(0), T.capabilityFlags & t.PLUGIN_AUTH ? T.authPluginDataLength = R.readInt8() : (T.authPluginDataLength = 0, R.skip(1)), R.skip(10)) : T.capabilityFlags = a.readUInt16LE(0), T.capabilityFlags & t.SECURE_CONNECTION) {
        const N = T.authPluginDataLength;
        if (N === 0)
...
function li() {
  ...
  class R {
    ...
    serializeResponse(a) {
      const e = (i) => this.clientFlags & E[i], N = new n(0, a, 0, a.length);
      ...
      let I;
      if (e("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (N.writeLengthCodedNumber(this.authToken.length), N.writeBuffer(this.authToken)) : e("SECURE_CONNECTION") ? (N.writeInt8(this.authToken.length), N.writeBuffer(this.authToken)) : (N.writeBuffer(this.authToken), N.writeInt8(0)), e("CONNECT_WITH_DB") && N.writeNullTerminatedString(this.database, A), e("PLUGIN_AUTH") && N.writeNullTerminatedString("mysql_native_password", "latin1"), e("CONNECT_ATTRS")) {
        const i = this.connectAttributes || {}, u = Object.keys(i);
        let O = 0;
        ...
        for (N.writeLengthCodedNumber(O), I = 0; I < u.length; ++I)
          N.writeLengthCodedString(u[I], A), N.writeLengthCodedString(
            i[u[I]],
            A
          );
      }
    ...
    static fromPacket(a) {
      ...
      let I;
      if (N("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (I = a.readLengthCodedNumber(A), e.authToken = a.readBuffer(I)) : N("SECURE_CONNECTION") ? (I = a.readInt8(), e.authToken = a.readBuffer(I)) : e.authToken = a.readNullTerminatedString(A), N("CONNECT_WITH_DB") && (e.database = a.readNullTerminatedString(A)), N("PLUGIN_AUTH") && (e.authPluginName = a.readNullTerminatedString(A)), N("CONNECT_ATTRS")) {
        const i = a.readLengthCodedNumber(A), u = a.offset + i, O = {};
        for (; a.offset < u; )
...
function Mi() {
  if (nA) return m_;
  nA = 1;
  const E = X(), t = tE(), n = ZE(), _ = iE();
  class R {
    constructor(a, e) {
      this.query = a, this.charsetNumber = e, this.encoding = _[e];
    }
    toPacket() {
      const a = n.encode(this.query, this.encoding), e = 5 + a.length, N = Buffer.allocUnsafe(e), A = new E(0, N, 0, e);
...
function Ui() {
  if (NA) return X_;
  NA = 1;
  const E = X(), t = rE(), n = HR(), _ = fi(), R = gi();
  class T {
    constructor(e, N) {
      const A = N.config.bigNumberStrings, I = N.serverEncoding, i = N._handshakePacket.capabilityFlags, u = function(c) {
        return i & t[c];
      };
          ...
          for (c > 0 && (O = {
            ...
            if (r = e.readInt8(), c = e.readLengthCodedNumber(), D = e.offset + c, r === R.SYSTEM_VARIABLES) {
              ...
              if (O.systemVariables[C] = B, C === "character_set_client") {
                const M = _[B];
                typeof M < "u" && (N.config.charsetNumber = M);
              }
            } else if (r === R.SCHEMA)
    ...
    static toPacket(e, N) {
      ...
      const I = Buffer.allocUnsafe(A), i = new E(0, I, 0, A);
      return i.offset = 4, i.writeLengthCodedNumber(e), typeof N < "u" && i.writeLengthCodedNumber(N), i;
    }
  }
  return X_ = T, X_;
}
...
function wi() {
  ...
  class t {
    ...
    static toPacket(_, R) {
      ...
      const e = Buffer.allocUnsafe(a + 4), N = new E(0, e, 0, a + 4);
...
function eE() {
  return rA || (rA = 1, (function(E, t) {
    const n = JE, _ = Ni(), R = Ti(), T = ai(), a = ri(), e = ui(), N = ci(), A = Oi(), I = si(), i = Ci(), u = Si(), O = Bi(), S = li(), c = Mi(), s = hi(), r = Li(), C = oi(), D = Ui(), B = di(), M = wi();
    Object.entries({
      AuthNextFactor: _,
      AuthSwitchRequest: R,
    ...
    class U {
      static toPacket(w, o) {
        w = w || {};
        const f = w.affectedRows || 0, l = w.insertId || 0, g = w.serverStatus || 0, V = w.warningCount || 0, Y = w.message || "";
        let v = 9 + L.lengthCodedNumberLength(f);
        v += L.lengthCodedNumberLength(l);
        const K = Buffer.allocUnsafe(v), y = new L(0, K, 0, v);
        return y.offset = 4, y.writeInt8(0), y.writeLengthCodedNumber(f), y.writeLengthCodedNumber(l), y.writeInt16(g), y.writeInt16(V), y.writeString(Y, o), y._name = "OK", y;
      }
    }
    t.OK = U;
    class d {
...
function Gi() {
  if (cA) return J_;
  cA = 1;
  const E = "sha256_password", t = dE, { xorRotating: n } = lE(), _ = wR, R = Buffer.from([1]), T = 0, a = 1, e = -1;
  function N(A, I, i) {
    const u = n(Buffer.from(`${A}\0`, "utf8"), I);
    return t.publicEncrypt(i, u);
  }
  return J_ = (A = {}) => ({ connection: I }) => {
    let i = 0, u = null;
    ...
    return (c) => {
      switch (i) {
        case T:
          return I.stream instanceof _.TLSSocket && I.stream.encrypted === !0 ? Buffer.from(`${O}\0`, "utf8") : (u = c.slice(0, 20), A.serverPublicKey ? S(A.serverPublicKey) : (i = a, R));
        case a:
...
function Fi() {
  if (OA) return z_;
  OA = 1;
  const E = "caching_sha2_password", t = dE, { xor: n, xorRotating: _ } = lE(), R = Buffer.from([2]), T = Buffer.from([3]), a = Buffer.from([4]), e = 0, N = 1, A = 2, I = -1;
  function i(S) {
  ...
  function u(S, c) {
    if (!S)
      return Buffer.alloc(0);
    const s = i(Buffer.from(S)), r = i(s), C = i(Buffer.concat([r, c]));
    return n(s, C);
  }
  function O(S, c, s) {
  ...
  return z_ = (S = {}) => ({ connection: c }) => {
    ...
    return (B) => {
      switch (s) {
        case e:
          return r = B.slice(0, 20), s = N, u(C, r);
        case N:
          if (T.equals(B))
            return s = I, null;
          if (a.equals(B))
...
function Pi() {
  ...
  return q_ = (n) => function({ connection: R, command: T }) {
...
function Vi() {
  if (DA) return p_;
  DA = 1;
  const E = eE(), t = Gi(), n = Fi(), _ = Qi(), R = Pi(), T = {
    sha256_password: t({}),
  ...
  function e(I, i) {
    I.code = "AUTH_SWITCH_PLUGIN_ERROR", I.fatal = !0, i.emit("error", I);
  }
  function N(I, i, u) {
    const { pluginName: O, pluginData: S } = E.AuthSwitchRequest.fromPacket(I);
    let c = i.config.authPlugins && i.config.authPlugins[O];
    if (i.config.authSwitchHandler && O !== "mysql_native_password") {
      const s = i.config.authSwitchHandler;
    ...
    i._authPlugin = c({ connection: i, command: u }), Promise.resolve(i._authPlugin(S)).then((s) => {
      s && i.writePacket(new E.AuthSwitchResponse(s).toPacket());
    }).catch((s) => {
      e(s, u);
    });
  }
  function A(I, i, u) {
    ...
    if (i.config.authSwitchHandler) {
      ...
      a(), S({ pluginData: O }, (c, s) => {
        if (c)
          return e(c, u);
        i.writePacket(new E.AuthSwitchResponse(s).toPacket());
      });
      return;
    }
    if (!i._authPlugin)
    ...
    Promise.resolve(i._authPlugin(O)).then((S) => {
      S && i.writePacket(new E.AuthSwitchResponse(S).toPacket());
    }).catch((S) => {
      e(S, u);
    });
...
function bi() {
  return SA || (SA = 1, (function(E) {
    var t = TE.EventEmitter, n = QR, _ = 3e3, R = 0, T = "closed", a = "drained", e = function(A) {
      t.call(this), A && A > 0 ? this.timeout = A : this.timeout = _, this.status = N.STATUS_IDLE, this.curId = R, this.queue = [];
    };
    n.inherits(e, t), e.prototype.push = function(A, I, i) {
      if (this.status !== N.STATUS_IDLE && this.status !== N.STATUS_BUSY)
        return !1;
      if (typeof A != "function")
        throw new Error("fn should be a function.");
      if (this.queue.push({ fn: A, ontimeout: I, timeout: i }), this.status === N.STATUS_IDLE) {
      ...
      this.status !== N.STATUS_IDLE && this.status !== N.STATUS_BUSY || (A ? (this.status = N.STATUS_DRAINED, this.timerId && (clearTimeout(this.timerId), this.timerId = void 0), this.emit(a)) : (this.status = N.STATUS_CLOSED, this.emit(T)));
      ...
      if (!(A !== this.curId || this.status !== N.STATUS_BUSY && this.status !== N.STATUS_CLOSED)) {
        ...
        if (!I) {
          this.status === N.STATUS_BUSY ? (this.status = N.STATUS_IDLE, this.curId++) : (this.status = N.STATUS_DRAINED, this.emit(a));
          return;
        }
        var i = this;
        I.id = ++this.curId;
        ...
        u = u > 0 ? u : _, this.timerId = setTimeout(function() {
          process.nextTick(function() {
            i._next(I.id);
          }), i.emit("timeout", I), I.ontimeout && I.ontimeout();
        }, u);
        try {
          I.fn({
            done: function() {
          ...
          i.emit("error", O, I), process.nextTick(function() {
            i._next(I.id);
          });
        }
      }
...
function Yi() {
  ...
  const E = ft, t = vR();
  function n(T) {
    const a = this, e = T.readInt24(), N = T.readBuffer();
    ...
    (function(i) {
      N.deflateQueue.push((u) => {
        E.deflate(T, (O, S) => {
          ...
          let c = S.length;
          c < A ? (I.writeUInt8(c & 255, 0), I.writeUInt16LE(c >> 8, 1), I.writeUInt8(i, 3), I.writeUInt8(A & 255, 4), I.writeUInt16LE(A >> 8, 5), N.writeUncompressed(I), N.writeUncompressed(S)) : (c = A, A = 0, I.writeUInt8(c & 255, 0), I.writeUInt16LE(c >> 8, 1), I.writeUInt8(i, 3), I.writeUInt8(A & 255, 4), I.writeUInt16LE(A >> 8, 5), N.writeUncompressed(I), N.writeUncompressed(T)), u.done();
        });
      });
...
function yR() {
  if (MA) return _n;
  MA = 1;
  const E = AE(), t = eE(), n = rE(), _ = iE(), R = lE();
  function T(e) {
    const N = [];
    for (const A in n)
      e & n[A] && N.push(A.replace(/_/g, " ").toLowerCase());
    return N;
  }
  class a extends E {
    constructor(N) {
    ...
    calculateNativePasswordAuthToken(N) {
      ...
      let i;
      return this.passwordSha1 ? i = R.calculateTokenFromPasswordSha(
        this.passwordSha1,
        A,
        I
      ) : i = R.calculateToken(
    ...
    handshakeInit(N, A) {
      ...
      }), this.handshake = t.Handshake.fromPacket(N), A.config.debug && console.log(
      ...
      const I = this.handshake.capabilityFlags & n.SSL, i = this.handshake.capabilityFlags & n.MULTI_FACTOR_AUTHENTICATION;
      if (this.clientFlags = this.clientFlags | i, A.config.compress = A.config.compress && this.handshake.capabilityFlags & n.COMPRESS, this.clientFlags = this.clientFlags | A.config.compress, A.config.ssl) {
        if (!I) {
        ...
        this.sendCredentials(A);
      return i && (this.authenticationFactor = 1), a.prototype.handshakeResult;
    }
    handshakeResult(N, A) {
      const I = N.peekByte();
      if (I === 254 || I === 1 || I === 2) {
        const i = Vi();
        try {
          return I === 1 ? i.authSwitchRequestMoreData(N, A, this) : (this.authenticationFactor !== 0 && (A.config.password = this[`password${this.authenticationFactor}`], this.authenticationFactor += 1), i.authSwitchRequest(N, A, this)), a.prototype.handshakeResult;
        } catch (u) {
  ...
  class R extends n {
    ...
    start(a, e) {
      ...
      return this.serverHello = N, N.setScrambleData((A) => {
        ...
        e.writePacket(N.toPacket(0));
      }), R.prototype.readClientReply;
    }
    readClientReply(a, e) {
      const N = _.HandshakeResponse.fromPacket(a);
      return e.clientHelloReply = N, this.args.authCallback ? this.args.authCallback(
        {
          user: N.user,
          database: N.database,
    ...
    dispatchCommands(a, e) {
      ...
      switch (I) {
        case E.STMT_PREPARE:
          if (e.listeners("stmt_prepare").length) {
            const i = a.readString(void 0, A);
            e.emit("stmt_prepare", i);
          } else
            e.writeError({
        ...
        case E.STMT_EXECUTE:
          if (e.listeners("stmt_execute").length) {
            const { stmtId: i, flags: u, iterationCount: O, values: S } = _.Execute.fromPacket(a, A);
            e.emit(
              "stmt_execute",
              i,
              u,
        ...
        case E.QUERY:
          if (e.listeners("query").length) {
            const i = a.readString(void 0, A);
            this._isStatement(i, "PREPARE") || this._isStatement(i, "SET") ? e.emit("stmt_prepare", i) : this._isStatement(i, "EXECUTE") ? e.emit("stmt_execute", null, null, null, null, i) : e.emit("query", i);
          } else
            e.writeError({
              code: t.HA_ERR_INTERNAL_ERROR,
        ...
        case E.FIELD_LIST:
          if (e.listeners("field_list").length) {
            const i = a.readNullTerminatedString(A), u = a.readString(void 0, A);
            e.emit("field_list", i, u);
          } else
            e.writeError({
      ...
      return e.listeners("packet").length ? e.emit("packet", a.clone(), N, I) : N || console.log("Unknown command:", I), R.prototype.dispatchCommands;
    }
  }
  return nn = R, nn;
}
...
`);
  ...
  cE.printDebugWithCode = _;
  function R(e, N, A) {
    return Array.isArray(N) ? N.some((I) => e === A[I]) : !!N;
  }
  cE.typeMatch = R;
...
`), C = 0; C < r.length; C++)
    ...
    return c.scope = {}, c.formats = N, c.sym = function(s) {
...
`);
  ...
  return A.formats = N, Rn = A, Rn;
}
var tn, UA;
function yi() {
  if (UA) return tn;
  UA = 1;
  const E = NE(), t = ME(), n = qE(), _ = XR(), R = qn(), T = [];
  for (const A in E)
    T[E[A]] = A;
  function a(A, I, i, u, O) {
    ...
    switch (A) {
      ...
      default:
        return I === t.BINARY ? "packet.readLengthCodedBuffer()" : `packet.readLengthCodedString(${i})`;
    }
  }
  function e(A, I, i) {
    typeof i.typeCast == "function" && typeof I.typeCast != "function" && (I.typeCast = i.typeCast);
    function u(D, B) {
      return {
        type: T[D.columnType],
    ...
    return O("return result;"), O("}"), O("};")("})()"), i.debug && n.printDebugWithCode(
      "Compiled text protocol row parser",
      O.toString()
    ), typeof I.typeCast == "function" ? O.toFunction({ wrap: u }) : O.toFunction();
  }
  function N(A, I, i) {
    return R.getParser("text", A, I, i, e);
  }
  return tn = N, tn;
}
var In, dA;
function Xi() {
  if (dA) return In;
  dA = 1;
  const E = NE(), t = ME(), n = qE(), _ = [];
  for (const e in E)
    _[E[e]] = e;
  function R({ packet: e, type: N, charset: A, encoding: I, config: i, options: u }) {
    const O = !!(u.supportBigNumbers || i.supportBigNumbers), S = !!(u.bigNumberStrings || i.bigNumberStrings), c = u.timezone || i.timezone, s = u.dateStrings || i.dateStrings;
    switch (N) {
      case E.TINY:
      case E.SHORT:
      case E.LONG:
      ...
      case E.DECIMAL:
      case E.NEWDECIMAL:
        return i.decimalNumbers ? e.parseLengthCodedFloat() : e.readLengthCodedString("ascii");
      case E.DATE:
        return n.typeMatch(N, s, E) ? e.readLengthCodedString("ascii") : e.parseDate(c);
      case E.DATETIME:
      case E.TIMESTAMP:
        return n.typeMatch(N, s, E) ? e.readLengthCodedString("ascii") : e.parseDateTime(c);
      case E.TIME:
      ...
      case E.JSON:
        return i.jsonStrings ? e.readLengthCodedString("utf8") : JSON.parse(e.readLengthCodedString("utf8"));
      default:
    ...
    return {
      next(I, i, u) {
        ...
        for (let S = 0; S < i.length; S++) {
          const c = i[S], s = u.typeCast ? u.typeCast : A.typeCast, r = () => R({
            packet: I,
            type: c.columnType,
            encoding: c.encoding,
...
function KR() {
  if (wA) return Nn;
  wA = 1;
  const E = JE, t = zn, n = GR.Readable, _ = AE(), R = eE(), T = yi(), a = Xi(), e = HR(), N = new R.Packet(0, Buffer.allocUnsafe(4), 0, 4);
  class A extends _ {
    constructor(i, u) {
      super(), this.sql = i.sql, this.values = i.values, this._queryOptions = i, this.namedPlaceholders = i.namedPlaceholders || !1, this.onResult = u, this.timeout = i.timeout, this.queryTimeout = null, this._fieldCount = 0, this._rowParser = null, this._fields = [], this._rows = [], this._receivedFieldsCount = 0, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
    ...
    start(i, u) {
      ...
      if (this.queryTimeout && (t.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult) {
        let i, u;
        this._resultIndex === 0 ? (i = this._rows[0], u = this._fields[0]) : (i = this._rows, u = this._fields), u ? E.nextTick(() => {
          this.onResult(null, i, u);
        }) : E.nextTick(() => {
    ...
    doneInsert(i) {
      return this._localStreamError ? (this.onResult ? this.onResult(this._localStreamError, i) : this.emit("error", this._localStreamError), null) : (this._rows.push(i), this._fields.push(void 0), this.emit("fields", void 0), this.emit("result", i), i.serverStatus & e.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, this.resultsetHeader) : this.done());
    }
    resultsetHeader(i, u) {
      const O = new R.ResultSetHeader(i, u);
    ...
    _streamLocalInfile(i, u) {
      ...
      const O = () => {
        ...
        D.copy(B, 4), i.writePacket(
          new R.Packet(0, B, 0, B.length)
        );
      }, r = () => {
        i.removeListener("error", O), i.writePacket(N);
      }, C = (D) => {
        this._localStreamError = D, i.removeListener("error", O), i.writePacket(N);
      };
      return this._unpipeStream = () => {
        i.stream.removeListener("pause", c), i.stream.removeListener("drain", S), this._localStream.removeListener("data", s), this._localStream.removeListener("end", r), this._localStream.removeListener("error", C);
      }, i.stream.on("pause", c), i.stream.on("drain", S), this._localStream.on("data", s), this._localStream.on("end", r), this._localStream.on("error", C), i.once("error", O), this.infileOk;
    }
    readField(i, u) {
    ...
    fieldsEOF(i, u) {
      return i.isEOF() ? this.row : u.protocolError("Expected EOF packet");
    }
    row(i, u) {
      if (i.isEOF())
        return i.eofStatusFlags() & e.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, A.prototype.resultsetHeader) : this.done();
      let O;
      try {
    ...
    _setTimeout() {
      if (this.timeout) {
        ...
        this.queryTimeout = t.setTimeout(i, this.timeout);
      }
    }
    _handleTimeoutError() {
  ...
  return A.prototype.catch = A.prototype.then, Nn = A, Nn;
}
var Tn, GA;
function jR() {
  if (GA) return Tn;
...
function Ki() {
  if (FA) return an;
  FA = 1;
  const E = pn(), t = ME(), n = NE(), _ = qE(), R = XR(), T = qn(), a = [];
  for (const I in n)
    a[n[I]] = I;
  function e(I, i, u, O) {
    const S = !!(u.supportBigNumbers || i.supportBigNumbers), c = !!(u.bigNumberStrings || i.bigNumberStrings), s = u.timezone || i.timezone, r = u.dateStrings || i.dateStrings, C = I.flags & E.UNSIGNED;
    switch (I.columnType) {
      case n.TINY:
  ...
  function N(I, i, u) {
    ...
    O("(function(){"), O("return class BinaryRow {"), O("constructor() {"), O("}"), O("next(packet, fields, options) {"), i.rowsAsArray ? O(`const result = new Array(${I.length});`) : O("const result = {};"), typeof u.typeCast == "function" && typeof i.typeCast != "function" && (i.typeCast = u.typeCast), O("packet.readInt8();");
    for (let M = 0; M < S; ++M)
      O(`const nullBitmaskByte${M} = packet.readInt8();`);
    let s = "", r = 4, C = 0, D = "", B = "";
    for (let M = 0; M < I.length; M++) {
      if (D = _.fieldEscape(I[M].name), typeof i.nestTables == "string" ? s = `result[${_.fieldEscape(I[M].table + i.nestTables + I[M].name)}]` : i.nestTables === !0 ? (B = _.fieldEscape(I[M].table), O(`if (!result[${B}]) result[${B}] = {};`), s = `result[${B}][${D}]`) : i.rowsAsArray ? s = `result[${M.toString(10)}]` : s = `result[${D}]`, O(`if (nullBitmaskByte${C} & ${r}) `), O(`${s} = null;`), O("else {"), i.typeCast === !1)
        O(`${s} = packet.readLengthCodedBuffer();`);
      else {
  ...
  function A(I, i, u) {
    return T.getParser("binary", I, i, u, N);
  }
  return an = A, an;
}
var rn, QA;
function ji() {
  if (QA) return rn;
  QA = 1;
  const E = pn(), t = ME(), n = NE(), _ = qE(), R = [];
  for (const a in n)
    R[n[a]] = a;
  function T(a, e, N) {
    function A(I, i, u, O, S) {
      const c = !!(u.supportBigNumbers || i.supportBigNumbers), s = !!(u.bigNumberStrings || i.bigNumberStrings), r = u.timezone || i.timezone, C = u.dateStrings || i.dateStrings, D = I.flags & E.UNSIGNED;
      switch (I.columnType) {
        case n.TINY:
        ...
        case n.DECIMAL:
        case n.NEWDECIMAL:
          return i.decimalNumbers ? S.parseLengthCodedFloat() : S.readLengthCodedString("ascii");
        case n.GEOMETRY:
          return S.parseGeometryValue();
        case n.VECTOR:
          return S.parseVector();
        case n.JSON:
          return i.jsonStrings ? S.readLengthCodedString("utf8") : JSON.parse(S.readLengthCodedString("utf8"));
        case n.LONGLONG:
    ...
    return class {
      ...
      next(i, u, O) {
        ...
        for (let D = 0; D < u.length; D++) {
          const B = u[D], M = O.typeCast !== void 0 ? O.typeCast : N.typeCast;
          let h;
          if (c[C] & r)
            h = null;
          else if (O.typeCast === !1)
            h = i.readLengthCodedBuffer();
          else {
            const L = () => A(B, N, O, D, i);
            h = typeof M == "function" ? M(
              {
                ...
                string: function(U = B.encoding) {
                  ...
                  if (B.columnType === n.TINY) {
                    ...
                    return String(
                      d ? i.readInt8() : i.readSInt8()
                    );
                  }
                  return B.columnType === n.TIME ? i.readTimeString() : i.readLengthCodedString(U);
                },
                buffer: function() {
...
function kR() {
  if (PA) return un;
  PA = 1;
  const E = AE(), t = KR(), n = eE(), _ = Ki(), R = ji();
  class T extends E {
    constructor(e, N) {
      super(), this.statement = e.statement, this.sql = e.sql, this.values = e.values, this.onResult = N, this.parameters = e.values, this.insertId = 0, this.timeout = e.timeout, this.queryTimeout = null, this._rows = [], this._fields = [], this._result = [], this._fieldCount = 0, this._rowParser = null, this._executeOptions = e, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
      }, this._streamFactory = e.infileStreamFactory, this._connection = null;
    }
    buildParserFromFields(e, N) {
      return this.options.disableEval ? R(e, this.options, N.config) : _(e, this.options, N.config);
    }
    start(e, N) {
    ...
    fieldsEOF(e, N) {
      return e.isEOF() ? (this._rowParser = new (this.buildParserFromFields(
        this._fields[this._resultIndex],
        N
      ))(), T.prototype.row) : N.protocolError("Expected EOF packet");
    }
  }
  return T.prototype.done = t.prototype.done, T.prototype.doneInsert = t.prototype.doneInsert, T.prototype.resultsetHeader = t.prototype.resultsetHeader, T.prototype._findOrCreateReadStream = t.prototype._findOrCreateReadStream, T.prototype._streamLocalInfile = t.prototype._streamLocalInfile, T.prototype._setTimeout = t.prototype._setTimeout, T.prototype._handleTimeoutError = t.prototype._handleTimeoutError, T.prototype.row = t.prototype.row, T.prototype.stream = t.prototype.stream, un = T, un;
}
var cn, VA;
function ki() {
  if (VA) return cn;
  VA = 1;
  const E = eE(), t = AE(), n = jR(), _ = kR();
  class R {
    constructor(e, N, A, I, i) {
      this.query = e, this.id = N, this.columns = A, this.parameters = I, this.rowParser = null, this._connection = i;
    }
    close() {
      return this._connection.addCommand(new n(this.id));
    }
    execute(e, N) {
      return typeof e == "function" && (N = e, e = []), this._connection.addCommand(
        new _({ statement: this, values: e }, N)
      );
    }
  }
  class T extends t {
    constructor(e, N) {
      super(), this.query = e.sql, this.onResult = N, this.id = 0, this.fieldCount = 0, this.parameterCount = 0, this.fields = [], this.parameterDefinitions = [], this.options = e;
    }
    start(e, N) {
      ...
      return N.writePacket(i.toPacket(1)), T.prototype.prepareHeader;
    }
    prepareHeader(e, N) {
      const A = new E.PreparedStatementHeader(e);
      return this.id = A.id, this.fieldCount = A.fieldCount, this.parameterCount = A.parameterCount, this.parameterCount > 0 ? T.prototype.readParameter : this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(N);
    }
    readParameter(e, N) {
      if (e.isEOF())
        return this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(N);
      const A = new E.ColumnDefinition(e, N.clientEncoding);
    ...
    parametersEOF(e, N) {
      return e.isEOF() ? this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(N) : N.protocolError("Expected EOF packet after parameters");
    }
    fieldsEOF(e, N) {
      return e.isEOF() ? this.prepareDone(N) : N.protocolError("Expected EOF packet after fields");
    }
    prepareDone(e) {
      const N = new R(
        this.query,
      ...
      return e._statements.set(this.key, N), this.onResult && this.onResult(null, N), null;
    }
  }
  return cn = T, cn;
}
...
function qi() {
  ...
  class R extends E {
    ...
    start(I, i) {
      ...
      return i.writePacket(u.toPacket(1)), R.prototype.binlogData;
    }
    binlogData(I) {
      if (I.isEOF())
        return this.emit("eof"), null;
      I.readInt8();
      const i = new _(I), u = n[i.eventType];
      let O;
      return u ? O = new u(I) : O = {
        name: "UNKNOWN"
      }, O.header = i, this.emit("event", O), R.prototype.binlogData;
    }
  }
  class T {
    constructor(I) {
  ...
  class e {
    constructor(I) {
      ...
      this.schema = I.readString(u), I.readInt8(), this.statusVars = i(S), this.query = I.readString(), this.name = "QueryEvent";
    }
  }
  class N {
    constructor(I) {
  ...
  return n[2] = e, n[4] = T, n[15] = a, n[16] = N, Dn = R, Dn;
}
var Sn, vA;
function pi() {
  if (vA) return Sn;
  vA = 1;
  const E = AE(), t = eE(), n = rE(), _ = yR(), R = iE();
  class T extends E {
    constructor(e, N) {
      super(), this.onResult = N, this.user = e.user, this.password = e.password, this.password1 = e.password, this.password2 = e.password2, this.password3 = e.password3, this.database = e.database, this.passwordSha1 = e.passwordSha1, this.charsetNumber = e.charsetNumber, this.currentConfig = e.currentConfig, this.authenticationFactor = 0;
    }
    start(e, N) {
      ...
      return this.currentConfig.user = this.user, this.currentConfig.password = this.password, this.currentConfig.database = this.database, this.currentConfig.charsetNumber = this.charsetNumber, N.clientEncoding = R[this.charsetNumber], N._statements.clear(), N.writePacket(A.toPacket()), N.serverCapabilityFlags & n.MULTI_FACTOR_AUTHENTICATION && (this.authenticationFactor = 1), T.prototype.handshakeResult;
    }
  }
  return T.prototype.handshakeResult = _.prototype.handshakeResult, T.prototype.calculateNativePasswordAuthToken = _.prototype.calculateNativePasswordAuthToken, Sn = T, Sn;
}
var Bn, HA;
function xi() {
  if (HA) return Bn;
...
function $i() {
  if (yA) return ln;
  yA = 1;
  const E = yR(), t = Wi(), n = KR(), _ = ki(), R = jR(), T = kR(), a = Ji(), e = zi(), N = qi(), A = pi(), I = xi();
  return ln = {
    ClientHandshake: E,
    ServerHandshake: t,
    Query: n,
...
var Mn = {}, DE = { exports: {} }, fE = {}, XA;
function nI() {
  return XA || (XA = 1, Object.defineProperty(fE, "__esModule", { value: !0 }), fE.defaults = void 0, fE.defaults = [
    `-----BEGIN CERTIFICATE-----
...
var gE = {}, KA;
function eI() {
  return KA || (KA = 1, Object.defineProperty(gE, "__esModule", { value: !0 }), gE.proxies = void 0, gE.proxies = [
    `-----BEGIN CERTIFICATE-----
...
var AI = DE.exports, jA;
function RI() {
  if (jA) return DE.exports;
  jA = 1, Object.defineProperty(AI, "__esModule", { value: !0 });
  const E = nI(), t = eI(), n = {
    ca: t.proxies
  }, _ = {
    ca: [...E.defaults, ...t.proxies]
  };
  return DE.exports = _, DE.exports.proxyBundle = n, DE.exports.default = _, DE.exports;
}
var kA;
function tI() {
...
function xn() {
  if (JA) return hn;
  JA = 1;
  const { URL: E } = gt, t = rE(), n = ME(), { version: _ } = _I;
  let R = null;
  const T = {
    authPlugins: 1,
    authSwitchHandler: 1,
  ...
  class a {
    constructor(N) {
      if (typeof N == "string")
        ...
        const I = a.parseUrl(N.uri);
        for (const i in I)
          Object.prototype.hasOwnProperty.call(I, i) && (N[i] || (N[i] = I[i]));
      }
      for (const I in N)
        Object.prototype.hasOwnProperty.call(N, I) && T[I] !== 1 && console.error(
      ...
      ), this.timezone = "Z") : this.timezone = N.timezone || "local", this.queryFormat = N.queryFormat, this.pool = N.pool || void 0, this.ssl = typeof N.ssl == "string" ? a.getSSLProfile(N.ssl) : N.ssl || !1, this.multipleStatements = N.multipleStatements || !1, this.rowsAsArray = N.rowsAsArray || !1, this.namedPlaceholders = N.namedPlaceholders || !1, this.nestTables = N.nestTables === void 0 ? void 0 : N.nestTables, this.typeCast = N.typeCast === void 0 ? !0 : N.typeCast, this.disableEval = !!N.disableEval, this.timezone[0] === " " && (this.timezone = `+${this.timezone.slice(1)}`), this.ssl) {
      ...
      this.maxPacketSize = 0, this.charsetNumber = N.charset ? a.getCharsetNumber(N.charset) : N.charsetNumber || n.UTF8MB4_UNICODE_CI, this.compress = N.compress || !1, this.authPlugins = N.authPlugins, this.authSwitchHandler = N.authSwitchHandler, this.clientFlags = a.mergeFlags(
        a.getDefaultFlags(N),
        N.flags || ""
      );
      const A = {
    ...
    static mergeFlags(N, A) {
      ...
      for (i in N)
        A.indexOf(`-${N[i]}`) >= 0 || (I |= t[N[i]] || 0);
      for (i in A)
        A[i][0] !== "-" && (N.indexOf(A[i]) >= 0 || (I |= t[A[i]] || 0));
      return I;
    }
    ...
    static parseUrl(N) {
      ...
      return A.searchParams.forEach((i, u) => {
        try {
          I[u] = JSON.parse(i);
        } catch {
          I[u] = i;
...
function iI() {
  ...
  function R(e) {
    ...
    if (N) {
      do {
        ...
        O || (u.push(e.substring(I, i)), s.push(N[0].length === 1 ? r++ : N[1]), I = i + N[0].length, C = I), A = i + N[0].length;
      } while (N = E.exec(e));
      if (s.length)
  ...
  function T(e) {
    ...
    let N = 100, A;
    typeof e.cache == "number" && (N = e.cache), typeof e.cache == "object" && (A = e.cache), e.cache !== !1 && !A && (A = zE().createLRU({ max: N }));
    function I(S, c) {
      const s = [];
      if (S.length === 1)
        return [S[0], []];
    ...
    function u(S) {
      ...
      for (let r = 1; r < S[0].length; ++r)
        S[0][r - 1].slice(-1) === ":" && (c += e.placeholder), c += e.placeholder, c += i(S[0][r]);
  ...
  function a(e, N) {
    ...
    for (let s = 0; s < A[0].length; ++s)
      S = i[A[1][s]], S || (S = ++u, i[A[1][s]] = S), A[1][s] ? (c[S - 1] = A[1][s], O += `${A[0][s]}$${S}`) : O += A[0][s];
    return [O, c.map((s) => N[s])];
  }
  return WE.exports = T, WE.exports.toNumbered = a, WE.exports;
}
var Ln, ZA;
function pE() {
  if (ZA) return Ln;
  ZA = 1;
  const E = Lt, t = wR, n = zn, _ = TE.EventEmitter, R = GR.Readable, T = mR(), a = Zn(), { createLRU: e } = zE(), N = vR(), A = eE(), I = $i(), i = xn(), u = iE();
  let O = 0, S = null;
  class c extends _ {
    constructor(r) {
      ...
      }), this.serverCapabilityFlags = 0, this.authorized = !1, this.sequenceId = 0, this.compressedSequenceId = 0, this.threadId = null, this._handshakePacket = null, this._fatalError = null, this._protocolError = null, this._outOfOrderPackets = [], this.clientEncoding = u[this.config.charsetNumber], this.stream.on("error", this._handleNetworkError.bind(this)), this.packetParser = new N((D) => {
        this.handlePacket(D);
    ...
    _resolveNamedPlaceholders(r) {
      ...
      if (this.config.namedPlaceholders || r.namedPlaceholders) {
        if (Array.isArray(r.values))
          return;
        S === null && (S = iI()()), C = S(r.sql, r.values), r.sql = C[0], r.values = C[1];
      }
    }
    query(r, C, D) {
      let B;
    ...
    changeUser(r, C) {
      ...
      const D = r.charset ? i.getCharsetNumber(r.charset) : this.config.charsetNumber;
...
function JR() {
  ...
  return t.statementKey = E.statementKey, on = t, t.prototype._realEnd = E.prototype.end, on;
}
var fn, pA;
function wE() {
  if (pA) return fn;
...
function Ee() {
  if (ER) return dn;
  ER = 1;
  const E = TE.EventEmitter, t = II(), n = wE(), _ = $n(), R = pE();
  class T extends E {
    constructor(e, N) {
      super(), this.connection = e, this.Promise = N || Promise, _(e, this, [
        "error",
        "drain",
        "connect",
        "end",
    ...
    query(e, N) {
      ...
      return new this.Promise((i, u) => {
        const O = n(i, u, I);
        N !== void 0 ? A.query(e, N, O) : A.query(e, O);
      });
    }
    execute(e, N) {
      const A = this.connection, I = new Error();
      ...
      return new this.Promise((i, u) => {
        const O = n(i, u, I);
        N !== void 0 ? A.execute(e, N, O) : A.execute(e, O);
      });
    }
    end() {
    ...
    beginTransaction() {
      const e = this.connection, N = new Error();
    ...
    ping() {
      ...
      return new this.Promise((A, I) => {
        e.ping((i) => {
          i ? (N.message = i.message, N.code = i.code, N.errno = i.errno, N.sqlState = i.sqlState, N.sqlMessage = i.sqlMessage, I(N)) : A(!0);
        });
      });
    }
    connect() {
      ...
      return new this.Promise((A, I) => {
        e.connect((i, u) => {
          i ? (N.message = i.message, N.code = i.code, N.errno = i.errno, N.sqlState = i.sqlState, N.sqlMessage = i.sqlMessage, I(N)) : A(u);
        });
      });
    }
    prepare(e) {
    ...
    changeUser(e) {
      ...
      return new this.Promise((I, i) => {
        N.changeUser(e, (u) => {
          u ? (A.message = u.message, A.code = u.code, A.errno = u.errno, A.sqlState = u.sqlState, A.sqlMessage = u.sqlMessage, i(A)) : I();
        });
      });
    }
    get config() {
  ...
  return (function(a) {
    for (let e = 0; a && e < a.length; e++) {
      const N = a[e];
      typeof R.prototype[N] == "function" && T.prototype[N] === void 0 && (T.prototype[N] = /* @__PURE__ */ (function(I) {
        return function() {
...
function zR() {
  if (eR) return Fn;
  eR = 1;
  const E = JE, t = Zn(), n = TE.EventEmitter, _ = NI(), R = mR(), T = pE();
  function a(N, A) {
    const I = N.length;
    for (let i = 0; i < I; i++)
      if (N.get(i) === A) {
  ...
  class e extends n {
    ...
    getConnection(A) {
      ...
      }), this._allConnections.push(I), I.connect((i) => this._closed ? A(new Error("Pool is closed.")) : i ? A(i) : (this.emit("connection", I), this.emit("acquire", I), A(null, I)))) : this.config.waitForConnections ? this.config.queueLimit && this._connectionQueue.length >= this.config.queueLimit ? A(new Error("Queue limit reached.")) : (this.emit("enqueue"), this._connectionQueue.push(A)) : E.nextTick(() => A(new Error("No connections available.")));
    }
    releaseConnection(A) {
      let I;
      ...
      let I = !1, i = 0, u;
      const O = (function(S) {
        if (!I && (S || ++i >= this._allConnections.length)) {
          I = !0, A(S);
          return;
        }
      }).bind(this);
    ...
    execute(A, I, i) {
      typeof I == "function" && (i = I, I = []), this.getConnection((u, O) => {
        if (u)
          return i(u);
        try {
          O.execute(A, I, i).once("end", () => {
            O.release();
          });
        } catch (S) {
          return O.release(), i(S);
        }
      });
    }
    _removeConnection(A) {
...
function ZR() {
  if (AR) return Qn;
  AR = 1;
  const E = TE.EventEmitter, t = wE(), n = xE(), _ = $n(), R = zR();
  class T extends E {
    constructor(e, N) {
      super(), this.pool = e, this.Promise = N || Promise, _(e, this, ["acquire", "connection", "enqueue", "release"]);
    }
    getConnection() {
      const e = this.pool;
      return new this.Promise((N, A) => {
        e.getConnection((I, i) => {
          I ? A(I) : N(new n(i, this.Promise));
        });
      });
    }
    releaseConnection(e) {
    ...
    execute(e, N) {
      ...
      return new this.Promise((i, u) => {
        const O = t(i, u, I);
        N ? A.execute(e, N, O) : A.execute(e, O);
      });
    }
    end() {
      const e = this.pool, N = new Error();
      return new this.Promise((A, I) => {
        e.end((i) => {
          i ? (N.message = i.message, N.code = i.code, N.errno = i.errno, N.sqlState = i.sqlState, N.sqlMessage = i.sqlMessage, I(N)) : A();
        });
      });
    }
  }
...
function $R() {
  if (IR) return mn;
  IR = 1;
  const E = JE, t = qR(), n = pR(), _ = xR(), R = TE.EventEmitter, T = {
    RR() {
      let I = 0;
      return (i) => i[I++ % i.length];
    },
    RANDOM() {
    ...
    if (I instanceof RegExp)
      return I;
    const i = I.replace(/([.+?^=!:${}()|[\]/\\])/g, "\\$1").replace(/\*/g, ".*");
    return new RegExp(`^${i}$`);
  };
  class N {
    constructor(i, u, O) {
      this._cluster = i, this._pattern = u, this._selector = T[O]();
    }
    getConnection(i) {
      const u = this._getClusterNode();
      if (u === null) {
        let O = new Error("Pool does Not exist.");
        return O.code = "POOL_NOEXIST", this._cluster._findNodeIds(this._pattern, !0).length !== 0 && (O = new Error("Pool does Not have online node."), O.code = "POOL_NONEONLINE"), i(O);
      }
      return this._cluster._getConnection(u, (O, S) => O ? this._cluster._canRetry && this._cluster._findNodeIds(this._pattern).length !== 0 ? (this._cluster.emit("warn", O), this.getConnection(i)) : i(O) : i(null, S));
    }
    /**
     * pool cluster query
     * @param {*} sql
    ...
    _getClusterNode() {
      ...
      if (i.length === 0)
        return null;
      const u = i.length === 1 ? i[0] : this._selector(i);
  ...
  class A extends R {
    ...
    of(i, u) {
      i = i || "*", u = u || this._defaultSelector, u = u.toUpperCase(), !T[u] === "undefined" && (u = this._defaultSelector);
      const O = i + u;
      return typeof this._namespaces[O] > "u" && (this._namespaces[O] = new N(this, i, u)), this._namespaces[O];
    }
    add(i, u) {
      typeof i == "object" && (u = i, i = `CLUSTER::${++this._lastId}`), typeof this._nodes[i] > "u" && (this._nodes[i] = {
        id: i,
        errorCount: 0,
    ...
    getConnection(i, u, O) {
      let S;
      typeof i == "function" ? (O = i, S = this.of()) : (typeof u == "function" && (O = u, u = this._defaultSelector), S = this.of(i, u)), S.getConnection(O);
    }
    end(i) {
      const u = i !== void 0 ? i : (s) => {
        if (s)
          throw s;
      };
      if (this._closed) {
    ...
    _findNodeIds(i, u) {
      ...
      return this._findCaches[i] = S, u ? S : S.filter((c) => {
    ...
    _increaseErrorCount(i) {
      const u = ++i.errorCount;
      if (!(this._removeNodeErrorCount > u)) {
        if (this._restoreNodeTimeout > 0) {
          i._offlineUntil = a() + this._restoreNodeTimeout, this.emit("offline", i.id);
          return;
        }
    ...
    _decreaseErrorCount(i) {
      let u = i.errorCount;
      u > this._removeNodeErrorCount && (u = this._removeNodeErrorCount), u < 1 && (u = 1), i.errorCount = u - 1, i._offlineUntil && (i._offlineUntil = 0, this.emit("online", i.id));
    }
    _getConnection(i, u) {
      i.pool.getConnection((O, S) => O ? (this._increaseErrorCount(i), u(O)) : (this._decreaseErrorCount(i), S._clusterId = i.id, u(null, S)));
    }
    _removeNode(i) {
      const u = this._serviceableNodeIds.indexOf(i.id);
      u !== -1 && (this._serviceableNodeIds.splice(u, 1), delete this._nodes[i.id], this._clearFindCaches(), i.pool.end());
    }
    _clearFindCaches() {
      this._findCaches = {};
    }
...
function cI() {
  return uR || (uR = 1, (function(E) {
    const t = Zn(), n = TE.EventEmitter, _ = qn(), R = $R(), T = TI(), a = aI(), e = rI(), N = Ee(), A = ZR(), I = wE(), i = xE(), u = $n(), O = uI();
    function S(C) {
    ...
    E.createConnection = S, E.createPool = c, E.createPoolCluster = r, E.escape = t.escape, E.escapeId = t.escapeId, E.format = t.format, E.raw = t.raw, E.PromisePool = A, E.PromiseConnection = N, E.PromisePoolConnection = i, E.__defineGetter__("Types", () => NE()), E.__defineGetter__(
      "Charsets",
      () => ME()
    ), E.__defineGetter__(
      "CharsetToEncoding",
...
function lI(E) {
  let n = Q().prepare(`
    ...
    ORDER BY h.hora
  `).all(E, E);
  return Et(E) && (n = n.filter((_) => _.hora < "12:00")), n;
}
async function MI(E) {
  const t = new Date(E).toISOString().split("T")[0], n = await b(async (_) => {
    const [R] = await _.execute(
      `
        SELECT h.hora
        FROM horarios_base h
  ...
  if (n.ok) {
    let _ = n.value;
    return Et(E) && (_ = _.filter((R) => R.hora < "12:00")), _;
  }
  return lI(t);
}
function cR(E) {
...
async function LI(E) {
  if (console.log("[Service] Desactivando horario:", E), (await b(async (n) => {
...
async function fI() {
  console.log("[Service] Obteniendo horarios inactivos");
  const E = await b(async (t) => {
    const [n] = await t.execute(
      "SELECT id, hora FROM horarios_base WHERE activo = 0 ORDER BY hora"
    );
    return n;
  });
  return E.ok ? E.value : oI();
}
function sR(E) {
  const t = Q();
  t.transaction(() => {
...
function CR(E, t, n) {
  ...
  const _ = Q(), R = new Date(E).toISOString().split("T")[0], T = hE(t);
...
async function UI(E, t, n) {
  console.log("[Service] Bloqueando horario:", { fecha: E, hora: t, motivo: n });
  const _ = new Date(E).toISOString().split("T")[0], R = hE(t);
  if ((await b(async (a) => {
...
function DR(E, t) {
  ...
  const n = Q(), _ = new Date(E).toISOString().split("T")[0], R = hE(t);
...
async function dI(E, t) {
  console.log("[Service] Desbloqueando horario:", { fecha: E, hora: t });
  const n = new Date(E).toISOString().split("T")[0], _ = hE(t);
  if ((await b(async (T) => {
    await T.execute(
...
async function FI(E) {
  if (console.log("[Service] Borrando horario permanentemente:", E), (await b(async (n) => {
...
async function J(E) {
  return new Promise((t, n) => {
    const _ = `op_${++QI}`, R = Date.now();
    OE.push({ id: _, fn: E, resolve: t, reject: n, createdAt: R }), console.log(`[Lock] ${_} encolada. Cola: ${OE.length} operaciones. Locked: ${XE}`), _t();
  });
}
async function _t() {
  if (XE) {
  ...
  try {
    ...
    XE = !1, console.log(`[Lock] ${E.id} liberada. Quedan: ${OE.length}`), OE.length > 0 && (console.log("[Lock] Procesando siguiente..."), setImmediate(() => _t()));
  }
}
function PI() {
  P("horarios:base", async () => {
...
async function nt(E, t = 0) {
  try {
    return console.log(`[Service] Intento ${t + 1}/${BR}`), E();
  } catch (n) {
    if (n.code === "SQLITE_BUSY" && t < BR - 1)
      return console.warn(`[Service] SQLITE_BUSY, reintentando en ${lR}ms...`), await new Promise((_) => setTimeout(_, lR)), nt(E, t + 1);
    throw n;
  }
}
...
async function yI(E) {
  console.log("[Service] Iniciando crearReserva..."), mI(E), vI(E);
  const t = YI({ ...E }), n = new Date(t.fecha).toISOString().split("T")[0];
  console.log("[Service] Fecha normalizada:", t.fecha, "->", n);
  try {
    const _ = await HI(t, n);
    try {
      await MR(t, n);
    } catch (R) {
    ...
    return console.warn("[Service] MySQL no disponible, usando SQLite local"), await MR(t, n);
  }
}
async function yn(E) {
  console.log("[Service] Obteniendo reserva:", E);
  const t = await b(async (_) => {
...
async function XI(E) {
  if (console.log("[Service] Borrando reserva:", E), (await b(async (_) => {
...
async function KI(E, t, n) {
  if (console.log("[Service] Moviendo reserva:", { id: E, nuevaFecha: t, nuevaHora: n }), (await b(async (T) => {
...
async function jI(E, t) {
  ...
  if ((await b(async (e) => {
    ...
    await e.execute(
      ...
      [R.nombre, R.fecha, R.hora, R.estado, R.detalles, _, n]
    );
    for (const I of Object.keys(A))
      A[I] !== R[I] && await e.execute(
        `
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
            VALUES ( ?, ?, ?, ?, NOW())
    ...
    try {
      ...
      console.warn("[Service] Backup SQLite fallo en actualizarReserva:", e);
    }
    return;
  }
  const a = Q();
  try {
    ...
    throw console.error("[Service] Error en actualizarReserva:", e), e;
  }
}
function _e(E) {
  if (!(!Array.isArray(E) || E.length === 0))
    ...
    `);
      t.transaction(() => {
        for (const e of E) {
          const N = e != null && e.id ? Number(e.id) : null;
          let A = null;
          if (N) {
            const i = n.get(N);
            i != null && i.id && (A = i.id);
...
async function kI(E, t) {
  ...
  return R.ok ? (_e(R.value), R.value) : Q().prepare(`
    SELECT * FROM reservas
    WHERE fecha >= ? AND fecha <= ?
    ORDER BY fecha, hora
  `).all(n, _);
...
async function et(E) {
  const t = new Date(E).toISOString().split("T")[0], n = await b(async (R) => {
  ...
  return n.ok ? (_e(n.value), n.value) : Q().prepare(`
    SELECT * FROM reservas
    WHERE fecha = ?
    ORDER BY hora
  `).all(t);
...
async function JI() {
  ...
  return E.ok ? (_e(E.value), E.value) : Q().prepare(`
    SELECT * FROM reservas
    ORDER BY fecha DESC, hora DESC
  `).all();
}
async function zI(E, t) {
  if (console.log("[Service] Actualizando notas para reserva:", E), (await b(async (R) => {
    const [T] = await R.execute(
...
const qI = "daily-summary.json", SE = {
  enabled: !1,
  sendTime: "07:30",
  recipients: [],
  lastSentDate: ""
...
function Rt(E) {
  ...
  if (!/^\d{2}:\d{2}$/.test(t)) return SE.sendTime;
...
function GE() {
  try {
    ...
    return !n || typeof n != "object" ? { ...SE } : {
      enabled: !!n.enabled,
      sendTime: Rt(String(n.sendTime || SE.sendTime)),
      recipients: tt(Array.isArray(n.recipients) ? n.recipients : []),
...
function kE(E) {
  const t = GE(), n = {
    ...
    sendTime: E.sendTime ? Rt(E.sendTime) : t.sendTime,
    recipients: Array.isArray(E.recipients) ? tt(E.recipients) : t.recipients,
...
function _N() {
  const E = String(process.env.SMTP_HOST || "").trim(), t = String(process.env.SMTP_USER || "").trim(), n = String(process.env.SMTP_PASS || "").trim(), _ = String(process.env.SMTP_FROM || t).trim(), R = xI(), T = $I(R), a = EN();
...
function nN(E, t) {
  const n = [
    ...
    const T = R != null && R.hora ? String(R.hora) : "--:--", a = R != null && R.nombre ? String(R.nombre) : "Sin nombre", e = R != null && R.telefono ? String(R.telefono) : "-", N = R != null && R.matricula ? String(R.matricula) : "-", A = R != null && R.estado ? String(R.estado) : "Pendiente";
    return `${T} | ${a} | ${e} | ${N} | ${A}`;
  });
...
async function it(E) {
  ...
  const _ = await et(E), R = (Array.isArray(_) ? _ : []).filter((N) => String((N == null ? void 0 : N.estado) || "").trim().toLowerCase() !== "cancelada"), T = nN(E, R);
  return await (await import("./nodemailer-B0gEb-6s.js").then((N) => N.n)).createTransport({
    host: n.host,
    port: n.port,
    secure: n.secure,
    tls: {
    ...
    text: T
  }), kE({ lastSentDate: E }), { ok: !0, count: R.length };
}
function eN(E) {
...
function AN() {
  hR || (hR = !0, LR(), setInterval(LR, 60 * 1e3));
}
async function RN(E) {
  const t = String(E || Kn()).trim(), n = /^\d{4}-\d{2}-\d{2}$/.test(t) ? t : Kn(), _ = GE();
  _.enabled || kE({ enabled: !0 });
  try {
    return await it(n);
  } finally {
    _.enabled || kE({ enabled: !1 });
  }
}
function tN() {
  const E = (n, _) => {
...
` + "=".repeat(50)), console.log("[IPC] Recibiendo solicitud de reserva:"), console.log(_), console.log("=".repeat(50)), console.log("[IPC] Esperando lock...");
    const T = await J(async () => (console.log("[IPC] Lock adquirido, ejecutando crearReserva"), await yI(_))), a = Date.now() - R;
...
`), typeof T == "number" && await t("creada", T, {
  ...
  }), P("reservas:obtener", (n, _) => (console.log("[IPC] Obteniendo reserva:", _), yn(_))), P("reservas:borrar", async (n, _) => {
    ...
    return ZI(R, T, a);
  }), P("reservas:dia", async (n, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return R ? et(R) : [];
  }), P("resumen-diario:config:get", async () => GE()), P("resumen-diario:config:set", async (n, _) => kE({
  ...
  })), P("resumen-diario:enviar", async (n, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return RN(R);
  });
}
...
function ON() {
  P("vehiculos:todos", async () => await J(() => rN())), P("vehiculos:historial", async (E, t) => await J(() => cN(t))), P("vehiculos:mysql-by-matricula", async (E, t) => await J(() => uN(t)));
}
const sN = "mysql.env", CN = [
  "MYSQL_HOST",
  "MYSQL_PORT",
  "MYSQL_USER",
  "MYSQL_PASSWORD",
...
function Nt() {
  ...
  const t = z.readFileSync(E, "utf-8"), n = DN(t);
...
function Tt(E) {
  const t = ne(), n = x.dirname(t);
  z.existsSync(n) || z.mkdirSync(n, { recursive: !0 }), z.writeFileSync(t, E, "utf-8");
}
function lN() {
  P("config:env:get", async () => BN()), P("config:env:set", async (E, t) => (Tt(t || ""), Nt(), SI(), { ok: !0 })), P("config:db:test", async () => {
    const E = await b(async (t) => {
...
async function at() {
  return b(async (E) => (await E.query(
    `CREATE TABLE IF NOT EXISTS auditoria_usuarios (
...
function BE(E) {
  return E === "superadmin" ? [...jn] : E === "super" ? [...jn] : E === "admin" ? ["agenda", "reservas", "historial", "ajustes", "vehiculos"] : ["reservas", "historial"];
}
function nE(E) {
  return E === "superadmin" || E === "super" || E === "admin" || E === "user" ? E : "user";
}
function ee(E) {
  const t = jE.randomBytes(16), n = jE.scryptSync(E, t, 32);
  return `scrypt$${t.toString("hex")}$${n.toString("hex")}`;
}
function hN(E, t) {
  const n = t.split("$");
  if (n.length !== 3 || n[0] !== "scrypt") return !1;
  const _ = Buffer.from(n[1], "hex"), R = Buffer.from(n[2], "hex"), T = jE.scryptSync(E, _, 32);
  return jE.timingSafeEqual(R, T);
}
function QE(E, t) {
  const n = nE(E), _ = new Set(jn);
  if (!t || t.length === 0)
    return BE(n);
...
function kn(E, t) {
  ...
  try {
    ...
    return Array.isArray(_) ? QE(n, _) : BE(n);
  } catch {
    return BE(n);
  }
}
async function rt(E) {
  await uE();
  const t = JSON.stringify(QE(E.role, E.permissions)), n = E.activo ?? 1;
  return b(async (_) => (await _.query(
...
function Jn(E) {
  const t = Q(), n = JSON.stringify(QE(E.role, E.permissions)), _ = E.activo ?? 1;
  t.prepare(
    `INSERT INTO usuarios (nombre, username, password_hash, role, permissions_json, activo)
     VALUES ( ?, ?, ?, ?, ?, ?)
  ...
  t.transaction((R) => {
    for (const T of R)
      n.run(
        ...
        T.password_hash,
        nE(T.role),
        T.permissions_json ?? JSON.stringify(BE(nE(T.role))),
        Number(T.activo) || 0
      );
  })(E);
}
async function ut() {
  const E = process.env.SUPERADMIN_USER || "superadmin", t = process.env.SUPERADMIN_PASS || "rodrigoSayasDev724", n = process.env.SUPERADMIN_NAME || "", _ = "superadmin", R = BE(_), T = ee(t);
  await uE();
  const a = await b(async (I) => {
    const [i] = await I.query(
...
async function ct() {
  ...
  if (E.ok) {
    ...
    return E.value.map((_) => ({
      ...
      username: _.username,
      role: nE(_.role),
      permissions: kn(_.permissions_json, nE(_.role)),
...
async function oN() {
  return (await ct()).filter((t) => t.activo).map((t) => ({
    id: t.id,
    nombre: t.nombre,
    username: t.username,
    role: t.role,
...
async function fN(E, t) {
  ...
  return n.ok ? _ = n.value : _ = Q().prepare(
    "SELECT id, nombre, username, password_hash, role, permissions_json, activo FROM usuarios WHERE username = ? LIMIT 1"
  ).get(E), !_ || !_.password_hash ? { ok: !1, error: "Usuario o contraseña inválida" } : _.activo ? hN(t, _.password_hash) ? (await FE({
...
async function gN(E) {
  await uE();
  const t = nE(E.role), n = QE(t, E.permissions), _ = ee(E.password);
  if (!(await rt({
    nombre: E.nombre,
    username: E.username,
    passwordHash: _,
...
async function UN(E) {
  await uE();
  const t = nE(E.role), n = QE(t, E.permissions), _ = await b(async (T) => (await T.query(
  ...
  Q().prepare(
    `UPDATE usuarios SET nombre = ?, username = ?, role = ?, permissions_json = ?, activo = ?
     WHERE id = ?`
  ).run(E.nombre, E.username, t, JSON.stringify(n), E.activo ?? 1, E.id), _.ok || console.warn("[Usuarios] MySQL no disponible, actualizado solo en SQLite"), await FE({
    actor_username: E.actor_username || "sistema",
    actor_role: E.actor_role || "system",
...
async function dN(E, t) {
  const n = await Ot(E);
  await uE(), await b(async (R) => (await R.query("DELETE FROM usuarios WHERE id = ?", [E]), !0)), Q().prepare("DELETE FROM usuarios WHERE id = ?").run(E), await FE({
...
async function wN(E, t, n) {
  ...
  await uE();
  const R = ee(t);
  await b(async (a) => (await a.query("UPDATE usuarios SET password_hash = ? WHERE id = ?", [R, E]), !0)), Q().prepare("UPDATE usuarios SET password_hash = ? WHERE id = ?").run(R, E), await FE({
...
function GN() {
  P("usuarios:bootstrap", async () => (await ut(), { ok: !0 })), P("usuarios:login-list", async () => oN()), P("auth:login", async (E, t, n) => fN(t, n)), P("usuarios:list", async () => ct()), P("usuarios:create", async (E, t) => {
    try {
      return await gN(t), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al crear usuario" };
    }
  }), P("usuarios:update", async (E, t) => {
    try {
      return await UN(t), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al actualizar usuario" };
    }
  }), P("usuarios:delete", async (E, t) => {
    try {
      return await dN(t.id, t.actor), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al eliminar usuario" };
    }
  }), P("usuarios:password", async (E, t) => {
    try {
      return await wN(t.id, t.password, t.actor), { ok: !0 };
    } catch (n) {
...
function FN() {
  P("auditoria:list", async () => MN());
}
function QN() {
  console.log(` 
 Cargando IPC handlers  
`), PI(), tN(), TN(), ON(), lN(), GN(), FN(), console.log(` 
...
function fR() {
  try {
    ...
    const n = x.join(t, `reservas_${bN()}.db`);
...
function mN() {
  fR(), setInterval(fR, 3600 * 1e3);
}
const st = dR(import.meta.url), Ct = x.dirname(st);
globalThis.__filename = st;
globalThis.__dirname = Ct;
process.env.APP_ROOT = x.join(Ct, "..");
const KE = process.env.VITE_DEV_SERVER_URL, YN = x.join(process.env.APP_ROOT, "dist-electron"), Dt = x.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = KE ? x.join(process.env.APP_ROOT, "public") : Dt;
let CE = null;
function WN() {
  CE = new UR({
    width: 1366,
    ...
    webPreferences: {
      preload: x.join(YN, "preload.mjs"),
      nodeIntegration: !1,
      contextIsolation: !0
    }
  }), KE && CE.webContents.openDevTools({ mode: "detach" }), CE.maximize(), CE.on("page-title-updated", (E) => E.preventDefault()), KE ? CE.loadURL(KE) : CE.loadFile(x.join(Dt, "index.html"));
}
sE.whenReady().then(async () => {
  Nt();
  const E = VR();
  E ? console.log("[Main] Modo API remota activo. Se omite DB local y schedulers locales.") : (Q(), await ut()), QN(), E || (mN(), AN()), WN(), gR.on("settings:update", (t, n) => {
    if (!n || typeof n != "object") return;
    const _ = n.soundEnabled, R = n.theme;
    VI({
      soundEnabled: typeof _ == "boolean" ? _ : !0,
      theme: R === "light" ? "light" : "dark"
    });
  });
});
export {
