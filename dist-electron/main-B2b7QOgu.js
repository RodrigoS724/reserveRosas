...
import { ipcMain as tE, app as CE, Notification as Re, shell as Bt, BrowserWindow as UR } from "electron";
import { fileURLToPath as dR } from "node:url";
import x from "node:path";
import lt from "better-sqlite3";
import z from "node:fs";
import aE from "events";
import zE from "process";
import Mt from "net";
import wR from "tls";
import Zn from "timers";
import GR from "stream";
import FR from "buffer";
...
var ie;
function dt() {
  if (ie) return RE.exports;
  ie = 1;
  const E = Ct, t = Dt, n = St, _ = wE, T = Ut.version, a = /(?:^|^)\s*(?:export\s+)?([\w.-]+)(?:\s*=\s*?|:\s+?)(\s*'(?:\\'|[^'])*'|\s*"(?:\\"|[^"])*"|\s*`(?:\\`|[^`])*`|[^#\r\n]+)?\s*(?:#.*)?(?:$|$)/mg;
  function e(h) {
    const L = {};
    let U = h.toString();
    U = U.replace(/\r\n?/mg, `
...
`), G = G.replace(/\\r/g, "\r")), L[F] = G;
  ...
  function i(h) {
    ...
    const L = S(h);
    h.path = L;
    const U = M.configDotenv(h);
    if (!U.parsed) {
      const w = new Error(`MISSING_DATA: Cannot parse ${L} for an unknown reason`);
      throw w.code = "MISSING_DATA", w;
    }
    const d = u(h).split(","), F = d.length;
    let G;
    for (let w = 0; w < F; w++)
      try {
        const o = d[w].trim(), f = O(U, o);
        G = M.decrypt(f.ciphertext, f.key);
        break;
      } catch (o) {
        if (w + 1 >= F)
          throw o;
  ...
  function A(h) {
    console.log(`[dotenv@${T}][WARN] ${h}`);
  }
  function I(h) {
    console.log(`[dotenv@${T}][DEBUG] ${h}`);
  }
  function N(h) {
    console.log(`[dotenv@${T}] ${h}`);
  }
  function u(h) {
  ...
  function S(h) {
    ...
    if (h && h.path && h.path.length > 0)
      ...
      L = t.resolve(process.cwd(), ".env.vault");
    return E.existsSync(L) ? L : null;
  }
  function c(h) {
    return h[0] === "~" ? t.join(n.homedir(), h.slice(1)) : h;
  }
  function s(h) {
    const L = !!(h && h.debug), U = h && "quiet" in h ? h.quiet : !0;
    (L || !U) && N("Loading env from encrypted .env.vault");
    const d = M._parseVault(h);
    let F = process.env;
    return h && h.processEnv != null && (F = h.processEnv), M.populate(F, d, h), { parsed: d };
  }
  function r(h) {
    const L = t.resolve(process.cwd(), ".env");
    let U = "utf8";
    ...
    for (const l of G)
      try {
        const g = M.parse(E.readFileSync(l, { encoding: U }));
        M.populate(o, g, h);
      } catch (g) {
        d && I(`Failed to load ${l} ${g.message}`), w = g;
      }
    let f = process.env;
    if (h && h.processEnv != null && (f = h.processEnv), M.populate(f, o, h), d || !F) {
      const l = Object.keys(o).length, g = [];
      for (const P of G)
        try {
          const Y = t.relative(process.cwd(), P);
          g.push(Y);
        } catch (Y) {
          d && I(`Failed to load ${P} ${Y.message}`), w = Y;
        }
      N(`injecting env (${l}) from ${g.join(",")}`);
    }
    return w ? { parsed: o, error: w } : { parsed: o };
  }
  function C(h) {
    if (u(h).length === 0)
      return M.configDotenv(h);
    const L = S(h);
    return L ? M._configVault(h) : (A(`You set DOTENV_KEY but you are missing a .env.vault file at ${L}. Did you forget to build it?`), M.configDotenv(h));
  }
  function D(h, L) {
    ...
    try {
      ...
      const o = w instanceof RangeError, f = w.message === "Invalid key length", l = w.message === "Unsupported state or unable to authenticate data";
      if (o || f) {
  ...
  return RE.exports.configDotenv = M.configDotenv, RE.exports._configVault = M._configVault, RE.exports._parseVault = M._parseVault, RE.exports.config = M.config, RE.exports.decrypt = M.decrypt, RE.exports.parse = M.parse, RE.exports.populate = M.populate, RE.exports = M, RE.exports;
}
var E_, Ie;
function wt() {
  if (Ie) return E_;
...
function Gt() {
  ...
  return __ = function(n) {
    const _ = n.reduce(function(R, T) {
      const a = T.match(E);
...
const PR = dR(import.meta.url), Qt = x.dirname(PR);
typeof globalThis < "u" && (globalThis.__filename = PR, globalThis.__dirname = Qt);
let W = null, n_ = !1;
function Q() {
  if (W)
    return console.log(" [DB] Reutilizando conexión existente"), W;
  if (n_ && (console.log("⏳ [DB] Conexión en progreso, esperando..."), W))
    return W;
  n_ = !0;
  try {
    if (!CE.isReady())
      throw new Error("Electron app not ready");
    const E = CE.getPath("userData"), t = x.join(E, "reservas.db");
    if (z.existsSync(E) || z.mkdirSync(E, { recursive: !0 }), console.log(" [DB] Creando nueva conexión a:", t), W = new lt(t, {
      readonly: !1,
      fileMustExist: !1,
      timeout: 3e4
    }), console.log(" [DB] Configurando pragmas..."), W.pragma("query_only = FALSE"), W.pragma("journal_mode = OFF"), W.pragma("synchronous = OFF"), W.pragma("cache_size = -64000"), W.pragma("temp_store = MEMORY"), W.pragma("foreign_keys = ON"), W.pragma("busy_timeout = 100000"), console.log(" [DB] Pragmas configurados correctamente"), W.exec(`
  ...
  `).get().total === 0) {
    ...
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN notas TEXT"), console.log(' [DB] Columna "notas" agregada a reservas');
    } catch (_) {
    ...
    console.log(" DB inicializada en:", t);
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN particular_tipo TEXT"), console.log('âœ… [DB] Columna "particular_tipo" agregada a reservas');
    } catch (_) {
    ...
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN garantia_tipo TEXT"), console.log('âœ… [DB] Columna "garantia_tipo" agregada a reservas');
    } catch (_) {
    ...
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN garantia_fecha_compra TEXT"), console.log('âœ… [DB] Columna "garantia_fecha_compra" agregada a reservas');
    } catch (_) {
    ...
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN garantia_numero_service TEXT"), console.log('âœ… [DB] Columna "garantia_numero_service" agregada a reservas');
    } catch (_) {
    ...
    try {
      W.exec("ALTER TABLE reservas ADD COLUMN garantia_problema TEXT"), console.log('âœ… [DB] Columna "garantia_problema" agregada a reservas');
    } catch (_) {
...
function Pt() {
  return ae || (ae = 1, (function(E) {
    ...
    t.escapeId = function(I, N) {
      if (Array.isArray(I)) {
        for (var u = "", O = 0; O < I.length; O++)
          u += (O === 0 ? "" : ", ") + t.escapeId(I[O], N);
        return u;
      } else return N ? "`" + String(I).replace(n, "``") + "`" : "`" + String(I).replace(n, "``").replace(_, "`.`") + "`";
    }, t.escape = function(I, N, u) {
      if (I == null)
        return "NULL";
      switch (typeof I) {
        case "boolean":
        ...
        case "number":
          return I + "";
        case "object":
          return Object.prototype.toString.call(I) === "[object Date]" ? t.dateToString(I, u || "local") : Array.isArray(I) ? t.arrayToList(I, u) : Buffer.isBuffer(I) ? t.bufferToString(I) : typeof I.toSqlString == "function" ? String(I.toSqlString()) : N ? a(I.toString()) : t.objectToValues(I, u);
        default:
          return a(I);
      }
    }, t.arrayToList = function(I, N) {
      for (var u = "", O = 0; O < I.length; O++) {
        var S = I[O];
        Array.isArray(S) ? u += (O === 0 ? "" : ", ") + "(" + t.arrayToList(S, N) + ")" : u += (O === 0 ? "" : ", ") + t.escape(S, !0, N);
      }
      return u;
    }, t.format = function(I, N, u, O) {
      if (N == null)
        return I;
      Array.isArray(N) || (N = [N]);
      for (var S = 0, c = /\?+/g, s = "", r = 0, C; r < N.length && (C = c.exec(I)); ) {
        var D = C[0].length;
        if (!(D > 2)) {
          var B = D === 2 ? t.escapeId(N[r]) : t.escape(N[r], u, O);
          s += I.slice(S, C.index) + B, S = c.lastIndex, r++;
        }
      }
      return S === 0 ? I : S < I.length ? s + I.slice(S) : s;
    }, t.dateToString = function(I, N) {
      var u = new Date(I);
      if (isNaN(u.getTime()))
        return "NULL";
      var O, S, c, s, r, C, D;
      if (N === "local")
        O = u.getFullYear(), S = u.getMonth() + 1, c = u.getDate(), s = u.getHours(), r = u.getMinutes(), C = u.getSeconds(), D = u.getMilliseconds();
      else {
        var B = i(N);
        B !== !1 && B !== 0 && u.setTime(u.getTime() + B * 6e4), O = u.getUTCFullYear(), S = u.getUTCMonth() + 1, c = u.getUTCDate(), s = u.getUTCHours(), r = u.getUTCMinutes(), C = u.getUTCSeconds(), D = u.getUTCMilliseconds();
      }
      var M = e(O, 4) + "-" + e(S, 2) + "-" + e(c, 2) + " " + e(s, 2) + ":" + e(r, 2) + ":" + e(C, 2) + "." + e(D, 3);
      return a(M);
    }, t.bufferToString = function(I) {
      return "X" + a(I.toString("hex"));
    }, t.objectToValues = function(I, N) {
      var u = "";
      for (var O in I) {
        var S = I[O];
        typeof S != "function" && (u += (u.length === 0 ? "" : ", ") + t.escapeId(O) + " = " + t.escape(S, !0, N));
      }
      return u;
    }, t.raw = function(I) {
      if (typeof I != "string")
    ...
    function a(A) {
      for (var I = R.lastIndex = 0, N = "", u; u = R.exec(A); )
        N += A.slice(I, u.index) + T[u[0]], I = R.lastIndex;
      return I === 0 ? "'" + A + "'" : I < A.length ? "'" + N + A.slice(I) + "'" : "'" + N + "'";
    }
    function e(A, I) {
...
var R_, re;
function qn() {
  return re || (re = 1, R_ = Pt()), R_;
}
var oE = {}, ue;
function ZE() {
  if (ue) return oE;
  ue = 1, Object.defineProperty(oE, "__esModule", { value: !0 }), oE.createLRU = void 0;
  const E = (t) => {
    let { max: n } = t;
    if (!(Number.isInteger(n) && n > 0))
      throw new TypeError("`max` must be a positive integer");
    let _ = 0, R = 0, T = 0, a = [];
    const { onEviction: e } = t, i = /* @__PURE__ */ new Map(), A = new Array(n).fill(void 0), I = new Array(n).fill(void 0), N = new Array(n).fill(0), u = new Array(n).fill(0), O = (r) => {
      N[T] = r, u[r] = T, N[r] = 0, T = r;
    }, S = (r) => {
      if (r === T)
        return;
      const C = N[r], D = u[r];
      r === R ? R = C : N[D] = C, u[C] = D, O(r);
    }, c = (r) => {
      let C = T;
      const D = Math.min(_, r), B = _ - D, M = new Array(D), h = new Array(D);
      for (let L = 0; L < B; L++) {
        const U = A[R];
        e == null || e(U, I[R]), i.delete(U), R = N[R];
      }
      for (let L = D - 1; L >= 0; L--)
        M[L] = A[C], h[L] = I[C], i.set(A[C], L), C = u[C];
      R = 0, T = D - 1, _ = D, A.length = r, I.length = r, N.length = r, u.length = r;
      for (let L = 0; L < D; L++)
        A[L] = M[L], I[L] = h[L], N[L] = L + 1, u[L] = L - 1;
      a = [];
      for (let L = D; L < r; L++)
        a.push(L);
    }, s = (r) => {
      A.length = r, I.length = r, N.length = r, u.length = r, A.fill(void 0, n), I.fill(void 0, n), N.fill(0, n), u.fill(0, n);
    };
    return {
      ...
      set(r, C) {
        ...
        if (D === void 0) {
          if (_ === n) {
            D = R;
            const B = A[D];
            e == null || e(B, I[D]), i.delete(B), R = N[D], u[R] = 0;
          } else
            D = a.length > 0 ? a.pop() : _, _++;
          i.set(r, D), A[D] = r, I[D] = C, _ === 1 ? R = T = D : O(D);
        } else
          e == null || e(r, I[D]), I[D] = C, S(D);
      },
      ...
      *entries() {
      ...
      delete(r) {
        ...
        if (C === void 0)
          return !1;
        e == null || e(r, I[C]), i.delete(r), a.push(C), A[C] = void 0, I[C] = void 0;
        const D = u[C], B = N[C];
        return C === R ? R = B : N[D] = B, C === T ? T = D : u[B] = D, _--, !0;
      },
      ...
      evict: (r) => {
        ...
        for (; C > 0; ) {
          const D = R, B = A[D];
          e == null || e(B, I[D]), i.delete(B), A[D] = void 0, I[D] = void 0, R = N[D], u[R] = 0, _--, a.push(D), C--;
        }
        _ === 0 && (R = T = 0);
      },
      ...
      clear() {
        ...
        i.clear(), A.fill(void 0), I.fill(void 0), a = [], _ = 0, R = T = 0;
      },
  ...
  return oE.createLRU = E, oE;
}
var t_, ce;
function pn() {
  if (ce) return t_;
  ce = 1;
  const { createLRU: E } = ZE(), t = E({
    max: 15e3
  });
  function n(a, e, i, A) {
    const I = [
      ...
      i.nestTables,
      !!i.rowsAsArray,
      !!(i.supportBigNumbers || A.supportBigNumbers),
      !!(i.bigNumberStrings || A.bigNumberStrings),
      typeof i.typeCast == "boolean" ? i.typeCast : typeof i.typeCast,
      i.timezone || A.timezone,
      !!i.decimalNumbers,
      i.dateStrings
    ];
  ...
  function _(a, e, i, A, I) {
    const N = n(a, e, i, A);
    let u = t.get(N);
    return u || (u = I(e, i, A), t.set(N, u), u);
  }
  function R(a) {
    t.resize(a);
  }
...
function VR() {
  ...
  function E(t, _) {
    var _ = _ || {};
    this._capacity = _.capacity, this._head = 0, this._tail = 0, Array.isArray(t) ? this._fromArray(t) : (this._capacityMask = 3, this._list = new Array(4));
  }
  return E.prototype.peekAt = function(n) {
    ...
    if (n !== this._head) {
      ...
      return this._list[this._tail] = void 0, this._head < 2 && n > 1e4 && n <= _ >>> 2 && this._shrinkArray(), R;
    }
  }, E.prototype.removeOne = function(n) {
    var _ = n;
    if (_ === (_ | 0) && this._head !== this._tail) {
      var R = this.size(), T = this._list.length;
      if (!(_ >= R || _ < -R)) {
        _ < 0 && (_ += R), _ = this._head + _ & this._capacityMask;
        var a = this._list[_], e;
        if (n < R / 2) {
          for (e = n; e > 0; e--)
    ...
    if (R === (R | 0) && this._head !== this._tail) {
      var e = this.size(), i = this._list.length;
      if (!(R >= e || R < -e || _ < 1)) {
        if (R < 0 && (R += e), _ === 1 || !_)
          return T = new Array(1), T[0] = this.removeOne(R), T;
        if (R === 0 && R + _ >= e)
          return T = this.toArray(), this.clear(), T;
        R + _ > e && (_ = e - R);
        var A;
        for (T = new Array(_), A = 0; A < _; A++)
          T[A] = this._list[this._head + R + A & this._capacityMask];
        if (R = this._head + R & this._capacityMask, n + _ === e) {
          for (this._tail = this._tail - _ + i & this._capacityMask, A = _; A > 0; A--)
            this._list[R = R + 1 + i & this._capacityMask] = void 0;
          return T;
        }
        if (n === 0) {
          for (this._head = this._head + _ + i & this._capacityMask, A = _ - 1; A > 0; A--)
            this._list[R = R + 1 + i & this._capacityMask] = void 0;
          return T;
        }
        if (R < e / 2) {
          for (this._head = this._head + n + _ + i & this._capacityMask, A = n; A > 0; A--)
            this.unshift(this._list[R = R - 1 + i & this._capacityMask]);
          for (R = this._head - 1 + i & this._capacityMask; a > 0; )
            this._list[R = R - 1 + i & this._capacityMask] = void 0, a--;
          n < 0 && (this._tail = R);
        } else {
          for (this._tail = R, R = R + _ + i & this._capacityMask, A = e - (_ + n); A > 0; A--)
    ...
    if (R === (R | 0)) {
      ...
      if (R < 0 && (R += T), !(R > T))
        if (arguments.length > 2) {
          var a, e, i, A = arguments.length, I = this._list.length, N = 2;
          if (!T || R < T / 2) {
            for (e = new Array(R), a = 0; a < R; a++)
              e[a] = this._list[this._head + a & this._capacityMask];
            for (_ === 0 ? (i = [], R > 0 && (this._head = this._head + R + I & this._capacityMask)) : (i = this.remove(R, _), this._head = this._head + R + I & this._capacityMask); A > N; )
            ...
            var u = e.length;
            for (a = 0; a < u; a++)
              e[a] = this._list[this._head + R + _ + a & this._capacityMask];
            for (_ === 0 ? (i = [], R != T && (this._tail = this._head + R + I & this._capacityMask)) : (i = this.remove(R, _), this._tail = this._tail - u + I & this._capacityMask); N < A; )
    ...
    var _ = n.length, R = this._nextPowerOf2(_);
    ...
    var R = this._list, T = R.length, a = this.length;
    if (_ = _ | a, _ == a && this._head < this._tail)
      return this._list.slice(this._head, this._tail);
    var e = new Array(_), i = 0, A;
    if (n || this._head > this._tail) {
      for (A = this._head; A < T; A++) e[i++] = R[A];
      for (A = 0; A < this._tail; A++) e[i++] = R[A];
    } else
      for (A = this._head; A < this._tail; A++) e[i++] = R[A];
    return e;
    ...
    var _ = Math.log(n) / Math.log(2), R = 1 << _ + 1;
    return Math.max(R, 4);
  }, i_ = E, i_;
}
var I_ = {}, se;
...
var dE = { exports: {} }, Vt = dE.exports, Ce;
function bt() {
  return Ce || (Ce = 1, (function(E, t) {
    (function(n, _) {
      ...
      typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : Vt,
      function(n) {
        ...
        function A(f, l) {
          var g, P, Y;
          return l ? (f >>>= 0, (Y = 0 <= f && f < 256) && (P = i[f], P) ? P : (g = N(f, 0, !0), Y && (i[f] = g), g)) : (f |= 0, (Y = -128 <= f && f < 128) && (P = e[f], P) ? P : (g = N(f, f < 0 ? -1 : 0, !1), Y && (e[f] = g), g));
        }
        R.fromInt = A;
        function I(f, l) {
          if (isNaN(f)) return l ? h : M;
          if (l) {
            if (f < 0) return h;
            if (f >= C) return G;
          } else {
        ...
        function O(f, l, g) {
          ...
          var P;
          if ((P = f.indexOf("-")) > 0) throw Error("interior hyphen");
          if (P === 0)
            return O(f.substring(1), l, g).neg();
          for (var Y = I(u(g, 8)), H = M, K = 0; K < f.length; K += 8) {
            var y = Math.min(8, f.length - K), EE = parseInt(f.substring(K, K + y), g);
            if (y < 8) {
              var q = I(u(g, y));
              H = H.mul(q).add(I(EE));
            } else
              H = H.mul(Y), H = H.add(I(EE));
          }
          return H.unsigned = l, H;
        }
        R.fromString = O;
        function S(f, l) {
          return typeof f == "number" ? I(f, l) : typeof f == "string" ? O(f, l) : N(
            f.low,
            f.high,
        ...
        R.fromValue = S;
        var c = 65536, s = 1 << 24, r = c * c, C = r * r, D = C / 2, B = A(s), M = A(0);
        R.ZERO = M;
        var h = A(0, !0);
        R.UZERO = h;
        var L = A(1);
        ...
        o.toInt = function() {
          ...
          if (this.isNegative())
            if (this.eq(w)) {
              var g = I(l), P = this.div(g), Y = P.mul(g).sub(this);
              return P.toString(l) + Y.toInt().toString(l);
            } else return "-" + this.neg().toString(l);
          for (var H = I(u(l, 6), this.unsigned), K = this, y = ""; ; ) {
            var EE = K.div(H), q = K.sub(EE.mul(H)).toInt() >>> 0, k = q.toString(l);
            if (K = EE, K.isZero()) return k + y;
          ...
          return T(l) || (l = S(l)), this.unsigned !== l.unsigned && this.high >>> 31 === 1 && l.high >>> 31 === 1 ? !1 : this.high === l.high && this.low === l.low;
        }, o.eq = o.equals, o.notEquals = function(l) {
          return !this.eq(
            /* validates */
            l
          );
        }, o.neq = o.notEquals, o.ne = o.notEquals, o.lessThan = function(l) {
          return this.comp(
            /* validates */
            l
          ) < 0;
        }, o.lt = o.lessThan, o.lessThanOrEqual = function(l) {
          return this.comp(
            /* validates */
            l
          ) <= 0;
        }, o.lte = o.lessThanOrEqual, o.le = o.lessThanOrEqual, o.greaterThan = function(l) {
          return this.comp(
            /* validates */
            l
          ) > 0;
        }, o.gt = o.greaterThan, o.greaterThanOrEqual = function(l) {
          return this.comp(
            /* validates */
            l
          ) >= 0;
        }, o.gte = o.greaterThanOrEqual, o.ge = o.greaterThanOrEqual, o.compare = function(l) {
          if (T(l) || (l = S(l)), this.eq(l)) return 0;
          var g = this.isNegative(), P = l.isNegative();
          return g && !P ? -1 : !g && P ? 1 : this.unsigned ? l.high >>> 0 > this.high >>> 0 || l.high === this.high && l.low >>> 0 > this.low >>> 0 ? -1 : 1 : this.sub(l).isNegative() ? -1 : 1;
        }, o.comp = o.compare, o.negate = function() {
          return !this.unsigned && this.eq(w) ? w : this.not().add(L);
        }, o.neg = o.negate, o.add = function(l) {
          T(l) || (l = S(l));
          var g = this.high >>> 16, P = this.high & 65535, Y = this.low >>> 16, H = this.low & 65535, K = l.high >>> 16, y = l.high & 65535, EE = l.low >>> 16, q = l.low & 65535, k = 0, _E = 0, J = 0, $ = 0;
          return $ += H + q, J += $ >>> 16, $ &= 65535, J += Y + EE, _E += J >>> 16, J &= 65535, _E += P + y, k += _E >>> 16, _E &= 65535, k += g + K, k &= 65535, N(J << 16 | $, k << 16 | _E, this.unsigned);
        }, o.subtract = function(l) {
          return T(l) || (l = S(l)), this.add(l.neg());
        }, o.sub = o.subtract, o.multiply = function(l) {
          ...
          if (T(l) || (l = S(l)), _) {
            ...
            return N(g, _.get_high(), this.unsigned);
          }
          if (l.isZero()) return this.unsigned ? h : M;
          if (this.eq(w)) return l.isOdd() ? w : M;
          if (l.eq(w)) return this.isOdd() ? w : M;
          if (this.isNegative())
          ...
          var P = this.high >>> 16, Y = this.high & 65535, H = this.low >>> 16, K = this.low & 65535, y = l.high >>> 16, EE = l.high & 65535, q = l.low >>> 16, k = l.low & 65535, _E = 0, J = 0, $ = 0, VE = 0;
          return VE += K * k, $ += VE >>> 16, VE &= 65535, $ += H * k, J += $ >>> 16, $ &= 65535, $ += K * q, J += $ >>> 16, $ &= 65535, J += Y * k, _E += J >>> 16, J &= 65535, J += H * q, _E += J >>> 16, J &= 65535, J += K * EE, _E += J >>> 16, J &= 65535, _E += P * k + Y * q + H * EE + K * y, _E &= 65535, N($ << 16 | VE, _E << 16 | J, this.unsigned);
        }, o.mul = o.multiply, o.divide = function(l) {
          if (T(l) || (l = S(l)), l.isZero()) throw Error("division by zero");
          if (_) {
          ...
          if (this.isZero()) return this.unsigned ? h : M;
          var P, Y, H;
          if (this.unsigned) {
            ...
            if (this.eq(w)) {
              ...
              if (l.eq(w)) return L;
              var K = this.shr(1);
              return P = K.div(l).shl(1), P.eq(M) ? l.isNegative() ? L : d : (Y = this.sub(l.mul(P)), H = P.add(Y.div(l)), H);
            } else if (l.eq(w)) return this.unsigned ? h : M;
            if (this.isNegative())
          ...
          for (Y = this; Y.gte(l); ) {
            P = Math.max(1, Math.floor(Y.toNumber() / l.toNumber()));
            for (var y = Math.ceil(Math.log(P) / Math.LN2), EE = y <= 48 ? 1 : u(2, y - 48), q = I(P), k = q.mul(l); k.isNegative() || k.gt(Y); )
              P -= EE, q = I(P, this.unsigned), k = q.mul(l);
            q.isZero() && (q = L), H = H.add(q), Y = Y.sub(k);
          }
          return H;
        }, o.div = o.divide, o.modulo = function(l) {
          if (T(l) || (l = S(l)), _) {
        ...
        }, o.ctz = o.countTrailingZeros, o.and = function(l) {
          return T(l) || (l = S(l)), N(
            this.low & l.low,
            this.high & l.high,
            this.unsigned
        ...
        }, o.shiftLeft = function(l) {
          return T(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l < 32 ? N(
            this.low << l,
            this.high << l | this.low >>> 32 - l,
            this.unsigned
          ) : N(0, this.low << l - 32, this.unsigned);
        }, o.shl = o.shiftLeft, o.shiftRight = function(l) {
          return T(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l < 32 ? N(
        ...
        }, o.shr = o.shiftRight, o.shiftRightUnsigned = function(l) {
          return T(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l < 32 ? N(
            this.low >>> l | this.high << 32 - l,
            this.high >>> l,
            this.unsigned
          ) : l === 32 ? N(this.high, 0, this.unsigned) : N(this.high >>> l - 32, 0, this.unsigned);
        }, o.shru = o.shiftRightUnsigned, o.shr_u = o.shiftRightUnsigned, o.rotateLeft = function(l) {
          var g;
          return T(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l === 32 ? N(this.high, this.low, this.unsigned) : l < 32 ? (g = 32 - l, N(
        ...
        }, o.rotl = o.rotateLeft, o.rotateRight = function(l) {
          var g;
          return T(l) && (l = l.toInt()), (l &= 63) === 0 ? this : l === 32 ? N(this.high, this.low, this.unsigned) : l < 32 ? (g = 32 - l, N(
        ...
        }, R.fromBytes = function(l, g, P) {
          return P ? R.fromBytesLE(l, g) : R.fromBytesBE(l, g);
        ...
        }, typeof BigInt == "function" && (R.fromBigInt = function(l, g) {
          var P = Number(BigInt.asIntN(32, l)), Y = Number(BigInt.asIntN(32, l >> BigInt(32)));
          return N(P, Y, g);
        }, R.fromValue = function(l, g) {
          return typeof l == "bigint" ? R.fromBigInt(l, g) : S(l, g);
        }, o.toBigInt = function() {
...
var bE = {}, yE = { exports: {} }, N_, De;
function rE() {
  if (De) return N_;
  De = 1;
  var E = FR, t = E.Buffer, n = {}, _;
  for (_ in E)
    E.hasOwnProperty(_) && (_ === "SlowBuffer" || _ === "Buffer" || (n[_] = E[_]));
  var R = n.Buffer = {};
  for (_ in t)
    t.hasOwnProperty(_) && (_ === "allocUnsafe" || _ === "allocUnsafeSlow" || (R[_] = t[_]));
  if (n.Buffer.prototype = t.prototype, (!R.from || R.from === Uint8Array.from) && (R.from = function(T, a, e) {
    if (typeof T == "number")
    ...
    return t(T, a, e);
  }), R.alloc || (R.alloc = function(T, a, e) {
    if (typeof T != "number")
      throw new TypeError('The "size" argument must be of type number. Received type ' + typeof T);
    if (T < 0 || T >= 2 * (1 << 30))
      throw new RangeError('The value "' + T + '" is invalid for option "size"');
    var i = t(T);
    return !a || a.length === 0 ? i.fill(0) : typeof e == "string" ? i.fill(a, e) : i.fill(a), i;
  }), !n.kStringMaxLength)
    try {
  ...
  }, n.kStringMaxLength && (n.constants.MAX_STRING_LENGTH = n.kStringMaxLength)), N_ = n, N_;
}
var mE = {}, Se;
function mt() {
  if (Se) return mE;
...
function Yt() {
  ...
  function t(A, I) {
    this.enc = A.encodingName, this.bomAware = A.bomAware, this.enc === "base64" ? this.encoder = T : this.enc === "utf8" ? this.encoder = i : this.enc === "cesu8" && (this.enc = "utf8", this.encoder = a, E.from("eda0bdedb2a9", "hex").toString() !== "💩" && (this.decoder = e, this.defaultCharUnicode = I.defaultCharUnicode));
  }
  t.prototype.encoder = R, t.prototype.decoder = _;
  var n = ht.StringDecoder;
  function _(A, I) {
    this.decoder = new n(I.enc);
  }
  ...
  a.prototype.write = function(A) {
    for (var I = E.alloc(A.length * 3), N = 0, u = 0; u < A.length; u++) {
      var O = A.charCodeAt(u);
      O < 128 ? I[N++] = O : O < 2048 ? (I[N++] = 192 + (O >>> 6), I[N++] = 128 + (O & 63)) : (I[N++] = 224 + (O >>> 12), I[N++] = 128 + (O >>> 6 & 63), I[N++] = 128 + (O & 63));
    }
    return I.slice(0, N);
  ...
  e.prototype.write = function(A) {
    for (var I = this.acc, N = this.contBytes, u = this.accBytes, O = "", S = 0; S < A.length; S++) {
      var c = A[S];
      (c & 192) !== 128 ? (N > 0 && (O += this.defaultCharUnicode, N = 0), c < 128 ? O += String.fromCharCode(c) : c < 224 ? (I = c & 31, N = 1, u = 1) : c < 240 ? (I = c & 15, N = 2, u = 1) : O += this.defaultCharUnicode) : N > 0 ? (I = I << 6 | c & 63, N--, u++, N === 0 && (u === 2 && I < 128 && I > 0 ? O += this.defaultCharUnicode : u === 3 && I < 2048 ? O += this.defaultCharUnicode : O += String.fromCharCode(I))) : O += this.defaultCharUnicode;
    }
    return this.acc = I, this.contBytes = N, this.accBytes = u, O;
...
function Wt() {
  ...
  NE.utf32le = { type: "_utf32", isLE: !0 }, NE.utf32be = { type: "_utf32", isLE: !1 }, NE.ucs4le = "utf32le", NE.ucs4be = "utf32be", t.prototype.encoder = n, t.prototype.decoder = _;
  function n(A, I) {
    this.isLE = I.isLE, this.highSurrogate = 0;
  }
  n.prototype.write = function(A) {
    for (var I = E.from(A, "ucs2"), N = E.alloc(I.length * 2), u = this.isLE ? N.writeUInt32LE : N.writeUInt32BE, O = 0, S = 0; S < I.length; S += 2) {
      var c = I.readUInt16LE(S), s = c >= 55296 && c < 56320, r = c >= 56320 && c < 57344;
      if (this.highSurrogate)
        if (s || !r)
          u.call(N, this.highSurrogate, O), O += 4;
        else {
          var C = (this.highSurrogate - 55296 << 10 | c - 56320) + 65536;
          u.call(N, C, O), O += 4, this.highSurrogate = 0;
          continue;
        }
      s ? this.highSurrogate = c : (u.call(N, c, O), O += 4, this.highSurrogate = 0);
    }
  ...
  _.prototype.write = function(A) {
    if (A.length === 0)
      return "";
    var I = 0, N = 0, u = E.alloc(A.length + 4), O = 0, S = this.isLE, c = this.overflow, s = this.badChar;
    if (c.length > 0) {
      for (; I < A.length && c.length < 4; I++)
        c.push(A[I]);
      c.length === 4 && (S ? N = c[I] | c[I + 1] << 8 | c[I + 2] << 16 | c[I + 3] << 24 : N = c[I + 3] | c[I + 2] << 8 | c[I + 1] << 16 | c[I] << 24, c.length = 0, O = R(u, O, N, s));
    }
    for (; I < A.length - 3; I += 4)
      S ? N = A[I] | A[I + 1] << 8 | A[I + 2] << 16 | A[I + 3] << 24 : N = A[I + 3] | A[I + 2] << 8 | A[I + 1] << 16 | A[I] << 24, O = R(u, O, N, s);
    for (; I < A.length; I++)
      c.push(A[I]);
    return u.slice(0, O).toString("ucs2");
  };
  function R(A, I, N, u) {
    if ((N < 0 || N > 1114111) && (N = u), N >= 65536) {
      N -= 65536;
      var O = 55296 | N >> 10;
      A[I++] = O & 255, A[I++] = O >> 8;
      var N = 56320 | N & 1023;
    }
    return A[I++] = N & 255, A[I++] = N >> 8, I;
  }
  _.prototype.end = function() {
    this.overflow.length = 0;
  }, NE.utf32 = T, NE.ucs4 = "utf32";
  function T(A, I) {
    this.iconv = I;
  }
  T.prototype.encoder = a, T.prototype.decoder = e;
  function a(A, I) {
  ...
  e.prototype.write = function(A) {
    if (!this.decoder) {
      ...
      for (var N = "", u = 0; u < this.initialBufs.length; u++)
        N += this.decoder.write(this.initialBufs[u]);
    ...
    if (!this.decoder) {
      ...
      for (var I = "", N = 0; N < this.initialBufs.length; N++)
        I += this.decoder.write(this.initialBufs[N]);
      var u = this.decoder.end();
      return u && (I += u), this.initialBufs.length = this.initialBufsLen = 0, I;
    }
    return this.decoder.end();
  };
  function i(A, I) {
    var N = [], u = 0, O = 0, S = 0, c = 0, s = 0;
    E:
      for (var r = 0; r < A.length; r++)
        for (var C = A[r], D = 0; D < C.length; D++)
          if (N.push(C[D]), N.length === 4) {
            if (u === 0) {
              if (N[0] === 255 && N[1] === 254 && N[2] === 0 && N[3] === 0)
                return "utf-32le";
              if (N[0] === 0 && N[1] === 0 && N[2] === 254 && N[3] === 255)
                return "utf-32be";
            }
            if ((N[0] !== 0 || N[1] > 16) && S++, (N[3] !== 0 || N[2] > 16) && O++, N[0] === 0 && N[1] === 0 && (N[2] !== 0 || N[3] !== 0) && s++, (N[0] !== 0 || N[1] !== 0) && N[2] === 0 && N[3] === 0 && c++, N.length = 0, u++, u >= 100)
              break E;
          }
    return s - S > c - O ? "utf-32be" : s - S < c - O ? "utf-32le" : I || "utf-32le";
  }
  return NE;
}
var YE = {}, he;
function Ht() {
  ...
  t.prototype.encoder = n, t.prototype.decoder = _, t.prototype.bomAware = !0;
  function n() {
  }
  n.prototype.write = function(i) {
    for (var A = E.from(i, "ucs2"), I = 0; I < A.length; I += 2) {
      var N = A[I];
      A[I] = A[I + 1], A[I + 1] = N;
    }
    return A;
  ...
  _.prototype.write = function(i) {
    if (i.length == 0)
      return "";
    var A = E.alloc(i.length + 1), I = 0, N = 0;
    for (this.overflowByte !== -1 && (A[0] = i[0], A[1] = this.overflowByte, I = 1, N = 2); I < i.length - 1; I += 2, N += 2)
      A[N] = i[I + 1], A[N + 1] = i[I];
    return this.overflowByte = I == i.length - 1 ? i[i.length - 1] : -1, A.slice(0, N).toString("ucs2");
  }, _.prototype.end = function() {
    this.overflowByte = -1;
  }, YE.utf16 = R;
  function R(i, A) {
  ...
  R.prototype.encoder = T, R.prototype.decoder = a;
  function T(i, A) {
    i = i || {}, i.addBOM === void 0 && (i.addBOM = !0), this.encoder = A.iconv.getEncoder("utf-16le", i);
  }
  ...
  a.prototype.write = function(i) {
    if (!this.decoder) {
      if (this.initialBufs.push(i), this.initialBufsLen += i.length, this.initialBufsLen < 16)
        return "";
      var A = e(this.initialBufs, this.options.defaultEncoding);
      this.decoder = this.iconv.getDecoder(A, this.options);
      for (var I = "", N = 0; N < this.initialBufs.length; N++)
        I += this.decoder.write(this.initialBufs[N]);
    ...
    if (!this.decoder) {
      var i = e(this.initialBufs, this.options.defaultEncoding);
      this.decoder = this.iconv.getDecoder(i, this.options);
      for (var A = "", I = 0; I < this.initialBufs.length; I++)
        A += this.decoder.write(this.initialBufs[I]);
      var N = this.decoder.end();
      return N && (A += N), this.initialBufs.length = this.initialBufsLen = 0, A;
    }
    return this.decoder.end();
  };
  function e(i, A) {
    ...
    E:
      for (var S = 0; S < i.length; S++)
        for (var c = i[S], s = 0; s < c.length; s++)
          if (I.push(c[s]), I.length === 2) {
            if (N === 0) {
              if (I[0] === 255 && I[1] === 254) return "utf-16le";
              if (I[0] === 254 && I[1] === 255) return "utf-16be";
            }
            if (I[0] === 0 && I[1] !== 0 && O++, I[0] !== 0 && I[1] === 0 && u++, I.length = 0, N++, N >= 100)
              break E;
          }
    return O > u ? "utf-16be" : O < u ? "utf-16le" : A || "utf-16le";
  }
  return YE;
}
var fE = {}, Le;
function vt() {
  ...
  Le = 1;
  var E = rE().Buffer;
  fE.utf7 = t, fE.unicode11utf7 = "utf7";
  function t(c, s) {
    this.iconv = s;
  }
  t.prototype.encoder = _, t.prototype.decoder = R, t.prototype.bomAware = !0;
  var n = /[^A-Za-z0-9'\(\),-\.\/:\? \n\r\t]+/g;
  function _(c, s) {
    this.iconv = s.iconv;
  }
  _.prototype.write = function(c) {
    return E.from(c.replace(n, (function(s) {
  ...
  for (var T = /[A-Za-z0-9\/+]/, a = [], e = 0; e < 256; e++)
    a[e] = T.test(String.fromCharCode(e));
  var i = 43, A = 45, I = 38;
  R.prototype.write = function(c) {
    for (var s = "", r = 0, C = this.inBase64, D = this.base64Accum, B = 0; B < c.length; B++)
      if (!C)
        c[B] == i && (s += this.iconv.decode(c.slice(r, B), "ascii"), r = B + 1, C = !0);
      else if (!a[c[B]]) {
        if (B == r && c[B] == A)
          s += "+";
        else {
          var M = D + this.iconv.decode(c.slice(r, B), "ascii");
          s += this.iconv.decode(E.from(M, "base64"), "utf16-be");
        }
        c[B] != A && B--, r = B + 1, C = !1, D = "";
      }
    if (!C)
      s += this.iconv.decode(c.slice(r), "ascii");
    else {
      var M = D + this.iconv.decode(c.slice(r), "ascii"), h = M.length - M.length % 8;
      D = M.slice(h), M = M.slice(0, h), s += this.iconv.decode(E.from(M, "base64"), "utf16-be");
    }
    return this.inBase64 = C, this.base64Accum = D, s;
  }, R.prototype.end = function() {
    var c = "";
    return this.inBase64 && this.base64Accum.length > 0 && (c = this.iconv.decode(E.from(this.base64Accum, "base64"), "utf16-be")), this.inBase64 = !1, this.base64Accum = "", c;
  }, fE.utf7imap = N;
  function N(c, s) {
    this.iconv = s;
  }
  N.prototype.encoder = u, N.prototype.decoder = O, N.prototype.bomAware = !0;
  function u(c, s) {
    this.iconv = s.iconv, this.inBase64 = !1, this.base64Accum = E.alloc(6), this.base64AccumIdx = 0;
  }
  u.prototype.write = function(c) {
    for (var s = this.inBase64, r = this.base64Accum, C = this.base64AccumIdx, D = E.alloc(c.length * 5 + 10), B = 0, M = 0; M < c.length; M++) {
      var h = c.charCodeAt(M);
      h >= 32 && h <= 126 ? (s && (C > 0 && (B += D.write(r.slice(0, C).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), B), C = 0), D[B++] = A, s = !1), s || (D[B++] = h, h === I && (D[B++] = A))) : (s || (D[B++] = I, s = !0), s && (r[C++] = h >> 8, r[C++] = h & 255, C == r.length && (B += D.write(r.toString("base64").replace(/\//g, ","), B), C = 0)));
    }
    return this.inBase64 = s, this.base64AccumIdx = C, D.slice(0, B);
  }, u.prototype.end = function() {
    var c = E.alloc(10), s = 0;
    return this.inBase64 && (this.base64AccumIdx > 0 && (s += c.write(this.base64Accum.slice(0, this.base64AccumIdx).toString("base64").replace(/\//g, ",").replace(/=+$/, ""), s), this.base64AccumIdx = 0), c[s++] = A, this.inBase64 = !1), c.slice(0, s);
  };
  function O(c, s) {
    this.iconv = s.iconv, this.inBase64 = !1, this.base64Accum = "";
  }
  var S = a.slice();
  return S[44] = !0, O.prototype.write = function(c) {
    for (var s = "", r = 0, C = this.inBase64, D = this.base64Accum, B = 0; B < c.length; B++)
      if (!C)
        c[B] == I && (s += this.iconv.decode(c.slice(r, B), "ascii"), r = B + 1, C = !0);
      else if (!S[c[B]]) {
        if (B == r && c[B] == A)
          ...
          var M = D + this.iconv.decode(c.slice(r, B), "ascii").replace(/,/g, "/");
          s += this.iconv.decode(E.from(M, "base64"), "utf16-be");
        }
        c[B] != A && B--, r = B + 1, C = !1, D = "";
      }
    if (!C)
      ...
      var M = D + this.iconv.decode(c.slice(r), "ascii").replace(/,/g, "/"), h = M.length - M.length % 8;
      D = M.slice(h), M = M.slice(0, h), s += this.iconv.decode(E.from(M, "base64"), "utf16-be");
    }
    return this.inBase64 = C, this.base64Accum = D, s;
...
function yt() {
  ...
  function t(R, T) {
    ...
    if (R.chars.length === 128) {
      for (var a = "", e = 0; e < 128; e++)
        a += String.fromCharCode(e);
      R.chars = a + R.chars;
    }
    this.decodeBuf = E.from(R.chars, "ucs2");
    for (var i = E.alloc(65536, T.defaultCharSingleByte.charCodeAt(0)), e = 0; e < R.chars.length; e++)
      i[R.chars.charCodeAt(e)] = e;
    this.encodeBuf = i;
  }
  t.prototype.encoder = n, t.prototype.decoder = _;
  function n(R, T) {
    this.encodeBuf = T.encodeBuf;
  }
  n.prototype.write = function(R) {
    for (var T = E.alloc(R.length), a = 0; a < R.length; a++)
      T[a] = this.encodeBuf[R.charCodeAt(a)];
    return T;
  }, n.prototype.end = function() {
  };
  function _(R, T) {
  ...
  return _.prototype.write = function(R) {
    for (var T = this.decodeBuf, a = E.alloc(R.length * 2), e = 0, i = 0, A = 0; A < R.length; A++)
      e = R[A] * 2, i = A * 2, a[i] = T[e], a[i + 1] = T[e + 1];
...
function jt() {
  ...
  var E = rE().Buffer;
  s_._dbcs = i;
  for (var t = -1, n = -2, _ = -10, R = -1e3, T = new Array(256), a = -1, e = 0; e < 256; e++)
    T[e] = t;
  function i(u, O) {
    ...
    if (typeof u.gb18030 == "function") {
      ...
      this.decodeTables.push(T.slice(0));
      for (var C = this.decodeTables[0], c = 129; c <= 254; c++)
        for (var D = this.decodeTables[R - C[c]], B = 48; B <= 57; B++) {
          if (D[B] === t)
            D[B] = R - s;
          else if (D[B] > R)
            throw new Error("gb18030 decode tables conflict at byte 2");
          for (var M = this.decodeTables[R - D[B]], h = 129; h <= 254; h++) {
            if (M[h] === t)
              M[h] = R - r;
            else {
              if (M[h] === R - r)
            ...
            for (var L = this.decodeTables[R - M[h]], U = 48; U <= 57; U++)
              L[U] === t && (L[U] = n);
          }
        }
    }
    ...
    var d = {};
    if (u.encodeSkipVals)
      for (var c = 0; c < u.encodeSkipVals.length; c++) {
        var F = u.encodeSkipVals[c];
        if (typeof F == "number")
          d[F] = !0;
        else
    ...
    if (this._fillEncodeTable(0, 0, d), u.encodeAdd)
      for (var G in u.encodeAdd)
        Object.prototype.hasOwnProperty.call(u.encodeAdd, G) && this._setEncodeChar(G.charCodeAt(0), u.encodeAdd[G]);
  ...
  i.prototype.encoder = A, i.prototype.decoder = I, i.prototype._getDecodeTrieNode = function(u) {
    for (var O = []; u > 0; u >>>= 8)
      O.push(u & 255);
    O.length == 0 && O.push(0);
    for (var S = this.decodeTables[0], c = O.length - 1; c > 0; c--) {
      var s = S[O[c]];
      if (s == t)
        S[O[c]] = R - this.decodeTables.length, this.decodeTables.push(S = T.slice(0));
      else if (s <= R)
        S = this.decodeTables[R - s];
      else
        throw new Error("Overwrite byte in " + this.encodingName + ", addr: " + u.toString(16));
    }
    return S;
  }, i.prototype._addDecodeChunk = function(u) {
    var O = parseInt(u[0], 16), S = this._getDecodeTrieNode(O);
    O = O & 255;
    for (var c = 1; c < u.length; c++) {
      var s = u[c];
      if (typeof s == "string")
        for (var r = 0; r < s.length; ) {
          var C = s.charCodeAt(r++);
          if (C >= 55296 && C < 56320) {
            var D = s.charCodeAt(r++);
            if (D >= 56320 && D < 57344)
              S[O++] = 65536 + (C - 55296) * 1024 + (D - 56320);
            else
              throw new Error("Incorrect surrogate pair in " + this.encodingName + " at chunk " + u[0]);
          } else if (C > 4080 && C <= 4095) {
            for (var B = 4095 - C + 2, M = [], h = 0; h < B; h++)
              M.push(s.charCodeAt(r++));
            S[O++] = _ - this.decodeTableSeq.length, this.decodeTableSeq.push(M);
          } else
            S[O++] = C;
        }
      else if (typeof s == "number")
        for (var L = S[O - 1] + 1, r = 0; r < s; r++)
          S[O++] = L++;
      else
        throw new Error("Incorrect type '" + typeof s + "' given in " + this.encodingName + " at chunk " + u[0]);
    }
    if (O > 255)
      throw new Error("Incorrect chunk in " + this.encodingName + " at addr " + u[0] + ": too long" + O);
  }, i.prototype._getEncodeBucket = function(u) {
    var O = u >> 8;
    return this.encodeTable[O] === void 0 && (this.encodeTable[O] = T.slice(0)), this.encodeTable[O];
  }, i.prototype._setEncodeChar = function(u, O) {
    var S = this._getEncodeBucket(u), c = u & 255;
    S[c] <= _ ? this.encodeTableSeq[_ - S[c]][a] = O : S[c] == t && (S[c] = O);
  }, i.prototype._setEncodeSequence = function(u, O) {
    var S = u[0], c = this._getEncodeBucket(S), s = S & 255, r;
    c[s] <= _ ? r = this.encodeTableSeq[_ - c[s]] : (r = {}, c[s] !== t && (r[a] = c[s]), c[s] = _ - this.encodeTableSeq.length, this.encodeTableSeq.push(r));
    for (var C = 1; C < u.length - 1; C++) {
      var D = r[S];
      typeof D == "object" ? r = D : (r = r[S] = {}, D !== void 0 && (r[a] = D));
    }
    S = u[u.length - 1], r[S] = O;
  }, i.prototype._fillEncodeTable = function(u, O, S) {
    for (var c = this.decodeTables[u], s = !1, r = {}, C = 0; C < 256; C++) {
      var D = c[C], B = O + C;
      if (!S[B])
        if (D >= 0)
          this._setEncodeChar(D, B), s = !0;
        else if (D <= R) {
          var M = R - D;
          if (!r[M]) {
            var h = B << 8 >>> 0;
            this._fillEncodeTable(M, h, S) ? s = !0 : r[M] = !0;
          }
        } else D <= _ && (this._setEncodeSequence(this.decodeTableSeq[_ - D], B), s = !0);
    }
    return s;
  };
  function A(u, O) {
  ...
  A.prototype.write = function(u) {
    for (var O = E.alloc(u.length * (this.gb18030 ? 4 : 3)), S = this.leadSurrogate, c = this.seqObj, s = -1, r = 0, C = 0; ; ) {
      if (s === -1) {
        if (r == u.length) break;
        var D = u.charCodeAt(r++);
      } else {
        var D = s;
        s = -1;
      }
      if (D >= 55296 && D < 57344)
        if (D < 56320)
          ...
          S !== -1 ? (D = 65536 + (S - 55296) * 1024 + (D - 56320), S = -1) : D = t;
      else S !== -1 && (s = D, D = t, S = -1);
      var B = t;
      if (c !== void 0 && D != t) {
        var M = c[D];
        if (typeof M == "object") {
          c = M;
          continue;
        } else typeof M == "number" ? B = M : M == null && (M = c[a], M !== void 0 && (B = M, s = D));
        c = void 0;
      } else if (D >= 0) {
        var h = this.encodeTable[D >> 8];
        if (h !== void 0 && (B = h[D & 255]), B <= _) {
          c = this.encodeTableSeq[_ - B];
          continue;
        }
        if (B == t && this.gb18030) {
          var L = N(this.gb18030.uChars, D);
          if (L != -1) {
            var B = this.gb18030.gbChars[L] + (D - this.gb18030.uChars[L]);
            O[C++] = 129 + Math.floor(B / 12600), B = B % 12600, O[C++] = 48 + Math.floor(B / 1260), B = B % 1260, O[C++] = 129 + Math.floor(B / 10), B = B % 10, O[C++] = 48 + B;
            continue;
          }
        }
      }
      B === t && (B = this.defaultCharSingleByte), B < 256 ? O[C++] = B : B < 65536 ? (O[C++] = B >> 8, O[C++] = B & 255) : B < 16777216 ? (O[C++] = B >> 16, O[C++] = B >> 8 & 255, O[C++] = B & 255) : (O[C++] = B >>> 24, O[C++] = B >>> 16 & 255, O[C++] = B >>> 8 & 255, O[C++] = B & 255);
    }
    return this.seqObj = c, this.leadSurrogate = S, O.slice(0, C);
  }, A.prototype.end = function() {
    if (!(this.leadSurrogate === -1 && this.seqObj === void 0)) {
      var u = E.alloc(10), O = 0;
      if (this.seqObj) {
        var S = this.seqObj[a];
        S !== void 0 && (S < 256 ? u[O++] = S : (u[O++] = S >> 8, u[O++] = S & 255)), this.seqObj = void 0;
      }
      return this.leadSurrogate !== -1 && (u[O++] = this.defaultCharSingleByte, this.leadSurrogate = -1), u.slice(0, O);
    }
  }, A.prototype.findIdx = N;
  function I(u, O) {
  ...
  I.prototype.write = function(u) {
    for (var O = E.alloc(u.length * 2), S = this.nodeIdx, c = this.prevBytes, s = this.prevBytes.length, r = -this.prevBytes.length, C, D = 0, B = 0; D < u.length; D++) {
      var M = D >= 0 ? u[D] : c[D + s], C = this.decodeTables[S][M];
      if (!(C >= 0)) if (C === t)
        C = this.defaultCharUnicode.charCodeAt(0), D = r;
      else if (C === n) {
        if (D >= 3)
          var h = (u[D - 3] - 129) * 12600 + (u[D - 2] - 48) * 1260 + (u[D - 1] - 129) * 10 + (M - 48);
        else
          var h = (c[D - 3 + s] - 129) * 12600 + ((D - 2 >= 0 ? u[D - 2] : c[D - 2 + s]) - 48) * 1260 + ((D - 1 >= 0 ? u[D - 1] : c[D - 1 + s]) - 129) * 10 + (M - 48);
        var L = N(this.gb18030.gbChars, h);
        C = this.gb18030.uChars[L] + h - this.gb18030.gbChars[L];
      } else if (C <= R) {
        S = R - C;
        continue;
      } else if (C <= _) {
        for (var U = this.decodeTableSeq[_ - C], d = 0; d < U.length - 1; d++)
          C = U[d], O[B++] = C & 255, O[B++] = C >> 8;
        C = U[U.length - 1];
      } else
        throw new Error("iconv-lite internal error: invalid decoding table value " + C + " at " + S + "/" + M);
      if (C >= 65536) {
        C -= 65536;
        var F = 55296 | C >> 10;
        O[B++] = F & 255, O[B++] = F >> 8, C = 56320 | C & 1023;
      }
      O[B++] = C & 255, O[B++] = C >> 8, S = 0, r = D + 1;
    }
    return this.nodeIdx = S, this.prevBytes = r >= 0 ? Array.prototype.slice.call(u, r) : c.slice(r + s).concat(Array.prototype.slice.call(u)), O.slice(0, B).toString("ucs2");
  }, I.prototype.end = function() {
...
function _i() {
  ...
  Qe = 1;
  var E = rE().Buffer;
  return S_ = function(t) {
    var n = t.Transform;
    function _(T, a) {
      this.conv = T, a = a || {}, a.decodeStrings = !1, n.call(this, a);
    }
    _.prototype = Object.create(n.prototype, {
      constructor: { value: _ }
    }), _.prototype._transform = function(T, a, e) {
      if (typeof T != "string")
        return e(new Error("Iconv encoding stream needs strings as its input."));
      try {
        var i = this.conv.write(T);
        i && i.length && this.push(i), e();
      } catch (A) {
        e(A);
      }
      ...
      try {
        var a = this.conv.end();
        a && a.length && this.push(a), T();
      } catch (e) {
        T(e);
      }
    }, _.prototype.collect = function(T) {
      var a = [];
      return this.on("error", T), this.on("data", function(e) {
        a.push(e);
    ...
    function R(T, a) {
      this.conv = T, a = a || {}, a.encoding = this.encoding = "utf8", n.call(this, a);
    }
    return R.prototype = Object.create(n.prototype, {
      constructor: { value: R }
    }), R.prototype._transform = function(T, a, e) {
      if (!E.isBuffer(T) && !(T instanceof Uint8Array))
        return e(new Error("Iconv decoding stream needs buffers as its input."));
      try {
        var i = this.conv.write(T);
        i && i.length && this.push(i, this.encoding), e();
      } catch (A) {
        e(A);
      }
      ...
      try {
        var a = this.conv.end();
        a && a.length && this.push(a, this.encoding), T();
      } catch (e) {
        T(e);
      }
...
function ni() {
  return Pe || (Pe = 1, (function(E) {
    var t = rE().Buffer, n = mt(), _ = mR();
    E.exports.encodings = null, E.exports.defaultCharUnicode = "�", E.exports.defaultCharSingleByte = "?", E.exports.encode = function(a, e, i) {
      a = "" + (a || "");
      var A = E.exports.getEncoder(e, i), I = A.write(a), N = A.end();
      return N && N.length > 0 ? t.concat([I, N]) : I;
    }, E.exports.decode = function(a, e, i) {
      typeof a == "string" && (E.exports.skipDecodeWarning || (console.error("Iconv-lite warning: decode()-ing strings is deprecated. Refer to https://github.com/ashtuchkin/iconv-lite/wiki/Use-Buffers-when-decoding"), E.exports.skipDecodeWarning = !0), a = t.from("" + (a || ""), "binary"));
      var A = E.exports.getDecoder(e, i), I = A.write(a), N = A.end();
      return N ? I + N : I;
    }, E.exports.encodingExists = function(a) {
      try {
        return E.exports.getCodec(a), !0;
      } catch {
        return !1;
      }
    }, E.exports.toEncoding = E.exports.encode, E.exports.fromEncoding = E.exports.decode, E.exports._codecDataCache = { __proto__: null }, E.exports.getCodec = function(a) {
      if (!E.exports.encodings) {
        var e = Ei();
        E.exports.encodings = { __proto__: null }, _(E.exports.encodings, e);
      }
      for (var i = E.exports._canonicalizeEncoding(a), A = {}; ; ) {
        var I = E.exports._codecDataCache[i];
        if (I)
          return I;
        var N = E.exports.encodings[i];
        switch (typeof N) {
          case "string":
            i = N;
            break;
          case "object":
            for (var u in N)
              A[u] = N[u];
            A.encodingName || (A.encodingName = i), i = N.type;
            break;
          case "function":
            return A.encodingName || (A.encodingName = i), I = new N(A, E.exports), E.exports._codecDataCache[A.encodingName] = I, I;
          default:
            throw new Error("Encoding not recognized: '" + a + "' (searched as: '" + i + "')");
        }
      }
    }, E.exports._canonicalizeEncoding = function(T) {
      return ("" + T).toLowerCase().replace(/:\d{4}$|[^0-9a-z]/g, "");
    }, E.exports.getEncoder = function(a, e) {
      var i = E.exports.getCodec(a), A = new i.encoder(e, i);
      return i.bomAware && e && e.addBOM && (A = new n.PrependBOM(A, e)), A;
    }, E.exports.getDecoder = function(a, e) {
      var i = E.exports.getCodec(a), A = new i.decoder(e, i);
      return i.bomAware && !(e && e.stripBOM === !1) && (A = new n.StripBOM(A, e)), A;
    }, E.exports.enableStreamingAPI = function(a) {
      if (!E.exports.supportsStreams) {
        var e = _i()(a);
        E.exports.IconvLiteEncoderStream = e.IconvLiteEncoderStream, E.exports.IconvLiteDecoderStream = e.IconvLiteDecoderStream, E.exports.encodeStream = function(A, I) {
...
function qE() {
  if (Ve) return bE;
  Ve = 1;
  const E = ni(), { createLRU: t } = ZE(), n = t({
    max: 500
  });
  return bE.decode = function(_, R, T, a, e) {
    if (Buffer.isEncoding(R))
      return _.toString(R, T, a);
    let i;
    if (!e)
      i = n.get(R), i || (i = E.getDecoder(R), n.set(R, i));
    else {
      const N = { encoding: R, options: e }, u = JSON.stringify(N);
      i = n.get(u), i || (i = E.getDecoder(N.encoding, N.options), n.set(u, i));
    }
    const A = i.write(_.slice(T, a)), I = i.end();
    return I ? A + I : A;
  }, bE.encode = function(_, R, T) {
    if (Buffer.isEncoding(R))
      return Buffer.from(_, R);
    const a = E.getEncoder(R, T || {}), e = a.write(_), i = a.end();
    return i && i.length > 0 ? Buffer.concat([e, i]) : e;
  }, bE;
}
var b = { exports: {} }, be;
function TE() {
  return be || (be = 1, b.exports = {
    0: "DECIMAL",
    // aka DECIMAL
    1: "TINY",
    // aka TINYINT, 1 byte
...
function X() {
  if (me) return B_;
  me = 1;
  const E = bR(), t = FR.Buffer, n = bt(), _ = qE(), R = TE(), T = /* @__PURE__ */ new Date(NaN), a = "000000000000";
  function e(S, c) {
    const s = c.toString();
    return s.length >= S ? s : (a + s).slice(-S);
  }
  const i = 45, A = 43, I = 46, N = 101, u = 69;
  class O {
    constructor(c, s, r, C) {
      this.sequenceId = c, this.numPackets = 1, this.buffer = s, this.start = r, this.offset = r + 4, this.end = C;
    }
    // ==============================
    // readers
    // ==============================
    ...
    readInt64JSNumber() {
      const c = this.readInt32(), s = this.readInt32();
      return new n(c, s, !0).toNumber();
    }
    readSInt64JSNumber() {
      const c = this.readInt32(), s = this.readInt32();
      return s & 2147483648 ? new n(c, s, !1).toNumber() : c + 4294967296 * s;
    }
    readInt64String() {
      const c = this.readInt32(), s = this.readInt32();
      return new n(c, s, !0).toString();
    }
    readSInt64String() {
      const c = this.readInt32(), s = this.readInt32();
      return new n(c, s, !1).toString();
    }
    readInt64() {
      const c = this.readInt32(), s = this.readInt32();
      let r = new n(c, s, !0);
      const C = r.toNumber(), D = r.toString();
      return r = C.toString() === D ? C : D, r;
    }
    readSInt64() {
      const c = this.readInt32(), s = this.readInt32();
      let r = new n(c, s, !1);
    ...
    readLengthCodedNumber(c, s) {
      const r = this.buffer[this.offset++];
      return r < 251 ? r : this.readLengthCodedNumberExt(r, c, s);
    }
    readLengthCodedNumberSigned(c) {
      return this.readLengthCodedNumber(c, !0);
    }
    readLengthCodedNumberExt(c, s, r) {
      let C, D, B;
      if (c === 251)
        return null;
      if (c === 252)
      ...
      if (c === 254) {
        if (C = this.readInt32(), D = this.readInt32(), D === 0)
          return C;
        if (D < 2097152)
          return D * 4294967296 + C;
        B = new n(C, D, !r);
        const M = B.toNumber(), h = B.toString();
        return B = M.toString() === h ? M : h, s ? h : B;
      }
      throw console.trace(), new Error(`Should not reach here: ${c}`);
    }
    readFloat() {
      const c = this.buffer.readFloatLE(this.offset);
      return this.offset += 4, c;
    }
    readDouble() {
      const c = this.buffer.readDoubleLE(this.offset);
      return this.offset += 8, c;
    }
    readBuffer(c) {
      return typeof c > "u" && (c = this.end - this.offset), this.offset += c, this.buffer.slice(this.offset - c, this.offset);
    }
    ...
    readDateTime(c) {
      if (!c || c === "Z" || c === "local") {
        ...
        if (r === 251)
          return null;
        let C = 0, D = 0, B = 0, M = 0, h = 0, L = 0, U = 0;
        return r > 3 && (C = this.readInt16(), D = this.readInt8(), B = this.readInt8()), r > 6 && (M = this.readInt8(), h = this.readInt8(), L = this.readInt8()), r > 10 && (U = this.readInt32() / 1e3), C + D + B + M + h + L + U === 0 ? T : c === "Z" ? new Date(Date.UTC(C, D - 1, B, M, h, L, U)) : new Date(C, D - 1, B, M, h, L, U);
      }
      let s = this.readDateTimeString(6, "T", null);
      return s.length === 10 && (s += "T00:00:00"), new Date(s + c);
    }
    readDateTimeString(c, s, r) {
      const C = this.readInt8();
      let D = 0, B = 0, M = 0, h = 0, L = 0, U = 0, d = 0, F;
      return C > 3 && (D = this.readInt16(), B = this.readInt8(), M = this.readInt8(), F = [e(4, D), e(2, B), e(2, M)].join("-")), C > 6 ? (h = this.readInt8(), L = this.readInt8(), U = this.readInt8(), F += `${s || " "}${[
        e(2, h),
        e(2, L),
        e(2, U)
      ].join(":")}`) : r === R.DATETIME && (F += " 00:00:00"), C > 10 && (d = this.readInt32(), F += ".", c && (d = e(6, d), d.length > c && (d = d.substring(0, c))), F += d), F;
    }
    // TIME - value as a string, Can be negative
    readTimeString(c) {
      const s = this.readInt8();
      if (s === 0)
        return "00:00:00";
      const r = this.readInt8() ? -1 : 1;
      let C = 0, D = 0, B = 0, M = 0, h = 0;
      return s > 6 && (C = this.readInt32(), D = this.readInt8(), B = this.readInt8(), M = this.readInt8()), s > 10 && (h = this.readInt32()), c ? (D += C * 24, B += D * 60, M += B * 60, h += M * 1e3, h *= r, h) : (r === -1 ? "-" : "") + [e(2, C * 24 + D), e(2, B), e(2, M)].join(":") + (h ? `.${h}`.replace(/0+$/, "") : "");
    }
    readLengthCodedString(c) {
      const s = this.readLengthCodedNumber();
      return s === null ? null : (this.offset += s, _.decode(
        this.buffer,
        c,
        this.offset - s,
        this.offset
    ...
    readLengthCodedBuffer() {
      ...
      return c === null ? null : this.readBuffer(c);
    }
    readNullTerminatedString(c) {
      const s = this.offset;
      let r = this.offset;
      for (; this.buffer[r]; )
        r = r + 1;
      return this.offset = r + 1, _.decode(this.buffer, c, s, r);
    }
    // TODO reuse?
    readString(c, s) {
      return typeof c == "string" && typeof s > "u" && (s = c, c = void 0), typeof c > "u" && (c = this.end - this.offset), this.offset += c, _.decode(
        this.buffer,
        s,
        this.offset - c,
        this.offset
    ...
    parseInt(c, s) {
      if (c === null)
        return null;
      if (c >= 14 && !s) {
        const U = this.buffer.toString("ascii", this.offset, this.offset + c);
        return this.offset += c, Number(U);
      }
      let r = 0;
      const C = this.offset, D = this.offset + c;
      let B = 1;
      if (c === 0)
        return 0;
      this.buffer[this.offset] === i && (this.offset++, B = -1);
      let M;
      const h = D - this.offset;
      if (s) {
        if (h >= 15)
          return M = this.readString(D - this.offset, "binary"), r = parseInt(M, 10), r.toString() === M ? B * r : B === -1 ? `-${M}` : M;
        if (h > 16)
          return M = this.readString(D - this.offset), B === -1 ? `-${M}` : M;
      }
      for (this.buffer[this.offset] === A && this.offset++; this.offset < D; )
        r *= 10, r += this.buffer[this.offset] - 48, this.offset++;
      const L = r * B;
      return !s || (M = this.buffer.toString("ascii", C, D), L.toString() === M) ? L : M;
    }
    // note that if value of inputNumberAsString is bigger than MAX_SAFE_INTEGER
    // ( or smaller than MIN_SAFE_INTEGER ) the parseIntNoBigCheck result might be
    // different from what you would get from Number(inputNumberAsString)
    // String(parseIntNoBigCheck) <> String(Number(inputNumberAsString)) <> inputNumberAsString
    parseIntNoBigCheck(c) {
      if (c === null)
        return null;
      let s = 0;
      const r = this.offset + c;
      let C = 1;
      if (c === 0)
        return 0;
      for (this.buffer[this.offset] === i && (this.offset++, C = -1), this.buffer[this.offset] === A && this.offset++; this.offset < r; )
        s *= 10, s += this.buffer[this.offset] - 48, this.offset++;
      return s * C;
    }
    // copy-paste from https://github.com/mysqljs/mysql/blob/master/lib/protocol/Parser.js
    parseGeometryValue() {
      const c = this.readLengthCodedBuffer();
      let s = 4;
      if (c === null || !c.length)
        return null;
      function r() {
        let C, D, B, M, h, L, U = null;
        const d = c.readUInt8(s);
        s += 1;
        const F = d ? c.readUInt32LE(s) : c.readUInt32BE(s);
        switch (s += 4, F) {
          case 1:
            C = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, D = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, U = { x: C, y: D };
            break;
          case 2:
            for (h = d ? c.readUInt32LE(s) : c.readUInt32BE(s), s += 4, U = [], B = h; B > 0; B--)
              C = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, D = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, U.push({ x: C, y: D });
            break;
          case 3:
            const G = d ? c.readUInt32LE(s) : c.readUInt32BE(s);
            for (s += 4, U = [], B = G; B > 0; B--) {
              for (h = d ? c.readUInt32LE(s) : c.readUInt32BE(s), s += 4, L = [], M = h; M > 0; M--)
                C = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, D = d ? c.readDoubleLE(s) : c.readDoubleBE(s), s += 8, L.push({ x: C, y: D });
              U.push(L);
            }
            break;
          case 4:
          ...
          // WKBMultiPolygon
          case 7:
            const w = d ? c.readUInt32LE(s) : c.readUInt32BE(s);
            for (s += 4, U = [], B = w; B > 0; B--)
              U.push(r());
            break;
        }
    ...
    parseVector() {
      const c = this.readLengthCodedNumber(), s = this.offset + c, r = [];
      for (; this.offset < s && this.offset < this.end; )
    ...
    parseDate(c) {
      ...
      return !c || c === "local" ? new Date(r, C - 1, D) : c === "Z" ? new Date(Date.UTC(r, C - 1, D)) : /* @__PURE__ */ new Date(
        `${e(4, r)}-${e(2, C)}-${e(2, D)}T00:00:00${c}`
      );
    }
    parseDateTime(c) {
      const s = this.readLengthCodedString("binary");
      return s === null ? null : !c || c === "local" ? new Date(s) : /* @__PURE__ */ new Date(`${s}${c}`);
    }
    parseFloat(c) {
      if (c === null)
        return null;
      ...
      for (this.buffer[this.offset] === i && (this.offset++, C = -1), this.buffer[this.offset] === A && this.offset++; this.offset < r; )
        if (B = this.buffer[this.offset], B === I)
          ...
          this.offset++;
          const M = this.parseInt(r - this.offset);
          return s / C * Math.pow(10, M);
        } else
          s *= 10, s += this.buffer[this.offset] - 48, this.offset++, D && (C = C * 10);
      return s / C;
    }
    parseLengthCodedIntNoBigCheck() {
      return this.parseIntNoBigCheck(this.readLengthCodedNumber());
    }
    parseLengthCodedInt(c) {
      return this.parseInt(this.readLengthCodedNumber(), c);
    }
    ...
    asError(c) {
      ...
      const s = this.readInt16();
      let r = "";
      this.buffer[this.offset] === 35 && (this.skip(1), r = this.readBuffer(5).toString());
      const C = this.readString(void 0, c), D = new Error(C);
      return D.code = E[s], D.errno = s, D.sqlState = r, D.sqlMessage = C, D;
    }
    writeInt32(c) {
      this.buffer.writeUInt32LE(c, this.offset), this.offset += 4;
    }
    writeInt24(c) {
      this.writeInt8(c & 255), this.writeInt16(c >> 8);
    }
    writeInt16(c) {
      this.buffer.writeUInt16LE(c, this.offset), this.offset += 2;
    }
    writeInt8(c) {
      this.buffer.writeUInt8(c, this.offset), this.offset++;
    }
    writeDouble(c) {
      this.buffer.writeDoubleLE(c, this.offset), this.offset += 8;
    }
    writeBuffer(c) {
      c.copy(this.buffer, this.offset), this.offset += c.length;
    }
    writeNull() {
      this.buffer[this.offset] = 251, this.offset++;
    }
    // TODO: refactor following three?
    writeNullTerminatedString(c, s) {
      const r = _.encode(c, s);
    ...
    writeLengthCodedString(c, s) {
      const r = _.encode(c, s);
      this.writeLengthCodedNumber(r.length), this.buffer.length && r.copy(this.buffer, this.offset), this.offset += r.length;
    }
    writeLengthCodedBuffer(c) {
      this.writeLengthCodedNumber(c.length), c.copy(this.buffer, this.offset), this.offset += c.length;
    }
    writeLengthCodedNumber(c) {
      return c < 251 ? this.writeInt8(c) : c < 65535 ? (this.writeInt8(252), this.writeInt16(c)) : c < 16777215 ? (this.writeInt8(253), this.writeInt24(c)) : c === null ? this.writeInt8(251) : (this.writeInt8(254), this.buffer.writeUInt32LE(c, this.offset), this.offset += 4, this.buffer.writeUInt32LE(c >> 32, this.offset), this.offset += 4, this.offset);
    }
    writeDate(c, s) {
      if (this.buffer.writeUInt8(11, this.offset), !s || s === "local")
        this.buffer.writeUInt16LE(c.getFullYear(), this.offset + 1), this.buffer.writeUInt8(c.getMonth() + 1, this.offset + 3), this.buffer.writeUInt8(c.getDate(), this.offset + 4), this.buffer.writeUInt8(c.getHours(), this.offset + 5), this.buffer.writeUInt8(c.getMinutes(), this.offset + 6), this.buffer.writeUInt8(c.getSeconds(), this.offset + 7), this.buffer.writeUInt32LE(c.getMilliseconds() * 1e3, this.offset + 8);
      else {
        if (s !== "Z") {
          const r = (s[0] === "-" ? -1 : 1) * (parseInt(s.substring(1, 3), 10) * 60 + parseInt(s.substring(4), 10));
          r !== 0 && (c = new Date(c.getTime() + 6e4 * r));
        }
        this.buffer.writeUInt16LE(c.getUTCFullYear(), this.offset + 1), this.buffer.writeUInt8(c.getUTCMonth() + 1, this.offset + 3), this.buffer.writeUInt8(c.getUTCDate(), this.offset + 4), this.buffer.writeUInt8(c.getUTCHours(), this.offset + 5), this.buffer.writeUInt8(c.getUTCMinutes(), this.offset + 6), this.buffer.writeUInt8(c.getUTCSeconds(), this.offset + 7), this.buffer.writeUInt32LE(c.getUTCMilliseconds() * 1e3, this.offset + 8);
      }
      this.offset += 12;
    }
    writeHeader(c) {
      const s = this.offset;
      this.offset = 0, this.writeInt24(this.buffer.length - 4), this.writeInt8(c), this.offset = s;
    }
    clone() {
      return new O(this.sequenceId, this.buffer, this.start, this.end);
    }
    type() {
      return this.isEOF() ? "EOF" : this.isError() ? "Error" : this.buffer[this.offset] === 0 ? "maybeOK" : "";
    }
    static lengthCodedNumberLength(c) {
      return c < 251 ? 1 : c < 65535 ? 3 : c < 16777215 ? 5 : 9;
    }
    static lengthCodedStringLength(c, s) {
      const C = _.encode(c, s).length;
      return O.lengthCodedNumberLength(C) + C;
    }
    static MockBuffer() {
      const c = function() {
      }, s = Buffer.alloc(0);
      for (const r in t.prototype)
        typeof s[r] == "function" && (s[r] = c);
      return s;
    }
  }
  return B_ = O, B_;
...
function YR() {
  if (Ye) return l_;
  Ye = 1;
  const E = X(), t = 16777215;
  function n(R, T) {
    const a = R[T], e = R[T + 1], i = R[T + 2];
    return e + i === 0 ? a : a + (e << 8) + (i << 16);
  }
  class _ {
    constructor(T, a) {
      typeof a > "u" && (a = 4), this.buffer = [], this.bufferLength = 0, this.packetHeaderLength = a, this.headerLen = 0, this.length = 0, this.largePacketParts = [], this.firstPacketSequenceId = 0, this.onPacket = T, this.execute = _.prototype.executeStart, this._flushLargePacket = a === 7 ? this._flushLargePacket7 : this._flushLargePacket4;
    }
    _flushLargePacket4() {
      const T = this.largePacketParts.length;
      this.largePacketParts.unshift(Buffer.from([0, 0, 0, 0]));
      const a = Buffer.concat(this.largePacketParts), e = new E(this.firstPacketSequenceId, a, 0, a.length);
      this.largePacketParts.length = 0, e.numPackets = T, this.onPacket(e);
    }
    _flushLargePacket7() {
      const T = this.largePacketParts.length;
      this.largePacketParts.unshift(Buffer.from([0, 0, 0, 0, 0, 0, 0]));
      const a = Buffer.concat(this.largePacketParts);
      this.largePacketParts.length = 0;
      const e = new E(this.firstPacketSequenceId, a, 0, a.length);
      e.numPackets = T, this.onPacket(e);
    }
    executeStart(T) {
      let a = 0;
      const e = T.length;
      for (; e - a >= 3; )
        if (this.length = n(T, a), e - a >= this.length + this.packetHeaderLength) {
          const i = T[a + 3];
          this.length < t && this.largePacketParts.length === 0 ? this.onPacket(
            new E(
              i,
              T,
              a,
              a + this.packetHeaderLength + this.length
            )
          ) : (this.largePacketParts.length === 0 && (this.firstPacketSequenceId = i), this.largePacketParts.push(
            T.slice(
              a + this.packetHeaderLength,
              a + this.packetHeaderLength + this.length
            )
          ), this.length < t && this._flushLargePacket()), a += this.packetHeaderLength + this.length;
        } else {
          this.buffer = [T.slice(a + 3, e)], this.bufferLength = e - a - 3, this.execute = _.prototype.executePayload;
          return;
        }
      e - a > 0 && (this.headerLen = e - a, this.length = T[a], this.headerLen === 2 ? (this.length = T[a] + (T[a + 1] << 8), this.execute = _.prototype.executeHeader3) : this.execute = _.prototype.executeHeader2);
    }
    executePayload(T) {
      let a = 0;
      const e = T.length, i = this.length - this.bufferLength + this.packetHeaderLength - 3;
      if (e - a >= i) {
        const A = Buffer.allocUnsafe(this.length + this.packetHeaderLength);
        let I = 3;
        for (let u = 0; u < this.buffer.length; ++u)
          this.buffer[u].copy(A, I), I += this.buffer[u].length;
        T.copy(A, I, a, a + i);
        const N = A[3];
        if (this.length < t && this.largePacketParts.length === 0 ? this.onPacket(
          new E(
            N,
            A,
            0,
        ...
        ), this.length < t && this._flushLargePacket()), this.buffer = [], this.bufferLength = 0, this.execute = _.prototype.executeStart, a += i, e - a > 0)
          return this.execute(T.slice(a, e));
      } else
...
function ei() {
  ...
  class t {
    ...
    toPacket(_) {
      const R = 6 + this.pluginName.length + this.pluginData.length, T = Buffer.allocUnsafe(R), a = new E(0, T, 0, R);
    ...
    static fromPacket(_, R) {
      _.readInt8();
      const T = _.readNullTerminatedString(R), a = _.readBuffer();
      return new t({
        pluginName: T,
        pluginData: a
      });
...
function Ai() {
  ...
  class t {
    ...
    toPacket() {
      const _ = 6 + this.pluginName.length + this.pluginData.length, R = Buffer.allocUnsafe(_), T = new E(0, R, 0, _);
      return T.offset = 4, T.writeInt8(254), T.writeNullTerminatedString(this.pluginName, "cesu8"), T.writeBuffer(this.pluginData), T;
    }
    static fromPacket(_) {
      _.readInt8();
      const R = _.readNullTerminatedString("cesu8"), T = _.readBuffer();
      return new t({
        pluginName: R,
        pluginData: T
      });
...
function Ri() {
  ...
  class t {
    ...
    toPacket() {
      const _ = 5 + this.data.length, R = Buffer.allocUnsafe(_), T = new E(0, R, 0, _);
...
function ti() {
  ...
  class t {
    ...
    toPacket() {
      const _ = 4 + this.data.length, R = Buffer.allocUnsafe(_), T = new E(0, R, 0, _);
...
function ii() {
  if (Xe) return f_;
  Xe = 1;
  const E = TE(), t = X(), n = new Array(256);
  class _ {
    constructor(T) {
      this.columns = T || [];
    }
    static toPacket(T, a) {
      ...
      T.forEach((O) => {
        ...
        i += t.lengthCodedStringLength(O.toString(10), a);
      }), i = i + 2;
      const A = Buffer.allocUnsafe(i + 4), I = new t(0, A, 0, i + 4);
      I.offset = 4, I.writeInt8(0);
      let N = 0, u = 1;
      return T.forEach((O) => {
        O.type === E.NULL && (N += u), u *= 2, u === 256 && (I.writeInt8(N), N = 0, u = 1);
      }), u !== 1 && I.writeInt8(N), T.forEach((O) => {
        if (O === null) {
          I.writeNull();
          return;
        }
        ...
        I.writeLengthCodedString(O.toString(10), a);
      }), I;
    }
    ...
    static fromPacket(T, a) {
      ...
      for (let A = 0; A < e.length; ++A)
        e[A] = n[T[A].columnType].apply(a);
      return new _(e);
    }
  }
  return n[E.DECIMAL] = t.prototype.readLengthCodedString, n[1] = t.prototype.readInt8, n[2] = t.prototype.readInt16, n[3] = t.prototype.readInt32, n[4] = t.prototype.readFloat, n[5] = t.prototype.readDouble, n[6] = t.prototype.assertInvalid, n[7] = t.prototype.readTimestamp, n[8] = t.prototype.readInt64, n[9] = t.prototype.readInt32, n[10] = t.prototype.readTimestamp, n[11] = t.prototype.readTime, n[12] = t.prototype.readDateTime, n[13] = t.prototype.readInt16, n[E.VAR_STRING] = t.prototype.readLengthCodedString, f_ = _, f_;
}
var g_, Ke;
function iE() {
  return Ke || (Ke = 1, g_ = {
...
function Ii() {
  ...
  class n {
    ...
    toPacket() {
      const R = 15 + Buffer.byteLength(this.filename, "utf8"), T = Buffer.allocUnsafe(R), a = new E(0, T, 0, R);
      return a.offset = 4, a.writeInt8(t.BINLOG_DUMP), a.writeInt32(this.binlogPos), a.writeInt16(this.flags), a.writeInt32(this.serverId), a.writeString(this.filename), a;
    }
  }
  return U_ = n, U_;
}
...
function ME() {
  return Je || (Je = 1, (function(E) {
    const t = wE;
    function n(a, e, i) {
      const A = t.createHash("sha1");
      return A.update(a), e && A.update(e), i && A.update(i), A.digest();
    }
    function _(a, e) {
      const i = Buffer.allocUnsafe(a.length);
    ...
    function R(a, e, i) {
      ...
      const A = n(a);
      return E.calculateTokenFromPasswordSha(A, e, i);
    }
    E.calculateTokenFromPasswordSha = function(a, e, i) {
      const A = e.slice(0, 8), I = i.slice(0, 12), N = n(a), u = n(A, I, N);
      return _(u, a);
    }, E.calculateToken = R, E.verifyToken = function(a, e, i, A) {
      const I = _(i, n(a, e, A));
      return n(I).compare(A) === 0;
    }, E.doubleSha1 = function(a) {
      return n(n(a));
    };
    function T(a, e) {
      const i = Buffer.allocUnsafe(a.length), A = e.length;
  ...
  class T {
    constructor(e) {
      ...
      this.passwordSha1 ? i = _.calculateTokenFromPasswordSha(
        ...
        this.authPluginData1,
        this.authPluginData2
      ), this.authToken = i, this.charsetNumber = e.charsetNumber;
    }
    // TODO
    // ChangeUser.fromPacket = function(packet)
    // };
    serializeToBuffer(e) {
      const i = (N) => this.flags & t[N], A = new n(0, e, 0, e.length);
      A.offset = 4;
      const I = R[this.charsetNumber];
      if (A.writeInt8(E.CHANGE_USER), A.writeNullTerminatedString(this.user, I), i("SECURE_CONNECTION") ? (A.writeInt8(this.authToken.length), A.writeBuffer(this.authToken)) : (A.writeBuffer(this.authToken), A.writeInt8(0)), A.writeNullTerminatedString(this.database, I), A.writeInt16(this.charsetNumber), i("PLUGIN_AUTH") && A.writeNullTerminatedString("mysql_native_password", "latin1"), i("CONNECT_ATTRS")) {
        const N = this.connectAttributes, u = Object.keys(N);
        let O = 0;
        for (let S = 0; S < u.length; ++S)
          O += n.lengthCodedStringLength(u[S], I), O += n.lengthCodedStringLength(
            N[u[S]],
            I
          );
        A.writeLengthCodedNumber(O);
        for (let S = 0; S < u.length; ++S)
          A.writeLengthCodedString(u[S], I), A.writeLengthCodedString(
            N[u[S]],
            I
          );
      }
    ...
    toPacket() {
      ...
      const e = this.serializeToBuffer(n.MockBuffer());
...
function Ti() {
  ...
  class n {
    ...
    toPacket() {
      ...
      return R.offset = 4, R.writeInt8(t.STMT_CLOSE), R.writeInt32(this.id), R;
    }
  }
  return F_ = n, F_;
}
var Z = {}, pe;
function xn() {
  return pe || (pe = 1, Z.NOT_NULL = 1, Z.PRI_KEY = 2, Z.UNIQUE_KEY = 4, Z.MULTIPLE_KEY = 8, Z.BLOB = 16, Z.UNSIGNED = 32, Z.ZEROFILL = 64, Z.BINARY = 128, Z.ENUM = 256, Z.AUTO_INCREMENT = 512, Z.TIMESTAMP = 1024, Z.SET = 2048, Z.NO_DEFAULT_VALUE = 4096, Z.ON_UPDATE_NOW = 8192, Z.NUM = 32768), Z;
}
var Q_, xe;
function ai() {
  if (xe) return Q_;
  xe = 1;
  const E = X(), t = qE(), n = IE(), _ = ["catalog", "schema", "table", "orgTable", "name", "orgName"];
  class R {
    constructor(e, i) {
      this._buf = e.buffer, this._clientEncoding = i, this._catalogLength = e.readLengthCodedNumber(), this._catalogStart = e.offset, e.offset += this._catalogLength, this._schemaLength = e.readLengthCodedNumber(), this._schemaStart = e.offset, e.offset += this._schemaLength, this._tableLength = e.readLengthCodedNumber(), this._tableStart = e.offset, e.offset += this._tableLength, this._orgTableLength = e.readLengthCodedNumber(), this._orgTableStart = e.offset, e.offset += this._orgTableLength;
      const A = e.readLengthCodedNumber(), I = e.offset;
      e.offset += A, this._orgNameLength = e.readLengthCodedNumber(), this._orgNameStart = e.offset, e.offset += this._orgNameLength, e.skip(1), this.characterSet = e.readInt16(), this.encoding = n[this.characterSet], this.name = t.decode(
        this._buf,
        this.encoding === "binary" ? this._clientEncoding : this.encoding,
        I,
        I + A
      ), this.columnLength = e.readInt32(), this.columnType = e.readInt8(), this.type = this.columnType, this.flags = e.readInt16(), this.decimals = e.readInt8();
    }
    inspect() {
      return {
    ...
    [Symbol.for("nodejs.util.inspect.custom")](e, i, A) {
      const I = TE(), N = [];
      for (const r in I)
        N[I[r]] = r;
      const u = xn(), O = [], S = this.flags;
      for (const r in u)
        S & u[r] && (r === "PRI_KEY" ? O.push("PRIMARY KEY") : r === "NOT_NULL" ? O.push("NOT NULL") : r === "BINARY" || r === "MULTIPLE_KEY" || r === "NO_DEFAULT_VALUE" || r === "BLOB" || r === "UNSIGNED" || r === "TIMESTAMP" || (r === "ON_UPDATE_NOW" ? O.push("ON UPDATE CURRENT_TIMESTAMP") : O.push(r)));
      if (e > 1)
        return A({
          ...this.inspect(),
          typeName: N[this.columnType],
          flags: O
        });
      const c = this.flags & u.UNSIGNED;
      let s = N[this.columnType];
      return s === "BLOB" ? this.columnLength === 4294967295 ? s = "LONGTEXT" : this.columnLength === 67108860 ? s = "MEDIUMTEXT" : this.columnLength === 262140 ? s = "TEXT" : this.columnLength === 1020 ? s = "TINYTEXT" : s = `BLOB(${this.columnLength})` : s === "VAR_STRING" ? s = `VARCHAR(${Math.ceil(this.columnLength / 4)})` : s === "TINY" ? this.columnLength === 3 && c || this.columnLength === 4 && !c ? s = "TINYINT" : s = `TINYINT(${this.columnLength})` : s === "LONGLONG" ? this.columnLength === 20 ? s = "BIGINT" : s = `BIGINT(${this.columnLength})` : s === "SHORT" ? c && this.columnLength === 5 || !c && this.columnLength === 6 ? s = "SMALLINT" : s = `SMALLINT(${this.columnLength})` : s === "LONG" ? c && this.columnLength === 10 || !c && this.columnLength === 11 ? s = "INT" : s = `INT(${this.columnLength})` : s === "INT24" ? c && this.columnLength === 8 || !c && this.columnLength === 9 ? s = "MEDIUMINT" : s = `MEDIUMINT(${this.columnLength})` : s === "DOUBLE" ? this.columnLength === 22 && this.decimals === 31 ? s = "DOUBLE" : s = `DOUBLE(${this.columnLength},${this.decimals})` : s === "FLOAT" ? this.columnLength === 12 && this.decimals === 31 ? s = "FLOAT" : s = `FLOAT(${this.columnLength},${this.decimals})` : s === "NEWDECIMAL" ? this.columnLength === 11 && this.decimals === 0 ? s = "DECIMAL" : this.decimals === 0 ? c ? s = `DECIMAL(${this.columnLength})` : s = `DECIMAL(${this.columnLength - 1})` : s = `DECIMAL(${this.columnLength - 2},${this.decimals})` : s = `${N[this.columnType]}(${this.columnLength})`, c && (s += " UNSIGNED"), `\`${this.name}\` ${[s, ...O].join(" ")}`;
    }
    static toPacket(e, i) {
      let A = 17;
      _.forEach((O) => {
      ...
      const I = Buffer.allocUnsafe(A), N = new E(i, I, 0, A);
      function u(O) {
        N.writeLengthCodedString(
          e[O],
          n[e.characterSet]
        );
      }
      return N.offset = 4, _.forEach(u), N.writeInt8(12), N.writeInt16(e.characterSet), N.writeInt32(e.columnLength), N.writeInt8(e.columnType), N.writeInt16(e.flags), N.writeInt8(e.decimals), N.writeInt16(0), N;
    }
  ...
  const T = function(a) {
    Object.defineProperty(R.prototype, a, {
      get: function() {
        const e = this[`_${a}Start`], i = e + this[`_${a}Length`], A = t.decode(
          this._buf,
          this.encoding === "binary" ? this._clientEncoding : this.encoding,
          e,
          i
  ...
  return T("catalog"), T("schema"), T("table"), T("orgTable"), T("orgName"), Q_ = R, Q_;
}
var P_, $e;
function ri() {
  return $e || ($e = 1, P_ = {
  ...
  function a(i, A, I) {
    ...
    if (i !== null)
      switch (typeof i) {
        ...
        case "number":
          N = n.DOUBLE, u = 8, O = _.prototype.writeDouble;
          break;
        case "boolean":
          i = i | 0, N = n.TINY, u = 1, O = _.prototype.writeInt8;
          break;
        case "object":
          Object.prototype.toString.call(i) === "[object Date]" ? (N = n.DATETIME, u = 12, O = function(S) {
            return _.prototype.writeDate.call(this, S, I);
          }) : T(i) ? (i = JSON.stringify(i), N = n.JSON) : Buffer.isBuffer(i) && (u = _.lengthCodedNumberLength(i.length) + i.length, O = _.prototype.writeLengthCodedBuffer);
          break;
        default:
          i = i.toString();
      }
      ...
      i = "", N = n.NULL;
    return u || (u = _.lengthCodedStringLength(i, A)), { value: i, type: N, length: u, writer: O };
  }
  class e {
    constructor(A, I, N, u) {
      this.id = A, this.parameters = I, this.encoding = R[N], this.timezone = u;
    }
    static fromPacket(A, I) {
      const N = A.readInt32(), u = A.readInt8(), O = A.readInt32();
      let S = A.offset;
      for (; S < A.end - 1 && !((A.buffer[S + 1] === n.VAR_STRING || A.buffer[S + 1] === n.NULL || A.buffer[S + 1] === n.DOUBLE || A.buffer[S + 1] === n.TINY || A.buffer[S + 1] === n.DATETIME || A.buffer[S + 1] === n.JSON) && A.buffer[S] === 1 && A.buffer[S + 2] === 0); )
        A.readInt8(), S++;
      const c = [];
      for (let r = A.offset + 1; r < A.end - 1; r++)
        (A.buffer[r] === n.VAR_STRING || A.buffer[r] === n.NULL || A.buffer[r] === n.DOUBLE || A.buffer[r] === n.TINY || A.buffer[r] === n.DATETIME || A.buffer[r] === n.JSON) && A.buffer[r + 1] === 0 && (c.push(A.buffer[r]), A.skip(2));
      A.skip(1);
      const s = [];
      for (let r = 0; r < c.length; r++)
        c[r] === n.VAR_STRING ? s.push(A.readLengthCodedString(I)) : c[r] === n.DOUBLE ? s.push(A.readDouble()) : c[r] === n.TINY ? s.push(A.readInt8()) : c[r] === n.DATETIME ? s.push(A.readDateTime()) : c[r] === n.JSON && s.push(JSON.parse(A.readLengthCodedString(I))), c[r] === n.NULL && s.push(null);
      return { stmtId: N, flags: u, iterationCount: O, values: s };
    }
    toPacket() {
      let A = 14, I;
      this.parameters && this.parameters.length > 0 && (A += Math.floor((this.parameters.length + 7) / 8), A += 1, A += 2 * this.parameters.length, I = this.parameters.map(
        (O) => a(O, this.encoding, this.timezone)
      ), A += I.reduce(
        (O, S) => O + S.length,
        0
      ));
      const N = Buffer.allocUnsafe(A), u = new _(0, N, 0, A);
      if (u.offset = 4, u.writeInt8(t.STMT_EXECUTE), u.writeInt32(this.id), u.writeInt8(E.NO_CURSOR), u.writeInt32(1), I) {
        let O = 0, S = 1;
        I.forEach((c) => {
          c.type === n.NULL && (O += S), S *= 2, S === 256 && (u.writeInt8(O), O = 0, S = 1);
        }), S !== 1 && u.writeInt8(O), u.writeInt8(1), I.forEach((c) => {
          u.writeInt8(c.type), u.writeInt8(0);
        }), I.forEach((c) => {
          c.type !== n.NULL && c.writer.call(u, c.value);
        });
      }
      return u;
    }
...
function ci() {
  ...
  class n {
    ...
    setScrambleData(R) {
      wE.randomBytes(20, (T, a) => {
        if (T) {
          R(T);
          return;
        }
        this.authPluginData1 = a.slice(0, 8), this.authPluginData2 = a.slice(8, 20), R();
      });
    }
    toPacket(R) {
      const T = 68 + Buffer.byteLength(this.serverVersion, "utf8"), a = Buffer.alloc(T + 4, 0), e = new E(R, a, 0, T + 4);
      e.offset = 4, e.writeInt8(this.protocolVersion), e.writeString(this.serverVersion, "cesu8"), e.writeInt8(0), e.writeInt32(this.connectionId), e.writeBuffer(this.authPluginData1), e.writeInt8(0);
      const i = Buffer.allocUnsafe(4);
      return i.writeUInt32LE(this.capabilityFlags, 0), e.writeBuffer(i.slice(0, 2)), e.writeInt8(this.characterSet), e.writeInt16(this.statusFlags), e.writeBuffer(i.slice(2, 4)), e.writeInt8(21), e.skip(10), e.writeBuffer(this.authPluginData2), e.writeInt8(0), e.writeString("mysql_native_password", "latin1"), e.writeInt8(0), e;
    }
    static fromPacket(R) {
      const T = {};
      T.protocolVersion = R.readInt8(), T.serverVersion = R.readNullTerminatedString("cesu8"), T.connectionId = R.readInt32(), T.authPluginData1 = R.readBuffer(8), R.skip(1);
      const a = Buffer.allocUnsafe(4);
      if (a[0] = R.readInt8(), a[1] = R.readInt8(), R.haveMoreData() ? (T.characterSet = R.readInt8(), T.statusFlags = R.readInt16(), a[2] = R.readInt8(), a[3] = R.readInt8(), T.capabilityFlags = a.readUInt32LE(0), T.capabilityFlags & t.PLUGIN_AUTH ? T.authPluginDataLength = R.readInt8() : (T.authPluginDataLength = 0, R.skip(1)), R.skip(10)) : T.capabilityFlags = a.readUInt16LE(0), T.capabilityFlags & t.SECURE_CONNECTION) {
        const i = T.authPluginDataLength;
        if (i === 0)
          T.authPluginDataLength = 20, T.authPluginData2 = R.readBuffer(12), R.skip(1);
        else {
          const A = Math.max(13, i - 8);
          T.authPluginData2 = R.readBuffer(A);
        }
      }
      return T.capabilityFlags & t.PLUGIN_AUTH && (T.authPluginName = R.readNullTerminatedString("ascii")), new n(T);
    }
  }
  return b_ = n, b_;
}
...
function Oi() {
  if (nA) return m_;
  nA = 1;
  const E = uE(), t = IE(), n = X(), _ = ME();
  class R {
    constructor(a) {
      this.user = a.user || "", this.database = a.database || "", this.password = a.password || "", this.passwordSha1 = a.passwordSha1, this.authPluginData1 = a.authPluginData1, this.authPluginData2 = a.authPluginData2, this.compress = a.compress, this.clientFlags = a.flags;
      let e;
      this.passwordSha1 ? e = _.calculateTokenFromPasswordSha(
        this.passwordSha1,
        this.authPluginData1,
        this.authPluginData2
      ) : e = _.calculateToken(
        ...
        this.authPluginData1,
        this.authPluginData2
      ), this.authToken = e, this.charsetNumber = a.charsetNumber, this.encoding = t[a.charsetNumber], this.connectAttributes = a.connectAttributes;
    }
    serializeResponse(a) {
      const e = (N) => this.clientFlags & E[N], i = new n(0, a, 0, a.length);
      i.offset = 4, i.writeInt32(this.clientFlags), i.writeInt32(0), i.writeInt8(this.charsetNumber), i.skip(23);
      const A = this.encoding;
      i.writeNullTerminatedString(this.user, A);
      let I;
      if (e("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (i.writeLengthCodedNumber(this.authToken.length), i.writeBuffer(this.authToken)) : e("SECURE_CONNECTION") ? (i.writeInt8(this.authToken.length), i.writeBuffer(this.authToken)) : (i.writeBuffer(this.authToken), i.writeInt8(0)), e("CONNECT_WITH_DB") && i.writeNullTerminatedString(this.database, A), e("PLUGIN_AUTH") && i.writeNullTerminatedString("mysql_native_password", "latin1"), e("CONNECT_ATTRS")) {
        const N = this.connectAttributes || {}, u = Object.keys(N);
        let O = 0;
        for (I = 0; I < u.length; ++I)
          O += n.lengthCodedStringLength(u[I], A), O += n.lengthCodedStringLength(
            N[u[I]],
            A
          );
        for (i.writeLengthCodedNumber(O), I = 0; I < u.length; ++I)
          i.writeLengthCodedString(u[I], A), i.writeLengthCodedString(
            N[u[I]],
            A
          );
      }
    ...
    toPacket() {
      ...
      if (typeof this.database != "string")
        throw new Error('"database" connection config property must be a string');
      const a = this.serializeResponse(n.MockBuffer());
      return this.serializeResponse(Buffer.alloc(a.offset));
    }
    static fromPacket(a) {
      const e = {};
      e.clientFlags = a.readInt32();
      function i(N) {
        return e.clientFlags & E[N];
      }
      e.maxPacketSize = a.readInt32(), e.charsetNumber = a.readInt8();
      const A = t[e.charsetNumber];
      e.encoding = A, a.skip(23), e.user = a.readNullTerminatedString(A);
      let I;
      if (i("PLUGIN_AUTH_LENENC_CLIENT_DATA") ? (I = a.readLengthCodedNumber(A), e.authToken = a.readBuffer(I)) : i("SECURE_CONNECTION") ? (I = a.readInt8(), e.authToken = a.readBuffer(I)) : e.authToken = a.readNullTerminatedString(A), i("CONNECT_WITH_DB") && (e.database = a.readNullTerminatedString(A)), i("PLUGIN_AUTH") && (e.authPluginName = a.readNullTerminatedString(A)), i("CONNECT_ATTRS")) {
        const N = a.readLengthCodedNumber(A), u = a.offset + N, O = {};
        for (; a.offset < u; )
          O[a.readLengthCodedString(A)] = a.readLengthCodedString(A);
        e.connectAttributes = O;
      }
      return e;
    }
  }
...
function si() {
  if (eA) return Y_;
  eA = 1;
  const E = X(), t = iE(), n = qE(), _ = IE();
  class R {
    constructor(a, e) {
      this.query = a, this.charsetNumber = e, this.encoding = _[e];
    }
    toPacket() {
      const a = n.encode(this.query, this.encoding), e = 5 + a.length, i = Buffer.allocUnsafe(e), A = new E(0, i, 0, e);
      return A.offset = 4, A.writeInt8(t.STMT_PREPARE), A.writeBuffer(a), A;
    }
  }
  return Y_ = R, Y_;
}
...
function Di() {
  ...
  class R {
    ...
    toPacket() {
      const a = n.encode(this.query, this.encoding), e = 5 + a.length, i = Buffer.allocUnsafe(e), A = new E(0, i, 0, e);
      return A.offset = 4, A.writeInt8(t.QUERY), A.writeBuffer(a), A;
    }
  }
  return H_ = R, H_;
}
...
function Si() {
  ...
  class n {
    ...
    toPacket() {
      ...
      Buffer.byteLength(this.slaveHostname, "utf8") + Buffer.byteLength(this.slaveUser, "utf8") + Buffer.byteLength(this.slavePassword, "utf8") + 3 + 4, T = Buffer.allocUnsafe(R), a = new E(0, T, 0, R);
      return a.offset = 4, a.writeInt8(t.REGISTER_SLAVE), a.writeInt32(this.serverId), a.writeInt8(Buffer.byteLength(this.slaveHostname, "utf8")), a.writeString(this.slaveHostname), a.writeInt8(Buffer.byteLength(this.slaveUser, "utf8")), a.writeString(this.slaveUser), a.writeInt8(Buffer.byteLength(this.slavePassword, "utf8")), a.writeString(this.slavePassword), a.writeInt16(this.slavePort), a.writeInt32(this.replicationRank), a.writeInt32(this.masterId), a;
    }
  }
  return v_ = n, v_;
}
...
function Mi() {
  if (TA) return K_;
  TA = 1;
  const E = X(), t = uE(), n = WR(), _ = Bi(), R = li();
  class T {
    constructor(e, i) {
      const A = i.config.bigNumberStrings, I = i.serverEncoding, N = i._handshakePacket.capabilityFlags, u = function(c) {
        return N & t[c];
      };
      if (e.buffer[e.offset] !== 0) {
        this.fieldCount = e.readLengthCodedNumber(), this.fieldCount === null && (this.infileName = e.readString(void 0, I));
        return;
      }
      this.fieldCount = e.readInt8(), this.affectedRows = e.readLengthCodedNumber(A), this.insertId = e.readLengthCodedNumberSigned(A), this.info = "", u("PROTOCOL_41") ? (this.serverStatus = e.readInt16(), this.warningStatus = e.readInt16()) : u("TRANSACTIONS") && (this.serverStatus = e.readInt16());
      let O = null;
      if (u("SESSION_TRACK") && e.offset < e.end) {
        if (this.info = e.readLengthCodedString(I), this.serverStatus && n.SERVER_SESSION_STATE_CHANGED) {
          let c = e.offset < e.end ? e.readLengthCodedNumber() : 0;
          const s = e.offset + c;
          let r, C, D;
          for (c > 0 && (O = {
            systemVariables: {},
            ...
            if (r = e.readInt8(), c = e.readLengthCodedNumber(), D = e.offset + c, r === R.SYSTEM_VARIABLES) {
              C = e.readLengthCodedString(I);
              const B = e.readLengthCodedString(I);
              if (O.systemVariables[C] = B, C === "character_set_client") {
                const M = _[B];
                typeof M < "u" && (i.config.charsetNumber = M);
              }
            } else if (r === R.SCHEMA)
              C = e.readLengthCodedString(I), O.schema = C;
            else if (r === R.STATE_CHANGE)
              O.trackStateChange = e.readLengthCodedString(I);
            else if (r === R.STATE_GTIDS) {
              e.readLengthCodedString(I);
              const B = e.readLengthCodedString(I);
              O.gtids = B.split(",");
            }
            e.offset = D;
        ...
        this.info = e.readString(void 0, I);
      O && (this.stateChanges = O);
      const S = this.info.match(/\schanged:\s*(\d+)/i);
      S !== null ? this.changedRows = parseInt(S[1], 10) : this.changedRows = 0;
    }
    // TODO: should be consistent instance member, but it's just easier here to have just function
    static toPacket(e, i) {
      let A = 4 + E.lengthCodedNumberLength(e);
      typeof i < "u" && (A += E.lengthCodedNumberLength(i));
      const I = Buffer.allocUnsafe(A), N = new E(0, I, 0, A);
      return N.offset = 4, N.writeLengthCodedNumber(e), typeof i < "u" && N.writeLengthCodedNumber(i), N;
    }
  }
  return K_ = T, K_;
}
...
function hi() {
  ...
  class n {
    constructor(R, T) {
      this.clientFlags = R | E.SSL, this.charset = T;
    }
    toPacket() {
      const T = Buffer.allocUnsafe(36), a = new t(0, T, 0, 36);
      return T.fill(0), a.offset = 4, a.writeInt32(this.clientFlags), a.writeInt32(0), a.writeInt8(this.charset), a;
    }
  }
  return j_ = n, j_;
}
  ...
  class t {
    ...
    static toPacket(_, R) {
      ...
      _.forEach((A) => {
        ...
        a += E.lengthCodedStringLength(A.toString(10), R);
      });
      const e = Buffer.allocUnsafe(a + 4), i = new E(0, e, 0, a + 4);
      return i.offset = 4, _.forEach((A) => {
        if (A === null) {
          i.writeNull();
          return;
        }
        ...
        i.writeLengthCodedString(A.toString(10), R);
      }), i;
    }
  }
  return k_ = t, k_;
...
function eE() {
  return uA || (uA = 1, (function(E, t) {
    const n = zE, _ = ei(), R = Ai(), T = Ri(), a = ti(), e = ii(), i = Ii(), A = Ni(), I = Ti(), N = ai(), u = ui(), O = ci(), S = Oi(), c = si(), s = Ci(), r = Di(), C = Si(), D = Mi(), B = hi(), M = Li();
    Object.entries({
      AuthNextFactor: _,
      AuthSwitchRequest: R,
      AuthSwitchRequestMoreData: T,
      AuthSwitchResponse: a,
      BinaryRow: e,
      BinlogDump: i,
      ChangeUser: A,
      CloseStatement: I,
      ...
      if (E.exports[G] = w, n.env.NODE_DEBUG && w.prototype.toPacket) {
        const o = w.prototype.toPacket;
    ...
    class U {
      static toPacket(w, o) {
        w = w || {};
        const f = w.affectedRows || 0, l = w.insertId || 0, g = w.serverStatus || 0, P = w.warningCount || 0, Y = w.message || "";
        let H = 9 + L.lengthCodedNumberLength(f);
        H += L.lengthCodedNumberLength(l);
        const K = Buffer.allocUnsafe(H), y = new L(0, K, 0, H);
        return y.offset = 4, y.writeInt8(0), y.writeLengthCodedNumber(f), y.writeLengthCodedNumber(l), y.writeInt16(g), y.writeInt16(P), y.writeString(Y, o), y._name = "OK", y;
      }
    }
    t.OK = U;
    class d {
      static toPacket(w, o) {
        typeof w > "u" && (w = 0), typeof o > "u" && (o = 0);
        const f = new L(0, Buffer.allocUnsafe(9), 0, 9);
        return f.offset = 4, f.writeInt8(254), f.writeInt16(w), f.writeInt16(o), f._name = "EOF", f;
      }
    }
    t.EOF = d;
    class F {
      static toPacket(w, o) {
        const f = 13 + Buffer.byteLength(w.message, "utf8"), l = new L(0, Buffer.allocUnsafe(f), 0, f);
        return l.offset = 4, l.writeInt8(255), l.writeInt16(w.code), l.writeString("#_____", o), l.writeString(w.message, o), l._name = "Error", l;
      }
      static fromPacket(w) {
        w.readInt8();
        const o = w.readInt16();
        w.readString(1, "ascii"), w.readString(5, "ascii");
        const f = w.readNullTerminatedString("utf8"), l = new F();
        return l.message = f, l.code = o, l;
      }
    }
    t.Error = F;
...
function AE() {
  ...
  class n extends E {
    ...
    execute(R, T) {
      if (this.next || (this.next = this.start, T._resetSequenceId()), R && R.isError()) {
        const a = R.asError(T.clientEncoding);
        return a.sql = this.sql || this.query, this.queryTimeout && (t.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult ? (this.onResult(a), this.emit("end")) : (this.emit("error", a), this.emit("end")), !0;
      }
      return this.next = this.next(R, T), this.next ? !1 : (this.emit("end"), !0);
    }
  }
  return J_ = n, J_;
}
...
function oi() {
  if (OA) return z_;
  OA = 1;
  const E = "sha256_password", t = wE, { xorRotating: n } = ME(), _ = wR, R = Buffer.from([1]), T = 0, a = 1, e = -1;
  function i(A, I, N) {
    const u = n(Buffer.from(`${A}\0`, "utf8"), I);
    return t.publicEncrypt(N, u);
  }
  return z_ = (A = {}) => ({ connection: I }) => {
    let N = 0, u = null;
    const O = I.config.password, S = (c) => {
      const s = i(O, u, c);
      return N = e, s;
    };
    return (c) => {
      switch (N) {
        case T:
          return I.stream instanceof _.TLSSocket && I.stream.encrypted === !0 ? Buffer.from(`${O}\0`, "utf8") : (u = c.slice(0, 20), A.serverPublicKey ? S(A.serverPublicKey) : (N = a, R));
        case a:
          return A.onServerPublicKey && A.onServerPublicKey(c), S(c);
        case e:
          throw new Error(
...
function fi() {
  if (sA) return Z_;
  sA = 1;
  const E = "caching_sha2_password", t = wE, { xor: n, xorRotating: _ } = ME(), R = Buffer.from([2]), T = Buffer.from([3]), a = Buffer.from([4]), e = 0, i = 1, A = 2, I = -1;
  function N(S) {
    const c = t.createHash("sha256");
    return c.update(S), c.digest();
  }
  function u(S, c) {
    if (!S)
      return Buffer.alloc(0);
    const s = N(Buffer.from(S)), r = N(s), C = N(Buffer.concat([r, c]));
    return n(s, C);
  }
  function O(S, c, s) {
    const r = _(Buffer.from(`${S}\0`, "utf8"), c);
    return t.publicEncrypt(
      {
        key: s,
        padding: t.constants.RSA_PKCS1_OAEP_PADDING
      },
      r
    );
  }
  return Z_ = (S = {}) => ({ connection: c }) => {
    let s = 0, r = null;
    const C = c.config.password, D = (B) => {
      const M = O(C, r, B);
      return s = I, M;
    };
    return (B) => {
      switch (s) {
        case e:
          return r = B.slice(0, 20), s = i, u(C, r);
        case i:
          if (T.equals(B))
            return s = I, null;
          if (a.equals(B))
            return (typeof S.overrideIsSecure > "u" ? c.config.ssl || c.config.socketPath : S.overrideIsSecure) ? (s = I, Buffer.from(`${C}\0`, "utf8")) : S.serverPublicKey ? D(S.serverPublicKey) : (s = A, R);
          throw new Error(
            `Invalid AuthMoreData packet received by ${E} plugin in STATE_TOKEN_SENT state.`
          );
        case A:
          return S.onServerPublicKey && S.onServerPublicKey(B), D(B);
        case I:
          throw new Error(
            `Unexpected data in AuthMoreData packet received by ${E} plugin in STATE_FINAL state.`
          );
      ...
      throw new Error(
        `Unexpected data in AuthMoreData packet received by ${E} plugin in state ${s}`
      );
    };
  }, Z_;
}
...
function gi() {
  ...
  CA = 1;
  const E = ME();
  return q_ = (t) => ({ connection: n, command: _ }) => {
    const R = _.password || t.password || n.config.password, T = _.passwordSha1 || t.passwordSha1 || n.config.passwordSha1;
    return (a) => {
      const e = a.slice(0, 8), i = a.slice(8, 20);
      let A;
      return T ? A = E.calculateTokenFromPasswordSha(
        T,
        e,
        i
      ) : A = E.calculateToken(
...
function Ui() {
  ...
  return p_ = (n) => function({ connection: R, command: T }) {
    const a = T.password || n.password || R.config.password;
    return function() {
      return E(a);
    };
  }, p_;
...
function di() {
  if (SA) return x_;
  SA = 1;
  const E = eE(), t = oi(), n = fi(), _ = gi(), R = Ui(), T = {
    sha256_password: t({}),
  ...
  function e(I, N) {
    I.code = "AUTH_SWITCH_PLUGIN_ERROR", I.fatal = !0, N.emit("error", I);
  }
  function i(I, N, u) {
    const { pluginName: O, pluginData: S } = E.AuthSwitchRequest.fromPacket(I);
    let c = N.config.authPlugins && N.config.authPlugins[O];
    if (N.config.authSwitchHandler && O !== "mysql_native_password") {
      const s = N.config.authSwitchHandler;
      a(), s({ pluginName: O, pluginData: S }, (r, C) => {
        if (r)
          return e(r, u);
        N.writePacket(new E.AuthSwitchResponse(C).toPacket());
      });
      return;
    }
    if (c || (c = T[O]), !c)
      throw new Error(
        `Server requests authentication using unknown plugin ${O}. See TODO: add plugins doco here on how to configure or author authentication plugins.`
      );
    N._authPlugin = c({ connection: N, command: u }), Promise.resolve(N._authPlugin(S)).then((s) => {
      s && N.writePacket(new E.AuthSwitchResponse(s).toPacket());
    }).catch((s) => {
      e(s, u);
    });
  }
  function A(I, N, u) {
    const { data: O } = E.AuthSwitchRequestMoreData.fromPacket(I);
    if (N.config.authSwitchHandler) {
      const S = N.config.authSwitchHandler;
      a(), S({ pluginData: O }, (c, s) => {
        if (c)
          return e(c, u);
        N.writePacket(new E.AuthSwitchResponse(s).toPacket());
      });
      return;
    }
    if (!N._authPlugin)
    ...
    Promise.resolve(N._authPlugin(O)).then((S) => {
      S && N.writePacket(new E.AuthSwitchResponse(S).toPacket());
    }).catch((S) => {
      e(S, u);
    });
...
function wi() {
  return BA || (BA = 1, (function(E) {
    var t = aE.EventEmitter, n = QR, _ = 3e3, R = 0, T = "closed", a = "drained", e = function(A) {
      t.call(this), A && A > 0 ? this.timeout = A : this.timeout = _, this.status = i.STATUS_IDLE, this.curId = R, this.queue = [];
    };
    n.inherits(e, t), e.prototype.push = function(A, I, N) {
      if (this.status !== i.STATUS_IDLE && this.status !== i.STATUS_BUSY)
        return !1;
      if (typeof A != "function")
        throw new Error("fn should be a function.");
      if (this.queue.push({ fn: A, ontimeout: I, timeout: N }), this.status === i.STATUS_IDLE) {
      ...
      this.status !== i.STATUS_IDLE && this.status !== i.STATUS_BUSY || (A ? (this.status = i.STATUS_DRAINED, this.timerId && (clearTimeout(this.timerId), this.timerId = void 0), this.emit(a)) : (this.status = i.STATUS_CLOSED, this.emit(T)));
    }, e.prototype._next = function(A) {
      if (!(A !== this.curId || this.status !== i.STATUS_BUSY && this.status !== i.STATUS_CLOSED)) {
        this.timerId && (clearTimeout(this.timerId), this.timerId = void 0);
        var I = this.queue.shift();
        if (!I) {
          this.status === i.STATUS_BUSY ? (this.status = i.STATUS_IDLE, this.curId++) : (this.status = i.STATUS_DRAINED, this.emit(a));
          return;
        }
        var N = this;
        I.id = ++this.curId;
        var u = I.timeout > 0 ? I.timeout : this.timeout;
        u = u > 0 ? u : _, this.timerId = setTimeout(function() {
          process.nextTick(function() {
            N._next(I.id);
          }), N.emit("timeout", I), I.ontimeout && I.ontimeout();
        }, u);
        try {
          I.fn({
            done: function() {
          ...
          N.emit("error", O, I), process.nextTick(function() {
            N._next(I.id);
          });
        }
      }
    ...
    var i = E.exports;
    i.STATUS_IDLE = 0, i.STATUS_BUSY = 1, i.STATUS_CLOSED = 2, i.STATUS_DRAINED = 3, i.createQueue = function(A) {
      return new e(A);
    };
  })($_)), $_.exports;
}
...
function Fi() {
  ...
  const E = Lt, t = YR();
  function n(T) {
    const a = this, e = T.readInt24(), i = T.readBuffer();
    e !== 0 ? a.inflateQueue.push((A) => {
      E.inflate(i, (I, N) => {
        if (I) {
          a._handleNetworkError(I);
          return;
        }
        a._bumpCompressedSequenceId(T.numPackets), a._inflatedPacketsParser.execute(N), A.done();
      });
    }) : a.inflateQueue.push((A) => {
      a._bumpCompressedSequenceId(T.numPackets), a._inflatedPacketsParser.execute(i), A.done();
    });
  }
  function _(T) {
    let e;
    if (T.length > 16777210) {
      for (e = 0; e < T.length; e += 16777210)
        _.call(
          // eslint-disable-next-line no-invalid-this
          this,
          T.slice(e, e + 16777210)
        );
      return;
    }
    const i = this;
    ...
    (function(N) {
      i.deflateQueue.push((u) => {
        E.deflate(T, (O, S) => {
          if (O) {
            i._handleFatalError(O);
            return;
          }
          let c = S.length;
          c < A ? (I.writeUInt8(c & 255, 0), I.writeUInt16LE(c >> 8, 1), I.writeUInt8(N, 3), I.writeUInt8(A & 255, 4), I.writeUInt16LE(A >> 8, 5), i.writeUncompressed(I), i.writeUncompressed(S)) : (c = A, A = 0, I.writeUInt8(c & 255, 0), I.writeUInt16LE(c >> 8, 1), I.writeUInt8(N, 3), I.writeUInt8(A & 255, 4), I.writeUInt16LE(A >> 8, 5), i.writeUncompressed(I), i.writeUncompressed(T)), u.done();
        });
      });
    })(i.compressedSequenceId), i._bumpCompressedSequenceId(1);
  }
  function R(T) {
    T._lastWrittenPacketId = 0, T._lastReceivedPacketId = 0, T._handleCompressedPacket = n, T._inflatedPacketsParser = new t((e) => {
      T.handlePacket(e);
    }, 4), T._inflatedPacketsParser._lastPacket = 0, T.packetParser = new t((e) => {
      T._handleCompressedPacket(e);
    }, 7), T.writeUncompressed = T.write, T.write = _;
    const a = Gi();
    T.inflateQueue = a.createQueue(), T.deflateQueue = a.createQueue();
  }
  return _n = {
    enableCompression: R
  }, _n;
...
function HR() {
  if (hA) return nn;
  hA = 1;
  const E = AE(), t = eE(), n = uE(), _ = IE(), R = ME();
  function T(e) {
    const i = [];
    for (const A in n)
      e & n[A] && i.push(A.replace(/_/g, " ").toLowerCase());
    return i;
  }
  class a extends E {
    constructor(i) {
    ...
    start() {
      return a.prototype.handshakeInit;
    }
    sendSSLRequest(i) {
      const A = new t.SSLRequest(
        this.clientFlags,
        i.config.charsetNumber
      );
      i.writePacket(A.toPacket());
    }
    sendCredentials(i) {
      ...
      const A = new t.HandshakeResponse({
    ...
    calculateNativePasswordAuthToken(i) {
      const A = i.slice(0, 8), I = i.slice(8, 20);
      let N;
      return this.passwordSha1 ? N = R.calculateTokenFromPasswordSha(
        this.passwordSha1,
        A,
        I
      ) : N = R.calculateToken(
    ...
    handshakeInit(i, A) {
      this.on("error", (u) => {
        A._fatalError = u, A._protocolError = u;
      }), this.handshake = t.Handshake.fromPacket(i), A.config.debug && console.log(
      ...
      const I = this.handshake.capabilityFlags & n.SSL, N = this.handshake.capabilityFlags & n.MULTI_FACTOR_AUTHENTICATION;
      if (this.clientFlags = this.clientFlags | N, A.config.compress = A.config.compress && this.handshake.capabilityFlags & n.COMPRESS, this.clientFlags = this.clientFlags | A.config.compress, A.config.ssl) {
        if (!I) {
          const u = new Error("Server does not support secure connection");
          return u.code = "HANDSHAKE_NO_SSL_SUPPORT", u.fatal = !0, this.emit("error", u), !1;
        }
        this.clientFlags |= n.SSL, this.sendSSLRequest(A), A.startTLS((u) => {
          if (u) {
            u.code = "HANDSHAKE_SSL_ERROR", u.fatal = !0, this.emit("error", u);
            return;
          }
        ...
        this.sendCredentials(A);
      return N && (this.authenticationFactor = 1), a.prototype.handshakeResult;
    }
    handshakeResult(i, A) {
      const I = i.peekByte();
      if (I === 254 || I === 1 || I === 2) {
        const N = di();
        try {
          return I === 1 ? N.authSwitchRequestMoreData(i, A, this) : (this.authenticationFactor !== 0 && (A.config.password = this[`password${this.authenticationFactor}`], this.authenticationFactor += 1), N.authSwitchRequest(i, A, this)), a.prototype.handshakeResult;
        } catch (u) {
          return u.code = "AUTH_SWITCH_PLUGIN_ERROR", u.fatal = !0, this.onResult ? this.onResult(u) : this.emit("error", u), null;
        }
      }
      if (I !== 0) {
        const N = new Error("Unexpected packet during handshake phase");
        return N.code = "HANDSHAKE_UNKNOWN_ERROR", N.fatal = !0, this.onResult ? this.onResult(N) : this.emit("error", N), null;
      }
      if (!A.authorized && (A.authorized = !0, A.config.compress)) {
        const N = Fi().enableCompression;
        N(A);
      }
...
function Qi() {
  ...
  class R extends n {
    ...
    start(a, e) {
      ...
      return this.serverHello = i, i.setScrambleData((A) => {
        if (A) {
          e.emit("error", new Error("Error generating random bytes"));
          return;
        }
        e.writePacket(i.toPacket(0));
      }), R.prototype.readClientReply;
    }
    readClientReply(a, e) {
      const i = _.HandshakeResponse.fromPacket(a);
      return e.clientHelloReply = i, this.args.authCallback ? this.args.authCallback(
        {
          user: i.user,
          database: i.database,
          address: e.stream.remoteAddress,
      ...
      ) : e.writeOk(), R.prototype.dispatchCommands;
    }
    _isStatement(a, e) {
      return a.split(" ")[0].toUpperCase() === e;
    }
    dispatchCommands(a, e) {
      let i = !0;
      const A = e.clientHelloReply.encoding, I = a.readInt8();
      switch (I) {
        case E.STMT_PREPARE:
          if (e.listeners("stmt_prepare").length) {
            const N = a.readString(void 0, A);
            e.emit("stmt_prepare", N);
          } else
            e.writeError({
              code: t.HA_ERR_INTERNAL_ERROR,
              message: "No query handler for prepared statements."
            });
          break;
        case E.STMT_EXECUTE:
          if (e.listeners("stmt_execute").length) {
            const { stmtId: N, flags: u, iterationCount: O, values: S } = _.Execute.fromPacket(a, A);
            e.emit(
              "stmt_execute",
              N,
              u,
          ...
          break;
        case E.QUIT:
          e.listeners("quit").length ? e.emit("quit") : e.stream.end();
          break;
        case E.INIT_DB:
          if (e.listeners("init_db").length) {
            const N = a.readString(void 0, A);
            e.emit("init_db", N);
          } else
            e.writeOk();
          break;
        case E.QUERY:
          if (e.listeners("query").length) {
            const N = a.readString(void 0, A);
            this._isStatement(N, "PREPARE") || this._isStatement(N, "SET") ? e.emit("stmt_prepare", N) : this._isStatement(N, "EXECUTE") ? e.emit("stmt_execute", null, null, null, null, N) : e.emit("query", N);
          } else
            e.writeError({
              code: t.HA_ERR_INTERNAL_ERROR,
          ...
          break;
        case E.FIELD_LIST:
          if (e.listeners("field_list").length) {
            const N = a.readNullTerminatedString(A), u = a.readString(void 0, A);
            e.emit("field_list", N, u);
          } else
            e.writeError({
              code: t.ER_WARN_DEPRECATED_SYNTAX,
              message: "As of MySQL 5.7.11, COM_FIELD_LIST is deprecated and will be removed in a future version of MySQL."
            });
          break;
        case E.PING:
          e.listeners("ping").length ? e.emit("ping") : e.writeOk();
          break;
        default:
          i = !1;
      }
      return e.listeners("packet").length ? e.emit("packet", a.clone(), i, I) : i || console.log("Unknown command:", I), R.prototype.dispatchCommands;
    }
  }
  return en = R, en;
}
...
function pE() {
  ...
  let t, n = !1;
  try {
    t = Pi("cardinal").highlight;
  } catch {
    t = (e) => (n || (console.log("For nicer debug output consider install cardinal@^2.0.0"), n = !0), e);
  }
  function _(e, i) {
    console.log(`
...
`);
  ...
  OE.printDebugWithCode = _;
  function R(e, i, A) {
    return Array.isArray(i) ? i.some((I) => e === A[I]) : !!i;
  }
  OE.typeMatch = R;
...
function vR() {
  ...
  var e = function(I) {
    ...
    var I = [], N = 0, u = {}, O = function(s) {
      for (var r = ""; r.length < N * 2; ) r += "  ";
      I.push(r + s);
    }, S = function(s) {
      if (_.test(s.trim()[0]) && n.test(s[s.length - 1])) {
        N--, O(s), N++;
        return;
      }
      if (n.test(s[s.length - 1])) {
        O(s), N++;
        return;
      }
      if (_.test(s.trim()[0])) {
      ...
      if (!s) return c;
      if (arguments.length === 1 && s.indexOf(`
`) > -1)
        for (var r = s.trim().split(`
`), C = 0; C < r.length; C++)
          S(r[C].trim());
      else
    ...
    return c.scope = {}, c.formats = i, c.sym = function(s) {
      return (!s || !e(s)) && (s = "tmp"), u[s] || (u[s] = 0), s + (u[s]++ || "");
    }, c.property = function(s, r) {
      return arguments.length === 1 && (r = s, s = ""), r = r + "", t(r) ? s ? s + "." + r : r : s ? s + "[" + JSON.stringify(r) + "]" : JSON.stringify(r);
    }, c.toString = function() {
      return I.join(`
`);
    }, c.toFunction = function(s) {
      s || (s = {});
      var r = "return (" + c.toString() + ")";
      Object.keys(c.scope).forEach(function(B) {
        s[B] || (s[B] = c.scope[B]);
      });
      var C = Object.keys(s).map(function(B) {
        return B;
      }), D = C.map(function(B) {
        return s[B];
      });
      return Function.apply(null, C.concat(r)).apply(null, D);
    }, arguments.length && c.apply(null, arguments), c;
  };
  return A.formats = i, tn = A, tn;
}
var In, dA;
function bi() {
  if (dA) return In;
  dA = 1;
  const E = TE(), t = hE(), n = pE(), _ = vR(), R = pn(), T = [];
  for (const A in E)
    T[E[A]] = A;
  function a(A, I, N, u, O) {
    const S = !!(O.supportBigNumbers || u.supportBigNumbers), c = !!(O.bigNumberStrings || u.bigNumberStrings), s = O.timezone || u.timezone, r = O.dateStrings || u.dateStrings;
    switch (A) {
      case E.TINY:
      case E.SHORT:
      case E.LONG:
      ...
      case E.LONGLONG:
        return S && c ? "packet.parseLengthCodedIntString()" : `packet.parseLengthCodedInt(${S})`;
      case E.FLOAT:
      case E.DOUBLE:
      ...
      case E.DECIMAL:
      case E.NEWDECIMAL:
        return u.decimalNumbers ? "packet.parseLengthCodedFloat()" : 'packet.readLengthCodedString("ascii")';
      case E.DATE:
        return n.typeMatch(A, r, E) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDate(${n.srcEscape(s)})`;
      case E.DATETIME:
      case E.TIMESTAMP:
        return n.typeMatch(A, r, E) ? 'packet.readLengthCodedString("ascii")' : `packet.parseDateTime(${n.srcEscape(s)})`;
      case E.TIME:
      ...
      case E.JSON:
        return u.jsonStrings ? 'packet.readLengthCodedString("utf8")' : 'JSON.parse(packet.readLengthCodedString("utf8"))';
      default:
        return I === t.BINARY ? "packet.readLengthCodedBuffer()" : `packet.readLengthCodedString(${N})`;
    }
  }
  function e(A, I, N) {
    typeof N.typeCast == "function" && typeof I.typeCast != "function" && (I.typeCast = N.typeCast);
    function u(D, B) {
      return {
        type: T[D.columnType],
        ...
        table: D.table,
        name: D.name,
        string: function(M = D.encoding) {
          return D.columnType === E.JSON && M === D.encoding && console.warn(
          ...
          ), B.packet.readLengthCodedString(M);
        },
        buffer: function() {
    ...
    const O = _();
    O("(function () {")("return class TextRow {"), O("constructor(fields) {"), typeof I.typeCast == "function" && (O("const _this = this;"), O("for(let i=0; i<fields.length; ++i) {"), O("this[`wrap${i}`] = wrap(fields[i], _this);"), O("}")), O("}"), O("next(packet, fields, options) {"), O("this.packet = packet;"), I.rowsAsArray ? O(`const result = new Array(${A.length});`) : O("const result = {};");
    const S = {};
    let c = [];
    if (I.nestTables === !0) {
      ...
      for (let D = 0; D < c.length; D++)
        O(`result[${n.fieldEscape(c[D])}] = {};`);
    }
    let s = "", r = "", C = "";
    for (let D = 0; D < A.length; D++)
      if (r = n.fieldEscape(A[D].name), typeof I.nestTables == "string" ? s = `result[${n.fieldEscape(A[D].table + I.nestTables + A[D].name)}]` : I.nestTables === !0 ? (C = n.fieldEscape(A[D].table), O(`if (!result[${C}]) result[${C}] = {};`), s = `result[${C}][${r}]`) : I.rowsAsArray ? s = `result[${D.toString(10)}]` : s = `result[${r}]`, I.typeCast === !1)
        O(`${s} = packet.readLengthCodedBuffer();`);
      else {
        const B = `fields[${D}].encoding`, M = a(
          A[D].columnType,
          A[D].characterSet,
          B,
          N,
        ...
        typeof I.typeCast == "function" ? O(
          `${s} = options.typeCast(this.wrap${D}, function() { return ${M} });`
        ) : O(`${s} = ${M};`);
      }
    return O("return result;"), O("}"), O("};")("})()"), N.debug && n.printDebugWithCode(
      "Compiled text protocol row parser",
      O.toString()
    ), typeof I.typeCast == "function" ? O.toFunction({ wrap: u }) : O.toFunction();
  }
  function i(A, I, N) {
    return R.getParser("text", A, I, N, e);
  }
  return In = i, In;
}
var Nn, wA;
function mi() {
  if (wA) return Nn;
  wA = 1;
  const E = TE(), t = hE(), n = pE(), _ = [];
  for (const e in E)
    _[E[e]] = e;
  function R({ packet: e, type: i, charset: A, encoding: I, config: N, options: u }) {
    const O = !!(u.supportBigNumbers || N.supportBigNumbers), S = !!(u.bigNumberStrings || N.bigNumberStrings), c = u.timezone || N.timezone, s = u.dateStrings || N.dateStrings;
    switch (i) {
      case E.TINY:
      case E.SHORT:
      case E.LONG:
      ...
      case E.LONGLONG:
        return O && S ? e.parseLengthCodedIntString() : e.parseLengthCodedInt(O);
      case E.FLOAT:
      case E.DOUBLE:
      ...
      case E.DECIMAL:
      case E.NEWDECIMAL:
        return N.decimalNumbers ? e.parseLengthCodedFloat() : e.readLengthCodedString("ascii");
      case E.DATE:
        return n.typeMatch(i, s, E) ? e.readLengthCodedString("ascii") : e.parseDate(c);
      case E.DATETIME:
      case E.TIMESTAMP:
        return n.typeMatch(i, s, E) ? e.readLengthCodedString("ascii") : e.parseDateTime(c);
      case E.TIME:
      ...
      case E.JSON:
        return N.jsonStrings ? e.readLengthCodedString("utf8") : JSON.parse(e.readLengthCodedString("utf8"));
      default:
        return A === t.BINARY ? e.readLengthCodedBuffer() : e.readLengthCodedString(I);
    }
  }
  function T(e, i) {
    return {
      ...
      table: e.table,
      name: e.name,
      string: function(A = e.encoding) {
        return e.columnType === E.JSON && A === e.encoding && console.warn(
  ...
  function a(e, i, A) {
    return {
      next(I, N, u) {
        const O = u.rowsAsArray ? [] : {};
        for (let S = 0; S < N.length; S++) {
          const c = N[S], s = u.typeCast ? u.typeCast : A.typeCast, r = () => R({
            packet: I,
            type: c.columnType,
            encoding: c.encoding,
          ...
          let C;
          if (u.typeCast === !1 ? C = I.readLengthCodedBuffer() : typeof s == "function" ? C = s(T(c, I), r) : C = r(), u.rowsAsArray)
            O.push(C);
          else if (typeof u.nestTables == "string")
            O[`${n.fieldEscape(c.table, !1)}${u.nestTables}${n.fieldEscape(c.name, !1)}`] = C;
          else if (u.nestTables) {
            const D = n.fieldEscape(c.table, !1);
            O[D] || (O[D] = {}), O[D][n.fieldEscape(c.name, !1)] = C;
          } else
            O[n.fieldEscape(c.name, !1)] = C;
        }
        return O;
      }
    };
...
function yR() {
  if (GA) return Tn;
  GA = 1;
  const E = zE, t = Zn, n = GR.Readable, _ = AE(), R = eE(), T = bi(), a = mi(), e = WR(), i = new R.Packet(0, Buffer.allocUnsafe(4), 0, 4);
  class A extends _ {
    constructor(N, u) {
      super(), this.sql = N.sql, this.values = N.values, this._queryOptions = N, this.namedPlaceholders = N.namedPlaceholders || !1, this.onResult = u, this.timeout = N.timeout, this.queryTimeout = null, this._fieldCount = 0, this._rowParser = null, this._fields = [], this._rows = [], this._receivedFieldsCount = 0, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
      }, this._streamFactory = N.infileStreamFactory, this._connection = null;
    }
    then() {
      const N = "You have tried to call .then(), .catch(), or invoked await on the result of query that is not a promise, which is a programming error. Try calling con.promise().query(), or require('mysql2/promise') instead of 'mysql2' for a promise-compatible version of the query interface. To learn how to use async/await or Promises check out documentation at https://sidorares.github.io/node-mysql2/docs#using-promise-wrapper, or the mysql2 documentation at https://sidorares.github.io/node-mysql2/docs/documentation/promise-wrapper";
      throw console.log(N), new Error(N);
    }
    /* eslint no-unused-vars: ["error", { "argsIgnorePattern": "^_" }] */
    start(N, u) {
      u.config.debug && console.log("        Sending query command: %s", this.sql), this._connection = u, this.options = Object.assign({}, u.config, this._queryOptions), this._setTimeout();
      const O = new R.Query(
        this.sql,
        u.config.charsetNumber
      );
      return u.writePacket(O.toPacket(1)), A.prototype.resultsetHeader;
    }
    done() {
      if (this._unpipeStream(), this.timeout && !this.queryTimeout)
        return null;
      if (this.queryTimeout && (t.clearTimeout(this.queryTimeout), this.queryTimeout = null), this.onResult) {
        let N, u;
        this._resultIndex === 0 ? (N = this._rows[0], u = this._fields[0]) : (N = this._rows, u = this._fields), u ? E.nextTick(() => {
          this.onResult(null, N, u);
        }) : E.nextTick(() => {
          this.onResult(null, N);
        });
      }
      return null;
    }
    doneInsert(N) {
      return this._localStreamError ? (this.onResult ? this.onResult(this._localStreamError, N) : this.emit("error", this._localStreamError), null) : (this._rows.push(N), this._fields.push(void 0), this.emit("fields", void 0), this.emit("result", N), N.serverStatus & e.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, this.resultsetHeader) : this.done());
    }
    resultsetHeader(N, u) {
      const O = new R.ResultSetHeader(N, u);
      return this._fieldCount = O.fieldCount, u.config.debug && console.log(
        `        Resultset header received, expecting ${O.fieldCount} column definition packets`
      ), this._fieldCount === 0 ? this.doneInsert(O) : this._fieldCount === null ? this._streamLocalInfile(u, O.infileName) : (this._receivedFieldsCount = 0, this._rows.push([]), this._fields.push([]), this.readField);
    }
    _streamLocalInfile(N, u) {
      if (this._streamFactory)
        this._localStream = this._streamFactory(u);
      else
        return this._localStreamError = new Error(
          `As a result of LOCAL INFILE command server wants to read ${u} file, but as of v2.0 you must provide streamFactory option returning ReadStream.`
        ), N.writePacket(i), this.infileOk;
      const O = () => {
        this._unpipeStream();
      }, S = () => {
        ...
        const B = Buffer.allocUnsafe(D.length + 4);
        D.copy(B, 4), N.writePacket(
          new R.Packet(0, B, 0, B.length)
        );
      }, r = () => {
        N.removeListener("error", O), N.writePacket(i);
      }, C = (D) => {
        this._localStreamError = D, N.removeListener("error", O), N.writePacket(i);
      };
      return this._unpipeStream = () => {
        N.stream.removeListener("pause", c), N.stream.removeListener("drain", S), this._localStream.removeListener("data", s), this._localStream.removeListener("end", r), this._localStream.removeListener("error", C);
      }, N.stream.on("pause", c), N.stream.on("drain", S), this._localStream.on("data", s), this._localStream.on("end", r), this._localStream.on("error", C), N.once("error", O), this.infileOk;
    }
    readField(N, u) {
      if (this._receivedFieldsCount++, this._fields[this._resultIndex].length !== this._fieldCount) {
        const O = new R.ColumnDefinition(
          N,
          u.clientEncoding
        );
        this._fields[this._resultIndex].push(O), u.config.debug && (console.log("        Column definition:"), console.log(`          name: ${O.name}`), console.log(`          type: ${O.columnType}`), console.log(`         flags: ${O.flags}`));
      }
      if (this._receivedFieldsCount === this._fieldCount) {
        const O = this._fields[this._resultIndex];
        return this.emit("fields", O), this.options.disableEval ? this._rowParser = a(O, this.options, u.config) : this._rowParser = new (T(
          O,
          this.options,
          u.config
        ))(O), A.prototype.fieldsEOF;
      }
      return A.prototype.readField;
    }
    fieldsEOF(N, u) {
      return N.isEOF() ? this.row : u.protocolError("Expected EOF packet");
    }
    row(N, u) {
      if (N.isEOF())
        return N.eofStatusFlags() & e.SERVER_MORE_RESULTS_EXISTS ? (this._resultIndex++, A.prototype.resultsetHeader) : this.done();
      let O;
      try {
        O = this._rowParser.next(
          N,
      ...
      return this.onResult ? this._rows[this._resultIndex].push(O) : this.emit("result", O, this._resultIndex), A.prototype.row;
    }
    infileOk(N, u) {
      const O = new R.ResultSetHeader(N, u);
      return this.doneInsert(O);
    }
    stream(N) {
      N = N || /* @__PURE__ */ Object.create(null), N.objectMode = !0;
      const u = new n({
        ...N,
        emitClose: !0,
        autoDestroy: !0,
      ...
      u.once("close", () => {
        u.readableEnded || u.emit("end");
      });
      const O = (r, C) => {
        u.destroyed || (u.push(r) || this._connection && this._connection.pause(), u.emit("result", r, C));
      }, S = (r) => {
        u.destroyed || u.emit("fields", r);
      }, c = () => {
        u.destroyed || u.push(null);
      }, s = (r) => {
        u.destroy(r);
      };
      return u._destroy = (r, C) => {
        this._connection && this._connection.resume(), this.removeListener("result", O), this.removeListener("fields", S), this.removeListener("end", c), this.removeListener("error", s), C(r);
      }, this.on("result", O), this.on("fields", S), this.on("end", c), this.on("error", s), u;
    }
    _setTimeout() {
      if (this.timeout) {
        const N = this._handleTimeoutError.bind(this);
        this.queryTimeout = t.setTimeout(N, this.timeout);
      }
    }
    _handleTimeoutError() {
      this.queryTimeout && (t.clearTimeout(this.queryTimeout), this.queryTimeout = null);
      const N = new Error("Query inactivity timeout");
      N.errorno = "PROTOCOL_SEQUENCE_TIMEOUT", N.code = "PROTOCOL_SEQUENCE_TIMEOUT", N.syscall = "query", this.onResult ? this.onResult(N) : this.emit("error", N);
    }
  }
  return A.prototype.catch = A.prototype.then, Tn = A, Tn;
}
var an, FA;
function XR() {
  if (FA) return an;
  ...
  class n extends E {
    ...
    start(R, T) {
      return T.writePacket(new t.CloseStatement(this.id).toPacket(1)), null;
    }
  }
  return an = n, an;
}
...
function Yi() {
  if (QA) return rn;
  QA = 1;
  const E = xn(), t = hE(), n = TE(), _ = pE(), R = vR(), T = pn(), a = [];
  for (const I in n)
    a[n[I]] = I;
  function e(I, N, u, O) {
    const S = !!(u.supportBigNumbers || N.supportBigNumbers), c = !!(u.bigNumberStrings || N.bigNumberStrings), s = u.timezone || N.timezone, r = u.dateStrings || N.dateStrings, C = I.flags & E.UNSIGNED;
    switch (I.columnType) {
      case n.TINY:
      ...
      case n.TIMESTAMP:
      case n.NEWDATE:
        return _.typeMatch(I.columnType, r, n) ? `packet.readDateTimeString(${parseInt(I.decimals, 10)}, null, ${I.columnType});` : `packet.readDateTime(${_.srcEscape(s)});`;
      case n.TIME:
        return "packet.readTimeString()";
      case n.DECIMAL:
      case n.NEWDECIMAL:
        return N.decimalNumbers ? "packet.parseLengthCodedFloat();" : 'packet.readLengthCodedString("ascii");';
      case n.GEOMETRY:
        return "packet.parseGeometryValue();";
      case n.VECTOR:
        return "packet.parseVector()";
      case n.JSON:
        return N.jsonStrings ? 'packet.readLengthCodedString("utf8")' : 'JSON.parse(packet.readLengthCodedString("utf8"));';
      case n.LONGLONG:
        return S ? c ? C ? "packet.readInt64String();" : "packet.readSInt64String();" : C ? "packet.readInt64();" : "packet.readSInt64();" : C ? "packet.readInt64JSNumber();" : "packet.readSInt64JSNumber();";
      default:
        return I.characterSet === t.BINARY ? "packet.readLengthCodedBuffer();" : `packet.readLengthCodedString(fields[${O}].encoding)`;
    }
  }
  function i(I, N, u) {
    const O = R(), S = Math.floor((I.length + 7 + 2) / 8);
    function c(M, h) {
      return {
        type: a[M.columnType],
        ...
        table: M.table,
        name: M.name,
        string: function(L = M.encoding) {
          if (M.columnType === n.JSON && L === M.encoding && console.warn(
            `typeCast: JSON column "${M.name}" is interpreted as BINARY by default, recommended to manually set utf8 encoding: \`field.string("utf8")\``
          ), [n.DATETIME, n.NEWDATE, n.TIMESTAMP, n.DATE].includes(
            M.columnType
          ))
            return h.readDateTimeString(
              parseInt(M.decimals, 10),
              " ",
              M.columnType
            );
          if (M.columnType === n.TINY) {
            const U = M.flags & E.UNSIGNED;
            return String(U ? h.readInt8() : h.readSInt8());
          }
          return M.columnType === n.TIME ? h.readTimeString() : h.readLengthCodedString(L);
        },
        buffer: function() {
    ...
    O("(function(){"), O("return class BinaryRow {"), O("constructor() {"), O("}"), O("next(packet, fields, options) {"), N.rowsAsArray ? O(`const result = new Array(${I.length});`) : O("const result = {};"), typeof u.typeCast == "function" && typeof N.typeCast != "function" && (N.typeCast = u.typeCast), O("packet.readInt8();");
    for (let M = 0; M < S; ++M)
      O(`const nullBitmaskByte${M} = packet.readInt8();`);
    let s = "", r = 4, C = 0, D = "", B = "";
    for (let M = 0; M < I.length; M++) {
      if (D = _.fieldEscape(I[M].name), typeof N.nestTables == "string" ? s = `result[${_.fieldEscape(I[M].table + N.nestTables + I[M].name)}]` : N.nestTables === !0 ? (B = _.fieldEscape(I[M].table), O(`if (!result[${B}]) result[${B}] = {};`), s = `result[${B}][${D}]`) : N.rowsAsArray ? s = `result[${M.toString(10)}]` : s = `result[${D}]`, O(`if (nullBitmaskByte${C} & ${r}) `), O(`${s} = null;`), O("else {"), N.typeCast === !1)
        O(`${s} = packet.readLengthCodedBuffer();`);
      else {
        const h = `fieldWrapper${M}`;
        O(`const ${h} = wrap(fields[${M}], packet);`);
        const L = e(I[M], u, N, M);
        typeof N.typeCast == "function" ? O(
          `${s} = options.typeCast(${h}, function() { return ${L} });`
        ) : O(`${s} = ${L};`);
      }
      O("}"), r *= 2, r === 256 && (r = 1, C++);
    }
    return O("return result;"), O("}"), O("};")("})()"), u.debug && _.printDebugWithCode(
      "Compiled binary protocol row parser",
      O.toString()
    ), O.toFunction({ wrap: c });
  }
  function A(I, N, u) {
    return T.getParser("binary", I, N, u, i);
  }
  return rn = A, rn;
}
var un, PA;
function Wi() {
  if (PA) return un;
  PA = 1;
  const E = xn(), t = hE(), n = TE(), _ = pE(), R = [];
  for (const a in n)
    R[n[a]] = a;
  function T(a, e, i) {
    function A(I, N, u, O, S) {
      const c = !!(u.supportBigNumbers || N.supportBigNumbers), s = !!(u.bigNumberStrings || N.bigNumberStrings), r = u.timezone || N.timezone, C = u.dateStrings || N.dateStrings, D = I.flags & E.UNSIGNED;
      switch (I.columnType) {
        case n.TINY:
          return D ? S.readInt8() : S.readSInt8();
        case n.SHORT:
          return D ? S.readInt16() : S.readSInt16();
        case n.LONG:
        case n.INT24:
          return D ? S.readInt32() : S.readSInt32();
        case n.YEAR:
          return S.readInt16();
        case n.FLOAT:
          return S.readFloat();
        ...
        case n.TIMESTAMP:
        case n.NEWDATE:
          return _.typeMatch(I.columnType, C, n) ? S.readDateTimeString(
        ...
        case n.DECIMAL:
        case n.NEWDECIMAL:
          return N.decimalNumbers ? S.parseLengthCodedFloat() : S.readLengthCodedString("ascii");
        case n.GEOMETRY:
          return S.parseGeometryValue();
        case n.VECTOR:
          return S.parseVector();
        case n.JSON:
          return N.jsonStrings ? S.readLengthCodedString("utf8") : JSON.parse(S.readLengthCodedString("utf8"));
        case n.LONGLONG:
          return c ? s ? D ? S.readInt64String() : S.readSInt64String() : D ? S.readInt64() : S.readSInt64() : D ? S.readInt64JSNumber() : S.readSInt64JSNumber();
        default:
          return I.characterSet === t.BINARY ? S.readLengthCodedBuffer() : S.readLengthCodedString(a[O].encoding);
      }
    }
    return class {
      constructor() {
      ...
      next(N, u, O) {
        N.readInt8();
        const S = Math.floor((u.length + 7 + 2) / 8), c = new Array(S);
        for (let D = 0; D < S; D++)
          c[D] = N.readInt8();
        const s = O.rowsAsArray ? new Array(u.length) : {};
        let r = 4, C = 0;
        for (let D = 0; D < u.length; D++) {
          const B = u[D], M = O.typeCast !== void 0 ? O.typeCast : i.typeCast;
          let h;
          if (c[C] & r)
            h = null;
          else if (O.typeCast === !1)
            h = N.readLengthCodedBuffer();
          else {
            const L = () => A(B, i, O, D, N);
            h = typeof M == "function" ? M(
              {
                type: R[B.columnType],
                ...
                string: function(U = B.encoding) {
                  if (B.columnType === n.JSON && U === B.encoding && console.warn(
                  ...
                  if (B.columnType === n.TINY) {
                    ...
                    return String(
                      d ? N.readInt8() : N.readSInt8()
                    );
                  }
                  return B.columnType === n.TIME ? N.readTimeString() : N.readLengthCodedString(U);
                },
                buffer: function() {
          ...
          if (O.rowsAsArray)
            ...
            const L = _.fieldEscape(
              B.table + O.nestTables + B.name,
              !1
            );
            s[L] = h;
          } else if (O.nestTables === !0) {
...
function KR() {
  if (VA) return cn;
  VA = 1;
  const E = AE(), t = yR(), n = eE(), _ = Yi(), R = Wi();
  class T extends E {
    constructor(e, i) {
      super(), this.statement = e.statement, this.sql = e.sql, this.values = e.values, this.onResult = i, this.parameters = e.values, this.insertId = 0, this.timeout = e.timeout, this.queryTimeout = null, this._rows = [], this._fields = [], this._result = [], this._fieldCount = 0, this._rowParser = null, this._executeOptions = e, this._resultIndex = 0, this._localStream = null, this._unpipeStream = function() {
      }, this._streamFactory = e.infileStreamFactory, this._connection = null;
    }
    buildParserFromFields(e, i) {
      return this.options.disableEval ? R(e, this.options, i.config) : _(e, this.options, i.config);
    }
    start(e, i) {
      this._connection = i, this.options = Object.assign({}, i.config, this._executeOptions), this._setTimeout();
      const A = new n.Execute(
        this.statement.id,
        this.parameters,
        i.config.charsetNumber,
        i.config.timezone
    ...
    readField(e, i) {
      let A;
      const I = new n.ColumnDefinition(
        e,
        i.clientEncoding
      );
      return this._receivedFieldsCount++, this._fields[this._resultIndex].push(I), this._receivedFieldsCount === this._fieldCount ? (A = this._fields[this._resultIndex], this.emit("fields", A, this._resultIndex), T.prototype.fieldsEOF) : T.prototype.readField;
    }
    fieldsEOF(e, i) {
      return e.isEOF() ? (this._rowParser = new (this.buildParserFromFields(
        this._fields[this._resultIndex],
        i
      ))(), T.prototype.row) : i.protocolError("Expected EOF packet");
    }
  }
  return T.prototype.done = t.prototype.done, T.prototype.doneInsert = t.prototype.doneInsert, T.prototype.resultsetHeader = t.prototype.resultsetHeader, T.prototype._findOrCreateReadStream = t.prototype._findOrCreateReadStream, T.prototype._streamLocalInfile = t.prototype._streamLocalInfile, T.prototype._setTimeout = t.prototype._setTimeout, T.prototype._handleTimeoutError = t.prototype._handleTimeoutError, T.prototype.row = t.prototype.row, T.prototype.stream = t.prototype.stream, cn = T, cn;
}
var On, bA;
function Hi() {
  if (bA) return On;
  bA = 1;
  const E = eE(), t = AE(), n = XR(), _ = KR();
  class R {
    constructor(e, i, A, I, N) {
      this.query = e, this.id = i, this.columns = A, this.parameters = I, this.rowParser = null, this._connection = N;
    }
    close() {
      return this._connection.addCommand(new n(this.id));
    }
    execute(e, i) {
      return typeof e == "function" && (i = e, e = []), this._connection.addCommand(
        new _({ statement: this, values: e }, i)
      );
    }
  }
  class T extends t {
    constructor(e, i) {
      super(), this.query = e.sql, this.onResult = i, this.id = 0, this.fieldCount = 0, this.parameterCount = 0, this.fields = [], this.parameterDefinitions = [], this.options = e;
    }
    start(e, i) {
      const A = i.constructor;
      this.key = A.statementKey(this.options);
      const I = i._statements.get(this.key);
      if (I)
        return this.onResult && this.onResult(null, I), null;
      const N = new E.PrepareStatement(
        this.query,
        i.config.charsetNumber,
        this.options.values
      );
      return i.writePacket(N.toPacket(1)), T.prototype.prepareHeader;
    }
    prepareHeader(e, i) {
      const A = new E.PreparedStatementHeader(e);
      return this.id = A.id, this.fieldCount = A.fieldCount, this.parameterCount = A.parameterCount, this.parameterCount > 0 ? T.prototype.readParameter : this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(i);
    }
    readParameter(e, i) {
      if (e.isEOF())
        return this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(i);
      const A = new E.ColumnDefinition(e, i.clientEncoding);
      return this.parameterDefinitions.push(A), this.parameterDefinitions.length === this.parameterCount ? T.prototype.parametersEOF : this.readParameter;
    }
    readField(e, i) {
      if (e.isEOF())
        return this.prepareDone(i);
      const A = new E.ColumnDefinition(e, i.clientEncoding);
      return this.fields.push(A), this.fields.length === this.fieldCount ? T.prototype.fieldsEOF : T.prototype.readField;
    }
    parametersEOF(e, i) {
      return e.isEOF() ? this.fieldCount > 0 ? T.prototype.readField : this.prepareDone(i) : i.protocolError("Expected EOF packet after parameters");
    }
    fieldsEOF(e, i) {
      return e.isEOF() ? this.prepareDone(i) : i.protocolError("Expected EOF packet after fields");
    }
    prepareDone(e) {
      const i = new R(
        this.query,
      ...
      return e._statements.set(this.key, i), this.onResult && this.onResult(null, i), null;
    }
  }
  return On = T, On;
}
...
function vi() {
  ...
  class _ extends E {
    ...
    start(T, a) {
      const e = new n(
        0,
        Buffer.from([1, 0, 0, 0, t.PING]),
        0,
        5
      );
      return a.writePacket(e), _.prototype.pingResponse;
    }
    pingResponse() {
...
function yi() {
  ...
  class n extends E {
    constructor(R, T) {
      super(), this.onResult = T, this.opts = R;
    }
    start(R, T) {
      const a = new t.RegisterSlave(this.opts);
      return T.writePacket(a.toPacket(1)), n.prototype.registerResponse;
    }
    registerResponse() {
      return this.onResult && process.nextTick(this.onResult.bind(this)), null;
    }
...
function Xi() {
  ...
  return Dn = function(n) {
    ...
    for (; R < n.length; )
      switch (T = n[R++], T) {
        case E.FLAGS2:
          _.flags = n.readUInt32LE(R), R += 4;
          break;
        case E.SQL_MODE:
          _.sqlMode = n.readUInt32LE(R), R += 8;
          break;
        case E.CATALOG:
          a = n[R++], _.catalog = n.toString("utf8", R, R + a), R += a + 1;
          break;
        case E.CHARSET:
          _.clientCharset = n.readUInt16LE(R), _.connectionCollation = n.readUInt16LE(R + 2), _.serverCharset = n.readUInt16LE(R + 4), R += 6;
          break;
        case E.TIME_ZONE:
          a = n[R++], _.timeZone = n.toString("utf8", R, R + a), R += a;
          break;
        case E.CATALOG_NZ:
          a = n[R++], _.catalogNz = n.toString("utf8", R, R + a), R += a;
          break;
        case E.LC_TIME_NAMES:
          _.lcTimeNames = n.readUInt16LE(R), R += 2;
          break;
        case E.CHARSET_DATABASE:
          _.schemaCharset = n.readUInt16LE(R), R += 2;
          break;
        case E.TABLE_MAP_FOR_UPDATE:
          _.mapForUpdate1 = n.readUInt32LE(R), _.mapForUpdate2 = n.readUInt32LE(R + 4), R += 8;
          break;
        case E.MASTER_DATA_WRITTEN:
          _.masterDataWritten = n.readUInt32LE(R), R += 4;
          break;
        case E.INVOKERS:
          a = n[R++], _.invokerUsername = n.toString(
            "utf8",
            R,
            R + a
          ), R += a, a = n[R++], _.invokerHostname = n.toString(
            "utf8",
            R,
            R + a
          ), R += a;
        ...
        case E.UPDATED_DB_NAMES:
          for (a = n[R++], _.updatedDBs = []; a; --a) {
            for (e = R; n[R++] && R < n.length; )
              ;
            _.updatedDBs.push(
              n.toString("utf8", e, R - 1)
            );
          }
          break;
        case E.MICROSECONDS:
          _.microseconds = // REVIEW: INVALID UNKNOWN VARIABLE!
          n.readInt16LE(R) + (n[R + 2] << 16), R += 3;
      }
    return _;
  }, Dn;
}
...
function Ki() {
  ...
  class R extends E {
    ...
    start(I, N) {
      const u = new t.BinlogDump(this.opts);
      return N.writePacket(u.toPacket(1)), R.prototype.binlogData;
    }
    binlogData(I) {
      if (I.isEOF())
        return this.emit("eof"), null;
      I.readInt8();
      const N = new _(I), u = n[N.eventType];
      let O;
      return u ? O = new u(I) : O = {
        name: "UNKNOWN"
      }, O.header = N, this.emit("event", O), R.prototype.binlogData;
    }
  }
  class T {
    constructor(I) {
  ...
  class e {
    constructor(I) {
      ...
      const O = I.readInt16(), S = I.readBuffer(O);
      this.schema = I.readString(u), I.readInt8(), this.statusVars = N(S), this.query = I.readString(), this.name = "QueryEvent";
    }
  }
  class i {
    constructor(I) {
  ...
  return n[2] = e, n[4] = T, n[15] = a, n[16] = i, Sn = R, Sn;
}
var Bn, vA;
function ji() {
  if (vA) return Bn;
  vA = 1;
  const E = AE(), t = eE(), n = uE(), _ = HR(), R = IE();
  class T extends E {
    constructor(e, i) {
      super(), this.onResult = i, this.user = e.user, this.password = e.password, this.password1 = e.password, this.password2 = e.password2, this.password3 = e.password3, this.database = e.database, this.passwordSha1 = e.passwordSha1, this.charsetNumber = e.charsetNumber, this.currentConfig = e.currentConfig, this.authenticationFactor = 0;
    }
    start(e, i) {
      const A = new t.ChangeUser({
      ...
      return this.currentConfig.user = this.user, this.currentConfig.password = this.password, this.currentConfig.database = this.database, this.currentConfig.charsetNumber = this.charsetNumber, i.clientEncoding = R[this.charsetNumber], i._statements.clear(), i.writePacket(A.toPacket()), i.serverCapabilityFlags & n.MULTI_FACTOR_AUTHENTICATION && (this.authenticationFactor = 1), T.prototype.handshakeResult;
    }
  }
  return T.prototype.handshakeResult = _.prototype.handshakeResult, T.prototype.calculateNativePasswordAuthToken = _.prototype.calculateNativePasswordAuthToken, Bn = T, Bn;
}
var ln, yA;
function ki() {
  if (yA) return ln;
  ...
  class _ extends E {
    ...
    start(T, a) {
      ...
      const e = new n(
        0,
        Buffer.from([1, 0, 0, 0, t.QUIT]),
        0,
        5
      );
      return this.onResult && this.onResult(), a.writePacket(e), null;
    }
  }
  return ln = _, ln;
}
...
function Ji() {
  if (XA) return Mn;
  XA = 1;
  const E = HR(), t = Qi(), n = yR(), _ = Hi(), R = XR(), T = KR(), a = vi(), e = yi(), i = Ki(), A = ji(), I = ki();
  return Mn = {
    ClientHandshake: E,
    ServerHandshake: t,
    Query: n,
...
var hn = {}, SE = { exports: {} }, gE = {}, KA;
function qi() {
  return KA || (KA = 1, Object.defineProperty(gE, "__esModule", { value: !0 }), gE.defaults = void 0, gE.defaults = [
    `-----BEGIN CERTIFICATE-----
...
var UE = {}, jA;
function pi() {
  return jA || (jA = 1, Object.defineProperty(UE, "__esModule", { value: !0 }), UE.proxies = void 0, UE.proxies = [
    `-----BEGIN CERTIFICATE-----
...
var xi = SE.exports, kA;
function $i() {
  if (kA) return SE.exports;
  kA = 1, Object.defineProperty(xi, "__esModule", { value: !0 });
  const E = qi(), t = pi(), n = {
    ca: t.proxies
  }, _ = {
    ca: [...E.defaults, ...t.proxies]
  };
  return SE.exports = _, SE.exports.proxyBundle = n, SE.exports.default = _, SE.exports;
}
var JA;
function EI() {
...
function $n() {
  if (zA) return Ln;
  zA = 1;
  const { URL: E } = ot, t = uE(), n = hE(), { version: _ } = Zi;
  let R = null;
  const T = {
    authPlugins: 1,
    authSwitchHandler: 1,
  ...
  class a {
    constructor(i) {
      if (typeof i == "string")
        ...
        const I = a.parseUrl(i.uri);
        for (const N in I)
          Object.prototype.hasOwnProperty.call(I, N) && (i[N] || (i[N] = I[N]));
      }
      for (const I in i)
        Object.prototype.hasOwnProperty.call(i, I) && T[I] !== 1 && console.error(
      ...
      ), this.timezone = "Z") : this.timezone = i.timezone || "local", this.queryFormat = i.queryFormat, this.pool = i.pool || void 0, this.ssl = typeof i.ssl == "string" ? a.getSSLProfile(i.ssl) : i.ssl || !1, this.multipleStatements = i.multipleStatements || !1, this.rowsAsArray = i.rowsAsArray || !1, this.namedPlaceholders = i.namedPlaceholders || !1, this.nestTables = i.nestTables === void 0 ? void 0 : i.nestTables, this.typeCast = i.typeCast === void 0 ? !0 : i.typeCast, this.disableEval = !!i.disableEval, this.timezone[0] === " " && (this.timezone = `+${this.timezone.slice(1)}`), this.ssl) {
      ...
      this.maxPacketSize = 0, this.charsetNumber = i.charset ? a.getCharsetNumber(i.charset) : i.charsetNumber || n.UTF8MB4_UNICODE_CI, this.compress = i.compress || !1, this.authPlugins = i.authPlugins, this.authSwitchHandler = i.authSwitchHandler, this.clientFlags = a.mergeFlags(
        a.getDefaultFlags(i),
        i.flags || ""
      );
      const A = {
    ...
    static mergeFlags(i, A) {
      let I = 0, N;
      Array.isArray(A) || (A = String(A || "").toUpperCase().split(/\s*,+\s*/));
      for (N in i)
        A.indexOf(`-${i[N]}`) >= 0 || (I |= t[i[N]] || 0);
      for (N in A)
        A[N][0] !== "-" && (i.indexOf(A[N]) >= 0 || (I |= t[A[N]] || 0));
      return I;
    }
    static getDefaultFlags(i) {
      ...
      return i && i.multipleStatements && A.push("MULTI_STATEMENTS"), A.push("PLUGIN_AUTH"), A.push("PLUGIN_AUTH_LENENC_CLIENT_DATA"), A;
    }
    ...
    static parseUrl(i) {
      ...
      return A.searchParams.forEach((N, u) => {
        try {
          I[u] = JSON.parse(N);
        } catch {
          I[u] = N;
...
var HE = { exports: {} }, ZA;
function _I() {
  if (ZA) return HE.exports;
  ZA = 1;
  const E = /(?:\?)|(?::(\d+|(?:[a-zA-Z][a-zA-Z0-9_]*)))/g, t = 34, n = 39, _ = 92;
  function R(e) {
    let i = E.exec(e), A = 0, I = 0, N;
    const u = [];
    let O = !1, S = !1, c;
    const s = [];
    let r = 0, C = 0, D;
    if (i) {
      do {
        for (D = A, N = i.index; D < N; ++D) {
          const B = e.charCodeAt(D);
          if (B === _) S = !S;
          else {
            if (S) {
              S = !1;
            ...
            if (O && B === c) {
              if (e.charCodeAt(D + 1) === c) {
                ++D;
                continue;
              }
              O = !1;
            } else !O && (B === t || B === n) && (O = !0, c = B);
          }
        }
        O || (u.push(e.substring(I, N)), s.push(i[0].length === 1 ? r++ : i[1]), I = N + i[0].length, C = I), A = N + i[0].length;
      } while (i = E.exec(e));
      if (s.length)
        return A < e.length && u.push(e.substring(C)), [u, s];
    }
    return [e];
  }
  function T(e) {
    e || (e = {}), e.placeholder || (e.placeholder = "?");
    let i = 100, A;
    typeof e.cache == "number" && (i = e.cache), typeof e.cache == "object" && (A = e.cache), e.cache !== !1 && !A && (A = ZE().createLRU({ max: i }));
    function I(S, c) {
      const s = [];
      if (S.length === 1)
        return [S[0], []];
    ...
    function u(S) {
      ...
      for (let r = 1; r < S[0].length; ++r)
        S[0][r - 1].slice(-1) === ":" && (c += e.placeholder), c += e.placeholder, c += N(S[0][r]);
      const s = S[0][S[0].length - 1];
      return S[0].length === S[1].length && (s.slice(-1) === ":" && (c += e.placeholder), c += e.placeholder), [c, S[1]];
    }
    function O(S, c) {
      let s;
      return A && (s = A.get(S)) || (s = u(R(S)), A && A.set(S, s)), I(s, c);
    }
    return O.parse = R, O;
  }
  function a(e, i) {
    ...
    for (let s = 0; s < A[0].length; ++s)
      S = N[A[1][s]], S || (S = ++u, N[A[1][s]] = S), A[1][s] ? (c[S - 1] = A[1][s], O += `${A[0][s]}$${S}`) : O += A[0][s];
    return [O, c.map((s) => i[s])];
  }
  return HE.exports = T, HE.exports.toNumbered = a, HE.exports;
}
var on, qA;
function xE() {
  if (qA) return on;
  qA = 1;
  const E = Mt, t = wR, n = Zn, _ = aE.EventEmitter, R = GR.Readable, T = VR(), a = qn(), { createLRU: e } = ZE(), i = YR(), A = eE(), I = Ji(), N = $n(), u = IE();
  let O = 0, S = null;
  class c extends _ {
    constructor(r) {
      ...
      }), this.stream.setNoDelay(!0)), this._internalId = O++, this._commands = new T(), this._command = null, this._paused = !1, this._paused_packets = new T(), this._statements = e({
        max: this.config.maxPreparedStatements,
        onEviction: function(D, B) {
          B.close();
        }
      }), this.serverCapabilityFlags = 0, this.authorized = !1, this.sequenceId = 0, this.compressedSequenceId = 0, this.threadId = null, this._handshakePacket = null, this._fatalError = null, this._protocolError = null, this._outOfOrderPackets = [], this.clientEncoding = u[this.config.charsetNumber], this.stream.on("error", this._handleNetworkError.bind(this)), this.packetParser = new i((D) => {
        this.handlePacket(D);
      }), this.stream.on("data", (D) => {
        this.connectTimeout && (n.clearTimeout(this.connectTimeout), this.connectTimeout = null), this.packetParser.execute(D);
      }), this.stream.on("end", () => {
        this.emit("end");
      ...
      let C;
      if (this.config.isServer || (C = new I.ClientHandshake(this.config.clientFlags), C.on("end", () => {
        ...
        this._closing = !0, this._notifyError(D);
      }), this.addCommand(C)), this.serverEncoding = "utf8", this.config.connectTimeout) {
        const D = this._handleTimeoutError.bind(this);
    ...
    _notifyError(r) {
      if (this.connectTimeout && (n.clearTimeout(this.connectTimeout), this.connectTimeout = null), this._fatalError)
        return;
      let C, D = !this._command;
      for (this._command && this._command.onResult ? (this._command.onResult(r), this._command = null) : this._command && this._command.constructor === I.ClientHandshake && this._commands.length > 0 || (D = !0); C = this._commands.shift(); )
        C.onResult ? C.onResult(r) : D = !0;
      (D || this._pool) && this.emit("error", r), r.fatal && this.close();
    }
    write(r) {
      this.stream.write(r, (D) => {
        D && this._handleNetworkError(D);
    ...
    writePacket(r) {
      ...
      if (D < 16777215)
        ...
        for (this.config.debug && (console.log(
          ...
          `${this._internalId} ${this.connectionId} <== ${this._command._commandName}#${this._command.stateName()}(${[this.sequenceId, r._name, r.length()].join(",")})`
        )), M = 4; M < 4 + D; M += 16777215)
          B = r.buffer.slice(M, M + 16777215), B.length === 16777215 ? h = Buffer.from([255, 255, 255, this.sequenceId]) : h = Buffer.from([
            B.length & 255,
            B.length >> 8 & 255,
            B.length >> 16 & 255,
            this.sequenceId
    ...
    startTLS(r) {
      ...
      }), D = this.config.ssl.rejectUnauthorized, B = this.config.ssl.verifyIdentity, M = E.isIP(this.config.host) ? void 0 : this.config.host;
      let h = !1;
      ...
      const L = t.connect(
        {
          ...
          requestCert: D,
          checkServerIdentity: B ? t.checkServerIdentity : function() {
          },
          secureContext: C,
          isServer: !1,
          socket: this.stream,
        ...
        () => {
          if (h = !0, D && typeof M == "string" && B) {
            const U = L.getPeerCertificate(!0), d = t.checkServerIdentity(
              M,
              U
            );
            if (d) {
    ...
    protocolError(r, C) {
      if (this._closing)
        return;
      const D = new Error(r);
      D.fatal = !0, D.code = C || "PROTOCOL_ERROR", this.emit("error", D);
    }
    get state() {
    ...
    handlePacket(r) {
      ...
      if (this.config.debug && r) {
        ...
        const C = this._command ? this._command._commandName : "(no command)", D = this._command ? this._command.stateName() : "(no command)";
        console.log(
          `${this._internalId} ${this.connectionId} ==> ${C}#${D}(${[r.sequenceId, r.type(), r.length()].join(",")})`
        );
      }
      if (!this._command) {
        if (r.peekByte() === 255) {
          const D = A.Error.fromPacket(r);
          this.protocolError(D.message, D.code);
        } else
          this.protocolError(
    ...
    format(r, C) {
      ...
      return this._resolveNamedPlaceholders(D), a.format(
        D.sql,
        D.values,
    ...
    escape(r) {
      return a.escape(r, !1, this.config.timezone);
    }
    escapeId(r) {
      return a.escapeId(r, !1);
    }
    raw(r) {
      return a.raw(r);
    }
    _resolveNamedPlaceholders(r) {
      ...
      if (this.config.namedPlaceholders || r.namedPlaceholders) {
        if (Array.isArray(r.values))
          return;
        S === null && (S = _I()()), C = S(r.sql, r.values), r.sql = C[0], r.values = C[1];
      }
    }
    query(r, C, D) {
      let B;
      r.constructor === I.Query ? B = r : B = c.createQuery(r, C, D, this.config), this._resolveNamedPlaceholders(B);
      const M = this.format(
        B.sql,
        B.values !== void 0 ? B.values : []
      );
      return B.sql = M, this.addCommand(B);
    }
    pause() {
    ...
    prepare(r, C) {
      return typeof r == "string" && (r = { sql: r }), this.addCommand(new I.Prepare(r, C));
    }
    unprepare(r) {
      let C = {};
      typeof r == "object" ? C = r : C.sql = r;
      const D = c.statementKey(C), B = this._statements.get(D);
      return B && (this._statements.delete(D), B.close()), B;
    }
    execute(r, C, D) {
      let B = {
      ...
      if (typeof r == "object" ? (B = {
        ...
        sql: r.sql,
        values: r.values
      }, typeof C == "function" ? D = C : B.values = B.values || C) : typeof C == "function" ? (D = C, B.sql = r, B.values = void 0) : (B.sql = r, B.values = C), this._resolveNamedPlaceholders(B), B.values) {
      ...
      const M = new I.Execute(B, D), h = new I.Prepare(B, (L, U) => {
        if (L) {
          M.start = function() {
            return null;
          }, D ? D(L) : M.emit("error", L), M.emit("end");
          return;
        }
        M.statement = U;
      });
      return this.addCommand(h), this.addCommand(M), M;
    }
    changeUser(r, C) {
      !C && typeof r == "function" && (C = r, r = {});
      const D = r.charset ? N.getCharsetNumber(r.charset) : this.config.charsetNumber;
    ...
    _registerSlave(r, C) {
      return this.addCommand(new I.RegisterSlave(r, C));
    }
    _binlogDump(r, C) {
      return this.addCommand(new I.BinlogDump(r, C));
    }
    ...
    createBinlogStream(r) {
      ...
      return D._read = function() {
        ...
        B.on("event", (M) => {
          ...
          D.push(null), r.flags && r.flags & 1 && this.close();
        });
      }), D;
    }
    connect(r) {
      ...
      function D(B) {
        return function(M) {
          C || (B ? r(M) : r(null, M)), C = 1;
        };
      }
      this.once("error", D(!0)), this.once("connect", D(!1));
    }
    // ===================================
    // outgoing server connection methods
    // ===================================
    writeColumns(r) {
      this.writePacket(A.ResultSetHeader.toPacket(r.length)), r.forEach((C) => {
        this.writePacket(
          A.ColumnDefinition.toPacket(C, this.serverConfig.encoding)
        );
      }), this.writeEof();
    }
    ...
    writeTextResult(r, C, D = !1) {
      this.writeColumns(C), r.forEach((B) => {
        const M = new Array(C.length);
        C.forEach((h) => {
          M.push(B[h.name]);
        }), D ? this.writeBinaryRow(M) : this.writeTextRow(M);
      }), this.writeEof();
    }
    writeEof(r, C) {
      this.writePacket(A.EOF.toPacket(r, C));
    }
    writeOk(r) {
    ...
    writeError(r) {
      ...
      this.writePacket(A.Error.toPacket(r, C));
    }
    serverHandshake(r) {
      return this.serverConfig = r, this.serverConfig.encoding = u[this.serverConfig.characterSet], this.addCommand(new I.ServerHandshake(r));
    }
    // ===============================================================
    end(r) {
      if (this.config.isServer) {
        ...
        const D = new _();
        return setImmediate(() => {
          this.stream.end(), D.emit("end");
        }), D;
      }
      const C = this.addCommand(new I.Quit(r));
      return this.addCommand = this._addCommandClosedState, C;
    }
    static createQuery(r, C, D, B) {
      let M = {
        rowsAsArray: B.rowsAsArray,
        infileStreamFactory: B.infileStreamFactory
      };
      return typeof r == "object" ? (M = {
        ...M,
        ...r,
        sql: r.sql,
        values: r.values
      }, typeof C == "function" ? D = C : C !== void 0 && (M.values = C)) : typeof C == "function" ? (D = C, M.sql = r, M.values = void 0) : (M.sql = r, M.values = C), new I.Query(M, D);
    }
...
function jR() {
  ...
  return t.statementKey = E.statementKey, fn = t, t.prototype._realEnd = E.prototype.end, fn;
}
var gn, xA;
function GE() {
  if (xA) return gn;
  ...
  function E(t, n, _) {
    return function(R, T, a) {
      R ? (_.message = R.message, _.code = R.code, _.errno = R.errno, _.sql = R.sql, _.sqlState = R.sqlState, _.sqlMessage = R.sqlMessage, n(_)) : t([T, a]);
    };
  }
  return gn = E, gn;
}
...
function nI() {
  ...
  class t {
    ...
    execute(_) {
      const R = this.statement, T = new Error();
      return new this.Promise((a, e) => {
        const i = E(a, e, T);
        _ ? R.execute(_, i) : R.execute(i);
      });
    }
    close() {
...
function Ee() {
  ...
  function E(t, n, _) {
    const R = {};
    n.on("newListener", (T) => {
      _.indexOf(T) >= 0 && !n.listenerCount(T) && t.on(
        T,
        R[T] = function() {
          const a = [].slice.call(arguments);
          a.unshift(T), n.emit.apply(n, a);
        }
      );
    }).on("removeListener", (T) => {
      _.indexOf(T) >= 0 && !n.listenerCount(T) && (t.removeListener(T, R[T]), delete R[T]);
    });
  }
  return dn = E, dn;
}
...
function _e() {
  if (_R) return wn;
  _R = 1;
  const E = aE.EventEmitter, t = nI(), n = GE(), _ = Ee(), R = xE();
  class T extends E {
    constructor(e, i) {
      super(), this.connection = e, this.Promise = i || Promise, _(e, this, [
        "error",
        "drain",
        "connect",
        "end",
    ...
    query(e, i) {
      ...
      return new this.Promise((N, u) => {
        const O = n(N, u, I);
        i !== void 0 ? A.query(e, i, O) : A.query(e, O);
      });
    }
    execute(e, i) {
      const A = this.connection, I = new Error();
      ...
      return new this.Promise((N, u) => {
        const O = n(N, u, I);
        i !== void 0 ? A.execute(e, i, O) : A.execute(e, O);
      });
    }
    end() {
    ...
    beginTransaction() {
      const e = this.connection, i = new Error();
      return new this.Promise((A, I) => {
        const N = n(A, I, i);
        e.beginTransaction(N);
      });
    }
    commit() {
      const e = this.connection, i = new Error();
    ...
    ping() {
      ...
      return new this.Promise((A, I) => {
        e.ping((N) => {
          N ? (i.message = N.message, i.code = N.code, i.errno = N.errno, i.sqlState = N.sqlState, i.sqlMessage = N.sqlMessage, I(i)) : A(!0);
        });
      });
    }
    connect() {
      ...
      return new this.Promise((A, I) => {
        e.connect((N, u) => {
          N ? (i.message = N.message, i.code = N.code, i.errno = N.errno, i.sqlState = N.sqlState, i.sqlMessage = N.sqlMessage, I(i)) : A(u);
        });
      });
    }
    prepare(e) {
      const i = this.connection, A = this.Promise, I = new Error();
      return new this.Promise((N, u) => {
        i.prepare(e, (O, S) => {
          if (O)
            I.message = O.message, I.code = O.code, I.errno = O.errno, I.sqlState = O.sqlState, I.sqlMessage = O.sqlMessage, u(I);
          else {
            const c = new t(
              S,
              A
    ...
    changeUser(e) {
      const i = this.connection, A = new Error();
      return new this.Promise((I, N) => {
        i.changeUser(e, (u) => {
          u ? (A.message = u.message, A.code = u.code, A.errno = u.errno, A.sqlState = u.sqlState, A.sqlMessage = u.sqlMessage, N(A)) : I();
        });
      });
    }
    get config() {
  ...
  return (function(a) {
    for (let e = 0; a && e < a.length; e++) {
      const i = a[e];
      typeof R.prototype[i] == "function" && T.prototype[i] === void 0 && (T.prototype[i] = /* @__PURE__ */ (function(I) {
        return function() {
...
function $E() {
  ...
  class n extends E {
    ...
    destroy() {
      return t.prototype.destroy.apply(
        this.connection,
        arguments
      );
    }
...
function kR() {
  if (AR) return Qn;
  AR = 1;
  const E = zE, t = qn(), n = aE.EventEmitter, _ = eI(), R = VR(), T = xE();
  function a(i, A) {
    const I = i.length;
    for (let N = 0; N < I; N++)
      if (i.get(N) === A) {
  ...
  class e extends n {
    ...
    getConnection(A) {
      ...
      }), this._allConnections.push(I), I.connect((N) => this._closed ? A(new Error("Pool is closed.")) : N ? A(N) : (this.emit("connection", I), this.emit("acquire", I), A(null, I)))) : this.config.waitForConnections ? this.config.queueLimit && this._connectionQueue.length >= this.config.queueLimit ? A(new Error("Queue limit reached.")) : (this.emit("enqueue"), this._connectionQueue.push(A)) : E.nextTick(() => A(new Error("No connections available.")));
    }
    releaseConnection(A) {
      let I;
      ...
      let I = !1, N = 0, u;
      const O = (function(S) {
        if (!I && (S || ++N >= this._allConnections.length)) {
          I = !0, A(S);
          return;
        }
      }).bind(this);
      ...
      for (let S = 0; S < this._allConnections.length; S++)
        u = this._allConnections.get(S), u._realEnd(O);
    }
    query(A, I, N) {
      const u = T.createQuery(
        A,
        I,
        N,
        this.config.connectionConfig
      );
      return typeof u.namedPlaceholders > "u" && (u.namedPlaceholders = this.config.connectionConfig.namedPlaceholders), this.getConnection((O, S) => {
        if (O) {
          typeof u.onResult == "function" ? u.onResult(O) : u.emit("error", O);
          return;
        }
        try {
          S.query(u).once("end", () => {
    ...
    execute(A, I, N) {
      typeof I == "function" && (N = I, I = []), this.getConnection((u, O) => {
        if (u)
          return N(u);
        try {
          O.execute(A, I, N).once("end", () => {
            O.release();
          });
        } catch (S) {
          return O.release(), N(S);
        }
      });
    }
    _removeConnection(A) {
      a(this._allConnections, A), a(this._freeConnections, A), this.releaseConnection(A);
    }
...
function JR() {
  if (RR) return Pn;
  RR = 1;
  const E = aE.EventEmitter, t = GE(), n = $E(), _ = Ee(), R = kR();
  class T extends E {
    constructor(e, i) {
      super(), this.pool = e, this.Promise = i || Promise, _(e, this, ["acquire", "connection", "enqueue", "release"]);
    }
    getConnection() {
      const e = this.pool;
      return new this.Promise((i, A) => {
        e.getConnection((I, N) => {
          I ? A(I) : i(new n(N, this.Promise));
        });
      });
    }
    releaseConnection(e) {
      e instanceof n && e.release();
    }
    query(e, i) {
    ...
    execute(e, i) {
      ...
      return new this.Promise((N, u) => {
        const O = t(N, u, I);
        i ? A.execute(e, i, O) : A.execute(e, O);
      });
    }
    end() {
      const e = this.pool, i = new Error();
      return new this.Promise((A, I) => {
        e.end((N) => {
          N ? (i.message = N.message, i.code = N.code, i.errno = N.errno, i.sqlState = N.sqlState, i.sqlMessage = N.sqlMessage, I(i)) : A();
        });
      });
    }
  }
...
function pR() {
  if (NR) return Yn;
  NR = 1;
  const E = zE, t = zR(), n = ZR(), _ = qR(), R = aE.EventEmitter, T = {
    RR() {
      let I = 0;
      return (N) => N[I++ % N.length];
    },
    RANDOM() {
    ...
    if (I instanceof RegExp)
      return I;
    const N = I.replace(/([.+?^=!:${}()|[\]/\\])/g, "\\$1").replace(/\*/g, ".*");
    return new RegExp(`^${N}$`);
  };
  class i {
    constructor(N, u, O) {
      this._cluster = N, this._pattern = u, this._selector = T[O]();
    }
    getConnection(N) {
      const u = this._getClusterNode();
      if (u === null) {
        let O = new Error("Pool does Not exist.");
        return O.code = "POOL_NOEXIST", this._cluster._findNodeIds(this._pattern, !0).length !== 0 && (O = new Error("Pool does Not have online node."), O.code = "POOL_NONEONLINE"), N(O);
      }
      return this._cluster._getConnection(u, (O, S) => O ? this._cluster._canRetry && this._cluster._findNodeIds(this._pattern).length !== 0 ? (this._cluster.emit("warn", O), this.getConnection(N)) : N(O) : N(null, S));
    }
    /**
     * pool cluster query
     * @param {*} sql
     ...
     */
    query(N, u, O) {
      const S = _.createQuery(N, u, O, {});
      return this.getConnection((c, s) => {
        if (c) {
          typeof S.onResult == "function" ? S.onResult(c) : S.emit("error", c);
          return;
        }
        try {
          s.query(S).once("end", () => {
     ...
     */
    execute(N, u, O) {
      typeof u == "function" && (O = u, u = []), this.getConnection((S, c) => {
        if (S)
          return O(S);
        try {
          c.execute(N, u, O).once("end", () => {
            c.release();
          });
        } catch (s) {
          throw c.release(), s;
    ...
    _getClusterNode() {
      const N = this._cluster._findNodeIds(this._pattern);
      if (N.length === 0)
        return null;
      const u = N.length === 1 ? N[0] : this._selector(N);
      return this._cluster._getNode(u);
    }
  }
  class A extends R {
    constructor(N) {
      super(), N = N || {}, this._canRetry = typeof N.canRetry > "u" ? !0 : N.canRetry, this._removeNodeErrorCount = N.removeNodeErrorCount || 5, this._restoreNodeTimeout = N.restoreNodeTimeout || 0, this._defaultSelector = N.defaultSelector || "RR", this._closed = !1, this._lastId = 0, this._nodes = {}, this._serviceableNodeIds = [], this._namespaces = {}, this._findCaches = {};
    }
    of(N, u) {
      N = N || "*", u = u || this._defaultSelector, u = u.toUpperCase(), !T[u] === "undefined" && (u = this._defaultSelector);
      const O = N + u;
      return typeof this._namespaces[O] > "u" && (this._namespaces[O] = new i(this, N, u)), this._namespaces[O];
    }
    add(N, u) {
      typeof N == "object" && (u = N, N = `CLUSTER::${++this._lastId}`), typeof this._nodes[N] > "u" && (this._nodes[N] = {
        id: N,
        errorCount: 0,
        pool: new t({ config: new n(u) }),
        _offlineUntil: 0
      }, this._serviceableNodeIds.push(N), this._clearFindCaches());
    }
    remove(N) {
      const u = this._findNodeIds(N, !0);
      for (let O = 0; O < u.length; O++) {
        const S = this._getNode(u[O]);
        S && this._removeNode(S);
      }
    }
    getConnection(N, u, O) {
      let S;
      typeof N == "function" ? (O = N, S = this.of()) : (typeof u == "function" && (O = u, u = this._defaultSelector), S = this.of(N, u)), S.getConnection(O);
    }
    end(N) {
      const u = N !== void 0 ? N : (s) => {
        if (s)
          throw s;
      };
      if (this._closed) {
      ...
      for (const s in this._nodes)
        S++, this._nodes[s].pool.end(c);
      S === 0 && E.nextTick(c);
    }
    _findNodeIds(N, u) {
      let O = 0, S = this._findCaches[N];
      if (S === void 0) {
        const c = e(N);
      ...
      return this._findCaches[N] = S, u ? S : S.filter((c) => {
        const s = this._getNode(c);
        return s._offlineUntil ? (O || (O = a()), s._offlineUntil <= O) : !0;
      });
    }
    _getNode(N) {
      return this._nodes[N] || null;
    }
    _increaseErrorCount(N) {
      const u = ++N.errorCount;
      if (!(this._removeNodeErrorCount > u)) {
        if (this._restoreNodeTimeout > 0) {
          N._offlineUntil = a() + this._restoreNodeTimeout, this.emit("offline", N.id);
          return;
        }
        this._removeNode(N), this.emit("remove", N.id);
      }
    }
    _decreaseErrorCount(N) {
      let u = N.errorCount;
      u > this._removeNodeErrorCount && (u = this._removeNodeErrorCount), u < 1 && (u = 1), N.errorCount = u - 1, N._offlineUntil && (N._offlineUntil = 0, this.emit("online", N.id));
    }
    _getConnection(N, u) {
      N.pool.getConnection((O, S) => O ? (this._increaseErrorCount(N), u(O)) : (this._decreaseErrorCount(N), S._clusterId = N.id, u(null, S)));
    }
    _removeNode(N) {
      const u = this._serviceableNodeIds.indexOf(N.id);
      u !== -1 && (this._serviceableNodeIds.splice(u, 1), delete this._nodes[N.id], this._clearFindCaches(), N.pool.end());
    }
    _clearFindCaches() {
      this._findCaches = {};
    }
...
function iI() {
  ...
  class n {
    constructor(R, T) {
      this.poolNamespace = R, this.Promise = T || Promise;
    }
    getConnection() {
      const R = this.poolNamespace;
      return new this.Promise((T, a) => {
        R.getConnection((e, i) => {
          e ? a(e) : T(new E(i, this.Promise));
        });
      });
    }
    query(R, T) {
      const a = this.poolNamespace, e = new Error();
...
function II() {
  return cR || (cR = 1, (function(E) {
    const t = qn(), n = aE.EventEmitter, _ = pn(), R = pR(), T = AI(), a = RI(), e = tI(), i = _e(), A = JR(), I = GE(), N = $E(), u = Ee(), O = iI();
    function S(C) {
      const D = T(C), B = new Error(), M = C.Promise || Promise;
      if (!M)
        throw new Error(
    ...
    function c(C) {
      const D = a(C), B = C.Promise || Promise;
      if (!B)
        throw new Error(
          "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
        );
    ...
    class s extends n {
      constructor(D, B) {
        super(), this.poolCluster = D, this.Promise = B || Promise, u(D, this, ["warn", "remove", "online", "offline"]);
      }
      getConnection(D, B) {
        const M = this.poolCluster;
        return new this.Promise((h, L) => {
          M.getConnection(
            D,
            B,
            (U, d) => {
              U ? L(U) : h(new N(d, this.Promise));
            }
          );
        });
      }
      query(D, B) {
        const M = this.poolCluster, h = new Error();
      ...
      of(D, B) {
        return new O(
          this.poolCluster.of(D, B),
          this.Promise
        );
      }
      end() {
        const D = this.poolCluster, B = new Error();
        return new this.Promise((M, h) => {
          D.end((L) => {
            L ? (B.message = L.message, B.code = L.code, B.errno = L.errno, B.sqlState = L.sqlState, B.sqlMessage = L.sqlMessage, h(B)) : M();
          });
        });
      }
    }
    (function(C) {
      for (let D = 0; C && D < C.length; D++) {
        const B = C[D];
        typeof R.prototype[B] == "function" && s.prototype[B] === void 0 && (s.prototype[B] = /* @__PURE__ */ (function(h) {
          return function() {
    ...
    function r(C) {
      const D = e(C), B = C && C.Promise || Promise;
      if (!B)
        throw new Error(
          "no Promise implementation available.Use promise-enabled node version or pass userland Promise implementation as parameter, for example: { Promise: require('bluebird') }"
        );
    ...
    E.createConnection = S, E.createPool = c, E.createPoolCluster = r, E.escape = t.escape, E.escapeId = t.escapeId, E.format = t.format, E.raw = t.raw, E.PromisePool = A, E.PromiseConnection = i, E.PromisePoolConnection = N, E.__defineGetter__("Types", () => TE()), E.__defineGetter__(
      "Charsets",
      () => hE()
    ), E.__defineGetter__(
      "CharsetToEncoding",
...
function rI() {
  if (!XE) {
    const E = process.env.MYSQL_PORT ? Number(process.env.MYSQL_PORT) : 3306, t = (process.env.MYSQL_SSL || "").toLowerCase(), n = t === "1" || t === "true" || t === "yes", _ = (process.env.MYSQL_SSL_REJECT_UNAUTHORIZED || "").toLowerCase(), R = !(_ === "0" || _ === "false" || _ === "no");
    XE = TI.createPool({
      ...
      ssl: n ? { rejectUnauthorized: R } : void 0,
...
async function V(E) {
  ...
  try {
    return { ok: !0, value: await E(rI()) };
  } catch (t) {
    return console.warn("[MySQL] Error:", t), { ok: !1, error: t };
  }
}
function LE(E) {
...
function OI(E) {
  let n = Q().prepare(`
    ...
    ORDER BY h.hora
  `).all(E, E);
  return xR(E) && (n = n.filter((_) => _.hora < "12:00")), n;
}
async function sI(E) {
  const t = new Date(E).toISOString().split("T")[0], n = await V(async (_) => {
    const [R] = await _.execute(
      `
        SELECT h.hora
        FROM horarios_base h
...
async function DI(E) {
  if (console.log("[Service] Desactivando horario:", E), (await V(async (n) => {
...
function SI() {
  ...
  `).all();
  return console.log("[Service] Horarios inactivos encontrados:", t.length), t;
}
async function BI() {
  console.log("[Service] Obteniendo horarios inactivos");
  const E = await V(async (t) => {
    const [n] = await t.execute(
      "SELECT id, hora FROM horarios_base WHERE activo = 0 ORDER BY hora"
    );
    return n;
  });
  return E.ok ? E.value : SI();
}
function CR(E) {
  const t = Q();
  t.transaction(() => {
...
function DR(E, t, n) {
  console.log("[Service] Bloqueando horario:", { fecha: E, hora: t, motivo: n });
  const _ = Q(), R = new Date(E).toISOString().split("T")[0], T = LE(t);
  console.log("[Service] Fecha normalizada:", E, "->", R), console.log("[Service] Hora normalizada:", t, "->", T);
  try {
    _.transaction(() => {
      if (_.prepare(`
      ...
      _.prepare(`
        ...
        VALUES ( ?, ?, ?)
      `).run(R, T, n ?? ""), console.log("[Service] Horario bloqueado");
    })();
  } catch (a) {
    throw console.error("[Service] Error en bloquearHorario:", a), a;
  }
}
async function MI(E, t, n) {
  console.log("[Service] Bloqueando horario:", { fecha: E, hora: t, motivo: n });
  const _ = new Date(E).toISOString().split("T")[0], R = LE(t);
  if ((await V(async (a) => {
    const [e] = await a.execute(
      "SELECT id FROM bloqueos_horarios WHERE fecha = ? AND hora = ?",
      [_, R]
    );
    e.length || await a.execute(
    ...
    try {
      ...
      console.warn("[Service] Backup SQLite fallo en bloquearHorario:", a);
    }
    return;
  }
  return DR(_, R, n);
...
function SR(E, t) {
  console.log("[Service] Desbloqueando horario:", { fecha: E, hora: t });
  const n = Q(), _ = new Date(E).toISOString().split("T")[0], R = LE(t);
  console.log("[Service] Fecha normalizada:", E, "->", _), console.log("[Service] Hora normalizada:", t, "->", R);
  try {
    n.transaction(() => {
      n.prepare(`
...
async function hI(E, t) {
  console.log("[Service] Desbloqueando horario:", { fecha: E, hora: t });
  const n = new Date(E).toISOString().split("T")[0], _ = LE(t);
  if ((await V(async (T) => {
    await T.execute(
...
function LI(E) {
  ...
  const t = Q(), n = new Date(E).toISOString().split("T")[0];
  ...
  const R = t.prepare("SELECT * FROM bloqueos_horarios ORDER BY fecha, hora").all();
...
async function fI(E) {
  if (console.log("[Service] Borrando horario permanentemente:", E), (await V(async (n) => {
...
async function j(E) {
  return new Promise((t, n) => {
    const _ = `op_${++gI}`, R = Date.now();
    sE.push({ id: _, fn: E, resolve: t, reject: n, createdAt: R }), console.log(`[Lock] ${_} encolada. Cola: ${sE.length} operaciones. Locked: ${KE}`), $R();
  });
}
async function $R() {
  if (KE) {
  ...
  KE = !0;
  const E = sE.shift(), n = Date.now() - E.createdAt;
  console.log(`[Lock] INICIANDO ${E.id} (esperó ${n}ms). Quedan: ${sE.length}`);
  try {
    const _ = E.fn();
    ...
    KE = !1, console.log(`[Lock] ${E.id} liberada. Quedan: ${sE.length}`), sE.length > 0 && (console.log("[Lock] Procesando siguiente..."), setImmediate(() => $R()));
  }
}
function UI() {
  m("horarios:base", async () => {
    ...
    "horarios:crear",
    async (E, t) => await j(() => CI(t))
  ), m("horarios:desactivar", async (E, t) => {
    console.log("[IPC] Desactivando horario:", t);
    const n = await j(() => DI(t));
    return console.log("[IPC] Horario desactivado exitosamente"), n;
  }), m("horarios:activar", async (E, t) => {
    console.log("[IPC] Activando horario:", t);
    const n = await j(() => lI(t));
    return console.log("[IPC] Horario activado exitosamente"), n;
  }), m("horarios:inactivos", async () => {
    ...
    "horarios:bloquear",
    async (E, t) => await j(
      () => MI(t.fecha, t.hora, t.motivo)
    )
  ), m("horarios:desbloquear", async (E, t) => {
    console.log("[IPC] Desbloqueando horario:", t);
    const n = await j(
      () => hI(t.fecha, t.hora)
    );
    return console.log("[IPC] Horario desbloqueado exitosamente"), n;
  }), m("horarios:bloqueados", async (E, t) => {
    console.log("[IPC] Obteniendo horarios bloqueados para:", t);
    const n = await j(() => oI(t));
    return console.log("[IPC] Horarios bloqueados obtenidos:", n), n;
  }), m("horarios:borrar", async (E, t) => {
    console.log("[IPC] Borrando horario permanentemente:", t);
    const n = await j(() => fI(t));
...
function FI(E) {
  const t = E.tipo_turno;
  return t !== "Garantía" && (E.garantia_tipo = null, E.garantia_fecha_compra = null, E.garantia_numero_service = null, E.garantia_problema = null), t !== "Particular" && (E.particular_tipo = null), E;
}
function QI(E) {
  return String(E || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
}
function PI(E) {
  const t = E.tipo_turno, n = /^\d+$/.test(String(E.km || "").trim());
  if (t === "Particular") {
...
async function Et(E, t = 0) {
  try {
    return console.log(`[Service] Intento ${t + 1}/${lR}`), E();
  } catch (n) {
    if (n.code === "SQLITE_BUSY" && t < lR - 1)
      return console.warn(`[Service] SQLITE_BUSY, reintentando en ${MR}ms...`), await new Promise((_) => setTimeout(_, MR)), Et(E, t + 1);
    throw n;
  }
}
async function hR(E, t) {
  return Et(() => {
    const n = Q(), _ = n.transaction(() => {
...
async function bI(E) {
  console.log("[Service] Iniciando crearReserva..."), GI(E), PI(E);
  const t = FI({ ...E }), n = new Date(t.fecha).toISOString().split("T")[0];
  console.log("[Service] Fecha normalizada:", t.fecha, "->", n);
  try {
    const _ = await VI(t, n);
    try {
      await hR(t, n);
    } catch (R) {
    ...
    return console.warn("[Service] MySQL no disponible, usando SQLite local"), await hR(t, n);
  }
}
async function Xn(E) {
  console.log("[Service] Obteniendo reserva:", E);
  const t = await V(async (_) => {
    const [R] = await _.execute("SELECT * FROM reservas WHERE id = ?", [E]);
    return R[0] ?? null;
  });
  return t.ok ? t.value : Q().prepare("SELECT * FROM reservas WHERE id = ?").get(E);
}
async function mI(E) {
  if (console.log("[Service] Borrando reserva:", E), (await V(async (_) => {
    const [R] = await _.execute("SELECT * FROM reservas WHERE id = ?", [E]), T = R[0];
    if (!T) {
  ...
  try {
    n.transaction(() => {
      const R = n.prepare("SELECT * FROM reservas WHERE id = ?").get(E);
      if (!R) {
...
async function YI(E, t, n) {
  if (console.log("[Service] Moviendo reserva:", { id: E, nuevaFecha: t, nuevaHora: n }), (await V(async (T) => {
    ...
    try {
      const T = Q();
      T.transaction(() => {
        const e = T.prepare("SELECT fecha, hora FROM reservas WHERE id = ?").get(E);
        e && (T.prepare("UPDATE reservas SET fecha = ?, hora = COALESCE( ?, hora) WHERE id = ?").run(t, n ?? null, E), t !== e.fecha && T.prepare(`
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
            VALUES ( ?, 'fecha', ?, ?, datetime('now'))
          `).run(E, e.fecha, t), n && n !== e.hora && T.prepare(`
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
  ...
  try {
    R.transaction(() => {
      const a = R.prepare("SELECT fecha, hora FROM reservas WHERE id = ?").get(E);
      if (!a) {
        console.log("[Service] Reserva no encontrada para mover:", E);
        return;
      }
      R.prepare("UPDATE reservas SET fecha = ?, hora = COALESCE( ?, hora) WHERE id = ?").run(t, n ?? null, E), t !== a.fecha && R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
          VALUES ( ?, 'fecha', ?, ?, datetime('now'))
        `).run(E, a.fecha, t), n && n !== a.hora && R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
          VALUES ( ?, 'hora', ?, ?, datetime('now'))
        `).run(E, a.hora, n);
...
async function WI(E, t) {
  console.log("[Service] Actualizando reserva:", E, t);
  const n = QI((t == null ? void 0 : t.matricula) || "");
  if (!/^[A-Z]{3}\d{3,4}$/.test(n))
  ...
  if ((await V(async (T) => {
    ...
    await T.execute(
      ...
      [t.nombre, t.fecha, t.hora, t.estado, t.detalles, n, t.id]
    );
    for (const i of Object.keys(e))
      e[i] !== t[i] && await T.execute(
        `
            INSERT INTO historial_reservas
            (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
            VALUES ( ?, ?, ?, ?, NOW())
    ...
    try {
      ...
      console.warn("[Service] Backup SQLite fallo en actualizarReserva:", T);
    }
    return;
  }
  const R = Q();
  try {
    ...
    throw console.error("[Service] Error en actualizarReserva:", T), T;
  }
}
function ne(E) {
  if (!(!Array.isArray(E) || E.length === 0))
    try {
      const t = Q(), n = t.prepare("SELECT id FROM reservas WHERE id = ?"), _ = t.prepare(`
    ...
    `);
      t.transaction(() => {
        for (const e of E) {
          const i = e != null && e.id ? Number(e.id) : null;
          let A = null;
          if (i) {
            const N = n.get(i);
            N != null && N.id && (A = N.id);
          ...
          if (!A) {
            const N = _.get(
              (e == null ? void 0 : e.fecha) ?? "",
              (e == null ? void 0 : e.hora) ?? "",
              (e == null ? void 0 : e.cedula) ?? "",
              (e == null ? void 0 : e.matricula) ?? ""
            );
            N != null && N.id && (A = N.id);
          }
          const I = [
            ...
            (e == null ? void 0 : e.cedula) ?? "",
            (e == null ? void 0 : e.telefono) ?? "",
            (e == null ? void 0 : e.marca) ?? "",
            (e == null ? void 0 : e.modelo) ?? "",
            (e == null ? void 0 : e.km) ?? "",
            (e == null ? void 0 : e.matricula) ?? "",
            (e == null ? void 0 : e.tipo_turno) ?? "",
            (e == null ? void 0 : e.particular_tipo) ?? null,
            (e == null ? void 0 : e.garantia_tipo) ?? null,
            (e == null ? void 0 : e.garantia_fecha_compra) ?? null,
            (e == null ? void 0 : e.garantia_numero_service) ?? null,
            (e == null ? void 0 : e.garantia_problema) ?? null,
            (e == null ? void 0 : e.fecha) ?? "",
            (e == null ? void 0 : e.hora) ?? "",
            (e == null ? void 0 : e.detalles) ?? null,
            (e == null ? void 0 : e.estado) ?? "pendiente",
            (e == null ? void 0 : e.notas) ?? null
          ];
          A ? T.run(
            ...I,
            A
      ...
      console.warn("[Service] Error sincronizando reservas MySQL -> SQLite:", t);
    }
}
async function HI(E, t) {
  console.log("[Service] Obteniendo reservas entre:", E, "y", t);
  const n = new Date(E).toISOString().split("T")[0], _ = new Date(t).toISOString().split("T")[0];
  console.log("[Service] Fechas normalizadas:", n, "a", _);
  const R = await V(async (e) => {
  ...
  return R.ok ? (ne(R.value), R.value) : Q().prepare(`
    SELECT * FROM reservas
    WHERE fecha >= ? AND fecha <= ?
    ORDER BY fecha, hora
  `).all(n, _);
...
async function _t(E) {
  const t = new Date(E).toISOString().split("T")[0], n = await V(async (R) => {
  ...
  return n.ok ? (ne(n.value), n.value) : Q().prepare(`
    SELECT * FROM reservas
    WHERE fecha = ?
    ORDER BY hora
  `).all(t);
...
async function vI() {
  ...
  return E.ok ? (ne(E.value), E.value) : Q().prepare(`
    SELECT * FROM reservas
...
async function yI(E, t) {
  if (console.log("[Service] Actualizando notas para reserva:", E), (await V(async (R) => {
    ...
    a && (await R.execute("UPDATE reservas SET notas = ? WHERE id = ?", [t, E]), await R.execute(
      `
        INSERT INTO historial_reservas
        (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
        VALUES ( ?, 'notas', ?, ?, NOW())
    ...
    try {
      const R = Q(), T = R.prepare("SELECT notas FROM reservas WHERE id = ?").get(E);
      if (!T) return;
      R.transaction(() => {
        R.prepare("UPDATE reservas SET notas = ? WHERE id = ?").run(t, E), R.prepare(`
          INSERT INTO historial_reservas
          (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
  ...
  try {
    ...
    _.transaction(() => {
      _.prepare("UPDATE reservas SET notas = ? WHERE id = ?").run(t, E), _.prepare(`
        INSERT INTO historial_reservas
        (reserva_id, campo, valor_anterior, valor_nuevo, fecha)
        VALUES ( ?, 'notas', ?, ?, datetime('now'))
      `).run(E, R.notas || "", t);
...
async function XI(E, t = 0, n = 200) {
  console.log("[Service] Buscando cambios de reservas desde:", E, "id>", t);
  const _ = await V(async (a) => {
    const [e] = await a.execute(
      `
...
const KI = "daily-summary.json", BE = {
  enabled: !1,
  sendTime: "07:30",
  recipients: [],
  lastSentDate: ""
...
function et(E) {
  ...
  if (!/^\d{2}:\d{2}$/.test(t)) return BE.sendTime;
  const [n, _] = t.split(":").map(Number);
  return !Number.isFinite(n) || !Number.isFinite(_) || n < 0 || n > 23 || _ < 0 || _ > 59 ? BE.sendTime : `${String(n).padStart(2, "0")}:${String(_).padStart(2, "0")}`;
}
function At(E) {
  const t = /* @__PURE__ */ new Set();
  for (const n of E || []) {
    const _ = String(n || "").trim().toLowerCase();
    _ && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(_) && t.add(_);
  }
  return Array.from(t);
}
function jn() {
...
function FE() {
  try {
    ...
    if (!z.existsSync(E))
      return { ...BE };
    const t = z.readFileSync(E, "utf-8"), n = JSON.parse(t);
    return !n || typeof n != "object" ? { ...BE } : {
      enabled: !!n.enabled,
      sendTime: et(String(n.sendTime || BE.sendTime)),
      recipients: At(Array.isArray(n.recipients) ? n.recipients : []),
      lastSentDate: String(n.lastSentDate || "")
    };
  } catch (E) {
    return console.warn("[ResumenDiario] No se pudo leer config:", E), { ...BE };
  }
}
function jI(E) {
  const t = nt(), n = x.dirname(t);
  z.existsSync(n) || z.mkdirSync(n, { recursive: !0 }), z.writeFileSync(t, JSON.stringify(E, null, 2), "utf-8");
}
function JE(E) {
  const t = FE(), n = {
    enabled: typeof E.enabled == "boolean" ? E.enabled : t.enabled,
    sendTime: E.sendTime ? et(E.sendTime) : t.sendTime,
    recipients: Array.isArray(E.recipients) ? At(E.recipients) : t.recipients,
    lastSentDate: typeof E.lastSentDate == "string" ? E.lastSentDate : t.lastSentDate
  };
  return jI(n), n;
}
function kI() {
...
function JI(E) {
  const t = (process.env.SMTP_SECURE || "").toLowerCase();
  return t === "1" || t === "true" || t === "yes" ? !0 : t === "0" || t === "false" || t === "no" ? !1 : E === 465;
}
function zI() {
...
function ZI() {
  const E = String(process.env.SMTP_HOST || "").trim(), t = String(process.env.SMTP_USER || "").trim(), n = String(process.env.SMTP_PASS || "").trim(), _ = String(process.env.SMTP_FROM || t).trim(), R = kI(), T = JI(R), a = zI();
  return { host: E, port: R, secure: T, user: t, pass: n, from: _, rejectUnauthorized: a };
}
function qI(E, t) {
  const n = [
    ...
    const T = R != null && R.hora ? String(R.hora) : "--:--", a = R != null && R.nombre ? String(R.nombre) : "Sin nombre", e = R != null && R.telefono ? String(R.telefono) : "-", i = R != null && R.matricula ? String(R.matricula) : "-", A = R != null && R.estado ? String(R.estado) : "Pendiente";
    return `${T} | ${a} | ${e} | ${i} | ${A}`;
  });
...
async function Rt(E) {
  const t = FE();
  if (!t.enabled) return { ok: !1, reason: "disabled" };
  if (!t.recipients.length) return { ok: !1, reason: "no_recipients" };
  const n = ZI();
  if (!n.host || !n.user || !n.pass || !n.from)
    return { ok: !1, reason: "smtp_missing" };
  const _ = await _t(E), R = (Array.isArray(_) ? _ : []).filter((i) => String((i == null ? void 0 : i.estado) || "").trim().toLowerCase() !== "cancelada"), T = qI(E, R);
  return await (await import("./nodemailer-7I4dxtao.js").then((i) => i.n)).createTransport({
    host: n.host,
    port: n.port,
    secure: n.secure,
    tls: {
    ...
    text: T
  }), JE({ lastSentDate: E }), { ok: !0, count: R.length };
}
function pI(E) {
  const t = /* @__PURE__ */ new Date(), n = String(t.getHours()).padStart(2, "0"), _ = String(t.getMinutes()).padStart(2, "0");
  return `${n}:${_}` === E.sendTime;
}
async function oR() {
  const E = FE();
  if (!E.enabled || !pI(E)) return;
  const t = jn();
  if (E.lastSentDate !== t)
    try {
      const n = await Rt(t);
...
function xI() {
  LR || (LR = !0, oR(), setInterval(oR, 60 * 1e3));
}
async function $I(E) {
  const t = String(E || jn()).trim(), n = /^\d{4}-\d{2}-\d{2}$/.test(t) ? t : jn(), _ = FE();
  _.enabled || JE({ enabled: !0 });
  try {
    return await Rt(n);
  } finally {
    _.enabled || JE({ enabled: !1 });
  }
}
function EN() {
  const E = (n, _) => {
    for (const R of UR.getAllWindows())
      R.isDestroyed() || R.webContents.send(n, _);
  }, t = async (n, _, R) => {
    let T = null;
    try {
      T = await Xn(_);
    ...
    } : { id: _, ...R || {} }, e = n === "creada" ? "Nueva reserva" : n === "eliminada" ? "Reserva eliminada" : "Reserva modificada", A = [
      a != null && a.nombre ? String(a.nombre) : "Cliente sin nombre",
      a != null && a.fecha ? String(a.fecha) : "",
      a != null && a.hora ? String(a.hora) : "",
      a != null && a.tipo_turno ? String(a.tipo_turno) : ""
    ].filter(Boolean).join(" · "), I = wI();
    if (Re.isSupported())
      try {
        new Re({
          title: e,
...
` + "=".repeat(50)), console.log("[IPC] Recibiendo solicitud de reserva:"), console.log(_), console.log("=".repeat(50)), console.log("[IPC] Esperando lock...");
    const T = await j(async () => (console.log("[IPC] Lock adquirido, ejecutando crearReserva"), await bI(_))), a = Date.now() - R;
    return console.log(`[IPC] Reserva creada exitosamente en ${a}ms, retornando ID:`, T), console.log("=".repeat(50) + `
`), typeof T == "number" && await t("creada", T, {
      nombre: _ == null ? void 0 : _.nombre,
      fecha: _ == null ? void 0 : _.fecha,
      hora: _ == null ? void 0 : _.hora,
      tipo_turno: _ == null ? void 0 : _.tipo_turno
    }), T;
  }), m("reservas:obtener", (n, _) => (console.log("[IPC] Obteniendo reserva:", _), Xn(_))), m("reservas:borrar", async (n, _) => {
    ...
    const T = await j(() => mI(_));
    return console.log("[IPC] Reserva borrada exitosamente"), await t("eliminada", _, {
      nombre: R == null ? void 0 : R.nombre,
      fecha: R == null ? void 0 : R.fecha,
      hora: R == null ? void 0 : R.hora,
      tipo_turno: R == null ? void 0 : R.tipo_turno
    }), T;
  }), m("reservas:mover", async (n, _) => {
    console.log("[IPC] Moviendo reserva:", _);
    const R = await j(
      () => YI(_.id, _.nuevaFecha, _.nuevaHora)
    );
    return console.log("[IPC] Reserva movida exitosamente"), _ != null && _.id && await t("modificada", _.id, {
      fecha: _.nuevaFecha,
      hora: _.nuevaHora
    }), R;
  }), m("reservas:actualizar", async (n, _) => {
    console.log("[IPC] Actualizando reserva:", _);
    const R = await j(
      () => WI(_.id, _)
    );
    return console.log("[IPC] Reserva actualizada exitosamente"), _ != null && _.id && await t("modificada", _.id, {
      nombre: _ == null ? void 0 : _.nombre,
      fecha: _ == null ? void 0 : _.fecha,
      hora: _ == null ? void 0 : _.hora,
      tipo_turno: _ == null ? void 0 : _.tipo_turno
    }), R;
  }), m("reservas:semana", async (n, _) => {
    console.log("[IPC] Obteniendo reservas de semana:", _);
    const R = await j(
      () => HI(_.desde, _.hasta)
    );
    return console.log("[IPC] Reservas de semana obtenidas:", R.length, "registros"), R;
  }), m("reservas:todas", async () => {
    console.log("[IPC] Obteniendo TODAS las reservas");
    const n = await j(() => vI());
    return console.log("[IPC] Total de reservas obtenidas:", n.length), n;
  }), m("reservas:actualizar-notas", async (n, _, R) => {
    console.log("[IPC] Actualizando notas para reserva:", _);
    const T = await j(() => yI(_, R));
    return console.log("[IPC] Notas actualizadas exitosamente"), _ && await t("modificada", _), T;
  }), m("reservas:cambios", async (n, _) => {
    const R = (_ == null ? void 0 : _.since) || (/* @__PURE__ */ new Date(0)).toISOString(), T = Number((_ == null ? void 0 : _.lastId) || 0), a = Number((_ == null ? void 0 : _.limit) || 200);
    return XI(R, T, a);
  }), m("reservas:dia", async (n, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return R ? j(() => _t(R)) : [];
  }), m("resumen-diario:config:get", async () => FE()), m("resumen-diario:config:set", async (n, _) => JE({
  ...
  })), m("resumen-diario:enviar", async (n, _) => {
    const R = String((_ == null ? void 0 : _.fecha) || "").trim();
    return $I(R);
  });
}
...
function tt(E, t, n) {
  return E === "creación" ? "Reserva creada" : E === "eliminación" ? "Reserva eliminada" : t === null && n !== null ? `Se estableció ${vE(E)}: ${n}` : t !== null && n === null ? `Se eliminó ${vE(E)}` : t !== n ? `Cambió ${vE(E)} de "${t}" a "${n}"` : `Actualización de ${vE(E)}`;
}
function _N(E) {
...
async function eN(E, t, n, _, R) {
  if ((await V(async (a) => {
    ...
    try {
      ...
      console.warn("[Service] Backup SQLite fallo en registrarEventoHistorial:", a);
    }
    return;
  }
  fR(E, t, n, _, R);
...
function NN() {
  m("vehiculos:todos", async () => await j(() => tN())), m("vehiculos:historial", async (E, t) => await j(() => IN(t))), m("vehiculos:mysql-by-matricula", async (E, t) => await j(() => iN(t)));
}
const TN = "mysql.env", aN = [
  "MYSQL_HOST",
  "MYSQL_PORT",
  "MYSQL_USER",
  "MYSQL_PASSWORD",
...
function rN(E) {
  ...
  for (const _ of n) {
    ...
    const T = R.indexOf("=");
    if (T === -1) continue;
    const a = R.slice(0, T).trim();
    let e = R.slice(T + 1).trim();
    (e.startsWith('"') && e.endsWith('"') || e.startsWith("'") && e.endsWith("'")) && (e = e.slice(1, -1)), a && (t[a] = e);
  }
  return t;
}
function uN() {
...
function it() {
  ...
  const t = z.readFileSync(E, "utf-8"), n = rN(t);
  for (const [_, R] of Object.entries(n))
    process.env[_] = R;
}
function cN() {
  const E = ee();
...
function It(E) {
  const t = ee(), n = x.dirname(t);
  z.existsSync(n) || z.mkdirSync(n, { recursive: !0 }), z.writeFileSync(t, E, "utf-8");
}
function ON() {
  m("config:env:get", async () => cN()), m("config:env:set", async (E, t) => (It(t || ""), it(), uI(), { ok: !0 })), m("config:db:test", async () => {
    const E = await V(async (t) => {
      const [n] = await t.query("SELECT 1 as ok");
      return n;
    });
...
async function QE(E) {
  ...
  await V(async (_) => (await _.query(
    `INSERT INTO auditoria_usuarios ( actor_username, actor_role, accion, target_username, detalle)
       VALUES ( ?, ?, ?, ?, ?)`,
    [t.actor_username, t.actor_role, t.accion, t.target_username, t.detalle]
  ), !0)), Q().prepare(
    `INSERT INTO auditoria_usuarios ( actor_username, actor_role, accion, target_username, detalle)
     VALUES ( ?, ?, ?, ?, ?)`
  ).run(t.actor_username, t.actor_role, t.accion, t.target_username, t.detalle);
}
async function sN() {
  await Nt();
  const E = await V(async (n) => {
...
function lE(E) {
  return E === "superadmin" ? [...kn] : E === "super" ? [...kn] : E === "admin" ? ["agenda", "reservas", "historial", "ajustes", "vehiculos"] : ["reservas", "historial"];
}
function nE(E) {
  return E === "superadmin" || E === "super" || E === "admin" || E === "user" ? E : "user";
}
function Ae(E) {
  const t = kE.randomBytes(16), n = kE.scryptSync(E, t, 32);
  return `scrypt$${t.toString("hex")}$${n.toString("hex")}`;
}
function CN(E, t) {
  const n = t.split("$");
  if (n.length !== 3 || n[0] !== "scrypt") return !1;
  const _ = Buffer.from(n[1], "hex"), R = Buffer.from(n[2], "hex"), T = kE.scryptSync(E, _, 32);
  return kE.timingSafeEqual(R, T);
}
function PE(E, t) {
  const n = nE(E), _ = new Set(kn);
  if (!t || t.length === 0)
    return lE(n);
...
function Jn(E, t) {
  ...
  try {
    ...
    return Array.isArray(_) ? PE(n, _) : lE(n);
  } catch {
    return lE(n);
  }
}
async function Tt(E) {
  await cE();
  const t = JSON.stringify(PE(E.role, E.permissions)), n = E.activo ?? 1;
  return V(async (_) => (await _.query(
    ...
    [E.nombre, E.username, E.passwordHash, E.role, t, n]
  ), !0));
}
function zn(E) {
  const t = Q(), n = JSON.stringify(PE(E.role, E.permissions)), _ = E.activo ?? 1;
  t.prepare(
    `INSERT INTO usuarios (nombre, username, password_hash, role, permissions_json, activo)
     VALUES ( ?, ?, ?, ?, ?, ?)
  ...
  t.transaction((R) => {
    for (const T of R)
      n.run(
        ...
        T.password_hash,
        nE(T.role),
        T.permissions_json ?? JSON.stringify(lE(nE(T.role))),
        Number(T.activo) || 0
      );
  })(E);
}
async function at() {
  const E = process.env.SUPERADMIN_USER || "superadmin", t = process.env.SUPERADMIN_PASS || "rodrigoSayasDev724", n = process.env.SUPERADMIN_NAME || "", _ = "superadmin", R = lE(_), T = Ae(t);
  await cE();
  const a = await V(async (I) => {
    const [N] = await I.query(
  ...
  let e = !1;
  try {
    e = !!Q().prepare("SELECT id FROM usuarios WHERE username = ? LIMIT 1").get(E);
  } catch {
    e = !1;
  }
  const i = a.ok, A = a.ok && a.value;
  if (!(!i && e) && (A || await Tt({
    nombre: n,
    username: E,
    passwordHash: T,
...
async function rt() {
  ...
  if (E.ok) {
    ...
    return E.value.map((_) => ({
      ...
      username: _.username,
      role: nE(_.role),
      permissions: Jn(_.permissions_json, nE(_.role)),
...
async function SN() {
  return (await rt()).filter((t) => t.activo).map((t) => ({
    id: t.id,
    nombre: t.nombre,
    username: t.username,
    role: t.role,
...
async function BN(E, t) {
  ...
  let _ = null;
  return n.ok ? _ = n.value : _ = Q().prepare(
    "SELECT id, nombre, username, password_hash, role, permissions_json, activo FROM usuarios WHERE username = ? LIMIT 1"
  ).get(E), !_ || !_.password_hash ? { ok: !1, error: "Usuario o contraseña inválida" } : _.activo ? CN(t, _.password_hash) ? (await QE({
...
async function lN(E) {
  await cE();
  const t = nE(E.role), n = PE(t, E.permissions), _ = Ae(E.password);
  if (!(await Tt({
    nombre: E.nombre,
    username: E.username,
    passwordHash: _,
...
async function MN(E) {
  await cE();
  const t = nE(E.role), n = PE(t, E.permissions), _ = await V(async (T) => (await T.query(
    `UPDATE usuarios SET nombre = ?, username = ?, role = ?, permissions_json = ?, activo = ?
       WHERE id = ?`,
    [E.nombre, E.username, t, JSON.stringify(n), E.activo ?? 1, E.id]
  ), !0));
  Q().prepare(
    `UPDATE usuarios SET nombre = ?, username = ?, role = ?, permissions_json = ?, activo = ?
     WHERE id = ?`
  ).run(E.nombre, E.username, t, JSON.stringify(n), E.activo ?? 1, E.id), _.ok || console.warn("[Usuarios] MySQL no disponible, actualizado solo en SQLite"), await QE({
    actor_username: E.actor_username || "sistema",
    actor_role: E.actor_role || "system",
    accion: "USUARIO_ACTUALIZADO",
    target_username: E.username,
    detalle: `Rol: ${t} | activo: ${E.activo ?? 1}`
  });
}
async function hN(E, t) {
  const n = await ut(E);
  await cE(), await V(async (R) => (await R.query("DELETE FROM usuarios WHERE id = ?", [E]), !0)), Q().prepare("DELETE FROM usuarios WHERE id = ?").run(E), await QE({
    actor_username: t.username || "sistema",
...
async function LN(E, t, n) {
  ...
  await cE();
  const R = Ae(t);
  await V(async (a) => (await a.query("UPDATE usuarios SET password_hash = ? WHERE id = ?", [R, E]), !0)), Q().prepare("UPDATE usuarios SET password_hash = ? WHERE id = ?").run(R, E), await QE({
...
async function ut(E) {
  const t = await V(async (R) => {
    var a;
    const [T] = await R.query(
      "SELECT username FROM usuarios WHERE id = ? LIMIT 1",
      [E]
    );
    return ((a = T[0]) == null ? void 0 : a.username) || null;
  });
  return t.ok ? t.value || null : Q().prepare("SELECT username FROM usuarios WHERE id = ? LIMIT 1").get(E).username || null;
}
function oN() {
  tE.handle("usuarios:bootstrap", async () => (await at(), { ok: !0 })), tE.handle("usuarios:login-list", async () => SN()), tE.handle("auth:login", async (E, t, n) => BN(t, n)), tE.handle("usuarios:list", async () => rt()), tE.handle("usuarios:create", async (E, t) => {
    try {
      return await lN(t), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al crear usuario" };
    }
  }), tE.handle("usuarios:update", async (E, t) => {
    try {
      return await MN(t), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al actualizar usuario" };
    }
  }), tE.handle("usuarios:delete", async (E, t) => {
    try {
      return await hN(t.id, t.actor), { ok: !0 };
    } catch (n) {
      return { ok: !1, error: n.message || "Error al eliminar usuario" };
    }
  }), tE.handle("usuarios:password", async (E, t) => {
    try {
      return await LN(t.id, t.password, t.actor), { ok: !0 };
    } catch (n) {
...
function fN() {
  m("auditoria:list", async () => sN());
}
function gN() {
  console.log(` 
 Cargando IPC handlers  
`), UI(), EN(), AN(), NN(), ON(), oN(), fN(), console.log(` 
...
function wN() {
  const E = /* @__PURE__ */ new Date(), t = (n) => String(n).padStart(2, "0");
...
function gR() {
  try {
    ...
    const t = dN();
    z.existsSync(t) || z.mkdirSync(t, { recursive: !0 });
    const n = x.join(t, `reservas_${wN()}.db`);
...
function GN() {
  gR(), setInterval(gR, 3600 * 1e3);
}
const ct = dR(import.meta.url), Ot = x.dirname(ct);
globalThis.__filename = ct;
globalThis.__dirname = Ot;
process.env.APP_ROOT = x.join(Ot, "..");
const jE = process.env.VITE_DEV_SERVER_URL, FN = x.join(process.env.APP_ROOT, "dist-electron"), st = x.join(process.env.APP_ROOT, "dist");
process.env.VITE_PUBLIC = jE ? x.join(process.env.APP_ROOT, "public") : st;
let DE = null;
function QN() {
  DE = new UR({
    width: 1366,
    ...
    webPreferences: {
      preload: x.join(FN, "preload.mjs"),
      nodeIntegration: !1,
      contextIsolation: !0
    }
  }), jE && DE.webContents.openDevTools({ mode: "detach" }), DE.maximize(), DE.on("page-title-updated", (E) => E.preventDefault()), jE ? DE.loadURL(jE) : DE.loadFile(x.join(st, "index.html"));
}
CE.whenReady().then(async () => {
  it(), Q(), await at(), gN(), GN(), xI(), QN(), tE.on("settings:update", (E, t) => {
    if (!t || typeof t != "object") return;
    const n = t.soundEnabled, _ = t.theme;
    dI({
      soundEnabled: typeof n == "boolean" ? n : !0,
