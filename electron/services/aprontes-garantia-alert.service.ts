...
} from './aprontes.service'
import { getAprontesAlertConfig } from './aprontes-alert-config.service'
...
function shouldNotify(apronte: any, defaultDays = 7) {
  ...
  const base = parseDate(apronte?.garantia_espera_desde)
    || parseDate(`${apronte?.fecha || ''} ${apronte?.hora || '00:00'}`)
...
function buildReminderText(apronte: any, limiteDias: number) {
...
async function enviarAlertaApronte(apronte: any) {
  ...
  await marcarApronteGarantiaNotificado(Number(apronte?.id || 0))
  return true
}
